...
import { pressKey, pressKeyCombo } from '@atlaskit/editor-test-helpers/page-objects/keyboard';
import {
...
async function initEditor(options?: InitOptions) {
	const { appearance = Appearance.fullPage, doc = toolbarAdf, editorProps = {} } = options || {};
...
describe.skip('Floating toolbars:', () => {
	...
	describe('with more items than can be displayed', () => {
		describe('snapshots', () => {
			...
			afterEach(async () => {
				await snapshot(page, { tolerance: 0.05, useUnsafeThreshold: true }, undefined, {
