	...
	(regex: RegExp, listType: TaskDecisionListType, itemAttrs?: AddItemAttrs) => {
		return createRule(
			...
			(state: EditorState, _match: Object | undefined, start: number, end: number) => {
				...
				const insertTr = insertTaskDecisionAction(editorAnalyticsAPI, getContextIdentifierProvider)(
					state,
					listType,
...
const processShortcutForNestedTask = (
	...
	tr.insert(
		...
		list.create({ localId: listLocalId }, [
			item.create({ localId: itemLocalId, ...itemAttrs }, contentWithoutShortcut),
		]),
	)
