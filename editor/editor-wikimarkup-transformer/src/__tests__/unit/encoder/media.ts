...
describe('ADF => WikiMarkup - Media', () => {
	...
	test('should convert mediaSingle node with no width and height to thumbnail', () => {
		const node = doc(mediaSingle()(media({ id: 'abc-123', type: 'file', collection: 'tmp' })()))(
			defaultSchema,
		);
		expect(transformer.encode(node, context)).toMatchSnapshot();
	});
	...
	test('should convert mediaSingle with context inside table', () => {
		const node = doc(
			table()(
				tr(td()(mediaSingle()(media({ id: 'adf-media-id', type: 'file', collection: 'tmp' })()))),
			),
		)(defaultSchema);
		const context: Context = {
			conversion: {
	...
	test('should convert mediaInline node with context', () => {
		const node = doc(p(mediaInline({ type: 'file', collection: 'tmp', id: 'abc-123' })()))(
			defaultSchema,
		);
		expect(transformer.encode(node, context)).toMatchSnapshot();
	});
	...
	test('should convert mediaInline node with no context', () => {
		const node = doc(p(mediaInline({ type: 'file', collection: 'tmp', id: 'abc-123-uuid' })()))(
			defaultSchema,
		);
		expect(transformer.encode(node)).toMatchSnapshot();
	});
