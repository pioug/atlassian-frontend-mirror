...
const i18n = defineMessages({
  ...
  emptyAutomationListImageAlt: {
    ...
    description:
      'Alternative text for the image that renders when the automation list is empty.',
  },
});
...
export const AutomationModalEmptyState = () => {
  ...
  const { canManageAutomation, analyticsSource, emptyStateDescription, emptyStateAdminDescription } = useAutomationMenu();

  const emptyStateDesc: React.ReactNode = emptyStateDescription ?? <FormattedMessage {...i18n.defaultEmptyStateDesc} />
  const adminEmptyStateDesc: React.ReactNode = emptyStateAdminDescription ?? <FormattedMessage {...i18n.defaultEmptyStateAdminDesc} />
  const displayedEmptyStateDesc = canManageAutomation
  ? adminEmptyStateDesc
  : emptyStateDesc;
