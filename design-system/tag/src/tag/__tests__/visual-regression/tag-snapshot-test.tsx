...
describe('Snapshot Test', () => {
	it('Tag-basic should match production example', async () => {
		const url = getExampleUrl('design-system', 'tag', 'basic-tag', global.__BASEURL__);
		const { page } = global;
	...
	it('Tag color variations should match production example', async () => {
		const url = getExampleUrl('design-system', 'tag', 'colors', global.__BASEURL__);
	...
	it('Removable avatar tag should match production example', async () => {
		const url = getExampleUrl('design-system', 'tag', 'appearance', global.__BASEURL__);
		...
		const imageAfterRemovingTag = await takeElementScreenShot(page, '#appearance');
		expect(imageAfterRemovingTag).toMatchProdImageSnapshot();
	});

	it('Removable tag shows focus ring', async () => {
		const url = getExampleUrl('design-system', 'tag', 'removable-tag', global.__BASEURL__);
		const { page } = global;
		await loadPage(page, url);
	...
	it('Should render simple tags', async () => {
		const url = getExampleUrl('design-system', 'tag', 'simple-tag', global.__BASEURL__);
	...
	it('Removable tag should change bg color on remove button hover & removed when clicked', async () => {
		...
		await page.waitForSelector(removableTag);
		const beforeClickingRemoveButton = await takeElementScreenShot(page, removableTag);
	...
	it('Tag with text max length should match production example', async () => {
		const url = getExampleUrl('design-system', 'tag', 'text-max-length', global.__BASEURL__);
