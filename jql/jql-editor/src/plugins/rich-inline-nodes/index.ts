...
const decorateNodesInRange = (
  ...
  state.doc.nodesBetween(from, to, (node, pos) => {
    if (node.type.spec.group === RICH_INLINE_NODE) {
      decorations.push(
        Decoration.node(pos, pos + node.nodeSize, {}, { type: decoration }),
      );
    }
  });
...
const RichInlineNodesPluginKey = new PluginKey<void>(
  'rich-inline-nodes-plugin',
);
  ...
  new Plugin<void>({
    ...
    props: {
      decorations: (state: EditorState) => {
        ...
        decorations.push(
          ...decorateNodesInRange(state, SELECTED_NODE, from, to),
        );
