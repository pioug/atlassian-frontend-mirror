...
describe('ADF => WikiMarkup - Paragraph', () => {
	...
	test('should convert plain-text with hardBreak in paragraph', () => {
		const node = doc(p('the first line of text', hardBreak(), 'a new line of text'))(defaultSchema);
		expect(transformer.encode(node)).toMatchSnapshot();
	});

	test('should convert em to _emphasis_', () => {
		const node = doc(p('This is an ', em('emphasised'), ' word.'))(defaultSchema);
		expect(transformer.encode(node)).toMatchSnapshot();
	});
	...
	test('should convert sub mark to subscript', () => {
		const node = doc(p('This is an ', subsup({ type: 'sub' })('subscript')))(defaultSchema);
		expect(transformer.encode(node)).toMatchSnapshot();
	});

	test('should convert sup mark to superscript', () => {
		const node = doc(p('This is an ', subsup({ type: 'sup' })('superscript')))(defaultSchema);
		expect(transformer.encode(node)).toMatchSnapshot();
	});

	test('should convert link mark to link', () => {
		const node = doc(p('This is an ', a({ href: 'https://www.atlassian.com' })('link')))(
			defaultSchema,
		);
		expect(transformer.encode(node)).toMatchSnapshot();
	});
	...
	test('should convert color mark to color macro', () => {
		const node = doc(p('This is an ', textColor({ color: '#FFFFFF' })('colorful'), ' text'))(
			defaultSchema,
		);
		expect(transformer.encode(node)).toMatchSnapshot();
	});
	...
	test('should convert color with strong mark', () => {
		const node = doc(
			p('This is an ', textColor({ color: '#FFFFFF' })(strong('strong colored')), ' text'),
		)(defaultSchema);
		expect(transformer.encode(node)).toMatchSnapshot();
	});

	test('should convert citation with strong mark', () => {
		const node = doc(p('This is an ', em(strong(`${EM_DASH} strong citation`)), ' text'))(
			defaultSchema,
		);
		expect(transformer.encode(node)).toMatchSnapshot();
	});
	...
	test('should convert link with strong mark', () => {
		const node = doc(
			p(
				'This is an ',
				a({ href: 'https://www.atlassian.com' })(strong('strong Atlassian link')),
				' text',
			),
		)(defaultSchema);
		expect(transformer.encode(node)).toMatchSnapshot();
	...
	test('[ESS-2507] Should only escape opening braces having macro keywords', () => {
		const node = doc(p('this is a \\{panel} \\{notmacro} \\{panel}'))(defaultSchema);
		expect(transformer.encode(node)).toMatchSnapshot();
	});
	...
	test('[ADFEXP-359] Should escape mediaGroup node', () => {
		const node = doc(p('This is a mediaGroup [^filename.txt] node'))(defaultSchema);
		expect(transformer.encode(node)).toMatchSnapshot();
	});

	test('[ADFEXP-359] Should not escape mediaGroup node - invalid syntax', () => {
		const node = doc(p('This is invalid syntax for mediaGroup [^filename.txt! node'))(
			defaultSchema,
		);
		expect(transformer.encode(node)).toMatchSnapshot();
	});
	...
	test('[ADFEXP-359] Should not escape content between square brackets', () => {
		const node = doc(p('This is should be a paragraph [filename.txt] node'))(defaultSchema);
		expect(transformer.encode(node)).toMatchSnapshot();
	});
});
