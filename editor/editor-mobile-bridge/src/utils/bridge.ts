...
const getTotalNodeSizeAndNodes = (docNode: JSONDocNode): TotalNodeSizeAndNodes => {
...
export function measureContentRenderedPerformance(
	docNode: JSONDocNode,
	callback: (totalNodeSize: number, nodes: string, actualRenderingDuration: number) => void,
) {
	measureRender(
