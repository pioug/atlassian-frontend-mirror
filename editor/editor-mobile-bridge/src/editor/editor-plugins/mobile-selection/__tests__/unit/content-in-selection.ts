...
describe('content-in-selection', () => {
	test.each([
		...
		['nested nodes', doc(panel()(p('{<}Hello{>}'))), ['panel', 'paragraph', 'text'], []],
		[
			'split nodes',
		...
		[
			'action',
			doc(taskList({ localId: 'dummy-list' })(taskItem({ localId: 'dummy-item' })('{<}Hello{>}'))),
			['taskList', 'taskItem', 'text'],
			[],
		],
		['bold', doc(p(strong('{<}Hello{>}'))), ['paragraph', 'text'], ['strong']],
		['split mark', doc(p('{<}Hel', strong('lo{>}'))), ['paragraph', 'text', 'text'], ['strong']],
		[
			'mixed marks',
