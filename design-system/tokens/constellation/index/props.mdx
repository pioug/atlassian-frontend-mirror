...
Use the `setGlobalTheme` method to switch themes globally at runtime. This updates the `data-theme` attribute on your page's HTML tag.
...
A React hook which returns the current theme set on `<html>`, or `null` if not set. It is useful for watching the theme and then performing side-effects when it changes.

<TokenPropsTable propName={'return value'} description={'Returns the current theme, or null if not set'} typing={'Themes < "light", "dark" > | null'}/>
