...
## Please avoid overflow scrolling if you can

<SectionMessage appearance="warning">

These challenges are why we call overflow scrolling "unsafe"

</SectionMessage>

### 1. It can be confusing for users
...
`getOverflow()` allows you to specify how overflow scrolling should work for each edge (eg `"top"`) of an element

```ts
unsafeOverflowAutoScrollForElements({
  getOverflow: () => ({
    // When will scroll on the "top" edge scroll?
    fromTopEdge: {
      top: 400, // for 400px above the top edge
      right: 100, // up to 100px on the right hand side of the top edge
      left: 100, // up to 100px on the left hand side of the top edge
...
![API visual explaination](images/from-top-edge-example.jpg)

```ts
unsafeOverflowAutoScrollForElements({
  element,
  // Allow overflow scrolling in every direction around an element
  getOverflow: () => ({
    fromTopEdge: {
      top: 400,
      right: 400,
...
// Only allow overflow scrolling above and below an element
unsafeOverflowAutoScrollForElements({
  element,
  getOverflow: () => ({
    fromTopEdge: {
...
Overflow scrolling is not applicable for window scrolling as the browser stops publishing events to the document once the user has left the window. If you want to have window scrolling, then use our "over element" window scrolling functions (eg `autoScrollWindowForElements`)
