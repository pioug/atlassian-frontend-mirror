...
2. **Separator:** The separator is positioned between each item in the list. It provides visual distinction between individual links using the same font as the rest of the breadcrumbs. Usually a forward slash symbol `/`.
3. **Icon or avatar (optional):** Some links may have an icon or avatar before or after the corresponding page title.
...
- Use `isNavigation={false}` for any instances of breadcrumbs that aren't used for main navigation areas, like a listing of search results, to reduce noise for users of assistive technologies.
