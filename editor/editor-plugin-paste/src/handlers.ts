      ...
      if (
        ...
        if (transformedSlice.content.lastChild?.type?.name === 'rule') {
          tr.setSelection(
            TextSelection.near(
              tr.doc.resolve(
                tr.selection.$from.pos + transformedSlice.content.size,
              ),
            ),
          );
        } else {
...
export function handlePasteNonNestableBlockNodesIntoList(
  ...
  return (state: EditorState, dispatch?: CommandDispatch): boolean => {
    ...
    slice.content.forEach(child => {
      if (
        !listItem ||
        (child.isBlock && !listItem.spec.content?.includes(child.type.name))
      ) {
...
export function handlePastePreservingMarks(
  ...
  return (state: EditorState, dispatch?): boolean => {
    ...
    const hasActiveCodeMark =
      ...
      anyMarkActive(state, codeMark);
    const hasAnnotationMark =
      annotationMark && annotationMark.isInSet(selectionMarks);
