...
level DOM element as \`data-testid\` attribute.

Here is the list of available css selectors (\`[data-test-...=""]\`) attributes that you might find
useful to use in integration/e2e tests available in media-card:
...
- \`[data-testid="media-card-loading"]\`: Media Card in loading state (with a spinner in the middle
  and nothing else)
...
- \`[data-test-loading]\`: This present for loading card (on the same element as
  \`media-card-loading\` (or 'testId')
- \`[data-testid="media-file-card-view"]\`: This selector can be used to get some extra meta
  information via following attributes:
...
- \`[data-testid="media-card-primary-action"]\`: Primary action button (there could be two of them
  max)
- \`[data-testid="media-card-actions-menu"]\`: Actions menu button (three dots icon) that open
  dropdown
...
- \`[data-testid="custom-media-player"]\`: Wrapper around custom media player
  ...
  - \`[data-testid="custom-media-player-playback-speed-toggle-button"]\`: Open/Close playback speed
    menu
