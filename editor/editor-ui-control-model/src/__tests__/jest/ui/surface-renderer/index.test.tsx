...
import {
	...
	type ContextToken,
	type IsHiddenOptions,
	type RegisterComponent,
} from '../../../../types';
import { SurfaceRenderer } from '../../../../ui/surface-renderer';
import type { SurfaceFallbacks } from '../../../../ui/surface-renderer/types';
...
describe('SurfaceRenderer', () => {
	...
	describe('parent chain', () => {
		...
		it('should pass an empty parents array to the root surface component', () => {
			...
			expect(rootSpy.mock.calls[0][0]).toHaveProperty('parents', []);
		});
	});
	...
	describe('surface context', () => {
		it('passes context to visibility and component props', () => {
			const token = Symbol('surface value') as ContextToken<string>;
			const surfaceContext = createSurfaceContext(token, 'surface value');
			const componentProps = jest.fn((_props: Record<string, unknown>) => (
				<div data-testid="context-item" />
			));
			const isHidden = jest.fn(
				({ surfaceContext: receivedContext } = {}) =>
					receivedContext?.get(token) !== 'surface value',
			);
			const surface = createMenuSurface();
			...
			expect(screen.getByTestId('context-item')).toBeInTheDocument();
			expect(isHidden).toHaveBeenCalledWith({ surfaceContext });
			expect(componentProps.mock.calls[0]?.[0]).toHaveProperty('surfaceContext', surfaceContext);
		});
	});
	...
	describe('isHidden', () => {
		...
		it('should pass surface context to root, descendants, and visibility predicates', () => {
			const token = createContextToken<string>('test');
			const surfaceContext = createSurfaceContext(token, 'context-value');
			const rootComponent = jest.fn(({ children }) => <div>{children}</div>);
			const childComponent = jest.fn((_props: CommonComponentProps) => (
				<div data-testid="context-child" />
			));
			const isHidden = jest.fn(
				(options?: IsHiddenOptions) => options?.surfaceContext?.get(token) !== 'context-value',
			);
			const components: RegisterComponent[] = [
				{
					key: 'context-surface',
				...
				},
				{
					...
					isHidden,
					parents: [{ key: 'context-surface', type: 'toolbar', rank: 1 }],
				},
			];
			...
			render(
				<SurfaceRenderer
					components={components}
					surface={{ key: 'context-surface', type: 'toolbar' }}
					surfaceContext={surfaceContext}
				/>,
			);

			expect(screen.getByTestId('context-child')).toBeInTheDocument();
			expect(rootComponent.mock.calls[0][0].surfaceContext).toBe(surfaceContext);
			expect(childComponent.mock.calls[0][0].surfaceContext).toBe(surfaceContext);
			expect(isHidden).toHaveBeenCalledWith({ surfaceContext });
		});
