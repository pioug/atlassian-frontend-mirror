...
test('Should tab through all interactive elements inside datepicker', async ({
  page,
}) => {
  await page.visitExample(
    'design-system',
  ...
  await expect(
    page.locator(tabcheckDatePickerInputOutsidePopup).first(),
  ).toBeFocused();
  await page.keyboard.press('Tab');
...
test('Should tab from input component to datepicker to next input in popup', async ({
  page,
}) => {
  await page.visitExample(
    'design-system',
  ...
  await expect(
    page.locator(tabcheckDatePickerInputInsidePopup).first(),
  ).toBeFocused();
  await page.keyboard.press('Tab');
  await expect(page.locator(previousMonthButton).first()).toBeFocused();
  await page.keyboard.press('Tab');
