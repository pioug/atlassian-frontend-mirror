...
describe('StoredDuplicateLimitedQueue', () => {
	...
	describe('storage write', () => {
		it('should update storage for enqueue', () => {
			...
			expect(mockSetItem.getCall(0).args[0]).toEqual(`${storagePrefix}.lastUsed`);
			expect(mockSetItem.getCall(0).args[1]).toEqual(JSON.stringify(['a', 'b', 'c']));
		});
		...
		it('should fail silently when storage write fails', () => {
			...
			expect(queue.getItemsOrderedByDuplicateCount().sort()).toEqual(['a', 'b', 'c']);
		});
	});
