import { computeIndex } from '../../../internal/compute-index';
...
const extractKeys = (rows: Array<RowType>) => rows.map((row: RowType) => row.key);
const getKeys = (params: Array<number>) => Array.from(params).map((rowIndex) => getKey(rowIndex));
...
const testReordering = (fromIndex: number, toIndex: number, expectedOrder: Array<number>) => {
