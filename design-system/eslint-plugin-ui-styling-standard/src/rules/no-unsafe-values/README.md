...
Statically written CSS at-rule keys may be annotated with a type-only `satisfies` expression. The
annotation is erased at runtime, so it does not make the key unsafe:
