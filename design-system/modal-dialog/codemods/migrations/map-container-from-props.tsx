...
import { getOverrideFromComponentsProp, replaceChildren } from '../internal/utils';

export const mapContainerFromProps = (j: core.JSCodeshift, source: Collection<Node>) => {
	...
	source.findJSXElements(defaultSpecifier).forEach((element: ASTPath<JSXElement>) => {
		const container = getOverrideFromComponentsProp(j, element, COMPONENTS_PROP_NAME, 'Container');
		...
		if ((container as StringLiteral).type === 'StringLiteral') {
			const wrappedChildren = j.jsxElement(
				j.jsxOpeningElement(j.jsxIdentifier((container as StringLiteral).value)),
				j.jsxClosingElement(j.jsxIdentifier((container as StringLiteral).value)),
			...
			const props = j.objectExpression([
				j.objectProperty(
					...
					j.jsxFragment(j.jsxOpeningFragment(), j.jsxClosingFragment(), element.value.children),
				),
			]);
