...
const setupTypeAhead = async (props?: Partial<Props>): Promise<RenderResult> =>
	renderWithIntl(
		<EmojiTypeAhead
			emojiProvider={props && props.emojiProvider ? props.emojiProvider : getEmojiResourcePromise()}
			query=""
...
describe('EmojiTypeAhead', () => {
	...
	it('should change selection when navigating next', async () => {
		...
		await waitFor(() => isEmojiTypeAheadItemSelected(container, allEmojis[1].shortName));
	});
	...
	it('should change selection when navigating previous', async () => {
		...
		// Last emoji selected
		await waitFor(() =>
			isEmojiTypeAheadItemSelected(container, allEmojis[defaultListLimit - 1].shortName),
		);
	});
	...
	it('should choose clicked selection when item clicked', async () => {
		...
		const item = await getEmojiTypeAheadItemById(container, allEmojis[2].shortName);
		...
		expect(fireEventSpy).toHaveBeenCalledWith(
			expect.objectContaining(
				withEmojiIds(
					withSessionId(typeaheadSelectedEvent(false, expect.any(Number), choseEmoji!, allEmojis)),
				),
			),
		);
	...
	it('should record selection on EmojiProvider even with no onSelection property', async () => {
		...
		expect(provider.recordedSelections[0].shortName).toEqual(allEmojis[2].shortName);
	});
	...
	it('should fire onClose when no matches', async () => {
		...
		await waitFor(() =>
			expect(container.querySelector('.ak-emoji-typeahead-item')).not.toBeInTheDocument(),
		);
		expect(onOpen).toHaveBeenCalledTimes(1);
		expect(onClose).toHaveBeenCalledTimes(1);
	});
	...
	it('should highlight correct emoji regardless of conflicting shortName', async () => {
		...
		const item = await getEmojiTypeAheadItemById(container, atlassianBoomEmoji.shortName);
		await userEvent.hover(item);
	...
	it('should retain selected match across search refinement', async () => {
		...
		let item = await getEmojiTypeAheadItemById(container, blackFlagEmoji.shortName);
		await userEvent.hover(item);
		...
		rerender(
			<EmojiTypeAhead emojiProvider={emojiProvider} query="flag_b" ref={emojiTypeAheadRef} />,
		);
	...
	it('should not fire onSelection if a query ends in a colon and an odd number of emoji have an exact shortName match', async () => {
		...
		expect(
			(await itemsVisibleCount(container)) > 1 && (await itemsVisibleCount(container)) % 2 === 1,
		).toEqual(true);
		expect(onSelection).not.toHaveBeenCalled();
	});
	...
	it('should display emojis using the skin tone preference provided by the EmojiResource', async () => {
		...
		expect((await itemsVisibleCount(container)) === 1).toEqual(true);
		const typeaheadEmoji = await getSelectedEmojiTypeAheadItem(container);
		expect(typeaheadEmoji).toHaveAttribute('data-emoji-id', ':raised_hand::skin-tone-2:');
	});
	...
	it('should include skin tone details for analytics', async () => {
		...
		expect((await itemsVisibleCount(container)) === 1).toEqual(true);
		const typeaheadEmoji = await getSelectedEmojiTypeAheadItem(container);
		expect(typeaheadEmoji).toHaveAttribute('data-emoji-id', ':raised_hand::skin-tone-6:');
		const item = await getEmojiTypeAheadItemById(container, ':raised_hand::skin-tone-6:');
		await userEvent.click(item);
		expect(fireEventSpy).toHaveBeenCalledWith(
			expect.objectContaining(
				withEmojiIds(
