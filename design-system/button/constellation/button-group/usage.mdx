...
A button group gives people access to frequently performed, related actions. Use button groups when there's a close relationship between multiple buttons.
...
- Avoid disabled buttons wherever possible. Never put tooltips on disabled buttons, this is not accessible.
...
- The button group is commonly used in [forms](/components/forms) and [modal dialogs](/components/modal-dialog).
