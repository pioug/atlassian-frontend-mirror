import { type MockContext, exactMatch, fillInResponse, type MediaCollectionFile } from '..';
import { type MockRequest, type MockResponse } from 'xhr-mock';

export const getFile = (context: () => MockContext) => (req: MockRequest, res: MockResponse): MockResponse | undefined => {
		...
		const resdata = {
			...
			body: JSON.stringify({
				data: {
					mediaType: 'image',
					mimeType: 'image/jpeg',
					name: availableFiles.reduce((ret, item) => (item.id === fileId ? item : ret), {
						details: { name: '' },
					}).details.name,
					size: availableFiles.reduce((ret, item) => (item.id === fileId ? item : ret), {
						details: { size: 0 },
					}).details.size,
