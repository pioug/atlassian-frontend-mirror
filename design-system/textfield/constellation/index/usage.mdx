...
Use text fields in [forms](/patterns/forms) to help people enter, select, and search for text. Text
fields are normally found within a form but can also be part of a modal, search, or card.
...
1. **Label:** Must indicate what sort of information the field requires and is left-aligned directly
...
2. **Placeholder text (optional):** Avoid placeholders when possible. Never use placeholder text to
   ...
   text to describe the purpose of the field and how to use it. Placeholders can be used for search
   fields or some short example entries.
3. **Input area:** This is where people enter text.
4. **Helper text (optional):** Use this to provide a hint about specific formats the text field will
   accept (for example, `Passwords must contain at least 8 characters`)
...
[field label](/components/form/examples#field) component, make sure the label is associated properly
to the field for accessibility.

Avoid using placeholder text whenever possible. Make sure any critical information is communicated
either in the field label or using helper text below the field. Search fields or brief examples are
the only exceptions where placeholder text is OK.

To announce errors in helper text area during custom validation (reference to
...
- Make sure your error container would have `aria-live` attribute value being explicitly set to
  `polite` by default.
- Add `aria-relevant="all" aria-atomic="false"` to error container when field lose focus and remove
  those when field is focused again.
