...
describe.skip('<QuickSearch />', () => {
	...
	const render = (props?: any, attachToDom = false) => {
		...
		wrapper = mount(
			<QuickSearch {...props} onSearchInput={noOp} onSearchSubmit={onSearchSubmitSpy}>
	...
	describe('Keyboard controls', () => {
		...
		it('should remove any selection when query changes', () => {
			...
			wrapper.update();
			expect(wrapper.find(ResultItem).filterWhere((n) => !!n.prop('isSelected'))).toHaveLength(0);
			expect(isInputFocused(searchInput)).toBe(true);
		});
	...
	it('should pass through the linkComponent prop', () => {
		...
		wrapper.update();
		expect(wrapper.find(ResultItem).first().prop('linkComponent')).toBe(MyLinkComponent);
		wrapper.unmount();
	});
});
