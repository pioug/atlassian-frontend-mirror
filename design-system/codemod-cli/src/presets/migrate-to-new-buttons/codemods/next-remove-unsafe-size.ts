...
export default function (file: FileInfo, api: API) {
  ...
  const newButtonImportDeclarations = getImportDeclaration(
    j,
    source,
    NEW_BUTTON_ENTRY_POINT,
  );
  ...
  newButtonJXSElements.forEach((newButtonJXSElement) => {
    for (const unsafeSizeProp in UNSAFE_SIZE_PROPS_MAP) {
      ...
      const iconAttributes =
        newButtonJXSElement.value.openingElement.attributes?.filter(
          (attribute) =>
            attribute.type === 'JSXAttribute' &&
            attribute.name.name === iconProp,
        );
      ...
      iconAttributes.forEach((iconAttribute) => {
        const unsafeSizeAttribute =
          newButtonJXSElement.value.openingElement.attributes?.find(
            (attribute) =>
              attribute.type === 'JSXAttribute' &&
              attribute.name.name === unsafeSizeProp,
          );
