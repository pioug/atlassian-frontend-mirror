...
export function useRendererReflowDetected(bridge: RendererBridge) {
	useLayoutEffect(() => {
		...
		eventDispatcher.on(EmitterEvents.SET_DOCUMENT_REFLOW_DETECTOR_STATUS, reflowCallBack);

		return () => {
			reflowDetector.disable();
			eventDispatcher.off(EmitterEvents.SET_DOCUMENT_REFLOW_DETECTOR_STATUS, reflowCallBack);
		};
	}, [bridge]);
}
