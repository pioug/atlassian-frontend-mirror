...
// eslint-disable-next-line import/no-extraneous-dependencies -- Removed import for fixing circular dependencies
import { doc, p, table, td, tr } from '@atlaskit/editor-test-helpers/doc-builder';
...
describe('Mobile Appearance Component', () => {
	...
	it('should wrap the children in ClickAreaMobile div with class ak-editor-content-area', () => {
		const { editorView } = editor(doc(table()(tr(td({})(p())), tr(td({})(p())), tr(td({})(p())))));
		const wrapper = mount(
		...
		expect(
			wrapper.find('ClickAreaMobile').find('.ak-editor-content-area').find('DummyChild'),
		).toHaveLength(1);
	});
	...
	it('should pass editorView prop to ClickAreaMobile', () => {
		...
		expect(wrapper.find('ClickAreaMobile').prop('editorView')).toEqual(editorView);
	});
	...
	it('should have a div inside ClickAreaMobile with className as ak-editor-content-area', () => {
		...
		expect(wrapper.find('ClickAreaMobile').find('ContentArea')).toHaveLength(1);
		expect(wrapper.find('ClickAreaMobile').find('.ak-editor-content-area')).toHaveLength(1);
	});
});
