...
import { type CustomMediaPlayerBase } from '../../index-emotion';
...
describe('CustomMediaPlayer Analytics', () => {
	const setup = (
		...
		const MockMediaPlayer: React.FC<VideoProps> = (props: VideoProps) => {
			...
			return (
				<div>
					{children(<div></div>, { ...videoState, currentTime: mockedCurrentTime }, actions, {
						current: null,
					})}
				</div>
			);
	...
	it('should emit "played" track event every 2s when playing', async () => {
		...
		const expectedAnalyticsPayloadForAbsoluteTime = (absoluteTimeInSec: number) => ({
			eventType: 'track',
			action: 'played',
