...
export const useValidateAqlText = (
	...
	const lastResult = useRef<Promise<string | undefined>>(Promise.resolve(undefined));
	const [lastValidationResult, setLastValidationResult] = useState<AqlValidationResult>(
	...
	const validateAqlText = useCallback(
		async (aql: string | undefined) => {
			if (aql?.trim()) {
				try {
					const validateAqlResponse = await validateAql(workspaceId, { qlQuery: aql }, fireEvent);
