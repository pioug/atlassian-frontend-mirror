...
describe('Collab Socket', () => {
	...
	it('should emit to bridge on emit', function () {
		...
		expect(toNativeBridge.emitCollabChanges).toHaveBeenCalledWith('test', '["foo","bar"]');
	});
	...
	describe('Error Handling', () => {
		it('should no throw with invalid payload', function () {
			...
			expect(() => socket.received('custom-event', undefined as any)).not.toThrow();
			expect(() => socket.received('custom-event', 'foo!bar/baz')).not.toThrow();
		});
		...
		it('should not emit an event received with an invalid payload', function () {
			const socket = new CollabSocket(path);
			const emitSpy = jest.spyOn((socket as any).emitter as EventEmitter2, 'emit');
