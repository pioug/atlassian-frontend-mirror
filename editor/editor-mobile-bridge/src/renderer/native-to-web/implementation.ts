...
import { nativeBridgeAPI, toNativeBridge } from '../web-to-native/implementation';
	...
	implements RendererBridge
{
	...
	async setContentPayload(uuid: string) {
		const adfContent: Serialized<JSONDocNode> = await this.fetchPayload('content', uuid);
	 ...
	 *
	 */
	scrollToContentNode(nodeType: ScrollToContentNode, id: string, index = -1): Promise<boolean> {
	 ...
	 */
	getContentNodeScrollOffset(nodeType: ScrollToContentNode, id: string, index = -1): string {
		if (nodeType === ScrollToContentNode.HEADING) {
			eventDispatcher.emit(EmitterEvents.SET_ACTIVE_HEADING_ID, id);
		}
		return JSON.stringify(getElementScrollOffsetByNodeType(nodeType, id, index));
	}

	/** @deprecated  Use `getContentNodeScrollOffset` instead */
	getContentNodeScrollOffsetY(nodeType: ScrollToContentNode, id: string, index = -1): string {
		const { y: axisYOffset } = getElementScrollOffsetByNodeType(nodeType, id, index);
	...
	setAnnotationFocus(annotationFocusPayload?: Serialized<AnnotationPayload>) {
		if (typeof annotationFocusPayload === 'string' && annotationFocusPayload.trim().length > 0) {
	...
	createAnnotationOnSelection(annotation: Serialized<AnnotationPayload>) {
		if (typeof annotation === 'string') {
			...
			eventDispatcher.emit(EmitterEvents.CREATE_ANNOTATION_ON_SELECTION, payload);
		}
	}
	...
	observeRenderedContentHeight(enabled: boolean) {
		eventDispatcher.emit(EmitterEvents.SET_DOCUMENT_REFLOW_DETECTOR_STATUS, enabled);
	}
