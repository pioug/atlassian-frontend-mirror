...
```tsx
// card.tsx
import {draggable, dropTargetForElements} from '@atlaskit/pragmatic-drag-and-drop/adapter/element';
...
Since this framework is not tied to `react`, you can also attach drag and drop behavior at some point after a component has already been rendered:
  ...
  useEffect(() => {
    ...
    (async () => {
      const modules = await Promise.all([
        await import('@atlaskit/pragmatic-drag-and-drop/adapter/element'),
        await import('@atlaskit/pragmatic-drag-and-drop/util/combine'),
      ]);
...
For more details, see our [deferred loading guide](/components/pragmatic-drag-and-drop/improving-performance/deferred-loading/deferred-loading).
