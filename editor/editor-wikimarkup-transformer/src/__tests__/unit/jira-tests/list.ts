...
\\- ccc`;
		...
		checkParseEncodeRoundTrips(
			...
			defaultSchema,
			WIKI_NOTATION,
			doc(p('&#42; aaa', hardBreak(), '&#35; bbb', hardBreak(), '&#45; ccc', hardBreak())),
		);
	});
...
* eee`;
		...
		checkParseEncodeRoundTrips(
			...
			defaultSchema,
			WIKI_NOTATION,
			doc(ul(li(p('aaa')), li(p('bbb')), li(p('ccc')), li(p('ddd')), li(p('eee')))),
		);
	});
...
** ggggg`;
		...
		checkParseEncodeRoundTrips(
			...
			WIKI_NOTATION,
			doc(
				ul(li(p('aaa')), li(p('bbb')), li(p('ccc', hardBreak(), 'ddd', hardBreak(), 'eee'))),
...
- ddd`;
		...
		checkParseEncodeRoundTrips(
			...
			defaultSchema,
			WIKI_NOTATION,
			doc(ul(li(p('aaa')), li(p('bbb &#124; another bbb')), li(p('ccc')), li(p('ddd')))),
		);
	});
...
* more jobs to do`;
		...
		checkParseEncodeRoundTrips(
			...
			defaultSchema,
			WIKI_NOTATION,
			doc(p('some texts here'), ul(li(p('jobs to do'))), ul(li(p('more jobs to do')))),
		);
	});
...
# unknown item`;
		...
		checkParseEncodeRoundTrips(
			...
			doc(
				...
				ol()(
					li(p('first item')),
					li(p('second item'), ul(li(p('another1 item')), li(p('another2 item')))),
					li(p('unknown item')),
				),
			),
		);
...
# five`;
		...
		checkParseEncodeRoundTrips(
			...
			doc(
				p('Just a list:'),
				ol()(li(p('one'), ol()(li(p('two')), li(p('three'))), ul(li(p('four')))), li(p('five'))),
			),
		);
	});
...
* normal test`;
		...
		checkParseEncodeRoundTrips(
			...
			defaultSchema,
			WIKI_NOTATION,
			doc(ul(li(p(strong('bold test')), ul(li(p(strong('deeper test'))))), li(p('normal test')))),
		);
	});
									...
									li(
										...
										ul(
											...
											li(
												...
												ul(li(p('Generating Reports from JSP and other sources'))),
											),
										),
									),
		...
		checkParseEncodeRoundTrips(
			...
			doc(
				ol()(
					...
					li(p('and')),
					li(p(link({ href: 'http://www.atlassian.com' })('link to atlassian'))),
					li(p('here')),
				),
			),
		);
...
h2. header`;
		...
		checkParseEncodeRoundTrips(
			...
			WIKI_NOTATION,
			doc(
				ul(li(p('item1')), li(p('item2', hardBreak(), 'still item2')), li(p('item3'))),
				h2('header'),
			),
		);
	});
