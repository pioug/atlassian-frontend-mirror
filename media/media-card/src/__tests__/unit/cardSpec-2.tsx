...
const getCardPreviewFromCache = jest.spyOn(getCardPreviewModule, 'getCardPreviewFromCache');
const removeCardPreviewFromCache = jest.spyOn(getCardPreviewModule, 'removeCardPreviewFromCache');
const getCardPreview = jest.spyOn(getCardPreviewModule, 'getCardPreview');
const shouldResolvePreview = jest.spyOn(getCardPreviewModule, 'shouldResolvePreview');

const getSSRCardPreview = jest.spyOn(getCardPreviewModule, 'getSSRCardPreview');
const fetchAndCacheRemotePreview = jest.spyOn(getCardPreviewModule, 'fetchAndCacheRemotePreview');

const getFilePreviewFromFileState = jest.spyOn(getCardPreviewModule, 'getFilePreviewFromFileState');

const createStateUpdater = jest.spyOn(stateUpdaterModule, 'createStateUpdater');
const getCardStateFromFileState = jest.spyOn(stateUpdaterModule, 'getCardStateFromFileState');

const fireOperationalEvent = jest.spyOn(cardAnalyticsModule, 'fireOperationalEvent');

const fireNonCriticalErrorEvent = jest.spyOn(cardAnalyticsModule, 'fireNonCriticalErrorEvent');
...
describe('Media Card', () => {
	...
	describe('Card Init', () => {
		it(`should start invisible if lazy load is enabled (default)`, () => {
			const mediaCard = shallow(
				<CardBase mediaClient={fakeMediaClient()} identifier={indentifiers.file} />,
			);
		...
		it(`should start visible if lazy load is disabled`, () => {
			const mediaCard = shallow(
				<CardBase mediaClient={fakeMediaClient()} identifier={indentifiers.file} isLazy={false} />,
			);
		...
		it('should resolve SSR preview when ssr is server', () => {
			...
			const mediaCard = shallow(
				<CardBase mediaClient={fakeMediaClient()} identifier={indentifiers.file} ssr={'server'} />,
			);
		...
		it('should resolve SSR preview when ssr is client and there is no SSR data in global scope', () => {
			...
			getSSRCardPreview.mockReturnValueOnce(expectedPreview);
			const mediaCard = shallow(
				<CardBase mediaClient={fakeMediaClient()} identifier={indentifiers.file} ssr={'client'} />,
			);
		...
		it(`should catch and log the error when refetch the preview if ssr client dimensions are bigger than server when card is visible`, async () => {
			...
			fetchAndCacheRemotePreview.mockRejectedValueOnce(new Error(`I've failed you`));
		...
		it(`should not refetch the preview if ssr client dimensions are smaller than server`, () => {
			...
			expect(getSSRData).toBeCalledTimes(1);
			expect(getSSRCardPreview).toBeCalledTimes(0);
			expect(mediaCard.state('cardPreview')).toEqual(filePreviews.ssrClientGlobalScope);
		});

		it.each(['server', 'client'] as const)(
			`should start invisible if lazy load is enabled and there is SSR preview (%s)`,
			(ssr) => {
				const expectedPreview = ssr === 'client' ? filePreviews.ssrClient : filePreviews.ssrServer;
		...
		it.each(['server', 'client'] as const)(
			...
			(ssr) => {
				getCardPreviewFromCache.mockReturnValueOnce(filePreviews.cachedRemote);
				const mediaCard = shallow(
					<CardBase mediaClient={fakeMediaClient()} identifier={indentifiers.file} ssr={ssr} />,
				);
				expect(getSSRCardPreview).toBeCalledTimes(0);
				expect(mediaCard.state('cardPreview')).toBe(filePreviews.cachedRemote);
			},
	...
	describe('Upfront Preview', () => {
		...
		it('resolves the preview upfront if card turned visible and has no preview', async () => {
			...
			const mediaCard = shallow(
				<CardBase mediaClient={fakeMediaClient()} identifier={indentifiers.file} isLazy={true} />,
			);
	...
	describe('File State Subscription', () => {
		it('should subscribe to file state after mounting if lazy load is off', () => {
			...
			shallow(<CardBase mediaClient={mediaClient} identifier={indentifiers.file} isLazy={false} />);
		...
		it('should not subscribe to file state when the card is out of the viewport', () => {
			const mediaClient = fakeMediaClient();
			shallow(<CardBase mediaClient={mediaClient} identifier={indentifiers.file} />);
		...
		it('should resubscribe to file state when identifier or Media Client are updated', () => {
			...
			const mediaClient = fakeMediaClientWithObservable(fakeObservable(unsubscribe));
			const newMediaClient = fakeMediaClient();
			const mediaCard = shallow(
				<CardBase mediaClient={mediaClient} identifier={indentifiers.file} isLazy={false} />,
			);
			mediaCard.setProps({
				identifier: { ...indentifiers.file, id: 'another-id' },
			});
	...
	describe('Card State from File State', () => {
		it('should set Card State derived from File State', () => {
			...
			expect(getCardStateFromFileState).toBeCalledWith(fileStates.processed, false);
	...
	describe('Card Preview', () => {
		...
		it(`should get card preview if shouldResolvePreview returns true`, async () => {
			...
			await flushPromises();
			expect(mediaCard.state('cardPreview')).toEqual(expect.objectContaining(filePreviews.remote));
		});

		it.each([
			['RemotePreviewError', new RemotePreviewError('remote-preview-fetch')],
			['Unexpected Error', new Error('this is an unexpected error')],
		])('should set state to error when getCardPreview throws %s', async (name, error) => {
			shouldResolvePreview.mockReturnValueOnce(true);
			...
			const expectedError =
				name === 'Unexpected Error' ? expect.objectContaining({ secondaryError: error }) : error;
		...
		it('should set isBannedLocalPreview to true when getCardPreview throws LocalPreviewError', async () => {
			shouldResolvePreview.mockReturnValueOnce(true);
			const error = new LocalPreviewError('some-message' as LocalPreviewPrimaryReason);
			getCardPreview.mockRejectedValueOnce(error);
		...
		it('should not pass filePreview to getCardPreview if local preview is banned', () => {
			...
			expect(getCardPreview).toBeCalledTimes(1);
			expect(getCardPreview).toBeCalledWith(expect.objectContaining({ filePreview: undefined }));
		});
	...
	describe('refetch on resize', () => {
		...
		])('should %s when dimensions are %s', async (_title1, _title2, width, height, callTimes) => {
	...
	describe('onImageError', () => {
		...
		it(`should not set status 'error' if onImageError is called when source is ssr-client`, () => {
			...
			expect(removeCardPreviewFromCache).toBeCalledWith(indentifiers.file.id, defaultMode);
		});
		...
		it(`should set status 'error' if onImageError is called when source is external image`, () => {
			const mediaCard = shallow(
				<CardBase mediaClient={fakeMediaClient()} identifier={indentifiers.externalImage} />,
			);
		...
		it('should set the status of both client and server to fail if the dataURI from server is reused in client and it fails loading', () => {
			...
			expect(fireOperationalEvent).toBeCalledWith(
				...
				},
				{
					server: {
						status: 'fail',
						...extractErrorInfo(new ImageLoadError(filePreviews.ssrClientGlobalScope.source)),
					},
					client: {
						status: 'fail',
						...extractErrorInfo(new ImageLoadError(filePreviews.ssrClientGlobalScope.source)),
