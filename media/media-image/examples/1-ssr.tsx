...
const createScenarios = (): Scenarios => {
	const createScenario = ({ hydrate, throwError }: Omit<RunSSRParams, 'containerId'>): Scenario => {
		const containerId = `${hydrate ? 'client' : 'server'}-${throwError || 'success'}`;
		return [containerId, () => runSSR({ containerId, hydrate, throwError })];
	};
	return {
		'media-image-server': (
			[{}, { throwError: 'getImageUrlSync' }, { throwError: 'dataURI' }] as const
		).map(createScenario),
