...
const editorPluginsPackageJsonPath = path.join(
  editorPluginsPath,
  'package.json',
);
...
function getEditorPluginsPackageJson(): Record<string, any> {
  ...
  if (!fs.existsSync(editorPluginsPackageJsonPath)) {
    throw new Error(
      `Package.json not found for ${editorPluginsPackageJsonPath}`,
    );
  }

  // Read and parse the package.json file
  const packageJson = JSON.parse(
    fs.readFileSync(editorPluginsPackageJsonPath, 'utf8'),
  );
...
function removeNonEditorPluginDeps(
...
function removeEditorPluginDeps(
  currentDeps: Record<string, string>,
): Record<string, string> {
  const filteredDeps: Record<string, string> = {};
...
function updatePluginsFiles(
  ...
  const newFolders = new Set(
    depFolderNames.map(name => extractFileNameFromDepName(name)),
  );

  const removedFiles = Array.from(existingFiles).filter(
    file => !newFolders.has(file),
  );
  const addedFolders = Array.from(newFolders).filter(
  ...
  if (removedFiles.length > 0) {
    console.log(
      `Removed (${removedFiles.length} file${
  ...
  if (addedFolders.length > 0) {
    console.log(
      `Added (${addedFolders.length} file${
        addedFolders.length > 1 ? 's' : ''
      }):`,
    );
...
function formatDependencyDiff(diff: DependenciesDiffResult): string {
  ...
  if (updatedCount > 0) {
    ...
    diff.updated.forEach(
      d =>
        (result += `  ^ ${d.dependency}: ${d.oldVersion} => ${d.newVersion}\n`),
    );
  }
...
async function run() {
  ...
  try {
    ...
    const newEditorPluginsDeps =
      getUpdatedDependenciesFromPackageJsons(packageJsons);
    ...
    if (!currentDeps) {
      throw new Error(
        `Dependencies object not found in editorPluginsPackageJsonPath`,
      );
    }
    ...
    depFolderNames.forEach(dep => {
      pluginEntryPointDataMap[extractFileNameFromDepName(dep)] =
        getEntryPointDataForPlugin(path.join(pluginsPath, dep));
    });
