  ...
  ({ nodeName, adfs, editorOptions }) => {
    test.describe(`inline-nodes - ${nodeName}`, () => {
      ...
      test.describe(`trailing spaces`, () => {
        ...
        test('Can click and drag to extend a selection to the start of the current line from the current position', async ({
          ...
          await editor.page.mouse.move(
            cursorPositionEndOfLine.x,
            cursorPositionEndOfLine.y,
          );
          await editor.page.mouse.down();
          await editor.page.mouse.move(
            cursorPositionStartOfLine.x,
            cursorPositionStartOfLine.y,
          );
      ...
      test.describe(`multiple nodes across lines`, () => {
        ...
        test('Can extend the selection one line up with shift + arrow up', async ({
          editor,
        }) => {
          fixTest({
            jiraIssueId: 'ED-20526',
        ...
        test('Can extend the selection one line down with shift + arrow down', async ({
          editor,
        }) => {
          fixTest({
            jiraIssueId: 'ED-20526',
