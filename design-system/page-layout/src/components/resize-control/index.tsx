...
const ResizeControl = ({
	...
	const onKeyDown = (event: ReactKeyboardEvent<HTMLButtonElement>) => {
		...
		const isLeftOrTopArrow =
			key === 'ArrowLeft' || key === 'ArrowUp' || key === 'Left' || key === 'Up';
		const isRightOrBottomArrow =
			key === 'ArrowRight' || key === 'ArrowDown' || key === 'Right' || key === 'Down';
	...
	return (
		<Fragment>
			...
			{leftSidebarState.isResizing ? <Global styles={globalResizingStyles} /> : null}
		</Fragment>
	);
};
