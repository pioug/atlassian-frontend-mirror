...
- If heading is specified, adding `titleId` prop value will set an id to a heading and reference it
  as an accessible name of the spotlight. Otherwise, accessible name of the spotlight will be
  defined by `aria-label` default value, see
  [label prop](/components/onboarding/code#Spotlight-label).
