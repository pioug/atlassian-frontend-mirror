...
describe('<EmojiPicker /> - compiled', () => {
	...
	const renderEmojiPicker = (pickerProps: Partial<EmojiPickerProps> = {}) =>
		renderWithIntl(
			<AnalyticsListener channel="fabric-elements" onEvent={onEvent}>
				<CompiledEmojiPicker emojiProvider={getEmojiResourcePromise()} {...pickerProps} />
			</AnalyticsListener>,
		);
	...
	describe('analytics for component lifecycle', () => {
		it('should fire analytics when component unmounts', async () => {
			const component = await helper.setupCompiledPicker(undefined, undefined, onEvent);
			component.unmount();
	...
	describe('display', () => {
		it('should display first set of emoji in viewport by default', async () => {
			await helper.setupCompiledPicker();
		...
		it('should adjust picker height if preview is shown', async () => {
			await helper.setupCompiledPickerWithoutToneSelector();
			const list = getUpdatedList();
		...
		it('media emoji should render placeholder while loading', async () => {
			...
			await helper.setupCompiledPicker(undefined, mockConfig);
	...
	describe('category', () => {
		it('selecting category should show that category', async () => {
			await act(async () => {
				helper.setupCompiledPicker(undefined, undefined, onEvent);
			});
	...
	describe('search', () => {
		...
		it('searching should fire analytics', async () => {
			await helper.setupCompiledPicker(undefined, undefined, onEvent);
	...
	describe('skin tone selection', () => {
		...
		it('should display emojis using the skin tone preference provided by the EmojiResource', async () => {
			...
			await helper.setupCompiledPicker({ emojiProvider });
			const list = getUpdatedList();
			const emojis = await helper.emojisVisible(list);
			const hoverOffset = helper.findHandEmoji(emojis);
			expect(hoverOffset).toBeGreaterThan(-1);
	...
	describe('with localStorage available', () => {
		it('should use localStorage to remember tone selection between sessions', async () => {
			...
			const findToneEmojiInNewPicker = async () => {
				({ unmount } = await helper.setupCompiledPicker());
				const list = getUpdatedList();
				const emojis = await helper.emojisVisible(list);
				const hoverOffset = helper.findHandEmoji(emojis);
				expect(hoverOffset).toBeGreaterThan(-1);
	...
	describe('with picker opened experiences', () => {
		it('should track picker opened UFO experience when picker rendered and unmounted', async () => {
			const { unmount } = await helper.setupCompiledPicker();
			unmount();
	...
	describe('Accessibility', () => {
		it('should have no accessibility violations', async () => {
			const { container } = await helper.setupCompiledPicker();
			const list = getUpdatedList();
			await helper.emojisVisible(list);
...
describe('<EmojiPicker /> - emotion', () => {
	...
	const renderEmojiPicker = (pickerProps: Partial<EmojiPickerProps> = {}) =>
		renderWithIntl(
			<AnalyticsListener channel="fabric-elements" onEvent={onEvent}>
				<EmotionEmojiPicker emojiProvider={getEmojiResourcePromise()} {...pickerProps} />
			</AnalyticsListener>,
		);
	...
	describe('analytics for component lifecycle', () => {
		it('should fire analytics when component unmounts', async () => {
			const component = await helper.setupEmotionPicker(undefined, undefined, onEvent);
			component.unmount();
			expect(onEvent).toHaveBeenCalledWith(
				expect.objectContaining({
					payload: openedPickerEvent(),
	...
	describe('display', () => {
		it('should display first set of emoji in viewport by default', async () => {
			await helper.setupEmotionPicker();
			const list = getUpdatedList();
			const emojis = await helper.emojisVisible(list);
		...
		it('should adjust picker height if preview is shown', async () => {
			await helper.setupEmotionPickerWithoutToneSelector();
			const list = getUpdatedList();
		...
		it('media emoji should render placeholder while loading', async () => {
			...
			await helper.setupEmotionPicker(undefined, mockConfig);
	...
	describe('category', () => {
		it('selecting category should show that category', async () => {
			await act(async () => {
				helper.setupEmotionPicker(undefined, undefined, onEvent);
			});
	...
	describe('search', () => {
		...
		it('searching should fire analytics', async () => {
			await helper.setupEmotionPicker(undefined, undefined, onEvent);
	...
	describe('skin tone selection', () => {
		...
		it('should display emojis using the skin tone preference provided by the EmojiResource', async () => {
			...
			await helper.setupEmotionPicker({ emojiProvider });
			const list = getUpdatedList();
			const emojis = await helper.emojisVisible(list);
			const hoverOffset = helper.findHandEmoji(emojis);
			expect(hoverOffset).toBeGreaterThan(-1);
	...
	describe('with localStorage available', () => {
		it('should use localStorage to remember tone selection between sessions', async () => {
			let unmount: RenderResult['unmount'];
			const findToneEmojiInNewPicker = async () => {
				({ unmount } = await helper.setupEmotionPicker());
				const list = getUpdatedList();
				const emojis = await helper.emojisVisible(list);
				const hoverOffset = helper.findHandEmoji(emojis);
				expect(hoverOffset).toBeGreaterThan(-1);
	...
	describe('with picker opened experiences', () => {
		it('should track picker opened UFO experience when picker rendered and unmounted', async () => {
			const { unmount } = await helper.setupEmotionPicker();
			unmount();
			expect(ufoPickerStartSpy).toBeCalled();
			expect(ufoPickerMarkFMPSpy).toBeCalled();
			expect(ufoPickerSuccessSpy).toBeCalled();
	...
	describe('Accessibility', () => {
		it('should have no accessibility violations', async () => {
			const { container } = await helper.setupEmotionPicker();
			const list = getUpdatedList();
			await helper.emojisVisible(list);
