...
export class JqlAnonymizerVisitor extends AbstractJastVisitor<string> {
  visitQuery = (query: Query): string => {
    const where: string | undefined = query.where
      ...
      const properties = field.properties
        .map(property => {
          ...
          const path = property.path
            ? property.path.map(arg => arg.text).join('')
            : '';
          return `[${key}]${path}`;
        })
        .join('');
  ...
  visitTerminalClause = (terminalClause: TerminalClause): string => {
    ...
    const operator = terminalClause.operator
      ? terminalClause.operator.text
      : undefined;
    const operand = terminalClause.operand
      ? terminalClause.operand.accept(this)
      : undefined;
    const predicates = terminalClause.predicates.map(predicate =>
      predicate.accept(this),
    );
    return [field, operator, operand, ...predicates]
      .filter(notUndefined)
      .join(' ');
  };
  ...
  visitFunctionOperand = (functionOperand: FunctionOperand): string => {
    ...
    const functionArguments = functionOperand.arguments
      .map(() => ANONYMIZED_VALUE)
      .join(', ');
  ...
  visitOrderBy = (orderBy: OrderBy): string => {
    const orderByFields = orderBy.fields.map(orderByField => {
      ...
      const direction = orderByField.direction
        ? orderByField.direction.value
        : undefined;
