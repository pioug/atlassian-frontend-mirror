...
describe('onboarding spotlight visual regression', () => {
	...
	it('should inherit button theme from parent buttons', async () => {
		const { __BASEURL__, page } = global;
		const url = getExampleUrl('design-system', 'onboarding', 'different-spotlights', __BASEURL__);

		await loadPage(page, url);

		expect(await takeElementScreenShot(page, spotlightContainer)).toMatchProdImageSnapshot();
	});
