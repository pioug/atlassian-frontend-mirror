import TableTreeDataHelper from '../../../utils/table-tree-data-helper';
...
describe('updateItems', () => {
	...
	it('should throw exception if parent item does not have key property', () => {
		...
		const tableTreeDataHelper = new TableTreeDataHelper<any>({
			key: 'author' as any,
		});
		expect(() => {
	...
	it('should return rootIds and itemsById for root items with custom id', () => {
		...
		const tableTreeDataHelperInstance = new TableTreeDataHelper<(typeof rootData)[number]>({
			key: 'title',
		});
...
describe('appendItems', () => {
	...
	it('should append the parent item with child ids when appending', () => {
		...
		const items = tableTreeDataHelperInstance.appendItems(childItem, rootItems, ...parentItem);
		const formatedData = tableTreeDataHelperInstance.appendItems(
			secondChildItem,
			items,
			...parentItem,
	...
	it('should update the root items when appending', () => {
		...
		const formatedData = tableTreeDataHelperInstance.appendItems(secondParentItem, items);
		expect(formatedData).toEqual([
			{
				id: 1,
				page: 10,
