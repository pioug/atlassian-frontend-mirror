...
export const fetchMessagesForLocale = async (locale: string): Promise<I18NMessages | undefined> => {
	try {
		const messages = await import(
			/* webpackChunkName: "@atlaskit-internal_@atlassian/link-datasource-i18n-[request]" */ `../../../i18n/${locale.replace(
				'-',
				'_',
			)}`
		);
	...
	try {
		...
		const messages = await import(
			/* webpackChunkName: "@atlaskit-internal_@atlassian/link-datasource-i18n-[request]" */ `../../../i18n/${parentLocale}`
		);
		return messages.default;
	} catch (e) {
		// ignore
