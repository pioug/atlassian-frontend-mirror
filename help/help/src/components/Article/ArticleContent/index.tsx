	...
	if (currentArticle && currentArticle.id) {
		if (currentArticle.type === ARTICLE_TYPE.HELP_ARTICLE) {
			...
			if (article && currentArticle.state === REQUEST_STATE.done) {
				...
				return (
					<ArticleLoadingFail onTryAgainButtonClick={onHelpArticleLoadingFailTryAgainButtonClick} />
				);
			}
		}
		...
		if (currentArticle.type === ARTICLE_TYPE.WHATS_NEW) {
			...
			if (article && currentArticle.state === REQUEST_STATE.done) {
				...
				return (
					<ArticleLoadingFail
						onTryAgainButtonClick={onWhatsNewArticleLoadingFailTryAgainButtonClick}
					/>
				);
			}
		}
