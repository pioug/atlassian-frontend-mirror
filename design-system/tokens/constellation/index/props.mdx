...
<TokenPropsTable
	propName={'themeState'}
	description={
		'Used to specify which themes the site is currently using. Omitted properties will fall back to the default value. To update properties, pass a function that returns the new theme state.'
	}
	typing={`{
...
<TokenPropsTable
	propName={'themes'}
	description={
		'A string in the format used by the `data-theme` html attribute; or a stringified theme state object as a URL query string'
	}
	typing={`string`}
	required={true}
/>
