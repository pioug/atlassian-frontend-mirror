...
Use a lozenge dropdown trigger when a [lozenge](/components/lozenge) needs interactivity. It shows
...
Try). [Tag](/components/tag) should be used instead when the label is just descriptive metadata that
...
- For non-interactive lozenges, use [lozenge](/components/lozenge).
- For tallies or scores, use a [badge](/components/badge).
- For descriptive metadata and information, use a [tag](/components/tag).
