...
export const createSelectionComponent = (nativeToWebAPI: RendererBridge) =>
	memo((props: InlineCommentSelectionComponentProps) => {
		...
		useLayoutEffect(() => {
			...
			mobileBridgeEventDispatcher.on(EmitterEvents.APPLY_DRAFT_ANNOTATION, onApplyDraftMode);
			mobileBridgeEventDispatcher.on(EmitterEvents.REMOVE_DRAFT_ANNOTATION, onRemoveDraftMode);
			...
			return () => {
				...
				mobileBridgeEventDispatcher.off(EmitterEvents.APPLY_DRAFT_ANNOTATION, onApplyDraftMode);

				mobileBridgeEventDispatcher.off(EmitterEvents.REMOVE_DRAFT_ANNOTATION, onRemoveDraftMode);
			};
		}, [
			onClose,
			onNativeSideCreatesAnnotation,
