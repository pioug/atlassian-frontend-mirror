...
describe('formatter', () => {
	...
	it('should preserve camelCase tokens', () => {
		...
		expect(result).toEqual(`'color.specialAccent': 'var(--${CSS_PREFIX}-specialAccent)',`);
	});
	...
	it('should parse deeply nested token', () => {
		...
		expect(result).toEqual(`'color.accent.brand': 'var(--${CSS_PREFIX}-accent-brand)',`);
	});
	...
	it('should omit [default] keywords in token paths', () => {
		...
		expect(result).toEqual(`'color.background.brand': 'var(--${CSS_PREFIX}-background-brand)',`);
	});
