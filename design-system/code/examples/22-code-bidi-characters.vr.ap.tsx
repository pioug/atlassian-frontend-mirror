	...
	return (
		<div>
			<div data-testid="bidi-code-warning">
				...
				<p>
					Bidi characters in code blocks and snippets are visually presented to mitigate against
					trojan source attacks.
				</p>
				<h3>Code block examples</h3>
				{Object.entries(codeSnippets).map(([snippetKey, snippetValue]) => {
					return (
						<React.Fragment key={snippetKey}>
							<h3>Example: {snippetKey}</h3>
							<CodeBlock testId={`bidi-warning-${snippetKey}`} text={snippetValue} />
						</React.Fragment>
					);
				})}
				...
				{Object.entries(characters).map(([characterKey, characterValue]) => {
					return (
						<React.Fragment key={characterKey}>
							...
							Inline code snippet{' '}
							<Code testId={`bidi-warning-${characterKey}`}>start{characterValue}end</Code>.
						</React.Fragment>
					);
				})}
			</div>
