...
Until a modal is closed, users can't interact with the rest of the page.

To help both mouse and keyboard users perform this action, the dialog can be closed by:

- Selecting **close** in the header
...
Always add a close button in the header. The control in the footer to cancel or close the modal is
optional.
...
Focus should move as follows:

1. **Container**: Once open, focus should immediately move to the container (modal window).
2. **Close button** in header.
3. **Main content**: For form fields or other interactive elements, focus should move to the first
   focusable element. If the content is text and not actionable, focus should move to the text.
4. **Secondary button**, if using.
5. **Primary button**.
6. **On close**: When the modal is closed, focus should return to the element that triggered the
   modal.
...
>
	Keep headings and body content concise and clear. Focus on a single task or message.
</DoDont>
