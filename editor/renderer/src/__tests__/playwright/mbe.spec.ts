...
test.describe('MBE Renderer', () => {
	...
	test(`Switch to tab 2 and confirm that only it should visible`, async ({ renderer }) => {
		await renderer.waitForRendererStable();
		const firstTabFrame = renderer.page.locator('div[data-extension-frame="true"]').nth(0);
		const secondTabFrame = renderer.page.locator('div[data-extension-frame="true"]').nth(1);
		const thirdTabFrame = renderer.page.locator('div[data-extension-frame="true"]').nth(2);
	...
	test(`Confirm that add and delete buttons should not be visible`, async ({ renderer }) => {
		await renderer.waitForRendererStable();
		const addButton = renderer.page.locator('button[data-testid="mbe-button-add-tab"]');
