...
The default link underline can be removed using `isUnderlined`, however it will still display on
hover. Only remove underlines if the context surrounding the link makes it clear that it is
interactive, such as in navigation.

Underlines help differentiate links from regular text and addresses a WCAG-A recommendation. Using
color alone isn't accessible, especially if links are surrounded by other text. For more information
about accessibility see
