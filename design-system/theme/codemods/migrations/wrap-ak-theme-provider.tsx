...
const cleanUpImports = (
	...
	if (themeImportSpecifiers.length === 1 && namedImport) {
		...
		themeImportSpecifiers
			.filter((path) => path.node.imported.name === AK_THEME_PROVIDER_IDENTIFIER)
			.remove();
	}
};

export const wrapAkThemeProvider = (j: JSCodeshift, source: Collection<Node>) => {
	...
	const packagePath = namedImportFromEntrypoint ? ENTRYPOINT_PATH : PACKAGE_PATH;
		...
		return j.jsxElement(
			j.jsxOpeningElement(
				...
				[
					...
					j.jsxAttribute(j.jsxIdentifier('provider'), j.jsxExpressionContainer(styledAliasImport)),
				],
				selfClose,
			),
			!selfClose ? j.jsxClosingElement(j.jsxIdentifier(DEPRECATED_THEME_PROVIDER)) : null,
