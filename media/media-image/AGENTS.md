...
| Field    | Value                                                                                                        |
| -------- | ------------------------------------------------------------------------------------------------------------ |
| NPM name | `@atlaskit/media-image`                                                                                      |
| Path     | `media-image/`                                                                                               |
| Owner    | Media Exif                                                                                                   |
| Purpose  | React component for rendering media images — wraps image fetching with auth, error boundary, and SSR support |
...
`.agents/knowledge-index/domains/media/units/atlaskit-media-image.md` **Standards page:**
`.agents/knowledge-index/domains/media/units/atlaskit-media-image/standards-and-patterns.md`
**CLI:**
...
- `src/mediaImage.tsx` — main `MediaImage` component
- `src/mediaImageErrorBoundary.tsx` — error boundary wrapper
- `src/types.tsx` — shared TypeScript types
- `content/` — props and example content docs

## Public API (key exports)

`MediaImage` (exported as `MediaImageWithErrorBoundary`), `MediaImageProps` (as
`MediaImageWithMediaClientConfigProps`), `MediaImageChildrenProps`

## Development notes

- React Compiler is enabled for this package
- Peer dependency: `react`
- 1 feature flag: `platform_media_cross_client_copy`
- Export subpaths: `.`, `./internal-types`, `./media-image-error-boundary`
- Includes SSR-safe image rendering
