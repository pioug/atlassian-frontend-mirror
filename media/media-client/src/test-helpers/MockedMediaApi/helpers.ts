...
export const createEmptyFileItem = (
  id: string,
  collection?: string,
): ResponseFileItem => {
...
export const createProcessingFileItem = (
  ...
  if (percent < 0 || percent > 1) {
    throw new Error(
      'Error createProcessingFileItem: percent must be between 0 and 1',
    );
  }
  ...
  const processedArtifactKeys = artifactsKeys.slice(
    0,
    Math.ceil(artifactsKeys.length * percent),
  );
  ...
  const processedArtifactEntries = artifactsEntries.map(
    ([key, artifact]): [string, MediaFileArtifact] => [
      ...
      {
        ...artifact,
        processingStatus: processedArtifactKeys.includes(key)
          ? 'succeeded'
          : 'pending',
      },
    ],
  );
  const artifactsProcessingPercent = Object.fromEntries(
    processedArtifactEntries,
  );
...
export const createErrorFileState = ({
  id,
}: ResponseFileItem): ErrorFileState => ({
  status: 'error',
  id,
