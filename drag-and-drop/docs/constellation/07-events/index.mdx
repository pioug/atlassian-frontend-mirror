...
Events will be fired on pieces (eg `draggable` and _drop targets_) if they are relevant to the event. If you want to listen for _all_ events of a particular entity type (eg files), you can use a [monitor](/components/drag-and-drop/monitors).
...
## Derived events on _drop targets_
...
All events are provided the following base data. Particular event types add additional data. Additionally, event listeners pieces of the system (eg `dropTargetForFiles({ onDragStart })`) can be given additional localized data for convenience.
...
All _drop targets_ add a `self` property (type: `DropTargetRecord`) that contain a convenience object with information about the drop target that the event is firing on. You could easily grab all this information from the outer scope (or from `location.current.dropTargets`), but having it available inline is convenient.
