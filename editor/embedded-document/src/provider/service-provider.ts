import { utils, type ServiceConfig } from '@atlaskit/util-service-support';
...
export default class ServiceProvider implements Provider {
	...
	async getDocument(documentId: string, language?: string): Promise<Document | null> {
		try {
	...
	async getDocumentByObjectId(objectId: string, language?: string): Promise<Document | null> {
		try {
			...
			const documents = await utils.requestService<Array<BatchDocumentResponse>>(this.config, {
