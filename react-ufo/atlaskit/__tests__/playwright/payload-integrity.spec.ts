...
test.describe('React UFO: Payload integrity - v1.0.1, with TTVC v1 fields', () => {
	test.use({
		examplePage: 'basic',
		viewport: {
			width: 1920,
	...
	test(`UFO payload contains expected fields for a basic implementation`, async ({
		...
		expect(ufoProperties).toMatchObject({
			...
			'event:schema': '1.0.0',
			'event:source': { name: 'react-ufo/web', version: '1.0.1' },
		...
		expect(Array.isArray(interactionMetrics.segments)).toBe(true);
		// @ts-ignore - this is an array in `v1.0.1` but an object in `v2.0.0`
		for (const segment of interactionMetrics.segments) {
			expect(Array.isArray(segment.labelStack)).toBe(true);
			for (const labelStack of segment.labelStack) {
				expect(typeof labelStack.n).toBe('string');
				expect(typeof labelStack.s).toBe('string');
			}
		}
		...
		for (const reactProfilerTiming of interactionMetrics.reactProfilerTimings) {
			expect(Array.isArray(reactProfilerTiming.labelStack)).toBe(true);
			for (const labelStack of reactProfilerTiming.labelStack) {
				expect(typeof labelStack.n).toBe('string');
				expect(typeof labelStack.s).toBe('string');
			}
...
test.describe('React UFO: Payload integrity - v1.0.1, without TTVC v1 fields', () => {
	test.use({
		examplePage: 'basic',
		viewport: {
			width: 1920,
