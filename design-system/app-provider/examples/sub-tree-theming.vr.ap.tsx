		...
		<AppProvider defaultColorMode="auto">
			<Stack xcss={contentStyles.body} space="space.400">
				<Heading as="h1" size="xxlarge">
					Sub-tree theming inside AppProvider
				</Heading>
				<PlainElement />
				<Stack space="space.150">
					<Text as="p">
						This example uses an AppProvider, which renders a root ThemeProvider, and nested
						ThemeProviders are within it to provide scoped sub-themes.
					</Text>
				</Stack>
...
const ControlledSubtreeThemeContent = ({
	...
	rootColorMode: ThemeColorModes;
	setRootColorMode: (colorMode: ThemeColorModes) => void;
}) => {
	const colorMode = useColorMode();
	const setColorMode = useSetColorMode();
	...
	return (
		<Box padding="space.300" xcss={contentStyles.section}>
			<Stack space="space.300">
				...
				<Text as="p">You can still control the rool color theme from within a sub-theme.</Text>
				<div>
					<Label htmlFor="color-mode">Root color mode</Label>
					<Select
						inputId="color-mode"
						options={colorModeOptions}
						value={colorModeOptions.find((option) => option.value === rootColorMode)}
						onChange={(option) => (option?.value ? setRootColorMode(option.value) : undefined)}
					/>
				</div>
			</Stack>
		</Box>
...
const ControlledSubtreeTheme = () => {
	...
	return (
		<ThemeProvider defaultColorMode="dark">
			<ControlledSubtreeThemeContent rootColorMode={colorMode} setRootColorMode={setColorMode} />
		</ThemeProvider>
	);
};
