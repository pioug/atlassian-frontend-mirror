...
export default () => {
  ...
  const [buttonToFocus, setButtonToFocus] = useState('1'); // Button 1

  return (
    <div css={spacerStyles}>
      <p>
        <strong>
          Choose a button to focus initially (Should be Button 1 on page open):
        </strong>
      </p>
      <RadioGroup
        onChange={({ currentTarget: { value } }) => setButtonToFocus(value)}
        defaultValue={radioValues[2].value} // Button 1
        options={radioValues}
      />
      <Popup
        isOpen={isOpen}
