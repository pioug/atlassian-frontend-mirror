...
const docAfterRemoveColumnZero = doc(
	table()(tr(createCellColorB(2)), tr(createCellColorA(1), tdColorC), tr(createCellColorC(2))),
);

const docAfterRemoveColumnTwo = doc(
	table()(tr(tdColorA, createCellColorB(1)), tr(createCellColorA(2)), tr(createCellColorC(2))),
);
...
describe('AddColumnStep', () => {
  ...
  `, () => {
		...
		describe('history plugin', () => {
			const testCases: [string, CreateTransaction, CreateTransaction[], DocBuilder][] = [
				[
				...
				[
					...
					addColumnAtFactory('table', 2),
					[removeColumnAtFactory('table', 0), removeColumnAtFactory('table', 0)],
					doc(table()(tr(createCellColorB(1)), tr(tdColorC), tr(createCellColorC(1)))),
				],
				[
			...
			testCases.forEach(([description, transaction, historyTransactions, expectDoc]) => {
				it(`should ${description}`, () => {
					const editorState = testHistory(originalDoc, transaction, historyTransactions);

					expect(editorState.doc.toJSON()).toEqual(expectDoc(defaultSchema).toJSON());
				});
			});
		});
