...
const appearanceOptions = [
	{
		label: 'Full page',
		value: 'full-page',
		description: 'should be used for a full page editor where it is the user focus of the page',
	},
	{
		label: 'Comment',
		value: 'comment',
...
const LOCALSTORAGE_orientationKey = 'fabric.editor.example.kitchen-sink.orientation';
...
const Comp = (props: any) => {
	...
	const extensionProviders = React.useCallback(
		(editorActions?: EditorActions) => [getExampleExtensionProviders(editorApi, editorActions)],
		[editorApi],
	);

	return <KitchenSinkEditor {...props} extensionProviders={extensionProviders} />;
};
...
> {
	...
	private getDefaultFeatureFlagInput = () => {
		...
		const maybeFeatureFlagsFromURL = FeatureFlagUrl.fromLocation<string>(window.parent.location);
	...
	private onOrientationChange = (vertical: boolean) => {
		...
		localStorage.setItem(LOCALSTORAGE_orientationKey, JSON.stringify({ vertical }));
	};
	...
	private onCopyLink = async () => {
		...
		if (this.state.featureFlagInput !== '{}') {
			url.searchParams.set('ff', FeatureFlagUrl.encode(this.state.featureFlagInput));
		} else {
		...
		window.parent.history.pushState(value, window.parent.document.title, result);
		copy(result);
	...
	private onFeatureFlagChange = (event: React.ChangeEvent<HTMLTextAreaElement>) => {
	...
	private onPositionDebuggerEnabled = (event: React.ChangeEvent<HTMLInputElement>) => {
	...
	private editorPlugins(): EditorPlugin[] {
		return this.state.positionDebuggerEnabled ? this.customPlugins : this.noopPlugins;
	}
				...
				<div>
					...
					<div css={container({ vertical: this.state.vertical, root: true })}>
						...
						>
							<Comp
								...
								editorAnnotationProviders={
									this.props.editorAndRendererAnnotationProviders.editorAnnotationProviders
								}
							/>
						</div>
						<div
						...
						>
							<KitchenSinkRenderer
								document={
									this.state.scrubContent ? scrubAdf(this.state.adf as ADFEntity) : this.state.adf
								}
								appearance={this.state.appearance}
								...
								rendererAnnotationProviders={
									this.props.editorAndRendererAnnotationProviders.rendererAnnotationProviders
								}
							/>
						</div>
						{this.state.showADF ? (
							<div css={rail}>
								<ContextPanel visible={this.state.showADF}>
									<div>
										<div css={container({})}>
											{this.state.errors.length > 0 && <ErrorReport errors={this.state.errors} />}
										</div>
										<label>
											Feature flags
											<KitchenSinkAdfInput
...
export function KitchenSink(props: Omit<KitchenSinkProps, 'editorAndRendererAnnotationProviders'>) {
