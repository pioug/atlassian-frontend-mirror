...
const handleRendererContentLoadedBridge = () => {
	if (
		...
		window.requestAnimationFrame(() => toNativeBridge.call('renderBridge', 'onContentRendered'));
	}
};
...
const withSmartCard =
	<P extends BasicRendererProps>(Component: React.ComponentType<React.PropsWithChildren<P>>) =>
		...
		const renderCallback = useCallback(
			(createAnalyticsEvent?: CreateUIAnalyticsEvent) => (
				<SmartCardProvider client={smartCardClient} authFlow={authFlow}>
					<Component createAnalyticsEvent={createAnalyticsEvent} {...(props as P)} />
				</SmartCardProvider>
			),
...
const withFabricAnalytics =
	<P extends MobileRendererProps>(Component: React.ComponentType<React.PropsWithChildren<P>>) =>
