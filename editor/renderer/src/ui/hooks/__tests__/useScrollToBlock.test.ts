...
describe("useScrollToBlock", () => {
  const mockFindNodeWithExpandParents =
    blockMenuUtils.findNodeWithExpandParents as jest.Mock;
  ...
  const mockExpandAllParentsThenScroll =
    blockMenuUtils.expandAllParentsThenScroll as jest.Mock;
  ...
  describe("scrolling without expand parents", () => {
    it("should scroll to element when node has no expand parents", () => {
      ...
      expect(mockWaitForStability).toHaveBeenCalledWith(
        containerDiv,
        expect.any(Function),
      );
  ...
  describe("cleanup", () => {
    it("should cleanup timers on unmount", () => {
      ...
      const { unmount } = renderHook(() =>
        useScrollToBlock(containerRef, adfDoc),
      );
      ...
      expect(mockGetLocalIdSelector).toHaveBeenCalledTimes(
  ...
  describe("edge cases", () => {
    ...
    it("should handle expand container not found in DOM", () => {
      ...
      expect(mockFindNodeWithExpandParents.mock.calls.length).toBeGreaterThan(
        initialCallCount,
      );
    });
    ...
    it("should handle expand element that fails to expand", () => {
      ...
      expect(mockExpandElement.mock.calls.length).toBeGreaterThan(
        initialCallCount,
      );
    });
