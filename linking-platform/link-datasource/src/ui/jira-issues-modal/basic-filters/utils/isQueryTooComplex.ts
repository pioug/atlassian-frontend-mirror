...
class JqlClauseCollectingVisitor extends AbstractJastVisitor<ClauseMap> {
  ...
  visitTerminalClause = (terminalClause: TerminalClause): ClauseMap => {
    ...
    const allowedOperators =
      fieldSpecificOperators[fieldName] || fallbackOperators;
...
export const isQueryTooComplex = (jql: string) => {
  ...
  try {
    ...
    const clauseMap: ClauseMap = jast.query
      ? jast.query.accept(jqlClauseCollectingVisitor)
      : {}; // jast.query is defined as void | Query, hence the fallback

    const hasAnyKeyWithComplexClause = Object.entries(clauseMap).some(
      ([key, clauses]) => isClauseTooComplex(clauses, key),
    );
