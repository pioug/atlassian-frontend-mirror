...
describe('cropToDataURI()', () => {
	...
	beforeEach(() => {
		...
		asMock(context.scale).mockImplementation(() => contextTransformationOrder.push('scale'));
		asMock(context.rotate).mockImplementation(() => contextTransformationOrder.push('rotate'));
	...
	it("should return empty string when canvas' context missing", () => {
		...
		const result = cropToDataURI(mockImage, defaultImageRect, defaultInputCropRect, 1, 1);
		expect(result).toBe('');
	});
	...
	describe('with orientation 2', () => {
		...
		it('should transform origin of context', () => {
			expect(context.translate).toHaveBeenCalledWith(defaultExpectedCrop.width, 0);
	...
	describe('with orientation 4', () => {
		...
		it('should transform origin of context', () => {
			expect(context.translate).toHaveBeenCalledWith(0, defaultExpectedCrop.height);
			expect(context.scale).toHaveBeenCalledWith(1, -1);
			expect(contextTransformationOrder).toEqual(['translate', 'scale']);
		});
	});
	...
	describe('with orientation 7', () => {
		...
		it('should transform origin of context', () => {
			...
			expect(context.rotate).toHaveBeenCalledWith(-Math.PI / 2);
			expect(context.scale).toHaveBeenCalledWith(1, -1);
			expect(contextTransformationOrder).toEqual(['translate', 'rotate', 'scale']);
		});
	});
