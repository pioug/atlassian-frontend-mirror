import { isBlank, isDigit, isNotBlank, StringBuffer } from '../../../parser/utils/text';

describe('StringBuffer', () => {
	it('should find last index of character', () => {
		expect(new StringBuffer('abcabc').lastIndexOf('b')).toEqual('abcabc'.lastIndexOf('b'));
	});

	it('should find index of character', () => {
		expect(new StringBuffer('abcabc').indexOf('b')).toEqual('abcabc'.indexOf('b'));
	});
