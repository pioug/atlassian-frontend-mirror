	...
	return (
		<>
			<ResourcedEmoji
				...
				fitToHeight={24}
				key={props.enableReMount ? `${grinEmoji.shortName}${props.count}` : undefined}
			/>
			<ResourcedEmoji
				emojiId={{
					id: emojiTest.id,
				...
				emojiProvider={props.emailProvider}
				fitToHeight={24}
				key={props.enableReMount ? `${emojiTest.shortName}${props.count}` : undefined}
			/>
			{!props.hideWrongEmojis && (
				<>
					<ResourcedEmoji
						...
						emojiProvider={props.emailProvider}
						fitToHeight={24}
						key={props.enableReMount ? `${emojiTest2.shortName}${props.count}` : undefined}
					/>
					<ResourcedEmoji
						emojiId={{
							id: wrongEmoji.id,
						...
						emojiProvider={props.emailProvider}
						fitToHeight={24}
						key={props.enableReMount ? `${wrongEmoji.shortName}${props.count}` : undefined}
					/>
				</>
			)}
		</>
...
const Parent = ({
	...
	return (
		<div data-id={count}>
			...
			<button onClick={toggleReMount}>re-mount {reMountEnabled ? 'enabled' : 'disabled'}</button>
		</div>
	);
};
