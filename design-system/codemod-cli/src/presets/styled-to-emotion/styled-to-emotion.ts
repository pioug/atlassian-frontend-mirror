...
function buildCoreImportDeclaration(j: core.JSCodeshift, path: ASTPath<ImportDeclaration>) {
	...
	j(path)
		...
		.filter((specifier: any) => coreExports.includes(specifier.value.imported.name))
...
function buildStyledImportDeclaration(j: core.JSCodeshift, path: ASTPath<ImportDeclaration>) {
	...
	return specifier && specifier.length
		? j.importDeclaration(
				[j.importDefaultSpecifier(j.identifier(specifier.get(0).node.local!.name))],
				j.literal('@emotion/styled'),
			)
		: null;
}

function buildThemingImportDeclaration(j: core.JSCodeshift, path: ASTPath<ImportDeclaration>) {
	...
	j(path)
		.find(j.ImportSpecifier)
		.filter((specifier: any) => themingExports.includes(specifier.value.imported.name))
		.forEach((specifier: any) => {
			specifiers.push(j.importSpecifier(j.identifier(specifier.value.imported.name)));
		});
