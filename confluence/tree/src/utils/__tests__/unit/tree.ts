...
describe('@atlaskit/tree - utils/tree', () => {
	...
	describe('#mutateTree', () => {
		it('mutates the root', () => {
			...
			expect(mutatedTree.items[rootId].data).toBe(treeWithThreeLeaves.items[rootId].data);
		...
		it('changes only the changed child', () => {
			...
			expect(mutatedTree.items['1-2']).not.toBe(treeWithThreeLeaves.items['1-2']);
		...
		it('changes only the changed item', () => {
			...
			expect(mutatedTree.items['1-2-1']).toBe(treeWithTwoBranches.items['1-2-1']);
			expect(mutatedTree.items['1-2-2']).not.toBe(treeWithTwoBranches.items['1-2-2']);
		...
		it('does not change if item not found', () => {
			expect(mutateTree(treeWithTwoBranches, 'notfound', { isExpanded: true })).toBe(
				treeWithTwoBranches,
			);
		});
	});
	...
	describe('#getItem', () => {
		it('returns item from the first level of tree', () => {
			expect(getItem(treeWithThreeLeaves, [1])).toBe(treeWithThreeLeaves.items['1-2']);
		});

		it('returns item from deep the tree', () => {
			expect(getItem(treeWithTwoBranches, [1, 1])).toBe(treeWithTwoBranches.items['1-2-2']);
		});
	...
	describe('#moveItemOnTree', () => {
		...
		it('should not append to subtree when destination index is not specified and children are not loaded', () => {
			const treeWithChildrenNotLoaded = mutateTree(treeWithTwoBranches, '1-2-1', {
				hasChildren: true,
			});
			const newPages = moveItemOnTree(
