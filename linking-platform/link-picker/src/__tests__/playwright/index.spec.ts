...
const testIdsToSelectors = <T extends Record<string, string>>(
  testIds: T,
): T => {
...
test('Link picker should be able to be edit link and title without plugins', async ({
  page,
}) => {
  ...
  await page
    .locator(testIds.urlInputField)
    .first()
    .fill('https://atlassian.com');
...
test('Link picker should be able to edit link and title from search results', async ({
  page,
}) => {
  ...
  await expect(page.locator('a').first()).not.toHaveJSProperty(
    'href',
    'https://google.com/',
  );
});
