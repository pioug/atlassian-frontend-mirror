...
describe('TableMap', () => {
	describe('creates correct TableMap', () => {
		...
		it.each([
			[
				'overlong rowspan',
				table()(tr(tableCell, td({ colspan: 2, rowspan: 3 })(p('1'))))(defaultSchema),
				[{ type: 'overlong_rowspan', pos: 6, n: 2 }],
			],
			[
				'missing',
				table()(tr(tableCell, td({ colspan: 3, rowspan: 1 })(p('1'))), tr(tableCell, tableCell))(
					defaultSchema,
				),
	...
	describe('finds the right shape', () => {
		it.each([
			...
			[
				'for deep rowspans',
				table()(
					tr(td({ colspan: 1, rowspan: 3 })(p('1')), td({ colspan: 2, rowspan: 1 })(p('1'))),
					tr(td({ colspan: 1, rowspan: 2 })(p('1')), td({ colspan: 1, rowspan: 2 })(p('1'))),
					tr(td()(p(''))),
				)(defaultSchema),
	...
	describe('given table with different colspans and rowspans', () => {
		const tableNode = table()(
			tr(td({ colspan: 2, rowspan: 3 })(p('1')), tableCell, td({ colspan: 1, rowspan: 2 })(p('1'))),
			tr(tableCell),
		...
		it('can find the rectangle between two cells', () => {
			expect(map.cellsInRect(map.rectBetween(1, 6)).join(', ')).toBe('1, 6, 18, 25');
			expect(map.cellsInRect(map.rectBetween(1, 25)).join(', ')).toBe('1, 6, 11, 18, 25');
			expect(map.cellsInRect(map.rectBetween(1, 1)).join(', ')).toBe('1');
			expect(map.cellsInRect(map.rectBetween(6, 25)).join(', ')).toBe('6, 11, 18, 25');
			expect(map.cellsInRect(map.rectBetween(6, 11)).join(', ')).toBe('6, 11, 18');
			expect(map.cellsInRect(map.rectBetween(11, 6)).join(', ')).toBe('6, 11, 18');
			expect(map.cellsInRect(map.rectBetween(18, 25)).join(', ')).toBe('18, 25');
