...
import { createCollabProviderFactory, StorageImpl, useCollabProvider } from '../../collab-provider';
...
function setupFactory(allowCollabProvider: boolean) {
	const config = new EditorConfiguration(JSON.stringify({ allowCollabProvider }));
	const bridge = new WebBridgeImpl();
	...
	const TestComponent = (props: React.PropsWithChildren<Props>) => {
		const configuration = useEditorConfiguration(props.bridge, props.editorConfiguration);
...
describe('useCollabProvider', () => {
	...
	it.skip('should create the collab provider if enabled', () => {
		...
		expect(
			renderResult.root.findAll((node) => node.props['data-test-id'] === 'has-collab').length,
		).toEqual(1);
	});
	...
	it.skip('should not create the collab provider if disabled', () => {
		...
		expect(
			renderResult.root.findAll((node) => node.props['data-test-id'] === 'no-collab').length,
		).toEqual(1);
	});
