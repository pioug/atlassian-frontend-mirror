	...
	describe('Comment', () => {
		...
		describe('props', () => {
			...
			describe('author prop', () => {
				it('should render the author in the correct container', () => {
					...
					expect(screen.getByTestId('comment-header')).toHaveTextContent('Joshua Nelson');
				});
			});
			...
			describe('content prop', () => {
				it('should render the provided content in the correct container', () => {
					const content = <p data-testid="comment-content">My sample content</p>;
			...
			describe('time prop', () => {
				it('should render the time in the correct container', () => {
					...
					expect(screen.getByTestId('comment-header')).toHaveTextContent('30 August, 2016');
				});
			});
			...
			describe('edited prop', () => {
				it('should render edited correctly', () => {
					...
					expect(screen.getByTestId('comment-header')).toHaveTextContent('Edited');
				});
			});
			...
			describe('type prop', () => {
				it('should render a Lozenge with the type in the correct container', () => {
					...
					expect(screen.getByTestId('comment-header')).toHaveTextContent('type');
				});
			});
			...
			describe('restrictedTo prop', () => {
				it('should render a Lock icon and restrictedTo text when supplied', () => {
					...
					expect(screen.getByTestId('comment')).toHaveTextContent(/atlassian\-staff/);
				});
			...
			describe('isSaving and savingText props', () => {
				describe('if isSaving prop is set', () => {
					it('should render the default savingText if no savingText is set', () => {
						...
						expect(screen.getByTestId('comment-header')).toHaveTextContent(/Sending\.\.\./);
					});

					it('should render the savingText text if it is set', () => {
						render(<Comment avatar="" isSaving savingText="Saving..." testId="comment" />);

						expect(screen.getByTestId('comment-header')).toHaveTextContent(/Saving\.\.\./);
					});
					...
					it('should not render CommentActions', () => {
						...
						expect(screen.queryByTestId('comment-footer')).not.toBeInTheDocument();
					});
				});

				describe('if isSaving prop is not set', () => {
					it('should not render savingText', () => {
						render(<Comment avatar="" savingText="Saving..." testId="comment" />);

						expect(screen.getByTestId('comment')).not.toHaveTextContent(/Saving\.\.\./);
					});
				});
			});
			...
			describe('isError, errorActions and errorLabel props', () => {
				...
				describe('if isError prop is set', () => {
					it('should render the default (empty) if no errorIconLabel is set', () => {
						const { container } = render(
							<Comment avatar="" isError errorActions={errorActions} testId="comment" />,
						);
				...
				describe('if isError prop is not set', () => {
					it('should not render the icon and errorActions', () => {
						const { container } = render(<Comment avatar="" errorActions={errorActions} />);
			...
			describe('headingLevel prop', () => {
				it('should change the heading level correctly', () => {
					render(<Comment avatar="" headingLevel="5" type="hello" author="DDC" testId="comment" />);
		...
		describe('nesting', () => {
			it('should reflect children to the CommentLayout', () => {
				const childComment = <Comment avatar="" content="child" testId="child" />;
				render(
