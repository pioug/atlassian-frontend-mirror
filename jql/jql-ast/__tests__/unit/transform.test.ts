...
const testCases: TestCase[] = [
  ...
  },
  {
    ...
    expected:
      'assignee is empty and issuetype = bug and status = open ORDER BY created',
    transform: (query: Query): void => {
      query.appendClause(getStatusEqualsOpenClause(), COMPOUND_OPERATOR_AND);
    },
  },
  ...
  },
  {
    ...
    expected:
      '(assignee is empty or issuetype = bug) and status = open ORDER BY created',
    transform: (query: Query): void => {
      query.appendClause(getStatusEqualsOpenClause(), COMPOUND_OPERATOR_AND);
    },
  },
  ...
  },
  {
    ...
    input: 'assignee is empty ORDER BY created',
    expected:
      'assignee is empty ORDER BY assignee, status DESC, issuetype ASC, created',
    transform: (query: Query): void => {
      query.prependOrderField(getOrderByTypeField());
  ...
  },
  {
    ...
    input: 'NOT (assignee is empty and issuetype = bug or not status = open)',
    expected:
      'not ((assignee is empty and issuetype = bug) or not created > -1w)',
    transform: {
      enterTerminalClause: (terminalClause: TerminalClause) => {
        if (terminalClause.field.value === 'status') {
  ...
  },
  {
    ...
    input:
      'assignee is empty and issuetype = bug and not reporter = currentUser()',
