...
certain actions and entities across Atlassian products. Use icons following these guidelines:
...
clear and established associations to features or concepts.
...
>
	Use caution when associating a new metaphor with an icon that isn’t widely established. It may be
	confused with another concept or may not be understood.
</DoDont>
...
descriptions in the [icon explorer](/components/icon/) or in Figma.
...
Most of the time icons are associated with a button or element that also has a text label. In these
cases, you don’t need to provide alternative text for the icon, because the label clarifies the
meaning of the icon.

If an icon doesn’t have an existing text label or accessible text, provide a clear label with the
`label` prop.

Sometimes an icon communicates additional information about a feature, such as a warning or external
link icon. In these cases, make sure that information is available to people using assistive
technology, either through the icon label or through other methods.
...
The majority of icons in products are 16 by 16px. Add padding equally around an icon following our
spacing system guidance to make icons fit larger spaces.

![Icon with padding around it.](./images/icon-spacing.png)

### Utility 12px icons

A select few icons are available at a smaller size for visual alignment or affordance in small
spaces. Because the size is smaller and harder to read, we only offer certain system icons at this
size.
...
color. Consider using [icon tile](/components/icon/icon-tile/) for icons larger than 16px.
