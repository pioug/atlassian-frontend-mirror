...
Sorting a dynamic table takes place using the `key` set on each cell. Be wary the that type of `key` will
...
An empty state can be supplied to the dynamic table via the `emptyView` prop.
Use empty states to communicate that the table has no content to show.
...
Drag and drop functionality is built into dynamic table. This is enabled via the `isRankable` prop.
...
This should not be used to reflect selection.
