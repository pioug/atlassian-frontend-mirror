...
const collapseSidebarIfOpen = async (page: Page) => {
  if (
    (await page.locator(resizeControl).getAttribute('aria-expanded')) === 'true'
  ) {
...
test('Hovering the sidebar opens the flyout', async ({ page }) => {
  ...
  await expect(page.locator(resizeControl)).toHaveAttribute(
    'aria-expanded',
    'false',
  );
...
test('The flyout is not locked by default', async ({ page }) => {
  ...
  const mainContentBounds = await page
    .locator(mainContent)
    .first()
    .boundingBox();
  const sideNavigationBounds = await page
    .locator(sideNavigation)
    .first()
    .boundingBox();
