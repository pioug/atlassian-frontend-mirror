...
export function Droppable({
  ...
  useEffect(() => {
    ...
    invariant(
      ...
      'innerRef must provide an `HTMLElement`',
    );
    ...
    return combine(
      ...
      dropTargetForElements({
        ...
        canDrop({ source }) {
          ...
          return (
            source.data.contextId === contextId && source.data.type === type
          );
        },
        onDragLeave() {
  ...
  const element = elementRef.current;
  const shouldPortalDropIndicator =
    isDraggingOver && mode === 'virtual' && element;
