	...
	describe('fitting rectangles', () => {
		jsc.property(
			...
			(w1, h1, w2, h2) => {
				...
				const fitted = original.scaled(original.scaleToFitLargestSide(containing));
				expect(
		...
		jsc.property(
			...
			(w1, h1, w2, h2) => {
				const containing = new Rectangle(w1, h1);
				const original = new Rectangle(w2, h2);
				const fitted = original.scaled(original.scaleToFitSmallestSide(containing));
				expect(
					Math.round(fitted.width) === Math.round(containing.width) ||
						Math.round(fitted.height) === Math.round(containing.height),
				).toBeTruthy();
		...
		jsc.property(
			...
			(w1, h1, w2, h2) => {
				...
				const original = new Rectangle(w2, h2);
				const fitted = original.scaled(original.scaleToFitSmallestSide(containing));
				expect(original.aspectRatio - fitted.aspectRatio <= ACCEPTABLE_FLOATING_ERROR).toBeTruthy();
				return original.aspectRatio - fitted.aspectRatio <= ACCEPTABLE_FLOATING_ERROR;
			},
		);
	});
});
	...
	describe('scaledImg', () => {
		jsc.property(
			...
			(side1, side2, side3, side4, scale) => {
				...
				expect(fitted.width < upscaled.width && fitted.height < upscaled.height).toBeTruthy();
		...
		jsc.property(
			...
			(side1, side2, side3, side4, scale) => {
				...
				expect(
					originalImg.width > downscaled.width && originalImg.height > downscaled.height,
				).toBeTruthy();
				return originalImg.width > downscaled.width && originalImg.height > downscaled.height;
			},
		);
	});
