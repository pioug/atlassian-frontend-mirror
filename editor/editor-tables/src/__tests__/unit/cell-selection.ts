...
describe('CellSelection.content', () => {
	...
	it('contains only the selected cells', () => {
		const tbl = createTable(tr(c11, cAnchor, cEmpty), tr(c11, cEmpty, cHead), tr(c11, c11, c11));
		const a = selectionFor(tbl)!.content();
		const b = slice(table()(tr(cAnchor, cEmpty), tr(cEmpty, cHead))(defaultSchema));

		expect(a.eq(b)).toEqual(true);
	});

	it('understands spanning cells', () => {
		const tbl = createTable(tr(cAnchor, c(2, 2), c11, c11), tr(c11, cHead, c11, c11));
		const a = selectionFor(tbl)!.content();
	...
	it('cuts off cells sticking out horizontally', () => {
		const tbl = createTable(tr(c11, cAnchor, c(2, 1)), tr(c(4, 1)), tr(c(2, 1), cHead, c11));
		const a = selectionFor(tbl)!.content();
		const b = slice(createTable(tr(cAnchor, c11), tr(td({ colspan: 2 })(p())), tr(cEmpty, cHead)));

		expect(a.eq(b)).toEqual(true);
	});

	it('cuts off cells sticking out vertically', () => {
		const tbl = createTable(tr(c11, c(1, 4), c(1, 2)), tr(cAnchor), tr(c(1, 2), cHead), tr(c11));
		const a = selectionFor(tbl)!.content();
		const b = slice(createTable(tr(cAnchor, td({ rowspan: 2 })(p()), cEmpty), tr(c11, cHead)));
	...
	it('preserves column widths', () => {
		...
		const a = selectionFor(tbl)!.content();
		const b = slice(createTable(tr(cAnchor), tr(td({ colwidth: [200] })(p())), tr(cHead)));
...
describe('isRowSelection', () => {
	describe('when there is a merged cell in the first column', () => {
		const tbl = createTableWithDoc(
			...
			tr(/* empty */ /* selection starts here */ cAnchor, /* selection ends here */ cHead),
