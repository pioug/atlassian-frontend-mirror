...
scenarios.forEach(({ axis, side }) => {
  describe(`Distance dampening on [axis: ${axis}, side: ${side}]`, () => {
    const scrollProperty = axis === 'vertical' ? 'scrollTop' : 'scrollLeft';
    const mainAxisClientPoint = axis === 'vertical' ? 'clientY' : 'clientX';
    const crossAxisClientPoint =
      mainAxisClientPoint === 'clientY' ? 'clientX' : 'clientY';

    const { mainAxis, crossAxis } = axisLookup[axis];

    it('should dampen acceleration based on the distance away from an edge', () => {
      ...
      const maxScrollInDirection =
        side === 'end' ? maxScrollPerFrame : -maxScrollPerFrame;
      {
        const before = parentScrollContainer[scrollProperty];
        advanceTimersToNextFrame();
        stepScrollBy();
      ...
      let currentMainAxisClientPoint =
        side === 'start'
          ? autoScrollHitbox[mainAxis.end]
          : autoScrollHitbox[mainAxis.start];
          ...
          const situation = (() => {
            ...
            if (
              ...
              currentMainAxisClientPoint <
                autoScrollHitbox[mainAxis.end] - maxSpeedBuffer
            ) {
              return 'accelerating';
            }
            ...
            if (
              side === 'start' &&
              currentMainAxisClientPoint >
                autoScrollHitbox[mainAxis.start] + maxSpeedBuffer
            ) {
              return 'accelerating';
            }
      ...
      // asserting that acceleration occurred, and that we where not just on a single speed
      {
        const uniques = Array.from(
          new Set(casesHit.accelerating.map(value => Math.round(value))),
        );
        const expected = Array.from(
          { length: maxScrollPerFrame },
          (_, index) => {
            const value = index + 1;
