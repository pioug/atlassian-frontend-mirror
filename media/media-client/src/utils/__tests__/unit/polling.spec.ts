...
describe('Polling Function', () => {
	...
	it('should use default options if none given', () => {
		...
		expect(poll.options.poll_intervalMs).toBe(defaultPollingOptions.poll_intervalMs);
		expect(poll.options.poll_maxAttempts).toBe(defaultPollingOptions.poll_maxAttempts);
		expect(poll.options.poll_backoffFactor).toBe(defaultPollingOptions.poll_backoffFactor);
	});
	...
	it.each([
		...
		(iteration, expectedIntervalMs) => {
			...
			expect(poll.getIntervalMsForIteration(iteration)).toBe(expectedIntervalMs);
		},
	);
