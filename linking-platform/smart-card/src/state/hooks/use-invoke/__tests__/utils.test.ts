...
describe('getInvokeFailureReason', () => {
  it('returns unknown reason if error is not invoke error', () => {
    expect(getInvokeFailureReason(new Error())).toBe(
  ...
  it('returns permission error if error is 403', () => {
    expect(
      getInvokeFailureReason(new InvokeError('You shall not passed!', 403)),
    ).toBe(TrackQuickActionFailureReason.PermissionError);
  });
...
describe('isInvokeCustomError', () => {
  it('return true if error is an InvokeError', () => {
    expect(isInvokeCustomError(new InvokeError('Yes, yes, I am.', 500))).toBe(
      true,
    );
  });
