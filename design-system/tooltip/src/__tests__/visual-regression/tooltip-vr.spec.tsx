...
describe('@atlaskit/tooltip', () => {
	it('should match production example', async () => {
		const url = getExampleUrl('design-system', 'tooltip', 'default-tooltip', global.__BASEURL__);
		const { page } = global;
	...
	it('should allow a custom tooltip', async () => {
		const url = getExampleUrl('design-system', 'tooltip', 'component-prop', global.__BASEURL__);
		const { page } = global;
		await loadPage(page, url, {
			reloadSameUrl: true,
			allowedSideEffects: { tooltips: true },
	...
	it('should work with render props', async () => {
		const url = getExampleUrl('design-system', 'tooltip', 'render-props', global.__BASEURL__);
		const { page } = global;
		await loadPage(page, url, {
			reloadSameUrl: true,
			allowedSideEffects: { tooltips: true },
	...
	it('should dynamically position with mouse positioning', async () => {
		const url = getExampleUrl('design-system', 'tooltip', 'position', global.__BASEURL__);
		const { page } = global;
		await loadPage(page, url, {
			reloadSameUrl: true,
			allowedSideEffects: { tooltips: true },
