...
export const useHydrateJqlQuery = (cloudId: string, jql: string): HydrateJqlState => {
	const [hydratedOptions, setHydratedOptions] = useState<HydrateJqlState['hydratedOptions']>({});
	...
	const fetchHydratedJqlOptions = useCallback(async () => {
		try {
			...
			const { assignee: mappedHydratedAssigneeValue, ...restOfMappedHydratedResponse } =
			...
			const mappedValues = {
				...
				...(textFieldValue ? { basicInputTextValue: removeFuzzyCharacter(textFieldValue) } : {}),
			};
