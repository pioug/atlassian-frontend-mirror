...
export const catchup = async (opt: CatchupOptions) => {
	...
	try {
		...
		opt.analyticsHelper?.sendErrorEvent(error, 'Error while fetching catchup from server');
		logger(`Fetch catchup from server failed:`, (error as InternalError).message);
		throw error;
	}
	...
	try {
			...
			if (typeof serverVersion === 'undefined') {
				...
				if (unconfirmedSteps?.length) {
					...
					const stepMaps = serverStepMaps!.map(
						({ ranges, inverted }: { ranges: [number, number, number]; inverted: boolean }) => {
					...
					logger(
						`${unconfirmedSteps.length} unconfirmed steps before rebased: ${JSON.stringify(
							unconfirmedSteps,
						)}`,
					);
					const newUnconfirmedSteps: Step[] = rebaseSteps(unconfirmedSteps, mapping);

					if (newUnconfirmedSteps?.length < unconfirmedSteps.length) {
						// Log the dropped steps after rebase
						opt.analyticsHelper?.sendActionEvent(EVENT_ACTION.DROPPED_STEPS, EVENT_STATUS.SUCCESS, {
							numOfDroppedSteps: unconfirmedSteps.length - newUnconfirmedSteps?.length,
						});
					}
					logger(
						`Re-aply ${
		...
		opt.analyticsHelper?.sendErrorEvent(error, 'Failed to apply catchup result in the editor');
