...
import { deviceViewPorts, Device } from '@atlaskit/editor-test-helpers/vr-utils/device-viewport';
/* eslint-disable import/no-extraneous-dependencies -- Removed from package.json to fix  circular depdencies */
...
export async function goToRendererTestingExample(page: PuppeteerPage, enableClickToEdit?: boolean) {
...
export const waitForText = async (page: PuppeteerPage, selector: string, text: string) =>
...
export async function setScrollPosition(
	...
	const { left, top } = await page.evaluate((scrollContainerSelector: string) => {
		const scrollContainer = document.querySelector(scrollContainerSelector) as HTMLElement;
		if (!scrollContainer) {
			if (!scrollContainer) {
				return {};
	...
	if (left !== posX || top !== posY) {
		if (retryCount > 1) {
			return await setScrollPosition(page, scrollContainerSelector, posX, posY, --retryCount);
		}
		throw new Error(
...
export async function getMaxScrollWidth(page: PuppeteerPage, scrollContainerSelector: string) {
