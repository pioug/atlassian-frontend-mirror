...
describe('Snapshot Test', () => {
	it('Basic should match production example', async () => {
		const url = getExampleUrl('design-system', 'pagination', 'basic#', global.__BASEURL__);
		const { page } = global;
		...
		await page.waitForSelector('button[data-testid="pagination--right-navigator"] svg');
		...
		const imageAfterClickingFivePage = await takeElementScreenShot(page, paginationSelector);
		expect(imageAfterClickingFivePage).toMatchProdImageSnapshot();

		await page.click(nextPageBtnSelector);
		const imageAfterClickingNextButton = await takeElementScreenShot(page, paginationSelector);
		expect(imageAfterClickingNextButton).toMatchProdImageSnapshot();

		await page.click(getPageSelectorForPage(7));
		const imageAfterClickingPageSeven = await takeElementScreenShot(page, paginationSelector);
		expect(imageAfterClickingPageSeven).toMatchProdImageSnapshot();

		await page.click(prevPageBtnSelector);
		const imageAfterClickingPrevButton = await takeElementScreenShot(page, paginationSelector);
		expect(imageAfterClickingPrevButton).toMatchProdImageSnapshot();

		await page.click(prevPageBtnSelector);
		await page.click(getPageSelectorForPage(4));
		const imageAfterHoverPageFourButton = await takeElementScreenShot(page, paginationSelector);
		expect(imageAfterHoverPageFourButton).toMatchProdImageSnapshot();

		await page.click(getPageSelectorForPage(4));
		const imageAfterClickPageFourButton = await takeElementScreenShot(page, paginationSelector);
