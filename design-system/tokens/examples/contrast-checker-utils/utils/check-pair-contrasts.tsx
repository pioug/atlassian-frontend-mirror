...
function ArrayToRGBA(array: [number, number, number, number] = [0, 0, 0, 0]): RGBA {
	return {
		r: array[0],
		g: array[1],
		b: array[2],
...
export default function checkThemePairContrasts(
	...
	// Iterate over each pairing of token types
	pairsToCheck.forEach(({ foreground, background, desiredContrast, layeredTokens }) => {
		const [foregroundMetadata, backgroundMetadata] = [foreground, background].map(
			(tokenName: string) => rawTokenSet.find((rawToken) => rawToken.cleanName === tokenName),
		);
