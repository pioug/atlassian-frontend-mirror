...
describe('splitCell', () => {
	...
	it('can split a row-spanning cell', () => {
		const input = createTable(tr(c11, td({ rowspan: 2 })(p('foo{anchor}')), c11), tr(c11, c11));
		const result = createTable(tr(c11, td()(p('foo')), c11), tr(c11, cEmpty, c11));
		testCommand(input, splitCell, result);
	});
	...
	it('distributes column widths', () => {
		const input = createTable(tr(td({ colspan: 3, colwidth: [100, 0, 200] })(p('a{anchor}'))));
		const result = createTable(
	...
	describe('with custom cell type', () => {
		...
		it('can split a row-spanning header cell into a header and normal cell ', () => {
			const input = createTable(tr(c11, td({ rowspan: 2 })(p('foo{anchor}')), c11), tr(c11, c11));
			const result = createTable(tr(c11, th()(p('foo')), c11), tr(c11, cEmpty, c11));
