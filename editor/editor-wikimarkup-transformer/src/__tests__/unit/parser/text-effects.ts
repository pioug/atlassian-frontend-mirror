...
describe('JIRA wiki markup - Text effects', () => {
	const testCases: Array<[string, string]> = [
		['should find strong marks in the text', 'This is a string with a *strong* text'],
		[
...
another one is here and it should* not be applied`,
		...
		["should use code mark first if it's the inner mark", 'This is a *bold {{monospace}} thingy*'],
		[
...
https://app.datadoghq.com/screen/282018/product-fabric-adf-service?tv_mode=true#close`,
		...
		['should not apply strong when ending line is two strong symbols', '*not valid strong **'],
		[
		...
		['should not let auto-links end with a period', 'text prefix https://example.com. text suffix'],
		['should not let auto-links end with a bang', 'text prefix https://example.com! text suffix'],
		[
		...
		['should auto-link ftp urls', 'text prefix ftp://example.com/~abc/ text suffix'],
		[
