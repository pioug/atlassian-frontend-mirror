...
describe('NameCell', () => {
	it('renders one Truncate component with correct text', async () => {
		...
		expect(nameCell.find(Truncate).get(0).props.text).toBe('This is the file name.png');
	});
	...
	it('renders one Tooltip component with correct content', async () => {
		...
		expect(nameCell.find(Tooltip).get(0).props.content).toBe('This is the file name.png');
	});

	it('renders one MediaTypeIcon component with the correct mediaType', async () => {
		const nameCell = shallow(<NameCell text="This is the file name.png" mediaType="image" />);
