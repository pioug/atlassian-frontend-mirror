...
Use a subtle link if the default link appearance is too prominent.

The underline is excluded in resting state, but will display on hover.

Ensure the surrounding context makes it clear that it is interactive, such as in navigation or
breadcrumbs. Avoid using this in body copy as it may be difficult to distinguish from the
surrounding text.
