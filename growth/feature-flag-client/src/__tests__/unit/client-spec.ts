...
describe('Feature Flag Client', () => {
	...
	const createDefaultClient = (isAutomaticExposuresEnabled: boolean = true): FeatureFlagClient => {
	...
	const assertCustomExposureSent = (
		...
		expect(analyticsHandler.sendOperationalEvent).toHaveBeenCalledWith({
			...
			tags: ['measurement', ExposureTriggerReason.hasCustomAttributes, exposureTag],
			highPriority: true,
			source: '@atlaskit/feature-flag-client',
		});
	};
	...
	describe('bootstrap', () => {
		...
		test('should allow to set flags later', () => {
			...
			expect(client.getBooleanValue('my.first.flag', { default: false })).toBe(true);
			expect(
	...
	describe('getters', () => {
		describe('getBooleanValue', () => {
			test('should throw if called without default', () => {
				expect(() => expect(client.getBooleanValue('my.flag', {} as any))).toThrow(
					'getBooleanValue: Missing default',
				);
			});

			test('should return default if flag is not set', () => {
				expect(client.getBooleanValue('missing.flag', { default: false })).toBe(false);
			});
			...
			test('should return default if flag is not boolean', () => {
				expect(client.getBooleanValue('detailed.variation', { default: false })).toBe(false);

				expect(client.getBooleanValue('variation', { default: false })).toBe(false);
			});
			...
			test('should fire the exposure event if the flag contains evaluation details (long format / feature flag)', () => {
				expect(client.getBooleanValue('detailed.boolean', { default: true })).toBe(false);
		...
		describe('getVariantValue', () => {
			...
			test('should throw if called without oneOf', () => {
				expect(() => client.getVariantValue('my.flag', { default: 'control' } as any)).toThrow(
					'getVariantValue: Missing oneOf',
				);
			});
		...
		describe('getFlagEvaluation', () => {
			test('should throw if called without default', () => {
				expect(() => client.getFlagEvaluation('missing.flag', {} as any)).toThrow(
					'getRawValue: Missing default',
				);
			});
	...
	describe('exposures', () => {
		...
		describe('trackFeatureFlag', () => {
			...
			const callTrackFeatureFlagWithFlagData = (flags: { [flagKey: string]: FlagShape }) => {
			...
			const triggerAutomaticExposureAndAssert = (
				...
				expect(sendAutomaticExposureSpy).toHaveBeenCalledTimes(Object.keys(flags).length);
				Object.entries(flags).forEach(([key, { value, explanation }]) =>
					expect(sendAutomaticExposureSpy).toHaveBeenCalledWith(key, value, explanation),
				);
			};
			...
			test('should call sendAutomaticExposure when flagValue from flags if falsey', async () => {
				triggerAutomaticExposureAndAssert(flasey_flags, callTrackFeatureFlagWithoutFlagData);
			});

			test('should call sendAutomaticExposure when flagValue from options if falsey', async () => {
				triggerAutomaticExposureAndAssert(flasey_flags, callTrackFeatureFlagWithFlagData);
			});
