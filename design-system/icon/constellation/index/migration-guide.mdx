...
#### Deprecated props: `LEGACY_size` and `LEGACY_secondaryColor`

Some props from the legacy icons are no longer available in the new icons, such as `size` and
`secondaryColor`. To control these, use `LEGACY_size` and `LEGACY_secondaryColor` props:
