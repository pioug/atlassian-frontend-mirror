...
These challenges are why overflow scrolling is considered unsafe.
It should only be used when there is a strong product justification.
...
import { autoScrollForNative } from '@atlaskit/pragmatic-drag-and-drop-auto-scroll/file';
...
- Distance dampening is applied when in the "cross axis" component of an edges hitbox. This is so that the scroll speed matches on the cross axis plane when moving in and out of an element. This sounds confusing, but it feels great!

![Cross axis hitbox](images/from-top-edge-example.jpg)

## Window scrolling

Overflow scrolling is not applicable for window scrolling as the browser stops publishing events to the document once the user has left the window. If you want to have window scrolling, then use our standard approach for [registering auto scrolling for the window](./about#registering-auto-scrolling-for-the-window).
