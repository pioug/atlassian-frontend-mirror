...
describe('APS', () => {
	...
	describe('#subscribe', () => {
		...
		it('should be able to fallback from websocket to http', async () => {
			...
			mockHttpTransport.subscribe.mockImplementation(() => Promise.resolve());
			mockWebsocketTransport.subscribe.mockImplementation(() => Promise.reject('mock error'));
		...
		it('should be able to fallback from http to websocket', async () => {
			...
			mockWebsocketTransport.subscribe.mockImplementation(() => Promise.resolve());
			mockHttpTransport.subscribe.mockImplementation(() => Promise.reject('mock error'));
	...
	describe('#url', () => {
		it('should use default relative path when constructor param is not passed', () => {
			const apsProtocol = new APSProtocol();
			expect(apsProtocol.url.toString()).toBe('https://jdog.jira-dev.com/gateway/wss/fps');
		});
