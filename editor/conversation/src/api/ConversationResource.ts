...
export class ConversationResource extends AbstractConversationResource {
	...
	async create(
		...
		try {
			result = await this.makeRequest<Conversation>('/conversation?expand=comments.document.adf', {
				method: 'POST',
				body: JSON.stringify({
					objectId,
					containerId,
	...
	protected getComment(conversationId: string, commentId: string): Comment | undefined {
		...
		const [conversation] = state.conversations.filter((c) => c.conversationId === conversationId);
		if (!conversation) {
			return undefined;
		}

		const [comment] = (conversation.comments || []).filter((c) => c.commentId === commentId);
		return {
			...comment,
		};
	}
