        ...
        errorBoundary(
          () => {
            ...
            function findObjectStyles(node: EslintNode): void {
              ...
              if (
                domains.length === 0 ||
                isDecendantOfGlobalToken(node.value)
              ) {
                return;
              }

              if (isNodeOfType(node.value, 'TemplateLiteral')) {
                const value = getValueFromTemplateLiteralRaw(
                  node.value,
                  context,
                );
            ...
            () => {
              ...
              const completeSource = processedCssLines.reduce(
                (currentSource, [resolvedCssLine, originalCssLine]) => {
                  ...
                  const replacedValuesPerProperty: string[] = [
                    originalProperty,
                  ];
