...
export class Viewport {
	...
	private applyConstraints() {
		...
		if (itemBounds.right > innerBounds.right && itemBounds.left > innerBounds.left) {
			x += deltaLeft;
		}
		if (itemBounds.bottom > innerBounds.bottom && itemBounds.top > innerBounds.top) {
			y += deltaTop;
		}
		if (itemBounds.top < innerBounds.top && itemBounds.bottom < innerBounds.bottom) {
			y += deltaBottom;
		}
		if (itemBounds.left < innerBounds.left && itemBounds.right < innerBounds.right) {
			x += deltaRight;
		}
	...
	setScale(scale: number) {
		...
		if (scale <= 1) {
			...
			const width =
				fittedItemBounds.width + (maxItemViewRect.width - fittedItemBounds.width) * floatingScale;
			const height =
				fittedItemBounds.height +
				(maxItemViewRect.height - fittedItemBounds.height) * floatingScale;
			const scaledBounds = new Bounds(itemBounds.x, itemBounds.y, width, height);
			...
			this.itemBounds = new Bounds(left, top, scaledBounds.width, scaledBounds.height);
