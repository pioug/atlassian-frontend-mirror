...
describe('takeSnapshot()', () => {
	...
	it('gracefully handle error loading video', async () => {
		...
		await expect(takeSnapshot(videoFile)).rejects.toStrictEqual(new Error('failed to load video'));
	});
	...
	it('gracefully handle error playing video', async () => {
		...
		await expect(takeSnapshot(videoFile)).rejects.toStrictEqual(new Error('failed to play video'));
	});
