...
function PresetPicker({
  ...
  const isDatabasePreset = useMemo(() => {
    return (
      disableDisplayDropdown &&
      overrideEntityTypes &&
      JSON.stringify(overrideEntityTypes.sort()) ===
        JSON.stringify(databaseATIs)
    );
...
export default () => {
  ...
  >(null);
  const [overrideEntityTypes, setOverrideEntityTypes] = useState<
    string[] | undefined
  >(undefined);
  ...
  useEffect(() => {
    ...
    if (generatedAdf.type === 'blockCard') {
      setParameters(
        generatedAdf.attrs.datasource
          .parameters as ConfluenceSearchDatasourceParameters,
      );
      setVisibleColumnKeys(
        generatedAdf.attrs.datasource.views[0].properties?.columns.map(
          c => c.key,
        ),
      );
    }
