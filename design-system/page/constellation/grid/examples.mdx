...
If a nested `Grid` is inside of a `GridColumn` with a provided value for `medium`, then the nested `Grid` will use that `medium` value as its default number of columns.
