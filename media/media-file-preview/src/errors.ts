...
const getImageLoadPrimaryReason = (
  source?: MediaFilePreview['source'],
): ImageLoadPrimaryReason => {
  switch (source) {
    case 'cache-remote':
...
export function isMediaFilePreviewError(
  err: Error,
): err is MediaFilePreviewError {
...
export const isUnsupportedLocalPreviewError = (err: Error) =>
  isMediaFilePreviewError(err) &&
  err.primaryReason === 'local-preview-unsupported';
