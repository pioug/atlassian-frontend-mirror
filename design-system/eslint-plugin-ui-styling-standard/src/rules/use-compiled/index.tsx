...
const SUPPORTED_EMOTION_IMPORTS = new Set([
  'css',
  'keyframes',
  'ClassNames',
  'jsx',
...
function isSupportedImportSource(
  importSource: string,
): importSource is SupportedImportSource {
...
export const rule: Rule.RuleModule = createLintRule({
  meta: {
    ...
    docs: {
      description:
        'Ensures usage of `@compiled/react` instead of other CSS-in-JS libraries',
      recommended: true,
      severity: 'warn',
    },
    messages: {
      ...
      Program() {
        ...
        context.report({
          ...
          *fix(fixer) {
            ...
            const hasEmotionImport = importDeclarations.some(
              (importDeclaration) => {
            ...
            const hasCompiledImport = importDeclarations.some(
              (importDeclaration) =>
                importDeclaration.source.value === '@compiled/react',
            );
  ...
  if (compiledImport) {
    ...
    yield fixer.replaceText(
      ...
      buildImportDeclaration(
        compiledImport.specifiers.concat(specifiers),
        '@compiled/react',
      ),
    );
  } else {
    yield fixer.replaceText(
      node,
      buildImportDeclaration(specifiers, '@compiled/react'),
    );
  }
}
