...
describe('useScrollbarWidth()', () => {
	...
	it('should return the difference of offset and scroll width', () => {
		const { getByText } = render(<Container refStub={{ offsetWidth: 100, scrollWidth: 80 }} />);
	...
	it('should rerender twice when setting width', () => {
		// Twice for initial.
		const onRender = jest.fn();
		render(<Container onRender={onRender} refStub={{ offsetWidth: 100, scrollWidth: 80 }} />);
	...
	it('should only rerender three when width has not changed', () => {
		...
		const { rerender } = render(
			<Container onRender={onRender} refStub={{ offsetWidth: 100, scrollWidth: 80 }} />,
		);

		rerender(<Container onRender={onRender} refStub={{ offsetWidth: 100, scrollWidth: 80 }} />);
