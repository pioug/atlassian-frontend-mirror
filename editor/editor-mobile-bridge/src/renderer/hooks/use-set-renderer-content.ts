...
function parseContent(content: Serialized<JSONDocNode>): JSONDocNode | undefined {
...
export function useRendererContent(initialDocument: string | JSONDocNode): DocNode {
	...
	useLayoutEffect(() => {
		...
		mobileBridgeEventDispatcher.on(EmitterEvents.SET_RENDERER_CONTENT, updateDocument);

		return () => {
			mobileBridgeEventDispatcher.off(EmitterEvents.SET_RENDERER_CONTENT, updateDocument);
		};
	}, []);
