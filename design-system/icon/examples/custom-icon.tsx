...
function CanonicalGlyph({ role }: CustomGlyphProps) {
	return (
		<svg width="24" height="24" viewBox="0 0 24 24" role={role}>
			<path
				fill="currentColor"
...
function CanonicalIcon({ label, size, primaryColor }: GlyphProps) {
...
export default function CustomIcon() {
	return (
		<div id="icon">
			...
			<CanonicalIcon label="" size="small" primaryColor={token('color.icon.brand')} />
			<CanonicalIcon label="" size="medium" primaryColor={token('color.icon.brand')} />
			<CanonicalIcon label="" size="large" primaryColor={token('color.icon.brand')} />
		</div>
	);
}
