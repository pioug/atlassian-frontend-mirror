...
describe.skip('WikiMarkup Transformer', () => {
	describe('strong', () => {
		...
		checkParseEncodeRoundTrips(
			...
			defaultSchema,
			WIKI_NOTATION,
			doc(p('well, both ', strong('space'), ' and ', strong('page'), ' words are in bold!')),
		);
		...
		checkParseEncodeRoundTrips(
			...
			defaultSchema,
			WIKI_NOTATION,
			doc(p('a bold letter ', strong('x'), ' followed by ', strong('more bold text'))),
		);
		...
		checkParseEncodeRoundTrips(
			...
			defaultSchema,
			WIKI_NOTATION,
			doc(p(strong('This should be strong'), '&nbsp; This should not be strong')),
		);
	...
	describe('citation', () => {
		...
		checkParseEncodeRoundTrips(WIKI_NOTATION, defaultSchema, WIKI_NOTATION, doc(p('English??')));
	});
