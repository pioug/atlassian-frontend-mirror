...
describe('Field', () => {
					...
					<Field name="username" defaultValue="Joe Bloggs" label="Username">
						{({ fieldProps, meta: { dirty } }) => (
							<>
								...
								<HelperMessage>Field is {dirty === true ? 'dirty' : 'pristine'}</HelperMessage>
							</>
						)}
					</Field>
				)}
	...
	it('should not show validation error when field is untouched', () => {
					...
					>
						{({ fieldProps, error }) => (
							<>
								...
								{error && <ErrorMessage>There is a problem with this field</ErrorMessage>}
							</>
						)}
					</Field>
				)}
		...
		expect(screen.queryByText('There is a problem with this field')).not.toBeInTheDocument();
		expect(screen.getByTestId('text-field-container')).not.toHaveAttribute('data-invalid');
	});
	...
	it('should show validation error when field is touched', async () => {
		...
		expect(screen.getByText('There is a problem with this field')).toBeInTheDocument();
		expect(screen.getByTestId('text-field-container')).toHaveAttribute('data-invalid', 'true');
	});
			...
			<WithState defaultState={{ value: { deep: 'value' } }}>
				{(defaultValue, setDefaultValue) => (
					<>
						<Form onSubmit={jest.fn()}>
							{() => (
								<Field<{ value: { deep: string } }>
									...
									label="Username"
								>
									{({ fieldProps: { value, onChange, ...otherFieldProps }, error }) => (
										<>
											<TextField
						...
						<Button onClick={() => setDefaultValue({ value: { deep: 'value' } })}>Submit</Button>
					</>
				)}
			</WithState>,
		);
	...
	it('should not reset form when array element identity changes', async () => {
			...
			<WithState defaultState={[{ val: '1' }, { val: '2' }]}>
				{(defaultValue, setDefaultValue) => (
					<>
									...
									{({ fieldProps: { value, onChange, ...otherFieldProps }, error }) => (
										<>
											<TextField
												...
												onChange={(event) => {
													onChange([...value, { val: event.currentTarget.value }]);
												}}
												{...otherFieldProps}
											/>
											{error && <ErrorMessage>There is a problem with this field</ErrorMessage>}
						...
						<Button onClick={() => setDefaultValue([{ val: '1' }, { val: '2' }])}>Submit</Button>
					</>
				)}
			</WithState>,
		);
		...
		expect(
			screen.getByDisplayValue(JSON.stringify([{ val: '1' }, { val: '2' }, { val: '3' }])),
		).toBeInTheDocument();
						...
						{({ formProps }) => (
							<form {...formProps}>
								<Field name={name} defaultValue="unchanged" label="Username">
									{({ fieldProps }) => <TextField {...fieldProps} testId="TextField" />}
								</Field>
								<Button testId="UpdateTrigger" onClick={() => setName('username')}>
									Change
								</Button>
			...
			<Form onSubmit={submitFn}>
				{({ formProps }) => (
					<form {...formProps}>
						...
						>
							{({ fieldProps }) => (
								<Select {...fieldProps} placeholder="select a value" isClearable testId="selecto" />
							)}
						</Field>
					</form>
				)}
						...
						>
							{({ fieldProps, error, valid }) => (
								<>
									...
									{!error && !valid && <HelperMessage testId="helper">Helper</HelperMessage>}
	...
	it('should associate label with field', () => {
		render(
			<Form onSubmit={jest.fn()}>
				{() => (
					<Field testId="field-id" name="username" id="username" label="User name" defaultValue="">
		...
		expect(screen.getByTestId('text-field')).toHaveAttribute('aria-labelledby', 'username-label');
				...
				{({ formProps: { onSubmit } }) => (
					<>
						<Field
							...
							label="Username"
							validate={(value = '') => (value === 'foo' ? 'TAKEN_USERNAME' : undefined)}
						>
							{({ fieldProps, error }) => (
								<>
									<TextField {...fieldProps} testId="TextField" />
									{error === 'TAKEN_USERNAME' ? <ErrorMessage>Username taken</ErrorMessage> : null}
								</>
							)}
						</Field>
					...
					<>
						<Field name="username" defaultValue="Jane Chan" label="Username">
							{({ fieldProps, error }) => (
								<>
									<TextField {...fieldProps} testId="text-field" />
									{error === 'TAKEN_USERNAME' && <ErrorMessage>{errorMessage}</ErrorMessage>}
								</>
							)}
						</Field>
						<Button onClick={onSubmit}>Submit</Button>
				...
				{({ formProps: { onSubmit } }) => (
					<>
						...
						<Field name="username[1]" defaultValue="Bar" label="Nickname">
							{({ fieldProps: { onChange, ...rest } }) => (
								<TextField {...rest} onChange={(event) => onChange(event.currentTarget.value)} />
							)}
						</Field>
						<Button testId="Submit" onClick={onSubmit}>
							Submit
			...
			<Form onSubmit={onSubmitMock}>
				{({ formProps: { onSubmit } }) => (
					<>
						...
						<Field name="username.email" defaultValue="johndoe@atlassian.com" label="email">
							{({ fieldProps: { onChange, ...rest } }) => (
								<TextField
									{...rest}
									testId="email"
	...
	it('should indicate validating status for async validation', async () => {
		...
		render(
			<Form onSubmit={onSubmitMock}>
				{({ formProps: { onSubmit } }) => (
					<>
						<Field name="password" label="Password" defaultValue="" isRequired validate={validate}>
							{({ fieldProps, error, valid, meta }) => {
								return (
									<>
										<TextField type="password" {...fieldProps} testId="password" />
										{error && (
					...
					>
						{({ fieldProps, error }) => (
							<>
								<TextField testId="TextField" {...fieldProps} />
								{error === 'TOO_SHORT' && <ErrorMessage>Too short</ErrorMessage>}
								{error === 'TAKEN_USERNAME' && <ErrorMessage>Username is in use</ErrorMessage>}
							</>
						)}
					</Field>
				)}
				...
				{(isRequired, setIsRequired) => (
					<>
						...
						<Button
							...
							onClick={() => setIsRequired(isRequired === 'required' ? '' : 'required')}
						>
							Change key
						</Button>
					</>
			...
			<WithState defaultState="1">
				{(key, setKey) => (
					<>
						...
						<Button testId="ChangeKeyButton" onClick={() => setKey('' + (+key + 1))}>
							Change key
						</Button>
					</>
				)}
