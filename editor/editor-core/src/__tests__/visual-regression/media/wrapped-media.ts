...
describe('Snapshot Test: Wrapped media', () => {
	...
	it('should wrap properly inside other block nodes', async () => {
		await initFullPageEditorWithAdf(page, wrappedInBlockMedia, undefined, viewport);
	...
	it('should show media even if width is not present on a wrapped media item', async () => {
		...
		await initFullPageEditorWithAdf(page, nonResizableMedia, undefined, viewport, {
			media: {
	...
	it('should render 2 media items in 1 line when wrapped with text in between', async () => {
		await initFullPageEditorWithAdf(page, wrappedMediaTextAdf, undefined, viewport, {
			media: {
				allowMediaSingle: true,
				allowResizing: true,
				allowMediaGroup: true,
	...
	it('should render 2 media items in 2 lines when wrapped with a large enough width', async () => {
		await initFullPageEditorWithAdf(page, wrappedMediaTextSplitAdf, undefined, viewport, {
			media: {
				allowMediaSingle: true,
				allowResizing: true,
				allowMediaGroup: true,
	...
	it('should have position multiple wrapped media properly', async () => {
		await initFullPageEditorWithAdf(page, wrappedMediasNextToEachOtherAdf, undefined, viewport, {
			media: {
				allowMediaSingle: true,
				allowResizing: true,
			},
	...
	describe('layout', () => {
		it('should render 2 media items in 1 line when wrapped with text in between', async () => {
			await initFullPageEditorWithAdf(page, wrappedMediaTextLayoutAdf, undefined, viewport, {
				media: {
					allowMediaSingle: true,
					allowResizing: true,
					allowMediaGroup: true,
		...
		it('should render 2 media items in 2 lines when wrapped with a large enough width', async () => {
			await initFullPageEditorWithAdf(page, wrappedMediaTextLayoutSplitAdf, undefined, viewport, {
				media: {
					allowMediaSingle: true,
					allowResizing: true,
					allowMediaGroup: true,
