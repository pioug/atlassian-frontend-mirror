...
const url = getExampleUrl('design-system', 'drawer', 'blanket-testing', global.__BASEURL__);
	...
	describe('animation', () => {
		it.skip.each(timeStampList)(
			...
			async (timeStamp: number) => {
				...
				await page.evaluate(
					(showButton: HTMLElement, blanketSelector: string, timeStamp: number) => {
						showButton.click();
		...
		it.skip.each(timeStampList)(
			...
			async (timeStamp: number) => {
				...
				await page.evaluate(
					(showButton: HTMLElement, blanketSelector: string, transitionDurationMs: number) => {
						showButton.click();
						...
						return new Promise<void>((resolve) => {
							setTimeout(() => {
								...
								if (animationList.length === 0 && opacity !== '1') {
									throw new Error(`Animations are empty when pausing. Opacity = ${opacity}`);
								}
								animationList.forEach((animation) => {
				...
				await page.evaluate(
					(blanketSelector: string, timeStamp: number) => {
						const blanket = document.querySelector<HTMLElement>(blanketSelector);
						if (blanket === null) {
							throw new Error('blanket not found');
						}
