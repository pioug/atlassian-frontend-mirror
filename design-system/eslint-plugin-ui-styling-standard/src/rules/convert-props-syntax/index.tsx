  ...
  if (parameter.type === 'ObjectPattern') {
    ...
    for (const property of properties) {
      ...
      const variable = currentScope.variables.find(
        (varb) => varb.name === propertyValue.name,
      );
    ...
    for (const reference of variable.references) {
      const memberExpression = (reference.identifier as IdentifierWithParent)
        .parent;
          ...
          ESTraverse.traverse(property.value, {
            enter: function (node, _parent) {
              if (node.type === 'ArrowFunctionExpression') {
                const propNames = parseArrowFunctionExpression(
                  node,
...
const convertPropUsage = (
  ...
  const arrowFunctionOffset = propUsage.arrowFunctionExpression.body.loc
    ? {
        startIndex: sourceCode.getIndexFromLoc(
          propUsage.arrowFunctionExpression.body.loc.start,
        ),
        endIndex: sourceCode.getIndexFromLoc(
          propUsage.arrowFunctionExpression.body.loc.end,
        ),
      }
    : undefined;
  ...
  newCode.push(
    sourceCode.getText().slice(prevEndIndex, arrowFunctionOffset.endIndex),
  );
