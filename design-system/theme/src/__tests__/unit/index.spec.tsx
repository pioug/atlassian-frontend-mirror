...
describe('AtlaskitThemeProvider', () => {
	...
	it('uses the default background color', () => {
		...
		const themeStyleSheet = document.getElementById('ds--theme--ak-body-background');
		expect(themeStyleSheet?.innerHTML).toContain(token('elevation.surface', '#FFFFFF'));
	});
	...
	it('uses the default background color and respects provided mode value', () => {
		...
		// CSS rules containing variables are not applied to the elements in tests
		const themeStyleSheet = document.getElementById('ds--theme--ak-body-background');
		expect(themeStyleSheet?.innerHTML).toContain(token('elevation.surface', '#1B2638'));
	});
	...
	it('uses a custom background color', () => {
		...
		expect(screen.getByTestId('theme-provider')).toHaveStyle('background: white');
	});
});
