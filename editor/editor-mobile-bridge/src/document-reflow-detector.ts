...
const isImageElement = (node: HTMLElement): node is HTMLImageElement => node.tagName === 'IMG';
...
export class DocumentReflowDetector {
	...
	private onReflow = (options: { forced: boolean }) => {
		if (!this.content) {
			throw new Error('Failed to find HTML element. Unable to utilise document reflow detector.');
		}
	...
	private onResizeObservation = (entries: ResizeObserverEntry[]) => {
		const height = (entries && entries[0].target.getBoundingClientRect().height) || 0;
