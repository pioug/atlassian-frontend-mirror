...
export const useFilePreview = ({
  ...
  const [nonCriticalError, setNonCriticalError] = useState<
    MediaFilePreviewError | undefined
  >();
  ...
  const isBackendPreviewReady =
    !!fileState && isImageRepresentationReady(fileState);
  const fileStateErrorMessage =
    fileState?.status === 'error' ? fileState.message : undefined;
      ...
      if (preview || (fileStatus === 'processed' && !isBackendPreviewReady)) {
        ...
        setError(
          new MediaFilePreviewError(
            'metadata-fetch',
            new Error(fileStateErrorMessage),
          ),
        );
      } else {
        setStatus('loading');
  ...
  useEffect(() => {
    ...
    if (
      !preview &&
      cachedPreview &&
      !isBigger(cachedPreview?.dimensions, requestDimensions)
    ) {
    ...
    // Remote Preview ----------------------------------------------------------------
      ...
      getAndCacheRemotePreviewRef
        ...
        .then(setPreview)
        .catch(e => {
          const wrappedError = ensureMediaFilePreviewError(
            'preview-fetch',
            e as Error,
          );
          if (!preview) {
  ...
  const onImageError = useCallback(
    (failedPreview?: MediaFilePreview) => {
      ...
      if (isSSRClientPreview(failedPreview)) {
        ssrReliabilityRef.current.client = createFailedSSRObject(
          failedPreview,
          traceContext,
        );
      }
      ...
      if (isSSRDataPreview(failedPreview)) {
        ssrReliabilityRef.current.server = createFailedSSRObject(
          failedPreview,
          traceContext,
        );
        ssrReliabilityRef.current.client = createFailedSSRObject(
