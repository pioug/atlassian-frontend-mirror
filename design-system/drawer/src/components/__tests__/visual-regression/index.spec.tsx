...
describe('Snapshot Test', () => {
	for (const width of widths) {
		it(`should match ${width} drawer screenshot`, async () => {
			const url = getExampleUrl('design-system', 'drawer', 'drawer-widths', global.__BASEURL__);
	...
	it('should match menu usage screenshot', async () => {
		const url = getExampleUrl('design-system', 'drawer', 'drawer-menu', global.__BASEURL__);
	...
	it('should prevent programmatic scroll when open', async () => {
		const url = getExampleUrl('design-system', 'drawer', 'scroll', global.__BASEURL__);
		...
		await page.evaluate(() =>
			document.querySelector('[data-testid="content-inner"]')?.parentElement?.scrollBy(0, 700),
		);
