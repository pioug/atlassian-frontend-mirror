...
import EditorConfiguration, { EditorAppearance } from '../../editor-configuration';
...
describe('Mobile Editor', () => {
	describe('on load', () => {
		...
		beforeEach(() => {
			...
			useEditorConfiguration = jest.spyOn(UseEditorConfigurationApi, 'useEditorConfiguration');
	...
	describe('Schema update on appearance mode change', () => {
		...
		it('schema constructed by default(full editor) should include taskList', async () => {
			const wrapper = mount(<App defaultValue={defaultValue} />);
			await waitFor(() => expect(!!bridge?.editorView?.state.schema.nodes.taskList).toBeTruthy());
			wrapper.unmount();
		});

		it('schema re-constructed by compact editor should not include taskList', (done) => {
			const config = JSON.stringify(
				new EditorConfiguration(`{"editorAppearance": "${EditorAppearance.COMPACT}"}`),
			);
