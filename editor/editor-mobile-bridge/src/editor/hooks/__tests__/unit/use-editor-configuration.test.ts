...
describe('use editor configuration', () => {
	...
	beforeEach(() => {
		...
		mockedSetEditorConfiguration = jest.spyOn(WebBridgeImpl.prototype, 'setEditorConfiguration');
	});
	...
	it('should return valid editor configuration', () => {
		const { result } = renderHook(() => useEditorConfiguration(bridge, initialConfig));
		expect(result.current).toEqual(initialConfig);
	});
});
