...
type ToolbarItem = ButtonToolbarItem | DropdownToolbarItem | SeperatorToolbarItem;
...
const Toolbar = ({ bridge }: Props) => {
	...
	useEffect(() => {
		(window as any).messageHandler.on('onNodeSelected', ({ items, nodeType }: any) => {
			setMenuItems(null);
	...
	const children = useMemo(() => {
		...
		if (menuItems) {
			...
			items.push(<Button onClick={() => setMenuItems(null)} iconName="EditorCloseIcon" />);
			...
			const dynamic = generateToolbarItems(toolbarConfig, bridge, setMenuItems) as JSX.Element[];
