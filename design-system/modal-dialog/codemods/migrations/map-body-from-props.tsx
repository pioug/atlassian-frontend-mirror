...
export const mapBodyFromProps = (j: core.JSCodeshift, source: Collection<Node>) => {
	...
	const modalBodySpecifier = getNamedSpecifier(j, source, PACKAGE_NAME, MODAL_BODY_COMPONENT_NAME);
	...
	source.findJSXElements(modalDialogComponentName).forEach((element: ASTPath<JSXElement>) => {
		const bodyFromIndividualProp = getOverrideFromIndividualProp(j, element, BODY_PROP_NAME);
		...
		if (!body) {
			const isSelfClosing = j(element).find(j.JSXOpeningElement).at(0).get('selfClosing').value;
	...
	if (shouldImportModalBody) {
		if (defaultSpecifier) {
			addToImport(j, source, [
				getComponentImportName(j, modalBodyComponentName, MODAL_BODY_COMPONENT_NAME),
			]);
