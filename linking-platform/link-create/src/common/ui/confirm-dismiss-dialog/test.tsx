...
describe('ConfirmDismissDialog', () => {
	const setup = (props?: Partial<ConfirmDismissDialogProps>) => {
		...
		const { rerender } = render(<ConfirmDismissDialog {...defaultProps} {...props} />, {
			wrapper: ({ children }) => (
	...
	it('should fire a screen event on mount', async () => {
		...
		expect(await screen.findByTestId('link-create-confirm-dismiss-dialog')).toBeInTheDocument();
		expect(analyticsSpy).toBeFiredWithAnalyticEventOnce(screenEvent, ANALYTICS_CHANNEL);
	});
