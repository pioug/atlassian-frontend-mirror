...
import FlagIconOverride from '../../examples/constellation/flag-icon-override';

## Default

All flags require a `title`. A dismiss button will auto-display on the
first flag in a `FlagGroup` or when the `appearance` prop is set.
...
Make sure the icon and color matches the intent of your
message appearance.

<Example Component={FlagIconOverride} packageName="@atlaskit/flag" />
