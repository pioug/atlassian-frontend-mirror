...
description: How @atlaskit/drag-and-drop reconciles pieces
---
...
During a drag operation, if a _drop target_ is removed and readded on the same `element`, then `@atlaskit/drag-and-drop` will treat the added _drop target_ as if it was the same _drop target_.
...
During a drag operation, if the dragging `draggable` is removed and readded on the same `element`, then `@atlaskit/drag-and-drop` will treat the added `draggable` as if it was the original `draggable`.
