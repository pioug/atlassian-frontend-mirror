		...
		test.describe(`when view port is ${viewport.width}x${viewport.height}`, () => {
			...
			test(`all revisions VC90 should matches when the 90th pixel section was visible`, async ({
				...
				const [p10, p25, p50, p75, p90, p95] = [
					Math.ceil(viewport.height * 0.1),
					Math.ceil(viewport.height * 0.25),
					Math.ceil(viewport.height * 0.5),
					Math.ceil(viewport.height * 0.75),
					Math.ceil(viewport.height * 0.9),
					Math.ceil(viewport.height * 0.95),
				];

				const sectionP10 = page.locator(`[data-testid="sectionHorizontal${p10}"]`);
				const sectionP25 = page.locator(`[data-testid="sectionHorizontal${p25}"]`);
				const sectionP50 = page.locator(`[data-testid="sectionHorizontal${p50}"]`);
				const sectionP75 = page.locator(`[data-testid="sectionHorizontal${p75}"]`);
				const sectionP90 = page.locator(`[data-testid="sectionHorizontal${p90}"]`);
				const sectionP95 = page.locator(`[data-testid="sectionHorizontal${p95}"]`);
				...
				const sectionTestid = `sectionHorizontal${p90}`;
		...
		test.describe(`when view port is ${viewport.width}x${viewport.height}`, () => {
			...
			test(`all revisions VC90 should matches when the 90th pixel section was visible`, async ({
				...
				const [p10, p25, p50, p75, p89, p90, p91, p95] = [
					...
					Math.ceil(viewport.height * 0.5),
					Math.ceil(viewport.height * 0.75),
					Math.ceil(viewport.height * 0.89),
					Math.ceil(viewport.height * 0.9),
					Math.ceil(viewport.height * 0.91),
					Math.ceil(viewport.height * 0.95),
				];
				...
				const p89VisibleAt = await getSectionVisibleAt(`sectionHorizontal${p89}`);
				expect(p89VisibleAt).toBeDefined();

				const p91VisibleAt = await getSectionVisibleAt(`sectionHorizontal${p91}`);
