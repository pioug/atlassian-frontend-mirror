...
describe('Status', () => {
	...
	it('should map colors to lozenge appearances', () => {
		...
		function checkColorMapping(color: Color, appearance: string) {
			const component = mountWithIntl(<Status text="In progress" color={color} />);
		...
		colors.forEach((color) => checkColorMapping(color, colorToLozengeAppearanceMap[color]));
	});
	...
	it('should use render data attributes for copy/paste', () => {
		const component = mountWithIntl(<Status text="TODO" color="blue" style="subtle" />);
	...
	describe('Status onHover', () => {
		...
		it('should fire analytics Status onHover', () => {
			...
			const lozengeContainer = component.find('span[className~="status-lozenge-span"]');
