...
import { loadPage, takeElementScreenShot } from '@atlaskit/visual-regression/helper';
...
const verifyElementIn =
	(url: string, options?: Options) => async (selector: string, interact?: () => Promise<void>) => {
		const { page } = global;
		...
		expect(await takeElementScreenShot(page, selector)).toMatchProdImageSnapshot();
	};
...
export { click, focus, hover, mouseDown, verifyElementIn, verifyAnimationTimestamps };
