...
describe('Text field input prompt', () => {
  test('should render correct props', () => {
    const textInputPrompt = renderWithDi(
      <TextInputPrompt userInputPrompt={userInputPrompts} />
    );

    expect(textInputPrompt.getByRole('textbox')).toHaveAttribute("name", userInputPrompts.variableName);
    expect(textInputPrompt.getByText(userInputPrompts.displayName)).toBeInTheDocument();
    expect(textInputPrompt.getByText(userInputPrompts.displayName)).toHaveAttribute("id", "testUserInput-uid1-label");
