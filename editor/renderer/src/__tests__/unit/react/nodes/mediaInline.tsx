...
describe('MediaInline', () => {
  ...
  it('should render a <span> tag', async () => {
    ...
    await waitFor(() =>
      expect(
        container.querySelector('span[data-node-type="mediaInline"'),
      ).toBeVisible(),
    );
  });
  ...
  it('should render with correct props passed down to component', async () => {
    ...
    await waitFor(() =>
      expect(
        container.querySelector('span[data-node-type="mediaInline"'),
      ).toHaveAttribute('data-id', '7113fcea-49ea-4a46-9ed5-c0f2991841e0'),
    );
    expect(
      container.querySelector('span[data-node-type="mediaInline"'),
    ).toHaveAttribute('data-collection', 'test-collection');
    expect(
      container.querySelector('span[data-node-type="mediaInline"'),
    ).toHaveAttribute('data-type', 'file');
  });
  ...
  it('should render with shouldOpenMediaViewer set to true when appearance is not mobile', async () => {
    ...
    wrapper.update();
    expect(wrapper.find(MediaInlineCard).prop('shouldOpenMediaViewer')).toEqual(
      true,
    );
    wrapper.unmount();
  });
  ...
  it('should add media attrs for copy and paste', async () => {
    ...
    expect(
      container.querySelector('span[data-node-type="mediaInline"'),
    ).not.toHaveAttribute('data-context-id');
    expect(
      container.querySelector('span[data-node-type="mediaInline"'),
    ).not.toHaveAttribute('data-width');
    expect(
      container.querySelector('span[data-node-type="mediaInline"'),
    ).not.toHaveAttribute('data-height');
  });
