...
test('Selected nav items should have aria-current="page" attribute', async ({
	...
	await expect(page.locator(linkItem2).first()).toHaveAttribute('aria-current', 'page');

	await page.locator(linkItem1).first().click();
	await expect(page.locator(linkItem1).first()).toHaveAttribute('aria-current', 'page');
