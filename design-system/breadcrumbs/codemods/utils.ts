...
export const createRenameJSXFunc =
	(component: string, from: string, to: string, fallback: string | undefined = undefined) =>
...
const createAddingPropFor =
	(component: string, options: AddingPorp) => (j: core.JSCodeshift, source: any) => {
		const defaultSpecifier = getDefaultSpecifier(j, source, component);
		...
		source.findJSXElements(defaultSpecifier).forEach((element: ASTPath<JSXAttribute>) => {
			...
			if (!isPropExist) {
				const value = j.jsxExpressionContainer(j.booleanLiteral(options.defaultValue));
...
export const elevateComponentToDefault =
	(pkg: string, innerElementName: string) => (j: core.JSCodeshift, root: any) => {
