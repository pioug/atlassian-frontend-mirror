...
describe('getNotificationsSrc', () => {
	it('returns the correct url when no locale or product is provided', () => {
		expect(getNotificationsSrc({})).toEqual('/home/notificationsDrawer/iframe.html');
	});
	...
	it('old route ignores the subproduct', () => {
		...
		).toEqual('/home/notificationsDrawer/iframe.html?locale=en-us&product=jira');
	});
	...
	it('returns the correct url for the new experience with a subproduct', () => {
		...
		).toEqual('/home/notificationList/index.html?locale=en-us&product=jira&subproduct=software');
	});
});
