...
describe('mapHydrateResponseData', () => {
	it('should correctly map response that includes each option type to SelectOption array', () => {
		const mappedOptions = mapHydrateResponseData(hydrateJqlStandardResponse as HydrateResponse);
...
describe('mapFieldValuesToFilterOptions', () => {
	it.each([
		...
		['status', fieldValuesResponseForStatuses, fieldValuesResponseForStatusesMapped, ''],
		['project', fieldValuesResponseForProjects, fieldValuesResponseForProjectsMapped, ''],
		['assignee', fieldValuesResponseForAssignees, fieldValuesResponseForAssigneesMapped, ''],
	])(
	...
	it('should correctly map an empty AGG response to an empty array', () => {
		const mappedOptions = mapFieldValuesToFilterOptions(fieldValuesEmptyResponse);
...
describe('mapFieldValuesTotalCount', () => {
	it('should correctly map response for option type to number', () => {
		const mappedTotalCount = mapFieldValuesToTotalCount(fieldValuesResponseForStatuses);

		expect(mappedTotalCount).toEqual(27);
	});

	it('should correctly map an empty AGG response to 0', () => {
		const mappedTotalCount = mapFieldValuesToTotalCount(fieldValuesEmptyResponse);
...
describe('mapFieldValuesPageCursor', () => {
	it('should correctly map response for option type to page cursor string', () => {
		const mappedPageCursor = mapFieldValuesToPageCursor(fieldValuesResponseForStatuses);

		expect(mappedPageCursor).toEqual(
			fieldValuesResponseForStatuses?.data?.jira?.jqlBuilder.fieldValues.pageInfo.endCursor,
		);
	});
});
