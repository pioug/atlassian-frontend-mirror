...
test('DynamicTable elements should be able to be identified, interacted and sorted by data-testid', async ({
  ...
  await expect(page.locator(`${tableRowA} > ${tableCell1}`).first()).toHaveText(
    'Republican',
  );
  await expect(page.locator(`${tableRowA} > ${tableCell2}`).first()).toHaveText(
    '1861-1865',
  );
  await expect(page.locator(`${tableRowA} > ${tableCell3}`).first()).toHaveText(
    '5',
  );
});
