...
const INITIAL_QUERY = `assignee = EMPTY AND status = 111 AND reporter in (Cristian Casais, Kyle Painter, Soney Mathew)`;
const INITIAL_USER_NODES = ['Cristian Casais', 'Kyle Painter', 'Soney Mathew'];

/**
 * Loads the user nodes example and waits for the initial query to be hydrated.
 *
 * The example resolves autocomplete data and hydrated users from mocked APIs, and the hydrated
 * users are then rendered by React into node views which ProseMirror inserts empty. Waiting for the
 * rendered node text first means the editor text assertions below only have to cover the final
 * state.
 */
const visitHydratedExample = async (jqlEditor: JQLEditorPage): Promise<void> => {
	await jqlEditor.visitExample<typeof import('../../examples/01-user-nodes.tsx')>('user-nodes');
	await jqlEditor.waitForHydratedNodes(INITIAL_USER_NODES);
};
...
test.describe('JQL Editor User Nodes', () => {
	test('User nodes are hydrated on initial load', async ({ page }) => {
		...
		await visitHydratedExample(jqlEditor);
		await expect(jqlEditor.input).toHaveText(INITIAL_QUERY);
	});
	test('User nodes are hydrated on paste', async ({ page }) => {
		const jqlEditor = new JQLEditorPage(page);
		await visitHydratedExample(jqlEditor);
		await expect(jqlEditor.input).toHaveText(INITIAL_QUERY);
		await jqlEditor.clearInput();
		await jqlEditor.pasteText('assignee = rjuedbergtlfrde');
		await jqlEditor.waitForHydratedNodes(['Cristian Casais']);
	...
	test('Correctly handles interactions before user nodes', async ({ page }) => {
		...
		await jqlEditor.waitForHydratedNodes(['Cristian Casais']);
		await expect(jqlEditor.input).toHaveText('status = Done and assignee = Cristian Casais');
		await jqlEditor.pressKeyNTimes('ArrowLeft', 2);
		await jqlEditor.pressKeyNTimes('Backspace', 5);
		await expect(jqlEditor.input).toHaveText('status = Done and assignCristian Casais');
	...
	test('Supports quoted AAIDs', async ({ page }) => {
		...
		await jqlEditor.waitForHydratedNodes(['Cristian Casais', 'Cristian Casais', 'Cristian Casais']);
