...
import MobileRendererWrapper, { MobileRenderer } from '../../mobile-renderer-element';
...
describe('renderer bridge', () => {
	...
	const initRenderer = (adf: DocNode, allowAnnotations: boolean): HTMLElement => {
	...
	describe('document reflow detector', () => {
		...
		it(`is enabled with container when SET_DOCUMENT_REFLOW_DETECTOR_STATUS emitted with true`, () => {
			eventDispatcher.emit(EmitterEvents.SET_DOCUMENT_REFLOW_DETECTOR_STATUS, true);
		...
		it(`is disabled when SET_DOCUMENT_REFLOW_DETECTOR_STATUS emitted with false`, () => {
			eventDispatcher.emit(EmitterEvents.SET_DOCUMENT_REFLOW_DETECTOR_STATUS, false);
	...
	describe('annotations', () => {
		...
		describe('when allowAnnotations is true', () => {
			it(`should call getAnnotationStates bridge method on renderer`, () => {
				...
				expect(nativeBridgeAPI.fetchAnnotationStates).toHaveBeenCalledWith(expected);
			});
...
describe('Mobile Renderer', () => {
	...
	it('should pass allowAnnotations to Mobile Renderer', () => {
		const result = mount(<App document={initialDocument} />);
		expect(result.find(MobileRendererWrapper).prop('allowAnnotations')).toBe(false);
		result.unmount();
	});
	it('should pass disableActions to Mobile Renderer', () => {
		const result = mount(<App document={initialDocument} />);
		expect(result.find(MobileRendererWrapper).prop('disableActions')).toBe(false);
		result.unmount();
	});
	it('should pass disableMediaLinking to Mobile Renderer', () => {
		const result = mount(<App document={initialDocument} />);
		expect(result.find(MobileRendererWrapper).prop('disableMediaLinking')).toBe(false);
		result.unmount();
	});
	it('should pass disableMediaLinking to Mobile Renderer', () => {
		const result = mount(<App document={initialDocument} />);
		expect(result.find(MobileRendererWrapper).prop('allowHeadingAnchorLinks')).toBe(false);
		result.unmount();
	});
	it('should pass allowCustomPanels to Mobile Renderer', () => {
		const result = mount(<App document={initialDocument} />);
		expect(result.find(MobileRendererWrapper).prop('allowCustomPanels')).toBe(false);
		result.unmount();
	});
});
