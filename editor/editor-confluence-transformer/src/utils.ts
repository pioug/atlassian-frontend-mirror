 ...
 */
export function addMarks(fragment: Fragment, marks: Mark[] | ReadonlyArray<Mark>): Fragment {
	let result = fragment;
...
export function getNodeMarkOfType(node: PMNode, markType: MarkType): Mark | null {
	if (!node.marks) {
		return null;
	}
...
export function createCodeFragment(
	...
	if (!!title) {
		const titleNode = schema.nodes.heading.createChecked({ level: 5 }, schema.text(title));
 ...
 */
export function calcPixelsFromCSSValue(value: string, parentPixels: number): number {
