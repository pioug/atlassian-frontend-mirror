...
import { fakeMediaClient, mountWithIntlWrapper } from '@atlaskit/media-test-helpers';
...
describe('download', () => {
	...
	describe('createItemDownloader', () => {
		...
		it('should not try to take name from errored file provided', () => {
			...
			expect(mediaClient.file.downloadBinary).toHaveBeenCalledWith('some-id', undefined, undefined);
		});

		it('should pass collection name', () => {
			const mediaClient = fakeMediaClient();
			createItemDownloader(processingFailedState, mediaClient, 'some-collection-name')();
			expect(mediaClient.file.downloadBinary).toHaveBeenCalledWith(
				'some-id',
				'some-name',
	...
	describe('ErrorViewDownloadButton', () => {
		it('should trigger an analytics event in the media channel', () => {
			...
			component.find('button[data-testid="media-viewer-download-button"]').simulate('click');
