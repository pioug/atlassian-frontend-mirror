...
describe('ExperimentController', () => {
	it('should provide context to a consumer', async () => {
		...
		mount(
			<ExperimentController experimentEnrollmentConfig={mockExperimentEnrollmentConfig}>
				<ExperimentConsumer>{(context) => mockContextReceiver(context)}</ExperimentConsumer>
		...
		expect(getExperimentValueForReceiverCall(0).isEnrollmentDecided).toBeFalsy();

		// call resolver
		const resolverPromise = getExperimentValueForReceiverCall(0).enrollmentResolver();
		...
		expect(getExperimentValueForReceiverCall(0).enrollmentResolver()).toBe(resolverPromise);
		expect(mockContextReceiver.mock.calls).toHaveLength(1);
		...
		expect(getExperimentValueForReceiverCall(1).isEnrollmentDecided).toBeTruthy();
		expect(getExperimentValueForReceiverCall(1).enrollmentDetails).toEqual(mockExperimentDetails);
	});
	...
	it('should provide context with options to a consumer if present', () => {
		...
		expect(receivedExperiments.myExperimentKey).toHaveProperty('enrollmentResolver');
		expect(receivedExperiments.myExperimentKey).toHaveProperty('isEnrollmentDecided', false);
