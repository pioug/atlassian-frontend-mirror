...
describe('fixTable', () => {
	...
	it('can add to multiple rows', () => {
		...
		const expected = createTable(tr(c11, cEmpty, cEmpty), tr(cEmpty, c11, c11), tr(c(3, 1)));
		expect(fixed.eq(expected)).toBeTruthy();
	});
	...
	it('will fix overlapping cells', () => {
		...
		const expected = createTable(tr(c11, c(1, 2), c11), tr(c11, cEmpty, cEmpty));
		expect(fixed.eq(expected)).toBeTruthy();
	});
	...
	it('makes sure column widths are coherent', () => {
		const fixed = fix(createTableWithDoc(tr(c11, c11, cw200), tr(cw100, c11, c11)))!;
	...
	it('can update column widths on colspan cells', () => {
		...
		const fixed = fix(createTableWithDoc(tr(c11, c11, cw200), tr(c(3, 2)), emptyTr))!;
	...
	it('will update the odd one out when column widths disagree', () => {
		const fixed = fix(
			createTableWithDoc(tr(cw100, cw100, cw100), tr(cw200, cw200, cw100), tr(cw100, cw200, cw200)),
		)!;
		const expected = createTable(
	...
	it('respects table role when inserting a cell', () => {
		...
		const expected = createTable(tr(h11, hEmpty, hEmpty), tr(cEmpty, c11, c11), tr(c(3, 1)));
