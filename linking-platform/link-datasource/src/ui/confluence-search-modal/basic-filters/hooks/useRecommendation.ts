...
const useRecommendation = (): UserRecommendationState => {
	...
	const [status, setStatus] = useState<UserRecommendationState['status']>('empty');
	const [errors, setErrors] = useState<UserRecommendationState['errors']>([]);
	const initialData = useRef<OptionData[]>();

	const convertRecommendationsToFilterOptions = (recommendations: OptionData[]): SelectOption[] => {
