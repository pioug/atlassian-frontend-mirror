...
const objectHas = <T extends string>(
  x: unknown,
  prop: T,
): x is { [prop in T]: unknown } => {
  return isObject(x) && prop in x;
};

expect.extend({
  toBeFiredWithAnalyticEventOnce(
     ...
     */
    const matchingEvents = spy.mock.calls.filter((arg: any[]) =>
      matches(event)(arg[0]),
    );
      ...
      if (channel && matchingEvents[0][1] !== channel) {
        return {
          message: () =>
            ...
            `Actually fired on: ${this.utils.printReceived(
              matchingEvents[0][1],
            )}`,
          pass: false,
        };
