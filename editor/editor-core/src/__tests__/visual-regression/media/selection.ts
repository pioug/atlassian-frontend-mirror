...
describe('Snapshot Test: Media', () => {
	...
	describe('full page editor', () => {
		beforeEach(async () => {
			await initEditor(Appearance.fullPage, { width: 800, height: 700 }, mediaSelectionAdf, {
	...
	describe('comment editor', () => {
		...
		describe('media group', () => {
			beforeEach(async () => {
				await initEditor(Appearance.comment, { width: 600, height: 400 }, mediaGroupAdf);
				await waitForMediaToBeLoaded(page);
				await page.click(`${editorSelector} p`);
			});
			...
			it('renders left side gap cursor', async () => {
				await pressKey(page, ['ArrowLeft', 'ArrowLeft', 'ArrowLeft', 'ArrowLeft', 'ArrowLeft']);
				await takeCommentSnapshot(page);
			});
		});
		...
		describe('media single', () => {
			beforeEach(async () => {
				await initEditor(Appearance.comment, { width: 550, height: 400 }, mediaSelectionAdf, {
					media: { allowMediaSingle: true },
				});
				await waitForMediaToBeLoaded(page);
				await changeMediaLayout(page, MediaLayout.alignStart);
