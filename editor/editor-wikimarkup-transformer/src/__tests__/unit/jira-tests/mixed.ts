...
|more|cells|`;
		...
		checkParseEncodeRoundTrips(
			...
			doc(
				...
				p('and ', strong('bold'), ' ', strike('strike'), ' and ', em('emphasised'), ' text.'),
				table()(
...
{{Or}} _how_ *about* {{combinations}}?`;
		...
		checkParseEncodeRoundTrips(
			...
			doc(
				...
				p(strong('Anything'), ' ', strong('wrong'), ' ', strong('with'), ' ', strong('this'), '?'),
				p(em('How'), ' ', em('about'), ' ', em('this'), '?'),
				p(code('Or'), ' ', em('how'), ' ', strong('about'), ' ', code('combinations'), '?'),
			),
		);
	});
