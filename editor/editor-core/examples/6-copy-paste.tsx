...
class ExampleEditorComponent extends React.Component<EditorProps & ExampleProps, State> {
	...
	private setFullWidthMode = (appearance: 'full-page' | 'full-width' | 'max') => {
		this.setState({
			appearance:
				appearance === 'max' && !expValEquals('editor_tinymce_full_width_mode', 'isEnabled', true)
					? 'full-width'
					: appearance,
		});
	};
	...
	renderEditor = (collectionName: string, contentStyles?: SerializedStyles) => {
		const defaultValue =
			(localStorage && localStorage.getItem(getLocalStorageKey(collectionName))) || undefined;
	...
	renderRenderer = () => {
		return (
			<div css={rendererWrapper}>
				...
				<ReactRenderer document={rendererDoc} adfStage="stage0" dataProviders={dataProviders} />
			</div>
		);
	};
...
const ClipboardWrapper = (props: ClipboardWrapperProps) => {
	...
	const [mediaClientConfig, setMediaClientConfig] = useState<MediaClientConfig>();
	...
	return (
		<div>
			...
			<p>
				If you paste an image you will see a preview. You should <b>NOT</b> see the image pasted in{' '}
				<b>Editor</b>.
			</p>
			<p>
				If you paste an image in <b>Editor</b>, you should <b>NOT</b> see a preview in here.
			</p>
			<p>Because Editor's paste event should only be listened within the editorDomElement.</p>

			<p>
				Notes: If you paste an image in <b>Renderer</b>, you will see a preview here, because
				external Clipboard bind paste event globally by default.
			</p>
