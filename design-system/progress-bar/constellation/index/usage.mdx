...
1. Progress line: Progress is visually represented by a filled line. It can be partially filled, complete, or indeterminate (partial completion of an unknown amount).
2. Progress bar container: This determines the length of the bar within the available screen space.

## Accessibility

- Always provide a clear `ariaLabel` that describes the current state of the progress bar. For example, "5 of 10 files uploaded", not "Files uploading".
- Use helper text with a progress bar if the process is complex or has a long wait time. This lets people know what sub-processes are taking place.
...
- For guidelines on an alternative indeterminate loading state for constrained spaces, see [spinner](/components/spinner).
- For guidelines on applying color to custom progress bars for data visualization, see [data visualization color](/foundations/color-new/data-visualization-color/).
