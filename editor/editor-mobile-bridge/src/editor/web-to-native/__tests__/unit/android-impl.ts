	...
	describe('Android', () => {
		...
		describe('Lifecycle bridge', () => {
			describe('editorReady', () => {
				it('should call native', function () {
					...
					expect(windowWithMockBridges.lifecycleBridge!.editorReady).toHaveBeenCalled();
				});
				...
				it('should call native only once when method is called a second time', function () {
					...
					expect(windowWithMockBridges.lifecycleBridge!.editorReady).toHaveBeenCalledTimes(1);
				});
				...
				it('should send analytics event called twice', function () {
					...
					expect(windowWithMockBridges.analyticsBridge!.trackEvent).toHaveBeenCalledWith(
						JSON.stringify({
			...
			describe('startWebBundle', () => {
				it('should call native', function () {
					...
					expect(windowWithMockBridges.lifecycleBridge!.startWebBundle).toHaveBeenCalled();
				});
				...
				it('should call native only once when method is called a second time', function () {
					...
					expect(windowWithMockBridges.lifecycleBridge!.startWebBundle).toHaveBeenCalledTimes(1);
				});
			...
			describe('editorError', () => {
				...
				it('should call native', () => {
					...
					expect(windowWithMockBridges.lifecycleBridge!.editorError).toHaveBeenCalledWith(
						errorString,
						undefined,
					);
				});
			...
			it('should call native editorDestroyed method', function () {
				...
				expect(windowWithMockBridges.lifecycleBridge!.editorDestroyed).toHaveBeenCalled();
			});
		});
		...
		describe('PageTitle Bridge', () => {
			it('should call updateTitle in native', function () {
				...
				expect(windowWithMockBridges.pageTitleBridge!.updateTitle).toHaveBeenCalledWith(title);
			});
		});
		...
		describe('TypeAhead Bridge', () => {
			it('should call dismissTypeAhead in native', function () {
				...
				expect(windowWithMockBridges.typeAheadBridge!.dismissTypeAhead).toHaveBeenCalled();
			});
		});
		...
		describe('Content Bridge', () => {
			it('should call onContentRendered in native', function () {
				...
				expect(windowWithMockBridges.contentBridge!.onContentRendered).toHaveBeenCalledWith(
					totalNodeSize,
					nodes,
					actualRenderingDuration,
					totalBridgeDuration,
		...
		describe('Toolbar Bridge', () => {
			it('should call onNodeSelected in native', () => {
				...
				expect(windowWithMockBridges.toolbarBridge!.onNodeSelected).toBeCalledWith(
					'panel',
					'items',
				);
			});
			...
			it('should call onNodeDeselected in native', () => {
				...
				expect(windowWithMockBridges.toolbarBridge!.onNodeDeselected).toBeCalled();
			});
		});
		...
		describe('Collab Bridge', () => {
			it('should call updateStepVersion in with version', () => {
				...
				expect(windowWithMockBridges.collabBridge?.updateStepVersion).toBeCalledWith(
					100,
					undefined,
				);
			});
