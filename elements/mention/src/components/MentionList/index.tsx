...
export default class MentionList extends React.PureComponent<Props, State> {
	...
	private selectIndexOnHover = (mention: MentionDescription, event?: React.SyntheticEvent<any>) => {
		if (!event) {
			return;
		}
	...
	render() {
		...
		if (mustShowError) {
			...
			resultSection = <Scrollable ref={this.handleScrollableRef}>{this.renderItems()}</Scrollable>;
		}
