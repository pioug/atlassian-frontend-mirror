...
describe.skip('@atlaskit/tree - Tree', () => {
	...
	describe('#render', () => {
		it('renders Droppable with the correct props', () => {
			const tree = mount(<Tree tree={treeWithThreeLeaves} renderItem={mockRender} />);
		...
		it('renders Draggable around each item if drag and drop is enabled', () => {
			const tree = mount(
				<Tree tree={treeWithThreeLeaves} renderItem={mockRender} isDragEnabled={true} />,
			);
		...
		it('renders Draggable around each item if drag and drop is conditionally enabled using a function', () => {
			...
			const tree = mount(
				<Tree tree={treeWithThreeLeaves} renderItem={mockRender} isDragEnabled={isDragEnabled} />,
			);
			const draggables = tree.find(Draggable);
			expect(draggables).toHaveLength(3);
		});
		...
		it('does not render Draggable around each item if drag and drop is explicitly disabled', () => {
			const tree = mount(
				<Tree tree={treeWithThreeLeaves} renderItem={mockRender} isDragEnabled={false} />,
			);
			const draggables = tree.find(Draggable);
		...
		it('re-renders only the items which have been changed', () => {
			const wrapper = mount(<Tree tree={treeWithThreeLeaves} renderItem={mockRender} />);
			expect(mockRender).toHaveBeenCalledTimes(3);
			mockRender.mockClear();
			const mutatedTree = {
	...
	describe('#onExpand', () => {
		it('calls with the right item', () => {
			...
			mount(<Tree tree={treeWithThreeLeaves} renderItem={mockRender} onExpand={mockOnExpand} />);
		...
		it('shows only the relevant children', () => {
			const DynamicTree = () => {
				...
				return (
					<Tree tree={tree} renderItem={renderItem} onExpand={onExpand} onCollapse={onCollapse} />
				);
			};
	...
	describe('#onCollapse', () => {
		it('calls with the right item', () => {
			...
			const firstItem = treeWithThreeLeaves.items['1-1'];
			mount(
				<Tree tree={treeWithThreeLeaves} renderItem={mockRender} onCollapse={mockOnCollapse} />,
			);
	...
	describe('#onDragStart', () => {
		...
		it('calls onDragStart if it is defined', () => {
			...
			const instance = mount<Tree>(
				<Tree tree={treeWithTwoBranches} renderItem={mockRender} onDragStart={mockOnStartCb} />,
			).instance();
			instance.onDragStart(dragStart);
	...
	describe('#onDragUpdate', () => {
		...
		it('expands parent after timeout', () => {
			...
			const instance = mount<Tree>(
				<Tree tree={treeWithClosedParent} renderItem={mockRender} onExpand={onExpand} />,
			).instance();
			instance.onDragStart(dragStart);
	...
	describe('#onDragEnd', () => {
		it('calls props.onDragEnd when drag ends successfully', () => {
			...
			const instance = mount<Tree>(
				<Tree tree={treeWithTwoBranches} renderItem={mockRender} onDragEnd={mockOnDragEnd} />,
			).instance();
