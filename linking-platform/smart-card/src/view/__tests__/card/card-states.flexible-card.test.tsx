...
describe('smart-card: card states, flexible block withUrl', () => {
	...
	describe('> state: resolved', () => {
		it('flexible block card: should render with metadata when resolved', async () => {
			...
			const resolvedCard = await waitFor(() => getByTestId('smart-block-resolved-view'));
	...
	describe('> state: forbidden', () => {
		...
		describe('with auth services available', () => {
			it('flexible block card: renders the forbidden view if no access, with auth prompt', async () => {
				...
				const frame = await waitFor(() => getByTestId('smart-block-forbidden-view'));
			...
			it('flexible block card: renders the forbidden view with "Direct Access" access type', async () => {
				...
				const frame = await waitFor(() => getByTestId('smart-block-forbidden-view'));
				expect(frame).toBeTruthy();
				const forbiddenLink = await waitFor(() => getByText('Join Google to view this content'));
				expect(forbiddenLink).toBeTruthy();

				const messageContainer = await waitFor(() => getByTestId('smart-element-text'));
			...
			it('flexible block card: renders the forbidden view with "Pending Request Exists" access type', async () => {
				...
				const frame = await waitFor(() => getByTestId('smart-block-forbidden-view'));
				expect(frame).toBeTruthy();
				const forbiddenLink = await waitFor(() => getByText('Access to Google is pending'));
				expect(forbiddenLink).toBeTruthy();
	...
	describe('> state: unauthorized', () => {
		it('renders correctly when provider details are not available', async () => {
			...
			const frame = await waitFor(() => getByTestId('smart-block-unauthorized-view'));
			expect(frame).toBeTruthy();
			...
			const unauthorizedContent = getByTestId('smart-block-unauthorized-view-content');
			...
			const providerImage = queryByTestId('smart-block-card-footer-provider-image');
			expect(providerImage).toBeFalsy();

			const providerLabel = queryByTestId('smart-block-card-footer-provider-label');
		...
		it('renders with connect flow when auth services are available', async () => {
			...
			const providerImage = getByTestId('smart-block-card-footer-provider-image');
			expect(providerImage).toBeTruthy();
			expect(providerImage.getAttribute('src')).toEqual(
				'https://developers.google.com/drive/images/drive_icon.png',
			);

			const providerLabel = getByTestId('smart-block-card-footer-provider-label');
	...
	describe('> state: error', () => {
		...
		it('flexible block card: renders not found card when link not found', async () => {
			...
			const frame = await waitFor(() => getByTestId('smart-block-not-found-view'));
			expect(frame).toBeTruthy();
			const link = await waitFor(() => getByText("We can't show you this Jira page"));
			expect(link).toBeTruthy();
			expect(mockFetch).toBeCalled();
			expect(mockFetch).toBeCalledTimes(1);
			expect(mockOnError).toHaveBeenCalledWith({
