...
const ModalOpenCloseEvents = () => {
	...
	useEffect(() => {
		...
		createAnalyticsEvent(createEventPayload(`ui.modalDialog.opened.linkCreate`, {})).fire(
			ANALYTICS_CHANNEL,
		);

		//will fire when modal is unmounted
		return () => {
			createAnalyticsEvent(createEventPayload(`ui.modalDialog.closed.linkCreate`, {})).fire(
				ANALYTICS_CHANNEL,
			);
		};
