...
describe('toEqualDocumentAndSelection matches', () => {
	...
	it('cursor positions', () => {
		...
		expect(editorView.state).toEqualDocumentAndSelection(doc(p('Hello{<>}World')));
	});

	it('moved cursor positions', () => {
		const { editorView } = editor(doc(p('Hello{<>}')));
		insertText(editorView, 'World');
		expect(editorView.state).toEqualDocumentAndSelection(doc(p('HelloWorld{<>}')));
	});
	...
	it('node selections', () => {
		const { editorView, refs } = editor(doc(p('HelloWorld{<>}'), '{nextPos}', hr()));
		const { state, dispatch } = editorView;
		dispatch(state.tr.setSelection(NodeSelection.create(state.doc, refs.nextPos)));
		expect(editorView.state).toEqualDocumentAndSelection(doc(p('HelloWorld'), '{<node>}', hr()));
	});

	it('gap cursor selections', () => {
		const { editorView, refs } = editor(doc(p('HelloWorld'), hr(), '{nextPos}'));
		setGapCursorSelection(editorView, refs.nextPos, GapCursorSide.RIGHT);

		expect(editorView.state).toEqualDocumentAndSelection(doc(p('HelloWorld'), hr(), '{<|gap>}'));
	});
