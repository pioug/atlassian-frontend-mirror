import type { ASTPath, CallExpression, Collection, JSCodeshift } from 'jscodeshift';

import { getNamedSpecifier, hasImportDeclaration, removeImport } from '@atlaskit/codemod-utils';
...
export const mathToBinaryExpressions = (j: JSCodeshift, source: Collection<Node>) => {
	const hasImportFromMathEntryPoint = hasImportDeclaration(j, source, '@atlaskit/theme/math');
	const namedImportFromTheme = getNamedSpecifier(j, source, '@atlaskit/theme', 'math');
	...
	const cleanUpImports = () => {
		...
		if (themeImportSpecifiers.length === 1 && namedImportFromTheme) {
			...
			themeImportSpecifiers.filter((path) => path.node.imported.name === 'math').remove();
		}
	};

	const grabArgsAndCreateReplacement = (path: ASTPath<CallExpression>, operator: Operator) => {
		// @ts-ignore
