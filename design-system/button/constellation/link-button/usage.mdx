...
Link buttons open new pages, websites, or new locations on a page. These are buttons that behave like links. For actions that only affect the current page, use a regular [button](/components/button/).
...
- For icon-only link buttons, use [link icon button](/components/button/link-icon-button).
