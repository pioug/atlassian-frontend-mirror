...
const removeDefaultPaths = (token: TransformedToken) => token.path.filter((p) => p !== '[default]');
...
const getSubpathIndex = (token: TransformedToken) => {
	...
	return rootPathIndex !== -1
		? tokenOrder[rootPathIndex].subpaths.findIndex((path) => path === token.path[1])
		: -1;
};
 ...
 * Semantic tokens
 */
const semanticTokens = ['brand', 'danger', 'warning', 'success', 'discovery', 'information'];
 ...
 */
const emphasisTokens = ['subtlest', 'subtler', 'subtle', 'bold', 'bolder', 'boldest'];

const isEmphasisToken = (token: TransformedToken, path: string) => {
	return token.original.attributes?.group !== 'fontWeight' && emphasisTokens.includes(path);
};
 ...
 * Font weights
 */
const fontWeightPaths = ['regular', 'medium', 'semibold', 'bold'];
 ...
 * Font family
 */
const fontFamilyPaths = ['heading', 'body', 'code'];
...
) => import('style-dictionary/types/TransformedToken').TransformedToken[] = (
	tokens: TransformedToken[],
) => tokens.sort(sortByPathName);
