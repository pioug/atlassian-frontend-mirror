...
export const checkIfSupportedExport = (
  ...
  if (
    root.type === 'ExportDefaultDeclaration' ||
    root.type === 'ExportNamedDeclaration'
  ) {
    return {
      isExport: true,
      node: root,
  ...
  for (const { identifier } of resolved?.references ?? []) {
    ...
    const { nodes: refs, scope: nextScope } = getStack(
      context,
      (identifier as Rule.Node).parent,
    );
