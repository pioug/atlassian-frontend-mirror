...
const findEmoji = (component: ReactWrapper) => component.update() && component.find(Emoji);
const emojiVisible = (component: ReactWrapper) => findEmoji(component).length === 1;
...
describe('<ResourcedEmoji />', () => {
	...
	it('should render emoji with correct data attributes', () => {
		...
		return waitUntil(() => emojiVisible(component)).then(() => {
			expect(
				component.find('span[data-emoji-id]').getDOMNode().attributes.getNamedItem('data-emoji-id')!
					.value,
	...
	it('should update emoji on shortName change', () => {
		...
		return waitUntil(() => emojiVisible(component)).then(() => {
			...
			return waitUntil(() => emojiVisibleById(component, evilburnsEmoji.id)).then(() => {
				expect(findEmoji(component).prop('emoji').id).toEqual(evilburnsEmoji.id);
			});
		});
	});
