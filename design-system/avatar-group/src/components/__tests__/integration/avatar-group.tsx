...
const overflowMenuItemInsideButton =
...
BrowserTestCase(
  'should have non interactive overflowed avatar items inside button',
  {},
  async (client: any) => {
    const testPage = new Page(client);
    await testPage.goto(exampleUrl);
    ...
    const actualTag = await testPage.getTagName(overflowMenuItemInsideButton);
    expect(['BUTTON', 'button'].includes(actualTag)).toBe(true);
  },
);
