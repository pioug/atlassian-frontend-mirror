...
const getKeyboardPreviewOffset = {
  ...
  home({
    ...
    rbdInvariant(
      placeholderRect,
      'the placeholder should exist if in home position',
    );
  ...
  away({
    ...
    rbdInvariant(
      dropIndicatorRect,
      'the drop indicator should exist if in away position',
    );
    ...
    return {
       ...
       */
      [mainAxis.name]:
        baseOffset[mainAxis.name] -
        0.5 * draggableDimensions.rect[mainAxis.style.length],
      /**
       * On the cross axis, the drag preview is offset by a fixed percentage
       * of its cross axis length.
       *
...
function startDrag(
  state: DraggableState,
  {
    start,
    previewOffset,
  }: { start: DragStart; previewOffset: DraggablePreviewOffset },
): DraggableDraggingState {
...
export function reducer(
  ...
  if (action.type === 'UPDATE_POINTER_PREVIEW') {
    ...
    const nextState = {
      ...
      previewOffset: {
        x:
          pointerLocation.current.input.clientX -
          pointerLocation.initial.input.clientX,
        y:
          pointerLocation.current.input.clientY -
          pointerLocation.initial.input.clientY,
      },
    };
