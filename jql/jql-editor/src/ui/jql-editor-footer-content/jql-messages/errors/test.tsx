...
const useJQLErrorMock = jest.fn<[JQLParseError | null, any], []>(() => [
  null,
  {},
]);
...
const syntaxError = new JQLSyntaxError(
...
describe('ErrorMessages', () => {
  ...
  it('renders JQL syntax errors', () => {
    ...
    expect(
      queryByText(syntaxError.message, { exact: false }),
    ).toBeInTheDocument();
  });
  ...
  it('renders external JQL errors', () => {
    ...
    expect(queryByTestId('jql-editor-validation')).toBeInTheDocument();
    expect(
      queryByText(String(normalizedExternalMessages.errors[0].message)),
    ).toBeInTheDocument();
    expect(
      queryByText(String(normalizedExternalMessages.errors[1].message)),
    ).toBeInTheDocument();
    expect(
      queryByText(String(normalizedExternalMessages.errors[2].message)),
    ).toBeInTheDocument();
  });
  ...
  it(`renders the unknown error message when a JQL error exists but it's not a syntax error`, () => {
    ...
    expect(queryByTestId('jql-editor-validation')).toBeInTheDocument();
    expect(
      queryByText(mockIntl.formatMessage(commonMessages.unknownError)),
    ).toBeInTheDocument();
  });
