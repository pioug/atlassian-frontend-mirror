...
describe('@atlaskit/editor-core slice utils', () => {
	describe('mapFragment', () => {
		...
		it('should support mutating child nodes', () => {
			...
			const expected = fragment(p(strong('start')), p(strong('middle')), p(strong('end')));
		...
		it('should call callback with (node, parent, offset)', () => {
			...
			expect(spy.mock.calls[0]).toEqual([content.firstChild!.firstChild!, content.firstChild!, 0]);
		...
		it('should allow replacing one node with multiple nodes', () => {
			...
			expect(mapFragment(content, replaceMultiple)).toEqual(fragment(p('start'), p('end')));
		});
	});
});
