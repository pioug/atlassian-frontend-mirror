...
const openAnotherButtonSelector = '[data-testid="modal-dialog-content--action-1"]';
...
describe('Snapshot Test', () => {
	it(`Portal stacking context should match prod`, async () => {
		const url = getExampleUrl('design-system', 'portal', 'stacking-context', global.__BASEURL__);
		const { page } = global;
	...
	it(`Changing z-index of portal should change stacking context`, async () => {
		const url = getExampleUrl('design-system', 'portal', 'portal-re-render', global.__BASEURL__);
		const { page } = global;
		await loadPage(page, url);
		await page.waitForSelector(portalSelector);
	...
	it.skip(`Portal should create portals with different stacking context for different layers`, async () => {
		const url = getExampleUrl('design-system', 'portal', 'complex-layering', global.__BASEURL__);
		const { page } = global;
		await loadPage(page, url);
		...
		const showOnboardingButton = await lastDialog?.$(showOnboardingButtonSelector);
