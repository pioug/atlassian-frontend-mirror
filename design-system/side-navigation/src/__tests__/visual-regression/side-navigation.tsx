...
describe('<SideNavigation /> integration tests', () => {
	...
	it('should match the simple sidebar', async () => {
		...
		expect(await takeElementScreenShot(global.page, sideNavigation)).toMatchProdImageSnapshot();
	});
	...
	it('should match nested navigation custom icon with production example', async () => {
		await openExamplesAndWaitFor('nested-side-navigation', '[data-custom-icon]');
	...
	it('should render a scrollable section when items overflow', async () => {
		...
		expect(await takeElementScreenShot(page, sideNavigation)).toMatchProdImageSnapshot();
	});
	...
	it('should match the spotlight example', async () => {
		...
		const disabledItemSpotlight = '[data-testid="disabledItemSpotlightMessage"]';
		const nestingItemSpotlight = '[data-testid="nestingItemSpotlightMessage"]';
		const selectedNestingItemSpotlight = '[data-testid="selectedNestingItemSpotlightMessage"]';
		...
		expect(await takeElementScreenShot(global.page, spotlightClone)).toMatchProdImageSnapshot();
	...
	it('should match the hover states', async () => {
		...
		expect(await takeElementScreenShot(global.page, settingsButton)).toMatchProdImageSnapshot();

		await page.hover(dropboxButton);

		expect(await takeElementScreenShot(global.page, dropboxButton)).toMatchProdImageSnapshot();
	});
	...
	it('should match styles for Section', async () => {
		...
		expect(await takeElementScreenShot(global.page, testId)).toMatchProdImageSnapshot();
	});
});
