...
const ucFirst = (text: string): string => text[0].toUpperCase() + text.substring(1);
...
class Example extends React.Component<{}, ExampleState> {
	...
	getGroupedUrls(mode: GroupingMode): GroupedExampleUrls {
		...
		const exampleSortCmp = (a: ExampleUrlData & WithTitle, b: ExampleUrlData & WithTitle) =>
			a.title > b.title ? 1 : -1;
		...
		if (mode === 'none') {
			...
			return urlsWithTitles.reduce(
				(grouped, example) => ({
					...grouped,
					[example[mode]]: [
						...(grouped[example[mode]] ? grouped[example[mode]].sort(exampleSortCmp) : []),
						example,
					],
				}),
	...
	renderGroups(mode: GroupingMode, appearanceMode: CardAppearance) {
		...
		return (
			<TableTree>
				<Headers>
					<Header width={'30%'}>{mode.substring(0, 1).toUpperCase() + mode.substring(1)}</Header>
					...
					render={({ title, idx, example, children }: CardExample) => (
						...
						>
							...
							<Cell css={patchedCellStyles}>
								{!example ? '' : <Card url={example.url} appearance={appearanceMode} />}
							</Cell>
						</Row>
					)}
				/>
			...
			<IntlProvider locale="en">
				<Provider client={new SmartCardClient(environment)} authFlow={authFlow}>
					<div css={divWithMarginStyles}>
						{this.renderButtons(groupingMode, appearanceMode, environment, authFlow)}
