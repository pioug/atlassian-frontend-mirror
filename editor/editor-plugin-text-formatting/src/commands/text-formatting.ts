...
export const moveRight = (): Command => {
	return (state, dispatch) => {
		...
		if (code) {
			...
			const exitingCode =
				!currentPosHasCode && !nextPosHasCode && (!storedMarks || !!storedMarks.length);
			const enteringCode =
				!currentPosHasCode && nextPosHasCode && (!storedMarks || !storedMarks.length);
	...
	return (state, dispatch) => {
		...
		if (code) {
			...
			const atRightEdge =
				...
				nextNextPosHasCode;
			const enteringCode =
				!currentPosHasCode && nextPosHasCode && Array.isArray(storedMarks) && !storedMarks.length;
...
const createInlineCodeFromTextInput = (from: number, to: number, text: string): Command => {
	return (state, dispatch) => {
