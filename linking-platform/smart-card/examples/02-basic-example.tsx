...
class Example extends React.Component<{}, ExampleState> {
	...
	renderCard(url: string, isSelected: boolean, appearance: CardAppearance) {
		if (url) {
			return <Card isSelected={isSelected} appearance={appearance} url={url} platform="web" />;
		}
		return null;
	}
						...
						<Grid>
									...
									{() => (
										<form>
											...
											<Field name="url" label="Url">
												{() => <Textfield onChange={this.handleUrlChange} value={url} />}
											</Field>
							...
							<GridColumn medium={6}>
								...
								{urlsJSON.map((example: any, i: number) => (
									<p key={i}>
										<Button spacing="compact" onClick={() => this.changeUrl(example.url)}>
											Try it
										</Button>
										&nbsp;
										<a href={example.url}>
