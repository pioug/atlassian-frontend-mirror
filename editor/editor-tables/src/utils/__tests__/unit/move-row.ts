...
import { p, tr as row, td, th, thEmpty } from '@atlaskit/editor-test-helpers/doc-builder';
...
describe('table__moveRow', () => {
	...
	describe('on a table with merged rows', () => {
		it('should move rows', () => {
			const original = createTableWithDoc(
				row(td({ rowspan: 2 })(p('---merged-row----')), td()(p('0')), td()(p('1'))),
				row(td()(p('2')), td()(p('3'))),
	...
	describe('on a complex table with merged columns and rows', () => {
		...
		describe('table 3x5', () => {
			describe('should move', () => {
				...
				const original = createTableWithDoc(
					...
					row(td()(p('c1')), td()(p('c2')), td({ colspan: 2 })(p('c3')), td()(p('c4'))),
				);
			...
			describe('should not move', () => {
				...
				const original = createTableWithDoc(
					row(td({ rowspan: 2 })(p('M1')), td()(p('0')), td()(p('1')), td()(p('2')), td()(p('3'))),
					row(td({ colspan: 2 })(p('4')), td({ rowspan: 2, colspan: 2 })(p('M2'))),
		...
		describe('table 3x4', () => {
			describe('should move', () => {
				const original = createTableWithDoc(
					row(td({ rowspan: 2 })(p('M1')), td()(p('0')), td()(p('1')), td()(p('2'))),
				...
				describe('with tryToFit false', () => {
					it('throw exeception on move row 2 to 1', () => {
						expect(() => moveTableRow(original, 2, 1, { tryToFit: false, direction: 0 })).toThrow();
					});
				});
			});
	...
	describe('with tryToFit true', () => {
		describe('overide the direction when move bottom-to-top', () => {
			...
			//     |______|______|______ ______|
			const original = createTableWithDoc(
				row(td()(p('A1')), td({ colspan: 2 })(p('B1')), td({ rowspan: 2 })(p('D1'))),
				row(td()(p('A2')), td()(p('B2')), td()(p('C2'))),
				row(td()(p('A3')), td({ colspan: 2 })(p('B3')), td({ rowspan: 2 })(p('D3'))),
		...
		describe('overide the direction when move top-to-bottom', () => {
			...
			const original = createTableWithDoc(
				...
				row(td()(p('A2')), td({ colspan: 2 })(p('B2')), td({ rowspan: 2 })(p('D2'))),
				row(td()(p('A3')), td()(p('B3')), td()(p('C3'))),
				row(td()(p('A4')), td({ colspan: 2 })(p('B4')), td({ rowspan: 2 })(p('D4'))),
	...
	describe('on a simple table with header', () => {
		...
		it('should move row header correctly within a single column table', () => {
			const original = createTableWithDoc(row(th()(p('1'))), row(td()(p('2'))), row(td()(p('3'))));

			const newTr = moveTableRow(original, 0, 2);

			const expected = createTableWithDoc(row(th()(p('2'))), row(td()(p('3'))), row(td()(p('1'))));
			expect(newTr.doc).toEqualDocument(expected);
		});

		it('should move col header correctly within a single column table', () => {
			const original = createTableWithDoc(row(th()(p('1'))), row(th()(p('2'))), row(th()(p('3'))));

			const newTr = moveTableRow(original, 0, 2);

			const expected = createTableWithDoc(row(th()(p('2'))), row(th()(p('3'))), row(th()(p('1'))));
			expect(newTr.doc).toEqualDocument(expected);
		});
	});
	...
	describe('on a table with header and varying row node sizes', () => {
		it('should move row from top-to-bottom', () => {
			const original = createTableWithDoc(
				row(th()(p('1')), thEmpty, thEmpty),
				row(td()(p('2'.repeat(10))), td()(p('2'.repeat(10))), td()(p('2222222{<>}222'))),
				row(td()(p('3'.repeat(100))), td()(p('3'.repeat(100))), td()(p('3'.repeat(100)))),
			);

			const newTr = moveTableRow(original, 0, 2);

			const expected = createTableWithDoc(
				row(th()(p('2'.repeat(10))), th()(p('2'.repeat(10))), th()(p('2222222{<>}222'))),
				row(td()(p('3'.repeat(100))), td()(p('3'.repeat(100))), td()(p('3'.repeat(100)))),
				row(td()(p('1')), cEmpty, cEmpty),
			);
			expect(newTr.doc).toEqualDocument(expected);
		...
		it('should move row from bottom-to-top', () => {
			...
			const expected = createTableWithDoc(
				row(th()(p('3'.repeat(100))), th()(p('3'.repeat(100))), th()(p('3'.repeat(100)))),
				row(td()(p('1')), cEmpty, cEmpty),
				row(td()(p('2'.repeat(10))), td()(p('2'.repeat(10))), td()(p('2222222{<>}222'))),
			);
			expect(newTr.doc).toEqualDocument(expected);
