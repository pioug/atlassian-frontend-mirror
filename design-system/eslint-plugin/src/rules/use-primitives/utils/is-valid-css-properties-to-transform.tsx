...
export const isValidCssPropertiesToTransform = (
  ...
  if (
    !cssObjectExpression ||
    !isNodeOfType(cssObjectExpression, 'ObjectExpression')
  ) {
    return false;
  }
  ...
  if (
    !config.patterns.includes('multiple-properties') &&
    Object.keys(cssObject).length > 1
  ) {
    return false;
  }
  ...
  const containsOnlyValidStyles = (
    Object.keys(cssObject) as Array<keyof CSSProperties>
  ).every((styleProperty) => {
    ...
    if (typeof styleValue === 'object') {
      ...
      if (!styleValue.fallbackValue) {
        return (
          ...
          Object.values(supportedStylesMap[styleProperty]).includes(
            styleValue.tokenName,
          )
        );
      }
      // token with fallback
      return (
        supportedStylesMap[styleProperty] &&
        supportedStylesMap[styleProperty][styleValue.fallbackValue] ===
          styleValue.tokenName
      );
    } else {
      // direct value used
