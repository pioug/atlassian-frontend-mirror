...
describe('Viewport', () => {
	describe('Layout', () => {
		...
		describe('visible source bounds', () => {
			...
			it('should give correct portion of total image width when half zoomed in', () => {
				...
				const visibleSourceBounds = viewport.visibleSourceBounds.map(Math.round);
			...
			it('should calculate correct max scale for itemBounds when image smaller than view', () => {
				const viewport = setup();
				viewport.setItemSize(DEFAULT_MAX_ITEM_VIEW_WIDTH / 2, DEFAULT_MAX_ITEM_VIEW_HEIGHT / 2);
	...
	describe('Dragging (panning)', () => {
		it('should update itemBounds when dragged', () => {
			...
			expect(afterDragItemBounds.left - beforeDragItemBounds.left).toEqual(DRAG_DELTA_X);
			expect(afterDragItemBounds.top - beforeDragItemBounds.top).toEqual(DRAG_DELTA_Y);
		});
	});
