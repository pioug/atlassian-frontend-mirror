...
export const useInvalidDateRange = (from?: string, to?: string) => {
  return useMemo(() => {
    ...
    if (dateFrom > now) {
      return (
        <FormattedMessage
          {...dateRangeMessages.dateRangeCustomInvalidDateAfterToday}
        />
      );
    }
    if (dateFrom > dateTo) {
      return (
        <FormattedMessage
          {...dateRangeMessages.dateRangeCustomInvalidDateAfterEnd}
        />
      );
    }
    if (dateTo > now) {
      return (
        <FormattedMessage
          {...dateRangeMessages.dateRangeCustomInvalidToDateAfterToday}
        />
      );
    }
    return null;
