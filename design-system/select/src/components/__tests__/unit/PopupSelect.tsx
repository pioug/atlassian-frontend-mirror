...
const ariaLabelSuffix =
	'Option 0 focused, 1 of 5. 5 results available. Use Up and Down to choose options, press Enter to select the currently focused option, press Escape to exit the menu.';
...
describe('Popup Select', () => {
	...
	describe('accessible labels and group labels', () => {
		it('screen reader should announce the option labels, option serial number, and number of all options', async () => {
			...
			await waitFor(() => {
				expect(
					screen.getByText(/Option 0 focused, 1 of 5. 5 results available/, { exact: false }),
				).toBeVisible();
			});
			...
			await waitFor(() => {
				expect(
					screen.getByText(/Option 1 focused, 2 of 5. 5 results available/, { exact: false }),
				).toBeVisible();
			});
		});
		...
		it('screen reader should announce the group labels, option serial number in the group, and number of options in the group', async () => {
			...
			await waitFor(() => {
				expect(
					screen.getByText(/Option 0, First group, item 1 out of 5./, { exact: false }),
				).toBeVisible();
			});
			...
			await waitFor(() => {
				expect(
					screen.getByText(/Option 1, First group, item 2 out of 5./, { exact: false }),
				).toBeVisible();
			});
			...
			await waitFor(() => {
				expect(
					screen.getByText(/Option 2, First group, item 3 out of 5./, {
						exact: false,
					}),
				).toBeVisible();
			});
