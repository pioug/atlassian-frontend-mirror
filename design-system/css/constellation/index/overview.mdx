...
Use canonical media-query types from granular `@atlaskit/css/at-rules/*` entrypoints with
`satisfies` to keep responsive style keys type-safe. See the
[breakpoint reference](/components/primitives/responsive/breakpoints) for the available breakpoint
queries.
