 ...
 */
export function ensureBlocks(fragment: Fragment, schema: Schema, nodeType?: NodeType): Fragment {
			...
			case 'P':
				if (node.firstElementChild && isMedia(node.firstElementChild)) {
					...
					if (isSchemaWithMedia(schema) && mediaArray.length) {
						const mediaNodeType = isMediaSingle(node.firstElementChild) ? mediaSingle : mediaGroup;
