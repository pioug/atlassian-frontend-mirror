...
describe('Highlight', () => {
  ...
  it('should render unhighlighted code and not call refractor if language is not set', () => {
    const highlight = (
      <Highlight {...props} language={undefined} text={javaCodeLine} />
    );
  ...
  it('should render highlighted code if valid language is set', () => {
    const highlight = (
      <Highlight {...props} language="java" text={javaCodeLine} />
    );
  ...
  it('should render correct number of tokens', () => {
    const highlightLine = (
      <Highlight {...props} showLineNumbers={true} text={javaCodeLine} />
    );
    const highlightBlock = (
      <Highlight {...props} showLineNumbers={true} text={javaCodeBlock} />
    );
    const { container: highlightLineContainer } = render(highlightLine);
    const { container: highlightBlockContainer } = render(highlightBlock);

    expect(highlightLineContainer.getElementsByClassName('token')).toHaveLength(
      3,
    );
    expect(
      highlightBlockContainer.getElementsByClassName('token'),
    ).toHaveLength(24);
  });
  ...
  it('should not render line number element if showLineNumbers flag is set to false', () => {
    const highlight = (
      <Highlight {...props} showLineNumbers={false} text={javaCodeLine} />
    );
    const { container } = render(highlight);
    const lineNumbers = container.getElementsByClassName('linenumber');
