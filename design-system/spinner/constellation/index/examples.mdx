...
`<FadeIn />` from [motion](/components/motion/about)
...
The `<Spinner />` is only visible to a user after `150-200ms` of being rendered because of it's
...
recommend that you use `<FadeIn />` from [motion](/components/motion/about/) to gracefully animate
...
For best results, use `<FadeIn />` from [motion](/components/motion/about/) to fade out the spinner.
