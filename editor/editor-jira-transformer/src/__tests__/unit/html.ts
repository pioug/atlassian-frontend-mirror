...
describe('JIRATransformer html:', () => {
	...
	describe('marks formatting:', () => {
		...
		checkParseEncodeRoundTrips(
			...
			doc(p('Text with ', subsup({ type: 'sub' })('subscript emphasised words'), '.')),
		);

		checkEncode(
			'<em> and <sub>',
			schema,
			doc(p('Text with ', em(subsup({ type: 'sub' })('subscript emphasised words')), '.')),
		...
		checkParseEncodeRoundTrips(
			...
			doc(p('Text with ', subsup({ type: 'sup' })('subscript emphasised words'), '.')),
		);

		checkEncode(
			'<em> and <sup>',
			schema,
			doc(p('Text with ', em(subsup({ type: 'sup' })('subscript emphasised words')), '.')),
	...
	describe('horizontal rule', () => {
		...
		checkParseEncodeRoundTrips('multiple <hr />', schema, '<hr /><hr />', doc(hr(), hr()));
	});
});
