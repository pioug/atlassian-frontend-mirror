...
export const selectNearNode: EditorCommandWithMetadata =
	(selectionRelativeToNode?: RelativeSelectionPos | undefined, selection?: Selection | null) =>
	({ tr }) => {
...
const arrowRightFromNode =
	(selection: NodeSelection): Command =>
	(state, dispatch) => {
...
const setSelectionInsideAtNodeStart =
	(selectionRelativeToNode: RelativeSelectionPos | undefined, node: PmNode, pos: number): Command =>
	(state, dispatch) => {
		if (isNodeEmpty(node)) {
			return findAndSetTextSelection(
				selectionRelativeToNode,
