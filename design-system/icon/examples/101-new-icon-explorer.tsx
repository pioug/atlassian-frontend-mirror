...
const IconAllExample = () => {
  const [singlePurposeAdsIcons, setSinglePurposeAdsIcons] =
    useState<IconsList>();
  ...
  const [singlePurposeLabIcons, setSinglePurposeLabIcons] =
    useState<IconsList>();
      ...
      setSinglePurposeAdsIcons(
        Object.fromEntries(
          Object.entries(icons).filter(
            ([_, icon]) => icon.categorization === 'Single-purpose',
          ),
        ),
      );
  ...
  return (
    <Box padding="space.200">
      <Stack space="space.300">
        ...
        {singlePurposeAdsIcons &&
          filterIcons(singlePurposeAdsIcons, query).length > 0 && (
            <Fragment>
              <Heading size="xsmall">{'Single-Purpose icons'}</Heading>
              {renderIcons(singlePurposeAdsIcons)}
            </Fragment>
          )}
        {multiPurposeAdsIcons &&
          filterIcons(multiPurposeAdsIcons, query).length > 0 && (
            <Fragment>
        ...
        {singlePurposeLabIcons &&
          filterIcons(singlePurposeLabIcons, query).length > 0 && (
            <Fragment>
              <Heading size="xsmall">{'Single-purpose icons'}</Heading>
              {renderIcons(singlePurposeLabIcons)}
            </Fragment>
          )}
        {multiPurposeLabIcons &&
          filterIcons(multiPurposeLabIcons, query).length > 0 && (
            <Fragment>
              <Heading size="xsmall">{'Multi-purpose icons'}</Heading>
