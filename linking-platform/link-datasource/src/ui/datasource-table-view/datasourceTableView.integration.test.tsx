...
describe('SLLV column fallback', () => {
	const actionsStore = defaultRegistry.getStore(ActionsStore);

	const renderTable = ({ visibleColumnKeys }: { visibleColumnKeys: string[] }) =>
		render(
			<DatasourceTableView
				datasourceId="datasource"
				parameters={{
	...
	const mockJiraResponseWithCustomFieldId = ({
		defaultProperties = ['issuekey', 'issuetype', 'summary'],
		...
		mockFetchDatasourceDataEndpoint({
			...
			data: {
				items: [
					{
						ari: { data: 'ari:cloud:jira::issue/1' },
						issuekey: { data: { url: 'https://x.atlassian.net/browse/TEST-1', text: 'TEST-1' } },
						issuetype: { data: { source: '', label: 'Story' } },
						summary: { data: 'Demonstrate the fallback fix' },
						customfield_10004: { data: '5' },
					},
				],
				schema: {
					properties: [
						{ key: 'issuekey', title: 'Key', type: 'link' },
						{ key: 'issuetype', title: 'Type', type: 'icon' },
						{ key: 'summary', title: 'Summary', type: 'string' },
						{ key: 'customfield_10004', title: 'Story Points', type: 'string' },
					],
					defaultProperties,
				},
			},
	...
	ffTest.on('fallback_to_default_columns_to_display_in_sllv', 'with the FG ON', () => {
		it('no overlap: renders the fallback default columns instead of the loading skeleton', async () => {
			mockJiraResponseWithCustomFieldId();

			renderTable({ visibleColumnKeys: ['Story Points'] });

			await screen.findByText('Demonstrate the fallback fix');

			expect(screen.queryByTestId('datasource-table-view-skeleton')).not.toBeInTheDocument();
		});

		it('partial overlap: renders the matching stored columns', async () => {
			mockJiraResponseWithCustomFieldId();

			renderTable({ visibleColumnKeys: ['summary', 'Story Points'] });
		...
		it('full overlap: renders normally without fallback', async () => {
			mockJiraResponseWithCustomFieldId();

			renderTable({ visibleColumnKeys: ['issuekey', 'summary'] });
		...
		it('no overlap AND empty defaultProperties: stays on the loading skeleton', async () => {
			mockJiraResponseWithCustomFieldId({ defaultProperties: [] });
			...
			expect(screen.queryByText('Demonstrate the fallback fix')).not.toBeInTheDocument();
			expect(screen.queryByTestId('datasource-table-view-skeleton')).toBeInTheDocument();
		});
	});

	ffTest.off('fallback_to_default_columns_to_display_in_sllv', 'with the FG OFF', () => {
		it('reproduces the bug: stays on the loading skeleton', async () => {
			mockJiraResponseWithCustomFieldId();
