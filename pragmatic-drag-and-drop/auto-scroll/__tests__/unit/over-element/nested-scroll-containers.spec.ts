...
it('should only scroll one scroll container per axis [case: inner is scrolling on vertical, parent on both]', () => {
  ...
  // validating setup
  expect(isWithin({ client, clientRect: parent.getBoundingClientRect() })).toBe(
    true,
  );
  expect(
    isWithin({ client, clientRect: grandParent.getBoundingClientRect() }),
  ).toBe(true);
  ...
  // scroll the parent until it cannot scroll any more
  {
    ...
    while (parent.scrollTop > 0) {
      ...
      expect(grandParentBefore.scrollLeft).toBeGreaterThan(
        grandParentAfter.scrollLeft,
      );
    }
    expect(hit.mock.calls.length).toBeGreaterThan(1);
  }
  ...
  // finish off scrolling the grand parent
  {
    ...
    while (grandParent.scrollTop > 0 || grandParent.scrollLeft > 0) {
      ...
      expect(grandParentBefore.scrollTop).toBeGreaterThan(
        grandParentAfter.scrollTop,
      );

      // There was already some scroll on the left, so expecting left
      // will finish scrolling before the top
      expect(grandParentBefore.scrollLeft).toBeGreaterThanOrEqual(
        grandParentAfter.scrollLeft,
      );
    }
    expect(hit.mock.calls.length).toBeGreaterThan(1);
...
it('should only scroll one scroll container per axis [case: inner is scrolling on horizontal, parent on both]', () => {
  ...
  // finish off scrolling the grand parent
  {
    ...
    while (grandParent.scrollTop > 0 || grandParent.scrollLeft > 0) {
      ...
      // will finish scrolling before the top
      expect(grandParentBefore.scrollTop).toBeGreaterThanOrEqual(
        grandParentAfter.scrollTop,
      );
    }
    expect(hit.mock.calls.length).toBeGreaterThan(1);
...
it('should ignore scroll containers that have `canScroll: () => false` [case: inner is scrolling on horizontal, parent on both]', () => {
  const [child, parent, grandParent, greatGrandParent] =
    setupNestedScrollContainers([
      // child
      { width: 10000, height: 10000 },
      // parent
  ...
  expect(greatGrandParentBefore.scrollLeft).toBe(
    greatGrandParentAfter.scrollLeft,
  );
  expect(greatGrandParentBefore.scrollTop).toBeGreaterThan(
    greatGrandParentAfter.scrollTop,
  );
