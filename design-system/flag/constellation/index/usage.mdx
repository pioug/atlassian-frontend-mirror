...
Bold flags are not dismissible and use [color](/foundations/color) to reinforce the severity or
...
- If a long message is needed, consider linking to an additional page for more information or using
  an alternative component such as [banner](/components/banner),
  [inline mesage](/components/inline-message), or [modal](/components/modal-dialog).
