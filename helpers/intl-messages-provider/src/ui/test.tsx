...
describe('IntlMessagesProvider', () => {
	...
	const setup = ({ locale, children, loaderFn, defaultMessages, parentMessages }: SetupProps) => {
		return render(
			<IntlProvider locale={locale} messages={parentMessages ?? { bar: 'Another string' }}>
				<IntlMessagesProvider loaderFn={loaderFn} defaultMessages={defaultMessages}>
					{children}
	...
	it('Should display translated messages from upstream IntlProviders', async () => {
		...
		expect(await screen.findByText(messages.bar.defaultMessage)).toBeInTheDocument();
	});
	...
	it('Should prefer to use inherited messages from parent provider instead of `defaultMessages`', async () => {
		 ...
		 */
		expect(screen.queryByText('BAR PARENT PROVIDED MESSAGE')).toBeInTheDocument();
		...
		expect(screen.queryByText('BAR PARENT PROVIDED MESSAGE')).not.toBeInTheDocument();
		expect(screen.queryByText('BAR DEFAULT MESSAGE')).not.toBeInTheDocument();
	});
});
