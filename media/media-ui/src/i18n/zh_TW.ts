...
export default {
  ...
  'fabric.media.video_seeker_assistive_text_time_value':
    '{currentTimeHours} {currentTimeMinutes} {currentTimeSeconds}，總共 {videoTotalHours} {videoTotalMinutes} {videoTotalSeconds}',
  ...
  'media.media-avatar-picker.image-cropper.arrow-keys.action':
    '{key, select, left {您已將圖片從左移到右，使用箭頭按鍵即可移動和裁剪圖片} right {您已將圖片從右移到左，使用箭頭按鍵即可移動和裁剪圖片} top {您已將圖片從頂端移到底部，使用箭頭按鍵即可移動和裁剪圖片} other {您已將圖片從底部移到頂端，使用箭頭按鍵即可移動和裁剪圖片}}',
  'media.media-avatar-picker.image-cropper.arrow-keys.label':
    '可拖曳圖片，避免您的畫面讀取器處於瀏覽模式。使用箭頭按鍵即可移動和裁剪圖片',
