...
1. [element adapter](./element) → handling the dragging of draggable elements
2. [text selection adapter](./text-selection) **(new)** → handling the dragging of text selections
3. [external adapter](./external) **(new)** → handling drag operations that started outside of the current `window` (eg files and text from other `window`s or applications)
...
```diff
- @atlaskit/pragmatic-drag-and-drop-drop-react-indicator
+ @atlaskit/pragmatic-drag-and-drop-drop-react-drop-indicator
```
...
```bash
# Using yarn
yarn remove @atlaskit/pragmatic-drag-and-drop-drop-react-indicator
yarn add @atlaskit/pragmatic-drag-and-drop-drop-react-drop-indicator
```
...
```bash
# Example usage
npx @atlaskit/codemod-cli --preset upgrade-pragmatic-drag-and-drop-to-stable --extensions tsx,ts,js --parser tsx /path/to/source
```
...
Our _file adapter_ has been replaced by a new [_external adapter_](../adapters/external) (and [_internal uncontrolled adapter_](../adapters/internal-uncontrolled)) which can be used for files, as well as any other native data (eg `"text/plain"`).
...
**Old:** [`@atlaskit/pragmatic-drag-and-drop-react-beautiful-dnd-autoscroll`](../../optional-packages/react-beautiful-dnd-autoscroll)

(previously called `@atlaskit/pragmatic-drag-and-drop-autoscroll`)

**New:** [`@atlaskit/pragmatic-drag-and-drop-auto-scroll`](../../optional-packages/auto-scroll)
