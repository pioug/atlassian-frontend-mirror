	...
	data: {
		...
		parameters: [
			...
			},
			{
				...
				type: 'boolean',
				description: 'Search for only entities that have a title that contains the given query',
			},
		],
		schema: {
			defaultProperties: ['type', 'title', 'description', 'space', 'status', 'updatedBy'],
			properties: [
				{
					key: 'type',
					title: 'Type',
...
const buildDataResponse = ({
	...
	const schema = {
		properties: defaultDetailsResponse.data.schema.properties.filter(({ key }) =>
			initialVisibleColumnKeys.includes(key),
		),
	};
