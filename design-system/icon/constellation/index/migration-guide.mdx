...
<SectionMessage
	title="Legacy icon support will end in December 2025. Please migrate your apps now."
	appearance="warning"
	actions={[
...
>
	<p>
		We recommend all apps and partners start migrating your interfaces from the legacy icons to the
		new iconography system to match Atlassian's new design language. Legacy icons will no longer be
		supported from December 2025. This migration guide is designed to guide you on how to complete
		this migration, however, if you have questions please reach out to us on our{' '}
		<a href="https://community.developer.atlassian.com/">developer community forum</a>.
	</p>
</SectionMessage>
...
At this stage, customers should see the new icons. Most of the migration process has been done and automated
through the [no-legacy-icons ESLint auto fixer](/components/eslint-plugin-design-system/no-legacy-icons/usage),
...
system to match Atlassian's new design language.

The new iconography system as part of the Atlassian Design System is now in General Availability,
and we have deprecated the legacy iconography system. It will be removed at the end of
December 2025.
...
1. Migration Support for Connect and Forge Custom UI:
   ...
   - Tools like the [no-legacy icons](/components/eslint-plugin-design-system/no-legacy-icons/usage)
     and the [no-utility-icons](/components/eslint-plugin-design-system/no-utility-icons/usage) lint
     rules can automate much of the process, including migrating away from legacy icons, i.e. the
     `@atlaskit/icon/glyph` package, migrating utility icons to core icons and switching to the
     recommended sizes.
   - The two-step migration (using temporary migration entry points) is no longer recommended. You
     can directly migrate to the @atlaskit/icon/core entry points.
2. Migration support for Forge UI Kit:
   - Please follow the
     [Icon documentation for Forge UI Kit](https://developer.atlassian.com/platform/forge/ui-kit/components/icon/).
