...
<SectionMessage title="Updates to link handling" appearance="information">
	<p>
		Link appearances now only support semantically correct links using link button. If you have a
		legacy button that's used as a link, consider replacing it with a link button or using another
		approach.
	</p>
</SectionMessage>
...
Similar to a subtle button, but looks like a link. Use subtle link buttons with caution. Because
there isn't any color or other visual affordance built in, this style relies on the context around
it to show that it can be interacted with.
