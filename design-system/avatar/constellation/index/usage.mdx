...
An avatar represents a user or entity (such as a project, a repository, or a space), in a product.
They're often combined with status or presence indicators to give more context. Users generally
upload their own image. If they haven't uploaded an image, the avatar will use a default image.

Check you’re using the right avatar shape, size, and features for your use case:

- Use circular avatars to identify users.
- Use square avatars to identify large product entities like projects, spaces, groups, rooms, or
  repositories.
...
- Don't use a [tooltip with an avatar](examples#displaying-a-tooltip) when it's non-interactive or
  disabled. The tooltip won't work for keyboard users and screen readers.
