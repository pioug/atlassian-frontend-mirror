...
import { APPEARANCE_PROP_NAME, IS_HEADING_MULTILINE_PROP_NAME, PACKAGE_NAME } from './constants';
...
export const getOverrideFromIndividualProp = (
	...
	let override;
	getJSXAttributesByName(j, element, attributeName).forEach((attribute: ASTPath<JSXAttribute>) => {
...
export const getOverrideFromComponentsProp = (
	...
	getJSXAttributesByName(j, element, attributeName).forEach((attribute: ASTPath<JSXAttribute>) => {
		...
		if (value && value.type === 'JSXExpressionContainer') {
			if ((value as JSXExpressionContainer).expression.type === 'ObjectExpression') {
...
export const getAppearanceFromProp = (j: core.JSCodeshift, element: ASTPath<JSXElement>) => {
	let appearance;
...
export const getIsMultilineFromProp = (j: core.JSCodeshift, element: ASTPath<JSXElement>) => {
	let isMultiline;
...
export const getComponentImportName = (
	...
	return isAliasImport(componentDefaultName, componentAliasName)
		? j.importSpecifier(j.identifier(componentAliasName), j.identifier(componentDefaultName))
		: j.importSpecifier(j.identifier(componentDefaultName));
};

const isAliasImport = (componentDefaultName: string, componentAliasName: string) =>
...
export const addToImport = (
	...
	source
		...
		.filter((path: ASTPath<ImportDeclaration>) => path.node.source.value === endpoint)
		.forEach((path: ASTPath<ImportDeclaration>) => {
			j(path).replaceWith(
				j.importDeclaration([...(path.value.specifiers || []), ...specifiers], j.literal(endpoint)),
			);
		});
};
