...
import type { DocBuilder, TypeAheadHandler } from '@atlaskit/editor-common/types';
...
import WebBridgeImpl, { MediaBridge, defaultSetList } from '../../implementation';
...
describe('Collab Web Bridge', () => {
	...
	describe('getStepVersion', () => {
		...
		it('calls updateStepVersion with error if it fails', async () => {
			...
			expect(toNativeBridge.updateStepVersion).toBeCalledWith(undefined, 'Error string');
		});
...
describe('AllowList Bridge methods', () => {
	...
	it('should return stringifyed default set from method getQuickInsertAllowList', () => {
		expect(bridgeVer.getQuickInsertAllowList()).toBe(JSON.stringify(defaultSetList));
	});
	...
	it('should return modifed data set when addQuickInsertAllowListItem is called', () => {
		...
		expect(bridgeVer.getQuickInsertAllowList()).toBe(JSON.stringify(newListExpected));
	});

	it('should return modifed data set when removeQuickInsertAllowListItem is called', () => {
		const removeList = ['heading1', 'heading2'];
		const newListExpected = [...defaultSetList].filter((item) => !removeList.includes(item));
...
describe('Bridge with editorConfiguration and onEditorConfigChange', () => {
	...
	it('should initialize the initial editor config', () => {
		...
		expect(bridge.getEditorConfiguration().isQuickInsertEnabled()).toEqual(true);
	});
	...
	it('should have a setter method to set the onEditorConfigChanged handler', async () => {
		...
		const getValuePromise = new Promise<any>((resolve) => resolve(currentDocumentValue));
		jest.spyOn(EditorActions.prototype, 'getValue').mockReturnValue(getValuePromise);

		const editorConfigChanged = jest.fn();
		let bridge: WebBridgeImpl = new WebBridgeImpl();
		const replaceDocumentSpy = jest.spyOn(bridge.editorActions, 'replaceDocument');
...
describe('replaceContent', () => {
	...
	beforeEach(async () => {
		...
		jest.spyOn(EditorActions.prototype, 'replaceDocument').mockReturnValue(true);
		jest
...
describe('Register and Unregister Editor', () => {
	...
	beforeEach(() => {
		bridge = new WebBridgeImpl();
		jest.spyOn(EditorActions.prototype, '_privateGetEditorView').mockReturnValueOnce(editorView);
		jest
	...
	it('should call _privateRegisterEditor when registerEditor method is called', () => {
		const privateRegisterEditor = jest.spyOn(EditorActions.prototype, '_privateRegisterEditor');
	...
	it('should call _privateUnregisterEditor when unregisterEditor method is called', () => {
		const privateUnregisterEditor = jest.spyOn(EditorActions.prototype, '_privateUnregisterEditor');
...
describe('media bridge', () => {
	...
	it('😊 onUploadStart should call uploader.notifyUploadStart and mediaPicker.emit with the expected parameters', async () => {
		...
		const mockParameter = '{"testId":"testContent","preview":{"value":"data:testValue"}}';
		...
		expect(bridge.mediaPicker.emit).toBeCalledWith('upload-preview-update', pickerPayload);
	});
	...
	it('😊 onUploadStart should call uploader.notifyUploadStart and mediaPicker.emit with the expected parameters (wrong scheme)', async () => {
		...
		const mockParameter = '{"testId":"testContent","preview":{"value":"http:testValue"}}';
		await bridge.onUploadStart(mockParameter);
		const startEvent = JSON.parse(mockParameter);
