...
export function applyFullConstraints(imageBounds: Bounds, visibleBounds: Bounds): Vector2 {
	...
	if (imageBounds.right > visibleBounds.right && imageBounds.left > visibleBounds.left) {
		deltaX += deltaLeft;
	}
	if (imageBounds.bottom > visibleBounds.bottom && imageBounds.top > visibleBounds.top) {
		deltaY += deltaTop;
	}
	if (imageBounds.top < visibleBounds.top && imageBounds.bottom < visibleBounds.bottom) {
		deltaY += deltaBottom;
	}
	if (imageBounds.left < visibleBounds.left && imageBounds.right < visibleBounds.right) {
		deltaX += deltaRight;
	}
...
export function applyPartialConstraints(imageBounds: Bounds, visibleBounds: Bounds): Vector2 {
