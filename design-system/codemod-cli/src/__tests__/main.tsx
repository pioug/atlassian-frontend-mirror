...
describe('main', () => {
	...
	it('should exit early if file path is not supplied', async () => {
		await expect(main([''], mockFlags)).rejects.toThrow(
			new ValidationError('Please supply a path to the source files you wish to modify'),
		);
	});
	...
	it('should prompt for a transform if supplied preset not found', async () => {
		...
		expect(global.console.warn).toHaveBeenCalledWith(`No preset found for: wrong-preset-name`);
	...
	describe('Packages flag', () => {
		...
		it('should throw when --packages flag is an invalid format', async () => {
			const invalidFormats = [
				['non-scoped-package', 'Package names must be fully qualified and begin with "@"'],
	...
	describe('Prompt rendering', () => {
		it('should render prompt with preset', async () => {
			...
			(
				AutoComplete as jest.Mock<any, [{ result: Function; choices: string[] }]>
		...
		it('should render prompt with module transform', async () => {
			const transformPath = path.parse('node_modules/@atlaskit/button/src/codemods/foo.ts');
			let choices: string[] = [];
			let result: string = '';
		...
		it('should render prompt with module transform enclosed in dedicated folder', async () => {
			const transformPath = path.parse('node_modules/@atlaskit/button/src/codemods/foo/index.ts');
			let choices: string[] = [];
			let result: string = '';
		...
		it('should render prompt with as file path if transform type is unknown', async () => {
			const transformPath = path.parse('node_modules/unknown/button/src/codemods/foo/index.ts');
			let choices: string[] = [];
			let result: string = '';
			...
			expect(choices).toEqual(['node_modules/unknown/button/src/codemods/foo/index']);
			expect(result).toEqual(expect.objectContaining(transformPath));
		});
	});
});
