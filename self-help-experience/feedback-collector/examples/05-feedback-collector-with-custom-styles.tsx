...
const FormContent = (
  <Fragment>
    <Field name="summary" label="Summary" isRequired validate={validateSummary}>
      {({ fieldProps, error }) => (
        <Fragment>
          ...
          {error === 'SUMMARY_EMPTY' && (
            <ErrorMessage>Summary is invalid.</ErrorMessage>
          )}
        </Fragment>
      )}
    </Field>
    <Field
      ...
      name="components"
      isRequired
      validate={validateSelectComponent}
    >
      {({ fieldProps, error }) => (
        <Fragment>
          <Select {...fieldProps} />
    ...
    <Field label="Description" name="description">
      {({ fieldProps }) => (
        <Fragment>
          ...
          <HelperMessage>
            Please include a full description of how to replicate the problem
            you are experiencing. This may be the steps to replicate, or the
...
const WarningContent = () => (
  <SectionMessage appearance="warning">
    ...
    <span>
      Please check{' '}
      <a
        href="https://ops.internal.atlassian.com/jira/issues/?filter=16189"
        target="_blank"
      >
