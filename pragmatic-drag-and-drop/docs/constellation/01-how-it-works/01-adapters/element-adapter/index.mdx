...
<SectionMessage appearance="discovery" title="Built in browser preview styles">

Web browsers apply their own styles to native drag previews that cannot be controled:

- The opacity of the drag preview is slightly lowered (approximately `opacity:0.8`)
- A box shadow is applied to the drag preview

  <!-- adding the new line for better spacing -->

{' '}

<br />

These styles are applied for all native drag and drop operations on the web, so users will be familiar with these affordances.
