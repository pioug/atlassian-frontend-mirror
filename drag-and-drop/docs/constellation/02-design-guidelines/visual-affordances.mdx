...
Atlassian drag and drop has been architected for fast performance. Atlassian drag and drop gets its performance benefits and bundle size reductions by using the browser's APIs. However, these APIs have limitations that affect the look and feel of the drag and drop experience.

Atlassian drag and drop uses drop indicators to communicate where a draggable object will be moved to at the completion of a drag operation.
...
In lieu of these physical interactions that would normally provide users with direct feedback during drag operations, Atlassian drag and drop changes the visual properties of drop targets instead.
