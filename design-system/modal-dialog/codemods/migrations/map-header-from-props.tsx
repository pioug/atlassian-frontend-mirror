...
export const mapHeaderFromProps = (j: core.JSCodeshift, source: Collection<Node>) => {
	...
	source.findJSXElements(defaultSpecifier).forEach((element: ASTPath<JSXElement>) => {
		const headerFromIndividualProp = getOverrideFromIndividualProp(j, element, HEADER_PROP_NAME);
		...
		const props = appearance
			? [j.objectExpression([j.objectProperty(j.identifier(APPEARANCE_PROP_NAME), appearance)])]
			: [];
...
const appendHeaderAsFirstChild = (
	...
	j(element)
		...
		.forEach((openingElement) => {
			...
			if (openingElementName.type === 'JSXIdentifier' && openingElementName.name === specifier) {
