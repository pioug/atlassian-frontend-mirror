...
test('InlineDialog with Open Modal button should open modal', async ({
  page,
}) => {
...
test('InlineDialog should stay open when user clicks modal blanket', async ({
  page,
}) => {
  await page.visitExample('design-system', 'inline-dialog', 'modal');
  await page.locator(inlineDialogBtn).first().click();
...
test('InlineDialog should close correctly after modal is closed', async ({
  page,
}) => {
  await page.visitExample('design-system', 'inline-dialog', 'modal');
  await page.locator(inlineDialogBtn).first().click();
...
test('InlineDialog should work with Select and set value correctly', async ({
  page,
}) => {
  ...
  await expect(page.locator(inlineDialogTestId).first()).toBeVisible();
  await expect(
    page.locator('.react-select__value-container').first(),
  ).toHaveText('value 1');
});

test('InlineDialog should work correctly with DatePicker component', async ({
  page,
}) => {
  await page.visitExample(
    'design-system',
