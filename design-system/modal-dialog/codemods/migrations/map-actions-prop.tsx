...
export const mapActionsProp = (j: core.JSCodeshift, source: Collection<Node>) => {
	...
	const buttonDefaultSpecifierName = getDefaultSpecifier(j, source, BUTTON_ENDPOINT);
	...
	source.findJSXElements(modalDialogComponentName).forEach((element: ASTPath<JSXElement>) => {
		getJSXAttributesByName(j, element, ACTIONS_PROP_NAME).forEach(
			(attribute: ASTPath<JSXAttribute>) => {
				const actionsPropValue = (attribute.node.value as JSXExpressionContainer)?.expression;

				if (!actionsPropValue || actionsPropValue.type === 'JSXEmptyExpression') {
					return;
				}
	...
	if (isActionsPropPresent) {
		if (defaultSpecifierName) {
			addToImport(j, source, [
				getComponentImportName(j, modalFooterComponentName, MODAL_FOOTER_COMPONENT_NAME),
			]);
...
const getAppearanceForButton = (
	...
	const shouldUseModalDialogAppearance = modalDialogAppearance && defaultValue === 'primary';
	...
	if (isAppearanceSetInActions && shouldUseModalDialogAppearance) {
		return j.logicalExpression(
			...
			j.logicalExpression(
				'||',
				j.memberExpression(j.identifier('props'), j.identifier(APPEARANCE_PROP_NAME)),
				// @ts-ignore
...
const getFooterComponent = (
	...
	const isAppearanceSetInActions = findAppearanceInActions(j, source, element, actions);
	...
	return j.callExpression(j.memberExpression(actions, j.identifier('map')), [
		j.arrowFunctionExpression(
			...
			j.jsxElement(
				...
				j.jsxClosingElement(j.jsxIdentifier(buttonComponentName)),
				[j.jsxExpressionContainer(j.memberExpression(j.identifier('props'), j.identifier('text')))],
			),
		),
	]);
};
...
const getAttributesForButtons = (j: core.JSCodeshift, element: core.ASTPath<JSXElement>) => {
	return [
		j.jsxSpreadAttribute(j.identifier('props')),
		j.jsxAttribute(
			j.jsxIdentifier(APPEARANCE_PROP_NAME),
...
const checkAppearancePropertyExistence = (actions: any) => {
	return (actions as ArrayExpression).elements.some((elem) => {
		return (elem as ObjectExpression).properties.some((property) => {
			return ((property as Property).key as Identifier).name === APPEARANCE_PROP_NAME;
		});
	});
};
...
const getVariableDeclarationByName = (
	...
	return source
		...
		.filter((variableDeclaration: ASTPath<VariableDeclaration>) => {
			return variableDeclaration.node.declarations.some((variableDeclarator) => {
