...
const getOpenTypeAhead = (
	...
	const typeAheadHandler = api?.typeAhead?.actions?.findHandlerByTrigger(trigger);
	...
	return new SafePlugin({
		...
		state: {
			...
			apply: (tr: ReadonlyTransaction, state: PlaceholderTextPluginState) => {
				const meta = tr.getMeta(pluginKey) as Partial<PlaceholderTextPluginState>;
		...
		appendTransaction(transactions, oldState, newState) {
			if (transactions.some((txn) => txn.docChanged)) {
				...
				if (
					...
					if (cursorMoved && (nodeBeforeHasContent || nodeBeforeIsInline)) {
						const { $from, $to } = NodeSelection.create(newState.doc, adjacentNodePos);
			...
			if (
				!(pluginKey.getState(oldState) as PlaceholderTextPluginState).showInsertPanelAt &&
				(pluginKey.getState(newState) as PlaceholderTextPluginState).showInsertPanelAt
			) {
...
const basePlaceholderTextPlugin: PlaceholderTextPlugin = ({ api, config: options }) => ({
	...
	getSharedState(editorState) {
		...
		const { showInsertPanelAt, allowInserting } = pluginKey.getState(editorState) || {
			showInsertPanelAt: null,
		};
		return {
			showInsertPanelAt,
