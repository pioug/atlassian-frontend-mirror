...
export default class MobileEditorToolbarActions {
	...
	private floatingToolbarItems?: Array<FloatingToolbarItem<Command> | undefined> | null;
	...
	notifyNativeBridgeForEditCapabilitiesChanges(
		...
		if (floatingToolbarConfig && Array.isArray(floatingToolbarConfigItems)) {
			...
			if (this.previousItemsJson === itemsJson && this.previousNode === floatingToolbarNode) {
				return;
			}
			...
			setTimeout(() => toNativeBridge.onNodeSelected(nodeTypeName, itemsJson), 0);
		} else {
				...
				switch (item.type) {
					...
					case 'select':
						if (item.selectType === 'list') {
							...
							newItem = this.translateDatePicker(item as FloatingToolbarDatePicker<Command>, index);
	...
	private translateColorPicker(
		...
		if (newItem.defaultValue) {
			newItem.defaultValue.border = this.convertRGBAtoHex(newItem.defaultValue.border);
		}
		return newItem;
	}
	...
	private translateToolbarSelect(
		...
		const newItem: MobileEditorToolbarItem & FloatingToolbarListPicker<Command> = {
			...item,
		};
		newItem.key = this.generateKey(index);
		newItem.options = item.options.map((option: SelectOption, optionIndex: number) => {
			return {
				...option,
				key: this.generateKey(index, optionIndex),
			};
	 ...
	 */
	performEditAction(key: string, editorView: EditorView, value?: string | null): void {
		...
		const [parentIndex, optionIndex] = key.split('.').map((key) => Number.parseInt(key, 10));
	...
	private performButtonClick(item: FloatingToolbarButton<Command>, editorView: EditorView) {
