import { doc, p, ul, li, ol, strong } from '@atlaskit/editor-test-helpers/doc-builder';
	...
	describe('lists', () => {
		...
		checkParseEncodeRoundTrips(
			...
			doc(
				ol()(
					li(p('one')),
					li(p('two'), ol()(li(p('two.one')), li(p('two.two')), li(p('two.three')))),
					li(p('three')),
				),
			),
		);
				...
				ul(
					...
					li(
						...
						ul(
							li(p('two.one')),
							li(p('two.two')),
							li(p('two.three'), ul(li(p('two.three.one')), li(p('two.three.two')))),
							li(p('two.four')),
						),
					),
					li(p('three')),
