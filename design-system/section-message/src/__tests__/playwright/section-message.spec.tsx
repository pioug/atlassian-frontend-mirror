...
test('SectionMessage and SectionMessage actions should be able to be identified and clicked by data-testid', async ({
	...
	await expect(page.locator(sectionMessageBitbucketAction).first()).toBeVisible();
	await expect(page.locator(sectionMessageGoogleAction).first()).toBeVisible();
	await expect(page.locator(sectionMessageInfo).first()).toHaveText(/Atlassian/);
	await expect(page.locator(sectionMessageInfo).first()).toHaveText(/Jira/);
	await expect(page.locator(sectionMessageInfo).first()).toHaveText(/Bitbucket/);
	await expect(page.locator(sectionMessageError).first()).toHaveText(/Google/);
	await expect(page.locator(sectionMessageJiraAction).first()).toHaveText(/Jira/);
	await expect(page.locator(sectionMessageBitbucketAction).first()).toHaveText(/Bitbucket/);
	await expect(page.locator(sectionMessageGoogleAction).first()).toHaveText(/Google/);
