...
describe('<Header />', () => {
	...
	it('passes isArchiveSideBarVisible as false if media type is not archive', () => {
		...
		const el = mountWithIntlContext(
			<Header intl={fakeIntl} mediaClient={mediaClient} identifier={identifier} />,
		);
	...
	describe('Metadata', () => {
		it('should work with external image identifier', () => {
			const element = mountWithIntlContext(
				<Header intl={fakeIntl} mediaClient={{} as any} identifier={externalIdentifierWithName} />,
			);
		...
		it('should default to dataURI as name when no name is passed in a external image identifier', () => {
			const element = mountWithIntlContext(
				<Header intl={fakeIntl} mediaClient={{} as any} identifier={externalIdentifier} />,
			);

			expect(element.find(MetadataFileName).text()).toEqual('some-external-src');
		});

		describe('File collectionName', () => {
			it('shows the title when loaded', () => {
				const mediaClient = fakeMediaClient();
				const fileStateSubscribable = createMediaSubscribable(processedImageState);
				asMockReturnValue(mediaClient.file.getFileState, fileStateSubscribable);
				const el = mountWithIntlContext(
					<Header intl={fakeIntl} mediaClient={mediaClient} identifier={identifier} />,
				);
		...
		describe('File metadata', () => {
			const testMediaTypeText = (mediaType: MediaType, expectedText: string) => {
				...
				const mediaClient = fakeMediaClient();
				asMockReturnValue(mediaClient.file.getFileState, createMediaSubscribable(testItem));
				const el = mountWithIntlContext(
					<Header intl={fakeIntl} mediaClient={mediaClient} identifier={identifier} />,
				);
				el.update();
				expect(el.find(MetadataSubText).text()).toEqual(
					fakeIntl.formatMessage({ defaultMessage: `${expectedText}` }) + ' · 11.7 MB',
				);
			};
			...
			it('should not render file size if unavailable', () => {
				...
				const mediaClient = fakeMediaClient();
				asMockReturnValue(mediaClient.file.getFileState, createMediaSubscribable(noSizeImage));
				const el = mountWithIntlContext(
					<Header intl={fakeIntl} mediaClient={mediaClient} identifier={identifier} />,
				);
				el.update();
		...
		it('MSW-720: passes the collectionName to getFile', () => {
			...
			const el = mountWithIntlContext(
				<Header intl={fakeIntl} mediaClient={mediaClient} identifier={identifierWithCollection} />,
			);
			el.update();
		...
		it('MSW-720: passes the collectionName to mediaClient.file.downloadBinary', () => {
			...
			el.update();
			el.find('button[data-testid="media-viewer-download-button"]').simulate('click');
			expect((mediaClient.file.downloadBinary as jest.Mock).mock.calls[0][2]).toEqual(
				collectionName,
			);
		});
	});
	...
	describe('Download button', () => {
		const assertDownloadButton = (el: ReactWrapper<any, any>, enabled: boolean) => {
	...
	describe('Sidebar button', () => {
		it('should render sidebar button if sidebar component is present', () => {
			...
			el.update();
			expect((el.find(MediaButton).at(0).prop('iconBefore') as any).type).toEqual(EditorPanelIcon);
		});
	});
	...
	describe('CodeViewer Support', () => {
		...
		it.each([
			...
			(filename, expectedHeaderText) => {
				const mediaClient = fakeMediaClient();
				const fileStateSubscribable = createMediaSubscribable(fakeFileState(filename));
				asMockReturnValue(mediaClient.file.getFileState, fileStateSubscribable);
				const el = mountWithIntlContext(
					<Header intl={fakeIntl} mediaClient={mediaClient} identifier={identifier} />,
				);
