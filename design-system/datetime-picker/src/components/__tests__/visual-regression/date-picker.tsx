...
async function waitForCalendarPicker(page: PuppeteerPage, testId: string) {
	...
	await page.waitForSelector(`[data-testid="${testId}--calendar--current-month-year"]`);
	await page.waitForSelector(`[data-testid="${testId}--calendar--month"]`);
	await page.waitForSelector(`[data-testid="${testId}--calendar--selected-day"]`);
}

describe('date picker snapshots', () => {
	it('should use the correct weekStartDay, when in a DatePicker', async () => {
		const url = getExampleUrl('design-system', 'datetime-picker', 'i18n', global.__BASEURL__);
		const { page } = global;
	...
	it('should use the correct weekStartDay, when in a DateTimePicker', async () => {
		...
		const datePickerSelector = '[data-testid="datetime-picker--datepicker--container"]';
		await page.waitForSelector(datePickerSelector);
		await page.click(datePickerSelector);
