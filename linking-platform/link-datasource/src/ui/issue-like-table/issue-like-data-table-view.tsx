...
const tableSidePadding = token('space.200');
...
// eslint-disable-next-line @atlaskit/ui-styling-standard/no-styled -- To migrate as part of go/ui-styling-standard
const ColumnPickerHeaderWithSettings = styled.th({
	// eslint-disable-next-line @atlaskit/ui-styling-standard/no-nested-selectors, @atlaskit/ui-styling-standard/no-unsafe-values, @atlaskit/ui-styling-standard/no-imported-style-values, @atlaskit/ui-styling-standard/no-unsafe-selectors -- Ignored via go/DSP-18766
	'.pm-table-wrapper > table thead &:last-of-type, .ProseMirror .pm-table-wrapper > table thead &:last-of-type, &:last-of-type':
		{
			boxSizing: 'border-box',
			border: 0,
			width: `${columnPickerWithSettingsWidth}px`,
			zIndex: 10,
			position: 'sticky',
			right: `calc(-1 * ${tableSidePadding})`,
			background: `linear-gradient( 90deg, rgba(255, 255, 255, 0) 0%, ${token(
...
const noResultsCellContentStyles = css({
	position: 'sticky',
	left: 0,
	display: 'flex',
...
const getHorizontalScrollParent = (element: HTMLElement): HTMLElement | undefined => {
...
export const IssueLikeDataTableView = ({
	...
	const isWrapTextSettingAvailable =
		Boolean(onWrappedColumnsChange) && fg('platform_lp_sllv_table_settings_menu');

	let areAllVisibleColumnsWrapped = false;
	if (isWrapTextSettingAvailable) {
		areAllVisibleColumnsWrapped =
			visibleColumnKeys.length > 0 &&
			visibleColumnKeys.every((columnKey) => wrappedColumnKeys?.includes(columnKey));
	}
	...
	const handleWrapTextOnAllColumnsChange = useCallback(() => {
		...
		const nextWrappedColumnKeys = areAllVisibleColumnsWrapped
			? (wrappedColumnKeys ?? []).filter((columnKey) => !visibleColumnKeys.includes(columnKey))
			: Array.from(new Set([...(wrappedColumnKeys ?? []), ...visibleColumnKeys]));
	...
	const tableSettingsMenuProps: TableSettingsMenuProps = {
	...
	const columnsWidthsSum = useMemo(
		() =>
			...
			(onVisibleColumnKeysChange && shouldShowTableSettingsMenu
				? columnPickerWithSettingsWidth
				: onVisibleColumnKeysChange
					? columnPickerWidth
					: 0),
		[
			...
			>
				...
				>
					<tr ref={tableHeaderRowRef}>
						...
						{onVisibleColumnKeysChange &&
							(shouldShowTableSettingsMenu ? (
								<ColumnPickerHeaderWithSettings>
									<Inline alignBlock="center" alignInline="end" space="space.050">
										...
										/>
										<TableSettingsMenu wrapTextSetting={tableSettingsMenuProps.wrapTextSetting} />
									</Inline>
								</ColumnPickerHeaderWithSettings>
							) : (
								<ColumnPickerHeader>
