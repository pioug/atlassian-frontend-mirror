...
function isNonNullSelectOption(edge: SelectOption | null): edge is SelectOption {
	return edge !== null;
}
...
const checkAndConvertToAbsoluteUrl = (url: string, siteUrl?: string): string => {
	if (!url) {
		return '';
	}
