...
describe('Snapshot Test', () => {
	...
	it('Calendar basic should match production example', async () => {
		const url = getExampleUrl('design-system', 'calendar', 'basic', global.__BASEURL__);
	...
	it('Calendar should switch dates', async () => {
		const url = getExampleUrl('design-system', 'calendar', 'controlled', global.__BASEURL__);
		...
		const imageOnHoveringDiffDate = await takeElementScreenShot(page, calendarSelector);
		expect(imageOnHoveringDiffDate).toMatchProdImageSnapshot();

		await page.mouse.down();
		const imageOnMouseDown = await takeElementScreenShot(page, calendarSelector);
		...
		const imageOnSwitchingToDiffDate = await takeElementScreenShot(page, calendarSelector);
	...
	it('Calendar should switch months', async () => {
		...
		const imageOnSwitchingToNextMonth = await takeElementScreenShot(page, calendarSelector);
		...
		const imageOnSwitchingBackToPrevMonth = await takeElementScreenShot(page, calendarSelector);
		expect(imageOnSwitchingBackToPrevMonth).toMatchProdImageSnapshot();
	});

	it('Calendar of different localization', async () => {
		const url = getExampleUrl('design-system', 'calendar', 'i18n', global.__BASEURL__);
		...
		const imageOfBrasilLocalizeCalendar = await takeElementScreenShot(page, calendarSelector);
	...
	it('Calendar with different week start day', async () => {
		...
		await page.keyboard.press('Enter');
		const imageOfCalendarStartingFromTuesday = await takeElementScreenShot(page, calendarSelector);
