...
class ExperimentController extends Component<Props, State> {
	...
	resolveEnrollmentForExperiment(experimentKey: ExperimentKey) {
		const { experimentEnrollmentConfig, experimentEnrollmentOptions: options } = this.props;

		const enrollmentResolver = experimentEnrollmentConfig[experimentKey];

		// updates context after resolving
		const enrollmentOptions = options instanceof Function ? options(experimentKey) : options;
		const enrollmentPromise = Promise.resolve(enrollmentResolver(enrollmentOptions));
