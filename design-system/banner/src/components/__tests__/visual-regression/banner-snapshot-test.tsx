...
describe('@atlaskit/banner visual regression', () => {
	...
	it('error banner example should match production example', async () => {
		const url = getExampleUrl('design-system', 'banner', 'error-banner', global.__BASEURL__);
		const { page } = global;
		await loadPage(page, url);
		const element = await waitForBannerVisible(page);
		const image = await element?.screenshot();
	...
	it('open/close behavior should match production example', async () => {
		const url = getExampleUrl('design-system', 'banner', 'open-close-example', global.__BASEURL__);
		const { page } = global;
		await loadPage(page, url);
	...
	it('banner with link example should match production example', async () => {
		const url = getExampleUrl('design-system', 'banner', 'banner-with-link', global.__BASEURL__);
		const { page } = global;
		await loadPage(page, url);
		const element = await waitForBannerVisible(page);
		const image = await element?.screenshot();
