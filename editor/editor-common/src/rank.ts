...
export function sortByOrder(item: 'plugins' | 'nodes' | 'marks') {
  return function (a: { name: string }, b: { name: string }): number {
    return (
      rankEditorPlugins[item].indexOf(a.name) -
      rankEditorPlugins[item].indexOf(b.name)
    );
  };
}
...
export function sortByOrderWithTypeName(item: 'plugins' | 'nodes' | 'marks') {
  return function (
    ...
    return (
      rankEditorPlugins[item].indexOf(a.type.name) -
      rankEditorPlugins[item].indexOf(b.type.name)
    );
  };
}
