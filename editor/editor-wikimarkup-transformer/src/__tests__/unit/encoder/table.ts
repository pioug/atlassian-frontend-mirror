...
describe('ADF => WikiMarkup - Table', () => {
	...
	test('should convert Table node without a header row', () => {
		const node = doc(
			table()(tr(td()(p('Cell 1')), td()(p('Cell 2'))), tr(td()(p('Cell 3')), td()(p('Cell 4')))),
		)(defaultSchema);
		expect(transformer.encode(node)).toMatchSnapshot();
	});
	...
	test('should convert advanced Table node with merged cells into {adf} macro', () => {
		const node = doc(
			table()(
				tr(td({ colspan: 2 })(code_block({ language: 'javascript' })('const i = 0;'))),
				tr(
					td()(code_block({ language: 'javascript' })('const i = 0;')),
					td()(code_block({ language: 'javascript' })('const i = 0;')),
				),
	...
	test('should convert empty table cells', () => {
		const node = doc(table()(tr(th()(p())), tr(td()(p())), tr(td()(p()))))(defaultSchema);
		expect(transformer.encode(node)).toMatchSnapshot();
	});

	test('should convert table with newline in cell', () => {
		const node = doc(table()(tr(td()(p('hello world'), p())), tr(td()(p('hello world')))))(
			defaultSchema,
		);
		expect(transformer.encode(node)).toMatchSnapshot();
	});
	...
	test('should convert table with header column', () => {
		const node = doc(
			table()(
				tr(th()(p('Header 0')), th()(p('Header Row 1')), th()(p('Header Row 2'))),
