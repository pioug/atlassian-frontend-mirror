...
3. **Controls:** Control items can include [buttons](/components/button),
   [checkboxes](/components/checkbox), and [text fields](/components/textfield).
...
- Use [inline messages](/components/inline-message) to alert users that important information is
  available or an action is required to complete the current task.
- See [flags](/components/flag) for messages containing confirmations, alerts, and acknowledgments
