...
describe('Media Viewer Navigation', () => {
	...
	const url = getExampleUrl('media', 'media-viewer', 'vr-mocked-viewer', global.__BASEURL__);
	...
	it.skip('renders a file and nav button given multiple files', async () => {
		...
		await page.waitForFunction(`window.areControlsRendered() && window.areControlsVisible()`);
	...
	it('hides nav for multiple files after a timeout', async () => {
		...
		await page.waitForSelector(pageSelector);
		await page.waitForSelector('img');
		await page.waitForFunction(`window.areControlsRendered() && window.areControlsHidden()`);
		await sleep(500);
