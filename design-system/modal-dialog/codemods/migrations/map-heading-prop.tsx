...
export const mapHeadingPropToModalTitle = (j: core.JSCodeshift, source: Collection<Node>) => {
		...
		getJSXAttributesByName(j, element, HEADING_PROP_NAME).forEach(
			(attribute: ASTPath<JSXAttribute>) => {
				...
				if (element.node.children) {
					element.node.children.unshift(j.jsxText('\n'), headerComponent, j.jsxText('\n'));
				} else {
					element.node.children = [j.jsxText('\n'), headerComponent, j.jsxText('\n')];
				}
			},
		);
	});
	...
	if (doesHeadingPropExist) {
		if (defaultSpecifierName) {
			addToImport(j, source, [
				getComponentImportName(j, modalTitleComponentName, MODAL_TITLE_COMPONENT_NAME),
				getComponentImportName(j, modalHeaderComponentName, MODAL_HEADER_COMPONENT_NAME),
			]);
...
const getModalTitleAttributes = (j: core.JSCodeshift, element: ASTPath<JSXElement>) => {
	...
	if (appearance !== undefined) {
		...
		const appearanceAttr = j.jsxAttribute(j.jsxIdentifier(APPEARANCE_PROP_NAME), appearanceValue);
