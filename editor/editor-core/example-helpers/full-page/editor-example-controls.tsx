...
export const EditorExampleControls = React.memo((props: EditorExampleControlsProps) => {
	const {
		adfEnabled,
		editorEnabled,
		errorsEnabled,
	...
	const onSanitizePrivateContentCb = React.useCallback(
		() => (onSanitizePrivateContent ? onSanitizePrivateContent(!sanitizePrivateContent) : null),
		...
		<div css={controls}>
			{props.appearanceOptions && props.onAppearanceChange && (
				<Select
					...
					defaultValue={props.appearanceOptions.find((opt: any) => opt.value === props.appearance)}
					onChange={({ value }: any) =>
