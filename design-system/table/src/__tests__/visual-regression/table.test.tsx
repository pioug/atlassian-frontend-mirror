...
describe('@atlaskit/table', () => {
	it.each(['light', 'dark'] as const)(
		...
		async (mode) => {
			...
			const url = getExampleUrl('design-system', 'table', 'basic', __BASEURL__, mode);
	...
	it('actions overlay should match production example', async () => {
		const { __BASEURL__, page } = global;
		const url = getExampleUrl('design-system', 'table', 'basic-with-actions', __BASEURL__);
	...
	it('multi direction header should match production example', async () => {
		const { __BASEURL__, page } = global;
		const url = getExampleUrl('design-system', 'table', 'multi-header', __BASEURL__);
	...
	it('table with sortable columns should match production example', async () => {
		...
		await loadPage(page, url);
		const button = await page.waitForSelector('[data-testid="column-name--button"]');
		await button?.click();
		const image = await takeElementScreenShot(page, selector);
		expect(image).toMatchProdImageSnapshot();
	});
