...
test('select.ts: Mod+a twice fires selectAllEscaped event', async ({ renderer }) => {
	await addSentinels(renderer.page);
...
test('select.ts: Mod+a twice, click, Mod+a does not select before', async ({ renderer }) => {
	await addSentinels(renderer.page);
...
test('select.ts: Mod+a twice, click, Mod+a does not select after', async ({ renderer }) => {
	await addSentinels(renderer.page);
...
test('select.ts: Mod+a twice, click, Mod+a selects renderer', async ({ renderer }) => {
	await addSentinels(renderer.page);
...
const shortcutSearchTabs = process.platform === 'darwin' ? 'Meta+Shift+a' : 'Control+Shift+a';

test("select.ts: Mod+Shift+a doesn't select all content", async ({ renderer }) => {
	const rendererLocator = renderer.page.locator(RENDERER_CONTAINER);
	await rendererLocator.click();
