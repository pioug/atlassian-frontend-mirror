...
- If using a field with a label other than "Date", use `datePickerSelectProps` to pass the `aria-label` prop. Assistive technologies will announce these along with the label from the field component when activated.
