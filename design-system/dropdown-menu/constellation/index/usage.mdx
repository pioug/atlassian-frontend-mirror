...
- When using a dropdown menu inside a modal dialog, use the `shouldRenderToParent` prop to prevent loss of focus and proper voicing by screen reading programs to render content in the nearest DOM node to the trigger element instead of `React.Portal`.
