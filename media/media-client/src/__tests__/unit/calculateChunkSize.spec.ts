...
import { calculateChunkSize, fileSizeError } from '../../uploader/calculateChunkSize';
...
describe('calculateChunkSize', () => {
	it.each(testCases)('chunk size should not be more than 200MB', (_, __, fileSize) =>
		expect(calculateChunkSize(fileSize)).toBeLessThanOrEqual(220200960),
	);

	it.each(testCases)('chunk size should not be less than 5MB', (_, __, fileSize) =>
		expect(calculateChunkSize(fileSize)).toBeGreaterThanOrEqual(5242880),
	);

	it.each(testCases)('chunk count should not be more than 10000', (_, __, fileSize) =>
		expect(fileSize / calculateChunkSize(fileSize)).toBeLessThanOrEqual(10000),
	);

	it.each(testCases)('chunk count should not be less than 1', (_, __, fileSize) =>
		expect(fileSize / calculateChunkSize(fileSize)).toBeGreaterThanOrEqual(1),
	);

	it.each(testCases)('to return %i when the file size is %s: %iB )', (expected, _, fileSize) =>
