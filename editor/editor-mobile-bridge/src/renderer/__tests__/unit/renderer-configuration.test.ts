...
describe('render configuration', () => {
	describe('default values', () => {
		it.each([
			...
			const rendererConfiguration = new RendererConfiguration(JSON.stringify(rendererConfig));
			expect(rendererConfiguration.getLocale()).toBe(expectedConfig.locale);
			expect(rendererConfiguration.isActionsDisabled()).toBe(expectedConfig.disableActions);
			expect(rendererConfiguration.isMedialinkingDisabled()).toBe(
				expectedConfig.disableMediaLinking,
			);
			expect(rendererConfiguration.isAnnotationsAllowed()).toBe(expectedConfig.allowAnnotations);
			expect(rendererConfiguration.isHeadingAnchorLinksAllowed()).toBe(
				expectedConfig.allowHeadingAnchorLinks,
			);
			expect(rendererConfiguration.isCustomPanelEnabled()).toBe(expectedConfig.allowCustomPanel);
		});
	});
	...
	describe('clone and update', () => {
		...
		it('should update the disableActions value when passed as config', () => {
			...
			expect(newRenderConfiguration.getLocale()).toBe(rendererConfiguration.getLocale());
		...
		it('should retain the original value when not configured explicitly', () => {
			...
			const newRenderConfiguration = rendererConfiguration.cloneAndUpdate(JSON.stringify({}));
