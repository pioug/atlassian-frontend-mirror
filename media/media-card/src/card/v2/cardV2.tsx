...
export class CardV2Base extends Component<CardV2BaseProps, CardState> {
  ...
  private subscribeFileState = (fileState: FileState) => {
    const { isBannedLocalPreview } = this.state;
  ...
  render() {
    ...
    if (shouldUpdateStateForIdentifier) {
      const { id, collectionName, occurrenceKey } = this.props
        .identifier as FileIdentifier;
      wrappedContent = (
        <FileStateListener
          id={id}
    ...
    return this.props.intl ? (
      ...
      <IntlProvider locale="en">{wrappedContent}</IntlProvider>
    );
  }
...
interface FileStateListenerProps {
