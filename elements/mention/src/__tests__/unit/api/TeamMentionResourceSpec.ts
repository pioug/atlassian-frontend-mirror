...
describe('TeamMentionResourceSpec', () => {
	...
	beforeEach(() => {
		...
		resource = new TeamMentionResource(apiUserMentionConfig, apiTeamMentionConfig);
		...
		fetchMock
			...
			.mock(/\/teams\/mentions\/search\?.*query=query-only-teams-fail(&|$)/, 500)
			.mock(/\/users\/mentions\/search\?.*query=query-only-users-fail(&|$)/, 500)
	...
	describe('#subscribe', () => {
		it('should receive 2 updates: from user request first and then from team request', (done) => {
			...
			resource.subscribe('craig', (mentions) => {
				...
				if (currentCount === 1) {
					...
					const teamLink = firstTeam.context ? firstTeam.context.teamLink : null;
		...
		it('should receive analytics events for both user and team search', async () => {
			...
			expect(analytics).toHaveBeenNthCalledWith(1, 'sli', 'searchUser', 'succeeded', undefined);
			expect(analytics).toHaveBeenNthCalledWith(2, 'sli', 'searchTeam', 'succeeded', undefined);
		});
		...
		it('should receive analytics events when team search fails', async () => {
			...
			expect(analytics).toHaveBeenCalledTimes(2);
			expect(analytics).toHaveBeenCalledWith('sli', 'searchUser', 'succeeded', undefined);
			expect(analytics).toHaveBeenCalledWith('sli', 'searchTeam', 'failed', undefined);
		});
		...
		it('should receive analytics events when both searches fail', async () => {
			...
			expect(analytics).toHaveBeenCalledTimes(2);
			expect(analytics).toHaveBeenCalledWith('sli', 'searchUser', 'failed', undefined);
			expect(analytics).toHaveBeenCalledWith('sli', 'searchTeam', 'failed', undefined);
		});
		...
		it('should receive analytics events when user search fails', async () => {
			...
			expect(analytics).toHaveBeenCalledTimes(2);
			expect(analytics).toHaveBeenCalledWith('sli', 'searchUser', 'failed', undefined);
			expect(analytics).toHaveBeenCalledWith('sli', 'searchTeam', 'succeeded', undefined);
		});
	});
	...
	describe('#recordTeamMentionSelection', () => {
		it('should send analytics event when a team is selected', async () => {
			...
			expect(analytics).toHaveBeenCalledWith('sli', 'selectTeam', 'succeeded', undefined);
		});
