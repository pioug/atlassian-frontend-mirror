...
after table`;
		...
		checkParseEncodeRoundTrips(
			...
			doc(
				p('before table'),
				table()(
					tr(th({})(p('colA')), th({})(p('colB')), th({})(p('colC')), th({})(p('colD'))),
					tr(td({})(p('cola')), td({})(p('colb')), td({})(p('colc')), td({})(p('cold'))),
				),
				p(),
				p('after table'),
			),
		...
		checkParseEncodeRoundTrips(
			...
			doc(
				table()(
					...
					tr(td({})(p('and')), td({})(p('here', hardBreak(), 'the ', strong('data'), ' and'))),
					tr(td({})(p('even more')), td({})(p(em('and more'), hardBreak(), 'data'))),
				),
			),
		);
	});
...
h2. header`;
		...
		checkParseEncodeRoundTrips(
			...
			doc(
				...
				table()(
					...
					tr(td({})(p('ddd')), td({})(p('eee')), td({})(p('fff', hardBreak(), '  gggg hhhh'))),
				),
				h2('header'),
			),
		);
	...
	describe('single cell with link', () => {
		...
		checkParseEncodeRoundTrips(
			...
			defaultSchema,
			WIKI_NOTATION,
			doc(table()(tr(td({})(p(link({ href: 'http://www.example.com' })('foo')))))),
		);
	});
...
# Item 5`;
		...
		checkParseEncodeRoundTrips(
			...
			doc(
				...
				ol()(
					li(p('Item 4', ol()(li(p('Item 4.1')), li(p('Item 4.2')), li(p('Item 4.3'))))),
					li(p('Item 5')),
				),
			),
		);
		...
		checkParseEncodeRoundTrips(
			...
			doc(
				table()(
					...
					tr(
						td({})(p('Here is my list', ul(li(p('Item A')), li(p('Item B')), li(p('Item C'))))),
...
* I like cheese |`;
		...
		checkParseEncodeRoundTrips(
			...
			defaultSchema,
			WIKI_NOTATION,
			doc(ul(li(p('I like | cheese')), li(p('I like | cheese')), li(p('I like cheese |')))),
		);
	});
