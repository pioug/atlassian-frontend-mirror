...
const Example = () => {
	...
	const mockClient = useMemo(() => {
		...
		client.setRequestHandler(UserAndGroupSearchQuery, async ({ query }: { query: string }) => {
			await new Promise((res) => setTimeout(res, 1000));
			return {
				data: {
					userGroupSearch: {
						users: users.filter(
							(user) => user.displayName && !user.displayName.toLowerCase().includes(query),
						),
	...
	const onError = useCallback(
		(error: any, request: RecommendationRequest) => {
			...
			return getRecommendations<RecommendationRequest, OptionData>(error, request, mockClient);
		},
	...
	return (
		<IntlProvider locale="en">
			...
			<ExampleCheckbox isChecked={ursRequestsSucceed} setIsChecked={setUrsRequestsSucceed}>
