...
describe('AssetsConfigModal', () => {
	const setup = (props: Partial<RenderAssetsContentProps> = {}) => {
		...
		const mockOnVisibleColumnKeysChange = jest.fn((visibleColumnKeys: string[]) => {});
	...
	it('Should display loading view when isFetchingInitialData', async () => {
		...
		expect(queryByTestId('assets-aql-datasource-modal--loading-state')).toBeInTheDocument();
	});
	...
	it('Should display error view when the status is rejected', async () => {
		...
		expect(queryByTestId('datasource-modal--loading-error')).toBeInTheDocument();
	});
