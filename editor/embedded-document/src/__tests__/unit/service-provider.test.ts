...
describe('ServiceProvider', () => {
	...
	describe('getDocumentByObjectId', () => {
		beforeAll(() => {
			fetchMock.get(`begin:${providerUrl}`, (url: string) => {
				if (url === `${providerUrl}/document?objectId=${encodeURIComponent(objectId)}`) {
		...
		it('should return null if document does not exist', async () => {
			const response = await serviceProvider.getDocumentByObjectId('does-not-exist');
			expect(response).toEqual(null);
		});
	});
	...
	describe('createDocument', () => {
		...
		it('should return new document from service', async () => {
			const response = await serviceProvider.createDocument(JSON.stringify(validContent), objectId);
