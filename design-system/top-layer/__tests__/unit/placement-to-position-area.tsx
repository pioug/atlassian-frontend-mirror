...
function positionAreaFor(placement: TPlacementOptions): string {
	return placementToPositionArea({ placement: resolvePlacement({ placement }) });
}
...
describe('placementToPositionArea()', () => {
	describe('single-axis (centered) placements', () => {
		it.each([
			...
			expect(positionAreaFor(placement)).toBe(expected);
		});
	});

	describe('defaults: empty object resolves to block-end', () => {
		it('maps {} to "block-end"', () => {
			expect(positionAreaFor({})).toBe('block-end');
		});
	});
...
describe('placementToTryFallbacks()', () => {
	describe('centered block placements include cross-axis shifts then flip-block', () => {
		it.each([
			...
			(placement, expected) => {
				expect(tryFallbacksFor(placement)).toBe(expected);
			},
		);
	});
	...
	describe('aligned placements re-centre and slide far on the same edge, flip, flip diagonally, then re-centre on the opposite edge', () => {
		it.each([
			[
				{ axis: 'block', edge: 'start', align: 'start' },
				'block-start, block-start span-inline-start, flip-block, flip-block flip-inline, block-end',
			],
			[
			...
			[
				...
				'block-end, block-end span-inline-end, flip-block, flip-block flip-inline, block-start',
			],
			[
				{ axis: 'inline', edge: 'start', align: 'end' },
				'inline-start, inline-start span-block-end, flip-inline, flip-inline flip-block, inline-end',
			],
			[
				{ axis: 'inline', edge: 'end', align: 'start' },
				'inline-end, inline-end span-block-start, flip-inline, flip-inline flip-block, inline-start',
			],
		] as [TPlacementOptions, string][])(
			'returns position-area fallbacks for %o',
			(placement, expected) => {
				expect(tryFallbacksFor(placement)).toBe(expected);
			},
		);
