...
import ChessboardDropTargetsCanDrop from '../../examples/pieces/getting-started/chessboard-drop-targets-can-drop.tsx';
import ChessboardWithMonitor from '../../examples/pieces/getting-started/chessboard-monitor';

Welcome to Pragmatic drag and drop!

This guide will walk you through the basic elements of the library including draggables, drop targets and monitors.
...
To see the full `draggable` documentation see [this page](how-it-works/adapters/element-adapter).

## Step 2: Making The Squares Into Drop Targets
...
function Square({ pieces, location, children }: SquareProps) {
  ...
  useEffect(() => {
    ...
    return dropTargetForElements({
      element: el,
      onDragEnter: ({ source }) => { // source is the piece being dragged over the drop target
        if ( // type guards
...
See [this page](how-it-works/drop-targets), for the full documentation on drop targets.
  ...
  useEffect(() => {
    return monitorForElements({
      onDrop({ source, location }) {
        ...
        if (!destination) { // if dropped outside of any drop targets
          return;
        }
...
See [this page](how-it-works/monitors), for the full documentation on monitors.

## Now it's Your Turn

You're now ready to start building your own projects with Pragmatic drag and drop.

For more examples see [this page](examples), or to see how to drag and drop files with Pragmatic drag and drop see [this page](how-it-works/adapters/file-adapter) .
