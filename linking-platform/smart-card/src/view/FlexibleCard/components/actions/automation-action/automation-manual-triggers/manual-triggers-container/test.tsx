...
describe('ManualRulesContainer', () => {
  ...
  const mockUseManualRules = (
    ...
    jest
      .fn()
      .mockImplementation(() => [async () => {}, initialising, error, rules]);
    ...
    renderWithDi(
      <ManualRulesContainer {...{ env, site, query, ...customProps }}>
        {({ invokeRuleOrShowDialog }) => (
          <InvokeButton
            onClick={() => invokeRuleOrShowDialog(ruleId, objects)}
          />
        )}
      </ManualRulesContainer>,
      customDeps ?? [],
  ...
  test('Should trigger failure callback if the api reports a failure', async () => {
    ...
    const mockInvokeManuallyTriggeredRule = jest
      .fn()
    ...
    const wrapper = renderWithInvokeButton(
      ...
      [
        ...
        injectable(
          ...
          mockInvokeManuallyTriggeredRule,
        ),
      ],
    );
    clickInvokeButton(wrapper);
