...
- Always provide an `ariaLabel` that explains what the indicator represents. For example, "Account
  setup". The progress number will be appended to the label.
- If interaction is enabled, use the `ariaControls` prop to tell assistive technology what elements
  are controlled by the progress indicator. For example, if interacting with the progress indicator
  changes tabs, specify this with `ariaControls`.
