...
describe('PNG Image Metadata', () => {
	describe('Parse PNG Chunks', () => {
		...
		const pHYsDataArrayInMeters = new Uint8Array([0, 0, 22, 37, 0, 0, 22, 37, 1]);
		const pHYsDataArrayNotInMeters = new Uint8Array([0, 0, 22, 37, 0, 0, 22, 37, 0]);
		const chunks = [
		...
		it('should find the iTXt chunk', async () => {
			...
			const expectedStr = String.fromCharCode.apply(null, Array.from(iTXtDataArray));
