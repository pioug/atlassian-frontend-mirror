...
export default class APSProtocol implements Protocol {
	...
	subscribe(config: APSProtocolConfig): void {
		this.activeTransport.subscribe(new Set(config.channels)).catch((errorPrimary) => {
			logDebug(
			...
			if (this.skipFallback) {
				...
				logDebug(`Retrying with fallback transport: ${this.fallbackTransport.transportType()}`);
				this.activeTransport.subscribe(new Set(config.channels)).catch((errorFallback) => {
