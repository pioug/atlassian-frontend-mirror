...
describe('SyncInfo Component', () => {
  beforeAll(() => {
    jest.useFakeTimers();
    Date.now = jest.fn(() =>
      new Date(Date.UTC(2022, 0, 20, 0, 0, 0)).valueOf(),
    );
  });
  ...
  it('should update the sync info right away when lastSyncTime changes', () => {
    const { getByText, rerender } = setup(
      new Date(Date.UTC(2022, 0, 19, 23, 59, 10)),
    );
    ...
    rerender(
      <IntlProvider locale={'en'}>
        <SyncInfo
          lastSyncTime={new Date(Date.UTC(2022, 0, 19, 23, 55, 10))}
        ></SyncInfo>
      </IntlProvider>,
    );
  ...
  it('should update the sync info when 1 second passed', () => {
    ...
    asMock(Date.now).mockReturnValue(
      new Date(Date.UTC(2022, 0, 20, 0, 2, 0)).valueOf(),
    );
    ...
    asMock(Date.now).mockReturnValue(
      new Date(Date.UTC(2022, 0, 20, 0, 4, 0)).valueOf(),
    );
