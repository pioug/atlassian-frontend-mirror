...
describe('TextArea', () => {
	...
	describe('Basic', () => {
		beforeAll(async () => {
			page = global.page;
			url = getExampleUrl('design-system', 'textarea', 'basic', global.__BASEURL__);
		...
		it.skip('example should match production', async () => {
			const image = await page.screenshot();
			expect(image).toMatchProdImageSnapshot();
		});

		// Need to investigate. Continuesly failing on branch build https://bitbucket.org/atlassian/atlassian-frontend/pipelines/results/3027371/steps/%7Bb57c83a3-a660-400f-8d8e-9c81094fd8a6%7D/test-report
		it.skip('should become blue and white onFocus & grey on blur', async () => {
	...
	describe('Appearance', () => {
		beforeAll(async () => {
			page = global.page;
			url = getExampleUrl('design-system', 'textarea', 'appearance', global.__BASEURL__);
			await loadPage(page, url);
	...
	describe('Resize', () => {
		beforeAll(async () => {
			page = global.page;
			url = getExampleUrl('design-system', 'textarea', 'resize', global.__BASEURL__);
			await loadPage(page, url);
		...
		it.skip('should auto increase/decrease height of resize:smart(default) textarea based on content', async () => {
			...
			// Manual input
			const imageBeforeTypingLongText = await takeElementScreenShot(page, selector);
			expect(imageBeforeTypingLongText).toMatchProdImageSnapshot();

			await page.type(selector, docsText);
			const imageAfterTypingLongText = await takeElementScreenShot(page, selector);
			expect(imageAfterTypingLongText).toMatchProdImageSnapshot();

			await page.click(selector, { clickCount: 3 });
			await page.type(selector, ' ');

			const imageHeightReduceOnRemovingText = await takeElementScreenShot(page, selector);
			expect(imageHeightReduceOnRemovingText).toMatchProdImageSnapshot();

			// ---- Value prop change
			// Insert text
			await page.click(insertTextSelector);
			const imageAfterValuePropSet = await takeElementScreenShot(page, selector);
			expect(imageAfterValuePropSet).toMatchProdImageSnapshot();

			// Clear text
			await page.click(clearTextSelector);
			const imageAfterValuePropCleared = await takeElementScreenShot(page, selector);
