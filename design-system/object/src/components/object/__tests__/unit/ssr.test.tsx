/* eslint-disable no-console */

import __noop from '@atlaskit/ds-lib/noop';
import { cleanup, hydrateWithAct, ssr } from '@atlaskit/ssr/emotion';
import { screen } from '@atlassian/testing-library';

jest.spyOn(console, 'error').mockImplementation(__noop);

afterEach(() => {
	jest.resetAllMocks();
});

test('Object should ssr then hydrate example component correctly', async () => {
	const examplePath = require.resolve('../../../../../examples/object.vr.ap');
	const elem = document.createElement('div');
	const { html, styles } = await ssr(examplePath);
	elem.innerHTML = html;

	await hydrateWithAct(examplePath, elem, styles, true);

	// Wait for our actual component to render following hydration
	await screen.findAllByLabelText('Blog');

	// Assert there's no hydration errors
	const mockCalls = (console.error as jest.Mock).mock.calls;
	expect(mockCalls).toHaveLength(0);

	cleanup();
});
