...
async function waitUntilElementIsDraggable(
  element: HTMLElement,
): Promise<void> {
...
const interactions: PublicInteractionTask[] = [
  {
    ...
    run: async ({
      ...
      invariant(
        itemA0.getAttribute('data-is-dragging') === 'true',
        'isDragging',
      );
  ...
  },
  {
    ...
    description:
      'Recording how long it takes to reorder a card in the same list',
    run: async ({
      container,
      controls,
    }: InteractionTaskArgs): Promise<void> => {
  ...
  },
  {
    ...
    run: async ({
      ...
      invariant(
        getColumnItems(container, 'A')[0].getAttribute('data-testid') ===
          'item-A1',
      );
      invariant(
        getColumnItems(container, 'B')[0].getAttribute('data-testid') ===
          'item-A0',
      );
    },
  },
