...
const getFieldValues = (operand: Operand): string[] => {
	...
	switch (operand.operandType) {
		case OPERAND_TYPE_LIST:
			return operand.values.map(mapValuesFromList).filter((value): value is string => !!value);
...
class JqlClauseCollectingVisitor extends AbstractJastVisitor<ResultMap> {
	...
	protected aggregateResult(aggregate: ResultMap, nextResult: ResultMap): ResultMap {
