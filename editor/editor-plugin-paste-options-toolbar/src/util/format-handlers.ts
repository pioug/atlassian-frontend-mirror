...
function pasteSliceIntoTransactionWithSelectionAdjust({
  ...
  const isLastNodeEmpty =
    emptyNodeReference?.nodeSize === lastInsertNode?.nodeSize;
  ...
  const $nextPosition =
    isStepSplitingTarget && $nextHead.depth > 0
      ? tr.doc.resolve($nextHead.before())
      : $nextHead;
...
export function getMarkdownSlice(
  ...
  try {
    ...
    const canMergeOpenStart =
      targetOpenStartNode.type === doc.content.firstChild?.type;
    const canMergeOpenEnd =
      targetOpenEndNode.type === doc.content.lastChild?.type;
