...
<SectionMessage title="Caution" appearance="warning">
  <p>
    This appearance has been carried over from the legacy button, but may be
    removed in the future and only supported on{' '}
    <a href="#link-button">Link Buttons</a>. Please use with caution.
  </p>
</SectionMessage>

Use a `link` appearance for buttons that should look similar to inline text links. This is only recommended for usage with <a href="#link-button">Link Buttons</a>. Avoid using for regular Buttons that are not links.
...
Similar to a `subtle` button, but behaves like a `link` appearance button. Use a `subtle-link` button to avoid overwhelming the page with blue links.
