...
describe('Snapshot Test: hover rows controlls', () => {
	...
	describe('when there are merged cells at first column', () => {
		it.each([2, 3, 4])('should display insert button on top for row %i ', async (a) => {
			await initFullPageEditorWithAdf(page, adf);

			await clickFirstCell(page);
			const bounds = await getBoundingRect(page, tableSelectors.nthRowControl(a));
		...
		it.each([2, 3, 4])('should display insert button on bottom for row %i ', async (a) => {
			await initFullPageEditorWithAdf(page, adf);
