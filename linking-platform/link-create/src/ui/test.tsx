...
describe('Link create', () => {
  ...
  describe('<LinkCreate />', () => {
    ...
    it('should display a custom hero when provided', async () => {
      ...
      const HeroModal = () => (
        <img src="some image here" alt="some alt text here" />
      );
      setUpLinkCreate({
        modalHero: <HeroModal />,
      });
  ...
  describe('<InlineCreate />', () => {
    const setUpInlineCreate = (props?: Partial<LinkCreateProps>) => {
    ...
    it("should find InlineCreate container by its testid when it's active", async () => {
      setUpInlineCreate();
    ...
    it('should trigger the callback onCreate and onComplete when it submits the form if onComplete is provided', async () => {
      setUpInlineCreate({ onComplete: onCompleteMock });
      screen.getByTestId('submit-button').click();
    ...
    // todo: EDM-10073 - reintroduce these tests after dismiss dialog behaviour has been correctly implemented

    // describe('<ConfirmDismissDialog />', () => {
    //   it('should display Confirm Dismiss Dialog when changes are made and user clicks cancel', async () => {
    //     setUpInlineCreate({ entityKey: 'plugin-with-create-form' });
    ...
    //   it('should dismiss create on confirm Dismiss Dialog', async () => {
    //     const { unmount } = setUpInlineCreate({
    //       entityKey: 'plugin-with-create-form',
    //     });
    ...
    //   it('with create form + edit view should render editView when edit button is clicked', async () => {
    //     const { rerender } = setUpInlineCreate({
    //       plugins: [pluginWithEdit],
    //       onComplete: onCompleteMock,
    //     });
