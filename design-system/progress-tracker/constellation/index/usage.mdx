...
1. **Progress bar:** A filled horizontal bar that shows people their progress and the number of steps required to complete the task.
2. **Current step:** The current step in the progress aligns with the color fill and has colored text.
3. **Unvisited:** Shows steps that haven't been visited yet. The step and label are more subtle to indicate this.
4. **Visited:** Steps that have already been visited are higher contrast and have clickable links, so that people can navigate back to them.
...
- Use a [progress indicator](/components/progress-indicator) for a simpler experience that also shows people where they are in a journey.
