...
export async function __temporaryFixForConfigPanel(
	...
	api: PublicPluginAPI<[OptionalPlugin<ExtensionPlugin>, OptionalPlugin<ContextPanelPlugin>]>,
) {
	...
	if (extensionPluginState && extensionPluginState.showContextPanel) {
		await new Promise<void>((resolve) => {
			api?.extension?.actions.forceAutoSave(api?.contextPanel?.actions.applyChange)(resolve)(
				editorView.state,
				editorView.dispatch,
			);
		});
