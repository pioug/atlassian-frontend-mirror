...
**Unit doc:** `.agents/knowledge-index/domains/media/units/atlaskit-media-state.md`
...
- **Singleton package** — declared with `"singleton": true` in `package.json`. Only one instance must exist in the app at runtime. Do not duplicate or mock the store in non-test code.
