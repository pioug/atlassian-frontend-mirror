...
const nonComplexCases: [string, boolean][] = [
	...
	['text ~ "EDM-6023*" or summary ~ "EDM-6023*" or key = EDM-6023 ORDER BY created DESC', false],

	// default jql with test, summary and key fields and order by ASC
	['text ~ "EDM-6023*" or summary ~ "EDM-6023*" or key = EDM-6023 ORDER BY created ASC', false],
...
const complexCases: [string, boolean][] = [
	...
	['text ~ "testing*" or summary ~ "testing*" and created >= -30d ORDER BY status ASC', true],
