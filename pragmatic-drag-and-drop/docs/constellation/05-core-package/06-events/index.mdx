...
`onDrop` also includes the final `dropEffect` which can be used as a small signal that a drag operation was accepted externally.

```ts
type DropData = {
  // the final `dropEffect` that was applied
  dropEffect: DataTransfer['dropEffect'];
};

type DropEventPayload = BaseEventPayload & {
  drop: DropData;
};
```
...
```ts
monitorForElements({
  onDrop({ location, drop }) {
    if (!location.current.dropTargets.length && drop.dropEffect !== 'none') {
      // a small signal that the drag was handled in some way externally
    }
  },
});
```
