import { useEffect, useCallback, useLayoutEffect, useState, useMemo } from 'react';
...
export function useHeadingLinks(allowHeadingAnchorLinks: boolean): HeadingAnchorLinksProps {
	const [activeHeadingId, setActiveHeadingId] = useState<undefined | string>();

	const receiveNewActiveHeadingId = useCallback((headingId: string | undefined) => {
	...
	useLayoutEffect(() => {
		...
		mobileBridgeEventDispatcher.on(EmitterEvents.SET_ACTIVE_HEADING_ID, receiveNewActiveHeadingId);
