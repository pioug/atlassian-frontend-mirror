...
export const cssVariableFormatter: Format['formatter'] = ({ dictionary, options }) => {
	...
	if (theme.attributes.type === 'typography' && themeId === 'typography-minor3') {
		...
		tokens
			...
			.forEach((token) => {
				const tokenPath = (token.attributes as TypographyDesignToken<any>['attributes'])
					.responsiveSmallerVariant;
				const responsiveToken = tokens.find((token) => token.path.join('.') === tokenPath);
