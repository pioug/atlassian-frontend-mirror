...
const getColorOptionsForMode = (themeMode: ThemeModes) => {
	return themeMode === 'light' ? lightModeStatusColorPalette : darkModeStatusColorPalette;
};
...
const getDefaultColorValueForMode = (
	...
	const defaultColor = colorOptions.find((item) => item.label === status?.color);
			...
			items: [
				{
					...
					onClick: (state: EditorState, dispatch?: CommandDispatch) => {
						const newInput = createInputToolbar(status, nodeType, showStatusPickerAt, intl, bridge);
				...
				},
				{
					...
					onChange: (selected: PaletteColor) => changeColor(status, selected, bridge),
				},
				{
					type: 'separator',
				},
