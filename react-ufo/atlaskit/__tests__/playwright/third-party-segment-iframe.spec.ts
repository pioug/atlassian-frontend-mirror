...
test.describe('ReactUFO: Third Party Segment with iframe', () => {
	...
	for (const featureFlags of featureFlagsCombos) {
		test.describe(`with feature flags ${featureFlags.join(', ')}`, () => {
			test.use({
				examplePage: 'third-party-segment-iframe',
				featureFlags,
			});
			...
			for (const viewport of viewports) {
				test.describe(`when view port is ${viewport.width}x${viewport.height}`, () => {
					...
					test('third party segment should be present in the UFO extra-interaction payload', async ({
						...
						expect(thirdPartySegment!.n).toBe('third-party-widget');
						...
						expect(
							holdInfo.some((hold) => hold.labelStack.indexOf('iframe-content-loading') > -1),
						).toBeTruthy();
						expect(
							holdInfo.some((hold) => hold.labelStack.indexOf('iframe-rendering') > -1),
						).toBeTruthy();
					});
				});
			}
