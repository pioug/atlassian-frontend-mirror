...
const useUserRecommendations = (
	...
	const { isUsed: isOnInputChangeUsed, trackingFunction: usageTrackedOnInputChange } =
		useFunctionUsageTracking<typeof onInputChange>(onInputChange);

	const { isUsed: isOnUserSelectUsed, trackingFunction: usageTrackedOnUserSelect } =
		useFunctionUsageTracking<typeof onUserSelect>(onUserSelect);

	return {
		recommendations:
			isOnInputChangeUsed && isOnUserSelectUsed ? recommendations : instrumentFailureOption,
