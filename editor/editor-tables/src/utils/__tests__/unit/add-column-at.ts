...
describe('addColumnAt', () => {
	...
	it('should return a new transaction that adds a new column at index 0', () => {
		...
		expect(newTr.doc).toEqualDocument(
			doc(
				table()(row(cEmpty, td()(p('1')), td()(p('2'))), row(cEmpty, td()(p('3')), td()(p('4')))),
			),
		);
	});
	...
	it('should return a new transaction that adds a new column in the middle', () => {
		...
		expect(newTr.doc).toEqualDocument(
			doc(
				table()(row(td()(p('1')), cEmpty, td()(p('2'))), row(td()(p('3')), cEmpty, td()(p('4')))),
			),
		);
	});
	...
	it('should return a new transaction that adds a new column at last index', () => {
		...
		expect(newTr.doc).toEqualDocument(
			doc(
				table()(row(td()(p('1')), td()(p('2')), cEmpty), row(td()(p('3')), td()(p('4')), cEmpty)),
			),
		);
	});
	...
	it('should return a new transaction that adds a new column with cell background matching right column if left column is header column', () => {
		const { tr } = createEditorState(
			doc(
				table()(
					row(th({ background: 'blue' })(p('1{<>}')), td({ background: 'red' })(p('2')), cEmpty),
					row(th({ background: 'blue' })(p('3')), th({ background: 'red' })(p('4')), cEmpty),
				),
			),
		);
		const newTr = addColumnAt(1)(tr);
