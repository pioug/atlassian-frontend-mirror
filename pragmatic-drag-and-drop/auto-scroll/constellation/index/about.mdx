...
order: 0
title: Auto scroll
description: Enables ergonomic automatic scrolling during a drag operation for any experience
---
...
This package depends on [the core package](/components/drag-and-drop/core-package).
...
import { autoScrollForInternalUncontrolled } from '@atlaskit/pragmatic-drag-and-drop-auto-scroll/internal-uncontrolled';
...
- _(optional)_: `getConfiguration: (args: ElementGetFeedbackArgs) => PublicConfig`: used to control some aspects of auto scrolling
...
- `maxScrollSpeed`: `'fast' | 'standard'`. We recommend using the default `"standard"` max scroll speed for most experiences. However, on _some_ larger experiences, a faster max scroll speed `"fast"` _can_ feel better.
