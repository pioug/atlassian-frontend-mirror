...
Instead, use [validation](/components/button/usage) or other techniques to show users how to proceed.
...
Buttons can expand to full width to fill the parent container. This is sometimes done in login forms. Follow the [alignment guidance](/components/button/usage#alignment-and-positioning) for more options.
...
<SectionMessage title="Coming soon" appearance="information">
  <p>
    The legacy{' '}
    <a href="/components/button/button-legacy/examples#custom-theme-button">
      CustomThemeButton
    </a>{' '}
