...
const allowedPrefix = [':', '&:'];
const allowedResponsiveImports = [
  ...
  if (node.type === 'MemberExpression') {
    if (
      ...
      if (
        ...
        allowedResponsiveImports.includes(
          definition.node.parent.source.value as string,
        )
      ) {
...
const isWidthMediaQuery = (rawSelector: string): boolean => {
  ...
  if (selectors[0].startsWith('@')) {
    ...
    return selectors.some(
      (selector) =>
        selector.includes('min-width') || selector.includes('max-width'),
    );
  }
  return false;
};
    ...
    return {
      'CallExpression[callee.name=css] > ObjectExpression Property,CallExpression[callee.name=xcss] > ObjectExpression Property':
        (node: Rule.Node) => {
          ...
          if (
            isUsingDirectDataAttribute(
              getKeyValue(node.key as Rule.Node, context),
            )
          ) {
            context.report({
              node,
