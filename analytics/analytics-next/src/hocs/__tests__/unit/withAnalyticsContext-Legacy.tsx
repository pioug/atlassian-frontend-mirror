...
const ContainerToBeWrapped = forwardRef<HTMLDivElement, { children: React.ReactNode }>(
	({ children }, ref) => {
...
const WrappedContainerOne = withAnalyticsContext({ ticket: 'AFP-123' })(ContainerToBeWrapped);

const WrappedContainerTwo = withAnalyticsContext({ board: 'AFPAI' })(ContainerToBeWrapped);
...
const FakeModernConsumer = memo<{}>(() => {
	...
	return (
		<button data-render-count={renderCounter} data-testid="button" onClick={onClick}>
			Button
		</button>
	);
});
...
class FakeLegacyConsumer extends PureComponent<{}> {
	...
	onClick = () => {
		const { getAtlaskitAnalyticsContext, getAtlaskitAnalyticsEventHandlers } = this.context;
	...
	render() {
		this.renderCounter++;
		return (
			<button data-render-count={this.renderCounter} data-testid="button" onClick={this.onClick}>
				Button
			</button>
		);
	}
...
describe('withAnalyticsContext LegacyContext', () => {
	...
	branches.forEach(({ FakeConsumer, description }) => {
		describe(description, () => {
			it('should render the wrapped component', () => {
				const { getByTestId } = render(
					<UnderTestSingleContext onEvent={() => null} FakeConsumer={FakeConsumer} />,
				);
			...
			it('should provide analytics context via default data to children', () => {
				...
				const { getByTestId } = render(
					<UnderTestSingleContext onEvent={onEvent} FakeConsumer={FakeConsumer} />,
				);
			...
			it('should provide analytics context work with many contexts to children', () => {
				...
				const { getByTestId, rerender } = render(
					<UnderTestManyContext onEvent={onEvent} FakeConsumer={FakeConsumer} />,
				);
			...
			it('should not re-render consumer components when analytics-context changes', () => {
				const { getByTestId, rerender } = render(
					<UnderTestSingleContext onEvent={() => {}} FakeConsumer={FakeConsumer} />,
				);

				expect(getByTestId('button').dataset.renderCount).toBe('1');

				rerender(<UnderTestSingleContext onEvent={() => {}} FakeConsumer={FakeConsumer} />);
		...
		it("should include the wrapped component's name in the wrapped name", () => {
			...
			expect(WrappedContainer.displayName).toBe('WithAnalyticsContext(Container)');
		});
	});
});
