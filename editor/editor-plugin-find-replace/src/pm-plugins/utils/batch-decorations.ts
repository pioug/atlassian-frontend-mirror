...
class BatchDecorations {
	...
	async applyAllSearchDecorations(
		...
		const pmElement = containerElement.querySelector<HTMLElement>('.ProseMirror');
		if (!pmElement) {
			return;
		}
	...
	private async updateDecorationsBetween(editorView: EditorView, startPos: number, endPos: number) {
	...
	private async addDecorationsBetween(editorView: EditorView, startPos: number, endPos?: number) {
		...
		return await this.batchRequests(
			(counter) => {
				...
				let useSelectionIndex =
					selectionIndex >= counter && selectionIndex < counter + batchIncrement;
