...
const CollabEditor = ({ userId, collabProps, setIsReady }: CollabEditorProps) => {
	...
	const onEditorReady = React.useCallback((editorActions: any) => {
		...
		(window as WindowForTesting).__setViewMode = (mode: 'edit' | 'view') => {
			...
			api?.core?.actions.execute(api?.editorViewMode?.commands.updateViewMode(mode));
		};
	}, []);
...
export default function EditorExampleForIntegrationTests() {
	...
	return (
		...
		<div id="editor-container" data-collab-is-ready={isReady} style={style}>
			<EditorContext>
				<CollabEditor userId={userId} collabProps={options} setIsReady={setIsReady} />
			</EditorContext>
		</div>
	);
}
