...
describe('getCardStatus()', () => {
	...
	it(`should return 'complete' if the file status is 'processed' and file is not previewable`, () => {
		expect(getCardStatus('processed', { ...defaultOptions, isPreviewable: false })).toEqual(
			'complete',
		);
	});

	it(`should return "complete" if the file status is "processed" and file has no preview`, () => {
		expect(getCardStatus('processed', { ...defaultOptions, hasPreview: false })).toEqual(
			'complete',
		);
	});

	it(`should return "loading-preview" if the file status is "processed" and file is previewable with preview`, () => {
		expect(getCardStatus('processed', { ...defaultOptions, hasPreview: true })).toEqual(
			'loading-preview',
		);
	});
	...
	it(`should return loading by default`, () => {
		...
		expect(getCardStatus('unhandled-file-status' as FileStatus, {} as FilePreviewStatus)).toEqual(
			'loading',
		);
	});
});
