		...
		it('init - first url is bad, good second', async () => {
			...
			if (isPromise(cacheStrategy)) {
				return cacheStrategy
					.catch((err) => {
						expect(err).toEqual('Unable to initialise cache based on provided url(s)');
...
describe('BrowserCacheStrategy', () => {
	describe('#supported', () => {
		...
		beforeEach(() => {
			...
			imageConstructorStub = sinon.stub(window, 'Image' as any).returns(mockImage);
		});
		...
		it('image loaded', () => {
			const promise = BrowserCacheStrategy.supported('cheese', mockMediaImageLoader).then(
				(supported) => {
		...
		it('exception loading image', () => {
			...
			imageConstructorStub = sinon.stub(window, 'Image' as any).throws(new Error('doh'));
