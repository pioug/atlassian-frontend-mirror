...
that can expand in height. Text areas are normally used in [forms](/patterns/forms).
...
2. ** Placeholder text (optional):** Avoid placeholders when possible. Never use placeholder text to
   communicate critical information — this is not accessible. Instead, use field labels or helper
   text to describe the purpose of the field and how to use it. Placeholders can be used for example
   entries that help clarify the text input.
3. ** Input area: ** This is where users enter text.
4. ** Helper text: ** Use this to provide a hint about specific formats the text area will accept
   (for example, `Enter up to 250 characters`).
...
- Avoid using placeholder text whenever possible. Make sure any critical information is communicated
  either in the field label or using helper text below the field. Search fields or brief examples
  are exceptions where placeholder text may be OK.

## Content guidelines

- Use a combination of a concise label and helper text to clarify the field input. Don’t rely on
  placeholder text as it is not visible at all times.
