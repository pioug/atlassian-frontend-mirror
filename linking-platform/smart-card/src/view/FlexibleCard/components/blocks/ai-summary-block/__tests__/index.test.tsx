...
import { ActionName, ElementName, SmartLinkStatus } from '../../../../../../constants';
...
describe('AISummaryBlock', () => {
	...
	describe('ai summary', () => {
		it('shows summarizing indicator when click on Summarize button', async () => {
			...
			const indicator = await findByTestId(`${testIdBase}-state-indicator-loading`);
		...
		it('shows acceptable use violation error state indicator on acceptable use violation error', async () => {
			...
			const { queryByTestId, findByTestId, rerenderTestComponent } = renderAISummaryBlock({
				testId: testIdBase,
			});
			...
			rerenderTestComponent();
			const indicatorA = queryByTestId(`${testIdBase}-error-indicator-error`);
			expect((await findByTestId(`${testIdBase}-error-indicator-error-message`)).textContent).toBe(
	...
	describe('metadata', () => {
		it('should render metadata', async () => {
			...
			const { findByTestId } = renderAISummaryBlock({
				metadata: [{ name: ElementName.Provider, testId: `${testIdBase}-provider` }],
			});
	...
	describe('actions', () => {
		it('should define AI Summary Action', async () => {
			...
			const aiSummaryAction = await findByTestId(`${testIdBase}-ai-summary-action`);
