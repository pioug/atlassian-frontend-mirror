...
describe('Fetcher', () => {
  ...
  describe('fetchClientSdkKey', () => {
    test('returns clientSdkKey if successful', async () => {
      fetchMock.mockResponseOnce(
        JSON.stringify(defaultClientSdkKeyResponseBody),
        {
          status: 200,
        },
      );
      ...
      expect(response.clientSdkKey).toEqual(
        defaultClientSdkKeyResponseBody.clientSdkKey,
      );
    });
  ...
  describe('fetchExperimentValues', () => {
    test('returns feature flags if successful', async () => {
      fetchMock.mockResponseOnce(
        JSON.stringify(defaultExperimentsResponseBody),
        {
          status: 200,
        },
      );
      const response: FrontendExperimentsResponse =
        await Fetcher.fetchExperimentValues(defaultFetcherOptions, {
          atlassianAccountId: 'account-abc',
        });
      expect(response.clientSdkKey).toEqual(
        defaultExperimentsResponseBody.clientSdkKey,
      );
      expect(response.experimentValues).toEqual(
        defaultExperimentsResponseBody.experimentValues,
      );
      expect(response.customAttributes).toEqual(
        defaultExperimentsResponseBody.customAttributes,
      );
    });
