...
import { uploadFile, MediaStore, type UploadableFileUpfrontIds, ChunkHashAlgorithm } from '../src';
...
class UploaderExample extends Component<UploaderExampleProps, UploaderExampleState> {
	...
	render() {
		...
		return (
			<Wrapper>
				<PreviewWrapper>
					<h1>Upload files using FedRAMP compatible SHA256 chunk identifiers</h1>
					<div>
	...
	private uploadFile(uploadableFile: UploadableFile) {
		const mediaStore = new MediaStore({
			authProvider: defaultMediaPickerAuthProvider(),
			chunkHashAlgorithm: ChunkHashAlgorithm.Sha256, // When running in a FedRAMP perimeter pass this flag to ensure all chunks are using SHA256 identifiers
		});
		const fileId = uuid();
