...
import { DEFAULT_MODE, FULL_WIDTH_MODE, LOCALSTORAGE_defaultMode } from './example-constants';
...
export default class FullWidthToggle extends React.Component<Props, State> {
	...
	private toggleFullWidthMode = (e: React.SyntheticEvent<HTMLButtonElement>) => {
		e.preventDefault();
		this.setState(
	...
	render() {
		return (
			<button
				...
				aria-label={this.state.fullWidthMode ? 'Make page fixed-width' : 'Make page full-width'}
			>
				{this.state.fullWidthMode ? <EditorCollapseIcon label="" /> : <EditorExpandIcon label="" />}
			</button>
		);
	}
}
