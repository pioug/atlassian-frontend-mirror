...
describe('Smart Card: Reducers', () => {
	...
	describe('ACTION_ERROR_FALLBACK', () => {
		it('successfully updates URL state to fallback and passes response data', () => {
			...
			store = reducer(
				store,
				cardAction(ACTION_ERROR_FALLBACK, mockActionParams, mockPayload, mockError),
			);
			expect(store).toHaveProperty('/some/url', {
				status: 'fallback',
				details: mockPayload,
	...
	describe('ACTION_RESOLVING', () => {
		it('successfully updates URL state to resolving (happy path)', () => {
			...
			store = reducer(store, cardAction(ACTION_RESOLVED, mockActionParams, mockPayload));
			expect(store).toHaveProperty('/some/url', {
				status: 'resolved',
				details: mockPayload,
			});
	...
	describe('ACTION_RESOLVED', () => {
		...
		it('successfully persists existing state when not transitioning between states', () => {
			...
			store = reducer(store, cardAction(ACTION_RESOLVED, mockActionParams, mockOtherPayload));
			expect(store).toHaveProperty('/some/url', {
				status: 'resolved',
				details: mockPayload,
			});
		...
		it('successfully persists new state when reload action is invoked', () => {
			...
			store = reducer(store, cardAction(ACTION_RELOADING, mockActionParams, mockOtherPayload));
		...
		it('successfully keeps URL state with empty payload', () => {
			mockDateNow.mockImplementationOnce(() => 123);
			store = reducer(store, cardAction(ACTION_RESOLVED, mockActionParams, undefined));
			expect(store).toHaveProperty('/some/url', {
				status: 'resolved',
			});
		});
	...
	describe('ACTION_UPDATE_METADATA_STATUS', () => {
		...
		const metadataStatuses: MetadataStatus[] = ['pending', 'resolved', 'errored'];
		it.each([...metadataStatuses, undefined])(
			...
			(status) => {
				...
				store = reducer(
					store,
					cardAction(ACTION_UPDATE_METADATA_STATUS, mockActionParams, undefined, undefined, status),
				);
				expect(store).toHaveProperty('/some/url', {
					status: 'resolved',
