...
async function generatePresetConfig(
  ...
  const pluginList = [
    `'${pluginName}'`,
    ...Array.from(new Set(externalScopes)),
  ];
  ...
  const code = outdent`
    export default {
      ...
      rules: {
        ${rules
          .map((rule) => {
            const { severity, recommended, pluginConfig } =
              rule.module.meta?.docs || {};
            ...
            if (typeof pluginConfig === 'object' && pluginConfig) {
              return `'${ruleName}': ['${calculatedSeverity}', ${JSON.stringify(
                pluginConfig,
              )}]`;
            }
...
async function generateRuleIndex(rules: FoundRule[]) {
  ...
  const code = outdent`
    ${internalRules
      .map(
        (rule) =>
          `import ${camelCase(rule.pathSafeName)} from './${
            rule.pathSafeName
          }'`,
      )
      .join('\n')}
    ...
    export default {
    ${internalRules
      .map((rule) => `'${rule.ruleName}': ${camelCase(rule.pathSafeName)}`)
      .join(',')}
    }
  `;
...
async function generateRulePages(rules: FoundRule[]): Promise<void> {
  ...
  for await (const rule of rules) {
    ...
    if (ruleIsExternal(rule)) {
      ...
      throw new Error(
        `invariant: found an unexpected rule type ${rule.ruleName}`,
      );
    }
  }
}

async function generateExternalRulePage(
  rule: ExternalRule,
  destination: string,
) {
...
async function generateInternalRulePage(
  rule: InternalRule,
  destination: string,
) {
...
async function generateRuleTable(
  ...
  if (!found) {
    throw new Error(
      `invariant: could not find the partial codegen section in ${filepath}`,
    );
  }
...
const validateInternalRule = (rule: InternalRule) => {
  ...
  if (rule.pathSafeName.includes('@') || rule.pathSafeName.includes('/')) {
    throw new Error(
      `invariant: name ${rule.pathSafeName} is not valid, specify a path-safe name`,
    );
  }
};
...
async function generate() {
  ...
  await generateRuleTable(
    join(srcDir, '../constellation/index/overview.mdx'),
    'docs',
    rules,
  );
}
