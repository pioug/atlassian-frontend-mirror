...
describe.skip('WikiMarkup Transformer', () => {
	...
	describe('plain link', () => {
		...
		checkParseEncodeRoundTrips(
			...
			defaultSchema,
			WIKI_NOTATION,
			doc(p(link({ href: 'http://confluence.atlassian.com' })('http://confluence.atlassian.com'))),
		);
	});
	...
	describe('link with text', () => {
		...
		checkParseEncodeRoundTrips(
			...
			defaultSchema,
			WIKI_NOTATION,
			doc(p(link({ href: 'http://confluence-url.atlassian.com:10090' })('confluence'))),
		);
	});
...
[_italic_|http://confluence.atlassian.com]`;
		...
		checkParseEncodeRoundTrips(
			...
			WIKI_NOTATION,
			doc(
				p(strong(link({ href: 'http://www.yahoo.com/search?query=hello' })('bold link'))),
				p(
	...
	describe('non links', () => {
		...
		checkParseEncodeRoundTrips(WIKI_NOTATION, defaultSchema, WIKI_NOTATION, doc(p('[ x ]')));
		...
		checkParseEncodeRoundTrips(
			...
			doc(
				p(
					'[',
					link({ href: 'http://confluence.atlassian.com' })('http://confluence.atlassian.com'),
					']',
				),
			),
		);
	...
	describe('links with marks to be ignored', () => {
		...
		checkParseEncodeRoundTrips(
			...
			defaultSchema,
			WIKI_NOTATION,
			doc(p(link({ href: 'http://foo.com/xx(yy)zz.java)' })('http://foo.com/xx(yy)zz.java)'))),
		);
	});
