...
import { AnalyticsContext, type CreateUIAnalyticsEvent } from '@atlaskit/analytics-next';
import {
	type MediaClient,
	type FileState,
	type FileIdentifier,
...
import { getCardPreview, getCardPreviewFromCache } from '../../card/getCardPreview';
...
const mockViewportDetectorOnce = () => {
	...
	asMockFunction(ViewportDetector).mockImplementationOnce(({ children, onVisible }) => {
...
describe('Card', () => {
	...
	const createMediaClientWithGetFile = (fileState: FileState = defaultFileState) => {
		const mockMediaClient = fakeMediaClient();

		asMockFunction(mockMediaClient.getImage).mockReturnValue(defaultImageBlob);
		asMockReturnValue(mockMediaClient.file.getFileState, createMediaSubscribable(fileState));
	...
	beforeEach(() => {
		...
		asMock(mediaClient.file.getFileState).mockReturnValue(fileStateSubscribable);
		defaultFileId = uuid();
		fileIdentifier = {
			id: defaultFileId,
			mediaItemType: 'file',
	...
	it('should update the card preview state when the file identifier is updated', () => {
		...
		const component = mount(
			<Card mediaClient={createMediaClientWithGetFile()} identifier={identifier} />,
		);
		expect(component.find(CardBase).state('cardPreview')).toEqual(defaultCardPreview);
		component.setProps({ identifier: newIdentifier });
		expect(component.find(CardBase).state('cardPreview')).toEqual(newCardPreview);
	});
	...
	it('should override creation date when applied with `DateOverrideContext`', () => {
		...
		expect(component.find(CardView).props().overriddenCreationDate).toEqual(123);
	});

	it('should attach default IntlProvider when an ancestor is not found', () => {
		const component = mount(<Card mediaClient={mediaClient} identifier={identifier} />);
	...
	it('should use "crop" as default resizeMode', () => {
		...
		const card = mount(
			<CardBase mediaClient={mediaClient} identifier={identifier} isLazy={false} />,
		);
	...
	it('should set error card state and wrap the error when filestate subscription sends error', async () => {
		...
		asMockReturnValue(mediaClient.file.getFileState, createMediaSubscribable(errorThrown));
		const { component } = setup(mediaClient);
	...
	it('should not lazy load if the preview was cached', async () => {
		asMockFunction(getCardPreviewFromCache).mockReturnValueOnce(await defaultCardPreview);
	...
	describe('External image identifier', () => {
		...
		it('should update the card preview when the external image identifier updates', () => {
			...
			expect(component.find(CardBase).state('cardPreview')).toEqual(cardPreview);
			component.setProps({ identifier: newIdentifier });
			expect(component.find(CardBase).state('cardPreview')).toEqual(newCardPreview);
		});
	...
	describe('MediaViewer integration', () => {
		...
		it('should pass items list to MediaViewer', () => {
			...
			expect(component.find(MediaViewer).prop('items')).toEqual([identifier, identifier]);
		});
		...
		it('should pass contextId to MV', async () => {
			...
			expect(component.find(MediaViewer).prop('contextId')).toEqual('some-context-id');
		});
		...
		it('should pass featureFlags to MV', async () => {
			...
			expect(component.find(MediaViewer).prop('featureFlags')).toEqual(featureFlags);
		});
		...
		it('should focus button', async () => {
			...
			const component = mount(
				<Card mediaClient={mediaClient} identifier={identifier} shouldOpenMediaViewer />,
	...
	describe('Analytics', () => {
		...
		it('should attach package attributes to Analytics Context', () => {
			const mediaClient = fakeMediaClient();
			const card = mount<CardProps>(<Card mediaClient={mediaClient} identifier={identifier} />);
		...
		it('should attach feature flags to Analytics Context', () => {
			...
			const card = mount<CardProps>(
				<Card mediaClient={mediaClient} identifier={identifier} featureFlags={featureFlags} />,
			);
		...
		describe('Operational Event', () => {
			...
			it('should complete the UFO experience when a serverRateLimited error occurs', () => {
				...
				expect((fireOperationalEvent as jest.Mock).mock.calls[0][5].secondaryError).toEqual(
					errorThrown,
				);
			});
			...
			it('should fire an operational event on card status change', () => {
				...
				expect(fireOperationalEvent).toHaveBeenLastCalledWith(
					createAnalyticsEvent,
					'some-status',
					getFileAttributes(getFileDetails(identifier, fileState), fileState.status),
					{
						overall: {
		...
		describe('Copy', () => {
			...
			it('should call fireCopiedEvent when a UI copy event has been dispatched', async () => {
				const handler = mount(<Card mediaClient={fakeMediaClient()} identifier={identifier} />);
				await callCopy();
