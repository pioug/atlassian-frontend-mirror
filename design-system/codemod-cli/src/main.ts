...
const getTransformPrompt = async (
  transforms: ParsedPath[],
): Promise<ParsedPath> => {
  ...
  return await new AutoComplete({
    ...
    limit: 18,
    choices: transformMeta.map(({ id, name, dir }) =>
      id ? id : `${dir}/${name}`,
    ),
...
const resolveTransform = async (
  ...
  if (flags.transform && !hasTransform(flags.transform)) {
    // eslint-disable-next-line no-console
    console.warn(
      `No available transform found for: ${chalk.bgRed(flags.transform)}`,
    );
  }
...
const runTransform = async (
  ...
  if (flags.filterPaths) {
    logger.log(
      chalk.green(`Running filtering logic for module ${transform.module}...`),
    );
  ...
  logger.log(
    chalk.green(
      `Transforming files matching these extensions '${chalk.bold(
        flags.extensions,
      )}'...`,
    ),
  );
  ...
  const transformedPackages = packages.map((pkg) => {
    ...
    if (pkg.version != null) {
      if (semver.validRange(pkg.version)) {
        ...
        errors.push(
          `Invalid version "${pkg.version}" for package "${pkg.name}"`,
        );
      }
    }
    return { ...pkg, version };
...
export default async function main(input: string[], userFlags: UserFlags) {
  ...
  logger.log(
    chalk.bgBlue(
      chalk.black(
        `📚 Atlassian-Frontend codemod library @ ${_PACKAGE_VERSION_} 📚`,
      ),
    ),
  );
  ...
  const shouldHavePackages =
    flags.sinceRef || (flags.packages && flags.packages.length > 0);
