...
describe('ZoomLevel', () => {
	it('should sort zoom levels correctly', () => {
		...
		expect(zoomLevel.zoomLevels).toEqual([0.12, 0.24, 0.48, 0.96, 1, 2, 3, 4, 8, 12, 16]);
	});

	jsc.property('selects the initialValue if no value is selected', scaleGenerator(), (scale) => {
	...
	jsc.property(
		...
		(scale) => {
			const zoomLevel = new ZoomLevel(scale);
			expect(zoomLevel.zoomLevels.filter((i) => i === 1).length === 1).toBeTruthy();
			return zoomLevel.zoomLevels.filter((i) => i === 1).length === 1;
		},
	);

	jsc.property('zooming in maintains the initialValue', scaleGenerator(), (scale) => {
	...
	jsc.property('zooming out maintains the initialValue', scaleGenerator(), (scale) => {
		const original = new ZoomLevel(scale);
	...
	jsc.property('increases the zoom level when zooming in', scaleGenerator(), (scale) => {
		const zoomLevel = new ZoomLevel(scale);
		expect(zoomLevel.zoomIn().value > zoomLevel.value).toBeTruthy();
		return zoomLevel.zoomIn().value > zoomLevel.value;
	});

	jsc.property('decreases the zoom level when zooming out', scaleGenerator(), (scale) => {
		const zoomLevel = new ZoomLevel(scale);
	...
	jsc.property('will report if zooming out is possible', scaleGenerator(), (scale) => {
		const zoomLevelDefault = new ZoomLevel(scale);
		const zoomLevelMin = new ZoomLevel(scale).fullyZoomOut();
		expect(zoomLevelDefault.canZoomOut && !zoomLevelMin.canZoomOut).toBeTruthy();
		return zoomLevelDefault.canZoomOut && !zoomLevelMin.canZoomOut;
	});

	jsc.property('will report if zooming in is possible', scaleGenerator(), (scale) => {
		const zoomLevelDefault = new ZoomLevel(scale);
		const zoomLevelMax = new ZoomLevel(scale).fullyZoomIn();
		expect(zoomLevelDefault.canZoomIn && !zoomLevelMax.canZoomIn).toBeTruthy();
