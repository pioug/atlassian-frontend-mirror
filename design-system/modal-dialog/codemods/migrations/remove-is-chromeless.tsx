...
export const removeIsChromeless = (j: core.JSCodeshift, source: Collection<Node>) => {
	...
	source.findJSXElements(defaultSpecifier).forEach((element) => {
		getJSXAttributesByName(j, element, IS_CHROMELESS_PROP_NAME).forEach((attribute: any) => {
			// if values is true then add comment before removing
			if (
				attribute.node.value === null ||
				(attribute.node.value.expression && attribute.node.value.expression.value !== false)
			) {
