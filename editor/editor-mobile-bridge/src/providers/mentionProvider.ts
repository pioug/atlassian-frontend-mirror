...
export async function createMentionProvider() {
	try {
		...
		console.error(`Could not construct a MentionProvider, the following exception occurred:`, err);
...
function selectAccountId(input: unknown): string {
	...
	switch (typeof input) {
		...
		case 'object':
			if (input !== null && hasAccountId(input) && typeof input.accountId === 'string') {
