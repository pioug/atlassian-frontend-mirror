...
const startCollab = (url: string, documentAri: string, bridge: WebBridgeImpl) => {
	...
	const onSubmitPromise = ({ name, uuid, args }: any) => {
		if (name === 'getCollabConfig') {
			bridge.onPromiseResolved(uuid, JSON.stringify({ baseUrl: url, documentAri }));
		}
	};
	...
	(window as any).messageHandler.on('disconnectFromCollabService', onDisconnect);
