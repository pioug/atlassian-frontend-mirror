...
export default {
  ...
  'media.media-avatar-picker.image-cropper.arrow-keys.action':
    '{key, select, left {您已将图像从左侧移至右侧，您可以使用箭头键来移动和裁剪图像} right {您已将图像从右侧移至左侧，您可以使用箭头键来移动和裁剪图像} top {您已将图像从顶部移至底部，您可以使用箭头键来移动和裁剪图像} other {您已将图像从底部移至顶部，您可以使用箭头键来移动和裁剪图像}}',
  'media.media-avatar-picker.image-cropper.arrow-keys.label':
    '可拖动图像，确保您的屏幕阅读器未处于浏览模式。可使用箭头键移动和裁剪图像',
