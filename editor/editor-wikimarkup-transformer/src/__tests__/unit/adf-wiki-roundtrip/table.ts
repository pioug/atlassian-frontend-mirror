...
describe('ADF => WikiMarkup - Table', () => {
	...
	test('should convert Table node with list and paragraph in the cell', () => {
		...
		const expected = doc(
			table()(
				tr(
					td()(ul(li(p('item 1')), li(p('item 2', hardBreak(), 'below the list')))),
					td()(ol()(li(p('item 1')), li(p('item 2', hardBreak(), 'below the list')))),
				),
				tr(
					td()(ol()(li(p('item 1')), li(p('item 2', hardBreak(), 'below the list')))),
					td()(ul(li(p('item 1')), li(p('item 2', hardBreak(), 'below the list')))),
	...
	test('should convert Table node with empty cells', () => {
		const node = doc(table()(tr(th()(p()), th()(p())), tr(td()(p()), td()(p()))))(defaultSchema);
		const wiki = transformer.encode(node);
		const adf = transformer.parse(wiki).toJSON();
		expect(adf).toEqual(node.toJSON());
	});
	...
	test('should convert table with header column', () => {
		const node = doc(
			table()(
				...
				tr(th()(p(strong('Header Column 1'))), td()(p('Cell 1')), td()(p('Cell 2'))),
				tr(th()(p(strong('Header Column 2'))), td()(p('Cell 3')), td()(p('Cell 4'))),
			),
		)(defaultSchema);
		const wiki = transformer.encode(node);
		const adf = transformer.parse(wiki).toJSON();
