...
describe('shouldResolvePreview()', () => {
	...
	const isPreviewableStatus = jest.spyOn(filePreviewStatusModule, 'isPreviewableStatus');
...
describe('getCardPreview()', () => {
	...
	it('should return card preview from passed file preview', async () => {
		asMockFunction(getCardPreviewFromFilePreview).mockResolvedValueOnce(localPreview);
		...
		expect(addFileAttrsToUrl).toBeCalledWith(localPreview.dataURI, mediaBlobUrlAttrs);
	...
	it('should return card preview from backend if there is no local preview and is image representation ready', async () => {
		asMockFunction(getCardPreviewFromBackend).mockResolvedValueOnce(remotePreview);
