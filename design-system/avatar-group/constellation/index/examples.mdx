---
order: 0
---

import SectionMessage from '@atlaskit/section-message';
import AvatarGroupStack from '../../examples/constellation/avatar-group-stack';
import AvatarGroupGrid from '../../examples/constellation/avatar-group-grid';
import AvatarGroupMaxCount from '../../examples/constellation/avatar-group-max-count';
import AvatarGroupBorderColor from '../../examples/constellation/avatar-group-border-color';
import AvatarGroupOverrides from '../../examples/constellation/avatar-group-overrides';
import AvatarGroupSize from '../../examples/constellation/avatar-group-size';

<SectionMessage
	title="Motion in Early Access"
	appearance="warning"
>
	The motion added into Avatar group is in Early Access. The motion updates are currently behind the feature flag: platform-dst-motion-uplift.
</SectionMessage>

## Appearance

You can display a group of avatars in a stack or a grid.

### Stack

Stacked avatar groups can contain up to five avatars, and should only be used with small or medium
sized avatars.

<Example
	Component={AvatarGroupStack}
	packageName="@atlaskit/avatar-group"
	featureFlagSwitchers={['motion']}
/>

### Grid

Avatar groups displayed in a grid can contain up to 11 avatars spread across its rows.

<Example
	Component={AvatarGroupGrid}
	packageName="@atlaskit/avatar-group"
	featureFlagSwitchers={['motion']}
/>

## Max count

Use the `maxCount` prop to customize the maximum number of avatars allowed in the list.

<Example
	Component={AvatarGroupMaxCount}
	packageName="@atlaskit/avatar-group"
	featureFlagSwitchers={['motion']}
/>

## Border color

The color of the border around the avatar. Any color that the CSS `border-color` property accepts
can be used.

<Example
	Component={AvatarGroupBorderColor}
	packageName="@atlaskit/avatar-group"
	featureFlagSwitchers={['motion']}
/>

## Overrides

Custom components can be passed in via the `overrides` prop.

For example, you can add extra behavior into the overflow menu using the `overrides` prop. Select
the example overflow avatar and you'll see a load more button powered by our custom
`AvatarGroupItem`.

<Example
	highlight="33-50"
	Component={AvatarGroupOverrides}
	packageName="@atlaskit/avatar-group"
	featureFlagSwitchers={['motion']}
/>

## Size

Avatars are available in multiple sizes. See [avatar](/components/avatar) for information on which
size to use when. Note the `xsmall` size is not available for `AvatarGroup` due to its size being
too small to render certain elements in an accessible manner.

<Example
	Component={AvatarGroupSize}
	packageName="@atlaskit/avatar-group"
	featureFlagSwitchers={['motion']}
/>
