jest.mock('@atlaskit/media-common', () => {
	...
	return {
		__esModule: true,
		...actualModule,
		isMimeTypeSupportedByBrowser: jest.fn(actualModule.isMimeTypeSupportedByBrowser),
	};
});
jest.mock('@atlaskit/media-client', () => {
	...
	return {
		...
		isImageRepresentationReady: jest.fn(() => 'isImageRepresentationReady-return'),
	};
});
jest.mock('../helpers');
...
describe('extractFilePreviewStatus()', () => {
	...
	it(`should use file state's file size`, () => {
		...
		expect(extractFilePreviewStatus(dummyFileStateWithoutSize, false)).toMatchObject(
			expect.objectContaining({
				hasFilesize: false,
			}),
		);

		expect(extractFilePreviewStatus(dummyFileStateWithSize, false)).toMatchObject(
			expect.objectContaining({
				hasFilesize: true,
			}),
		);
