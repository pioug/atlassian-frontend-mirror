...
describe('pastedCells', () => {
	function test(slice: RefsNode, width: number | null, height?: number, content?: any[]) {
	...
	it('returns multiple rows', () => {
		const table = createTable(tr('{a}', cEmpty, cEmpty), tr(cEmpty, c11), '{b}');
		test(table, 2, 2, [
	...
	it('can normalize a partially-selected row', () => {
		const table = createTable(tr(td()(p(), '{a}'), cEmpty, c11), tr(c11, c11), '{b}');
	...
	it('will make sure the result is rectangular', () => {
		const table = createTable('{a}', tr(c(2, 2), c11), tr(cEmpty), tr(c11, c11), '{b}');

		test(table, 3, 3, [[c22Node, c11Node], [cEmptyNode], [c11Node, c11Node, cEmptyNode]]);
	});
...
describe('clipCells', () => {
	function test(slice: RefsNode, width: number, height: number, content: any[]) {
	...
	it('clips off excess colspan', () => {
		const table = createTable('{a}', tr(c(2, 2), c11), tr(c11), '{b}');
		test(table, 4, 2, [[c22Node, c11Node, createTd({ colspan: 1, rowspan: 2 })], [c11Node]]);
	});
...
describe('insertCells', () => {
	...
	it('preserves headers while growing a table', () => {
		const aFoo = cellRef('a', '', 'foo');
		const table = createTable(tr(h11, h11, h11), tr(h11, c11, c11), tr(h11, c11, cAnchor));
	...
	it('will split interfering rowspan cells', () => {
		const table = createTable(tr(c11, c(1, 4), c11), tr(cAnchor, c11), tr(c11, c11), tr(c11, c11));
	...
	it('will split interfering colspan cells', () => {
		const table = createTable(tr(c11, cAnchor, c11), tr(c(2, 1), c11), tr(c11, c(2, 1)));
		const paste = createTable('{a}', tr(cEmpty), tr(cEmpty), tr(cEmpty), '{b}');
		const result = createTable(tr(c11, cEmpty, c11), tr(c11, cEmpty, c11), tr(c11, cEmpty, cEmpty));
