      ...
      `,
			...
			expect(mockFixer.replaceText).toHaveBeenNthCalledWith(
				...
				expect.objectContaining({
					...
					arguments: expect.arrayContaining([expect.objectContaining({ value: 'space.100' })]),
				}),
				"'space.100'",
			);
		...
		it('transforms a simple example with negative spacing tokens', () => {
			// ARRANGE
			const root = j(
				`
      import { styled } from '@compiled/react';
      const MyContainer = styled.div({ margin: '-8px' });
      <MyContainer>Hello, World!</MyContainer>
      `,
			);
