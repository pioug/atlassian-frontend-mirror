...
describe('validate Mark', () => {
	...
	it(
		'should invoke error callback with INVALID_CONTENT error code' + 'when an unknown mark appears',
		() => {
	...
	it(
		'should invoke error callback with REDUNDANT_MARKS error code' + 'when mark not allowed',
		() => {
			const unsupportedAdfMark = {
				type: 'unsupported',
				attrs: {
	...
	it(`should return the valid spec when there are multiple specs for a node`, () => {
		...
		const { entity: resultantEntity } = validate(initialEntity, errorCallbackMock);
