...
import { HDIconGroupWrapper, ImageWrapper, Img } from '../../../../../styleWrappers';
...
describe('InteractiveImg', () => {
	it('should have image and overflow invisible until camera is defined', () => {
		...
		expect(
			getComputedStyle(component.find('img').getDOMNode()).getPropertyValue('visibility'),
		).toBe('hidden');

		expect(getComputedStyle(component.getDOMNode()).getPropertyValue('overflow')).toBe('hidden');
	});
	...
	it('should have image and overflow visible when camera is defined', () => {
		...
		expect(getComputedStyle(component.getDOMNode()).getPropertyValue('overflow')).not.toBe(
			'hidden',
		);
	});
	...
	it('should set the correct scrollLeft and scrollTop values on the ImageWrapper', () => {
		...
		const expectedOffset = camera.scaledOffset(prevOffset, prevScale, nextScale);
	...
	it('should resize a fitted image when the window is resized', () => {
		...
		const expectedZoomLevel = zoomLevelAfterResize(newCamera, camera, oldZoomLevel);

		const { zoomLevel: actualZoomLevel, camera: actualCamera } = component.state();
	...
	describe('drag and drop', () => {
		...
		it('should move image after a mousedown event', () => {
			...
			component.find(imageClassName).simulate('mousedown', { screenX: 100, screenY: 100 });
	...
	describe('when HD image is loaded', () => {
		...
		it('should show active HD button when binary resource has loaded', () => {
			const HDButton = component.find('[data-testid="hd-active"]').last().getDOMNode();
			expect(HDButton).toBeDefined();
		});

		it('should load binary source', () => {
			expect(component.find(Img).props().src).toEqual('some-original-binary-url');
		});
	});
});
...
describe('zoomLevelAfterResize', () => {
	...
	jsverify.property(
		...
		(w1, h1, w2, h2) => {
			...
			const newZoomLevel = zoomLevelAfterResize(newCamera, oldCamera, oldZoomLevel);
