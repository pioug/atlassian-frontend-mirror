...
const messageTypeWhatsNewFix = intl.formatMessage(messages.help_whats_new_filter_select_option_fix);
...
describe('WhatsNewResultListItem', () => {
	...
	it(`Should display the correct type text if the type is defined `, () => {
		const mockWhatsNewArticleItem = getMockWhatsNewArticleItem('1', WHATS_NEW_ITEM_TYPES.FIX);
		const { queryByText } = render(
			<WhatsNewResultListItem intl={intl} {...mockWhatsNewArticleItem} onClick={mockOnClick} />,
		);
