...
> extends Router<MediaDatabaseSchema> {
	register(method: string, path: string, originalHandler: RouterHandler<MediaDatabaseSchema>) {
...
export function createApiRouter(
	...
	router.get('/collection/:collectionName/items', ({ params, query }, database) => {
		const { limit, inclusiveStartKey } = query;
		const { collectionName } = params;
		const startIndex = (inclusiveStartKey && parseInt(inclusiveStartKey, 10)) || 0;
		...
		const nextInclusiveStartKey = contents.length > endIndex ? endIndex : undefined;
	...
	router.post('/file/upload', ({ query, body }, database) => {
		...
		if (!uploadRecord) {
			return new KakapoResponse(404, { message: `upload ${uploadId} is not found` }, {});
		}
		const fileRecord = database.findOne('collectionItem', {
			id: replaceFileId,
		});
		if (fileRecord === undefined) {
			return new KakapoResponse(404, { message: `file ${replaceFileId} is not found` }, {});
		}
	...
	router.post('/file/copy/withToken', (request, database) => {
		...
		let { collection: destinationCollection, replaceFileId, occurrenceKey } = query;
	...
	urlsReturnErrorsTo &&
		urlsReturnErrorsTo.forEach((url) => {
			const return500 = () => new KakapoResponse(500, { message: 'As you wish, master!' });
