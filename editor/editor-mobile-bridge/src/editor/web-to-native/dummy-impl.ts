...
export default class DummyBridge implements NativeBridge {
	...
	updateTextWithADFStatus(content: string, isEmptyADF: boolean) {
		this.log(`updateTextWithADFStatus(content=${content}, isEmptyADF=${isEmptyADF})`);
	}
	...
	typeAheadDisplayItems(query: string, trigger: string, items: string): void {
		this.log(`typeAheadQuery(query=${query}, trigger=${trigger}), items=${items}`);
	}
	...
	onCollabError(message: string, status: number, code: string) {
		saveToDummyBridge('onCollabError', [message, status, code]);
	}
}
