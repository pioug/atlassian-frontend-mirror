...
test.describe('resizing [platform-dst-panel-splitter-drag-start-client-x enabled]', () => {
	test.beforeEach(async ({ page }) => {
		await page.visitExample('design-system', 'navigation-system', 'resizable-slots', {
			featureFlag: 'platform-dst-panel-splitter-drag-start-client-x',
		});
	});
	...
	test.describe('side nav - mouse resizing', () => {
		...
		test('should be resizable even when dragstart event has clientX of 0', async ({ page }) => {
			// This test reproduces a bug caused by some browser extensions, where the `dragstart` event incorrectly
			// returns `0` for the `clientX` property.
			...
			// Simulate the browser extension bug where the `dragstart` event incorrectly returns `0` for the `clientX` property.
			// eslint-disable-next-line @atlaskit/design-system/no-direct-use-of-web-platform-drag-and-drop
			await page.evaluate(() => {
				const splitter = document.querySelector(`[data-testid="side-nav-slot-panel-splitter"]`);
				if (!splitter) {
			...
			// Width should now be 420px (resize should work despite the bugged `dragstart` event)
			await expect(sideNav).toHaveWidth(420);
		});

		test('should use the correct clientX value when there are multiple mousedowns before the drag starts', async ({
			page,
		}) => {
			const sideNavPanelSplitter = page
				.getByTestId('side-nav-slot-panel-splitter')
			...
			// 1. Click the mouse a few pixels away from the start position
			// This is to make sure we use the last clientX values from the last mousedown event before the drag starts.
			await page.mouse.move(dragStartPosition.x - 3, dragStartPosition.y);
			await page.mouse.down();
			await page.mouse.up();

			// 2. Click the mouse at the actual start position
			await page.mouse.move(dragStartPosition.x, dragStartPosition.y);
			await page.mouse.down();

			// 3. Drag the mouse to the end position
			await page.mouse.move(dragEndPosition.x, dragEndPosition.y);
