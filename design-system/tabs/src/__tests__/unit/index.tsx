	...
	describe('Tabs', () => {
		...
		describe('content persisting', () => {
			...
			it('should not unmount a panel when changed and another panel is added', () => {
				const { getByText, queryByText, getAllByRole, rerender } = render(renderTabs());
		...
		describe('props', () => {
			...
			describe('selected', () => {
				it('should set the selected tab on initial mount', () => {
					const { getByText, getByTestId } = render(renderTabs({ selected: 1 }));
					expect(getByTestId('tab-2').getAttribute('aria-selected')).toBeTruthy();
					expect(getByText('Tab 2 panel')).toBeInTheDocument();
				});
				...
				it('changing this prop should update the selected tab after the initial mount', () => {
					const { getByText, getByTestId, rerender } = render(renderTabs({ selected: 1 }));

					rerender(renderTabs({ selected: 2 }));

					expect(getByTestId('tab-3').getAttribute('aria-selected')).toBeTruthy();
				...
				it('should default to the first tab if neither selected nor defaultSelected are set', () => {
					...
					expect(getByTestId('tab-1').getAttribute('aria-selected')).toBeTruthy();
					expect(getByText('Tab 1 panel')).toBeInTheDocument();
				});

				describe("setting this prop should make the component 'controlled'", () => {
					it('should listen to selected prop if defined', () => {
						const { getByTestId, getByText } = render(renderTabs({ selected: 1 }));
					...
					it('should maintain its own internal state in case selected is not provided', () => {
						const { getByTestId, getByText, rerender } = render(renderTabs({ selected: 1 }));
						getByTestId('tab-1').click();
			...
			describe('keyboard navigation', () => {
				describe('with 3 tabs, when the 2nd tab is selected', () => {
					...
					beforeEach(() => {
						...
						expect(wrapper.getByTestId('tab-2')).toHaveAttribute('aria-selected', 'true');
					...
					it('pressing LEFT arrow selects the first tab', () => {
						...
						expect(wrapper.getByTestId('tab-1').getAttribute('aria-selected')).toBeTruthy();
					...
					it('pressing the RIGHT arrow selects the last tab', () => {
						...
						expect(wrapper.getByTestId('tab-3').getAttribute('aria-selected')).toBeTruthy();
