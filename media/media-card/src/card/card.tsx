...
import { generateScriptProps, getSSRData, type MediaCardSsrData } from '../utils/globalScope';
...
export class CardBase extends Component<CardBaseProps, CardState> {
	...
	constructor(props: CardBaseProps) {
		...
		const isCardVisible = !this.props.isLazy || (!!cardPreview && !isSSRPreview(cardPreview));
	...
	componentDidMount() {
		...
		if (isCardVisible && !!ssr && !!cardPreview && isFileIdentifier(identifier)) {
			...
			if (isSSRDataPreview(cardPreview) && isBigger(this.ssrData?.dimensions, dimensions)) {
	...
	componentDidUpdate(prevProps: CardProps, prevState: CardState) {
		...
		const { isCardVisible: prevIsCardVisible, cardPreview: prevCardPreview } = prevState;
		const { mediaClient, identifier, dimensions, useInlinePlayer, disableOverlay, ssr } =
			this.props;
		const {
			isCardVisible,
			cardPreview,
		...
		if (
			isFileIdentifier(identifier) &&
			(turnedVisible || (!!this.subscription && (isNewMediaClient || isDiffIdentifier)))
		) {
			this.updateStateForIdentifier(identifier);
		}
	...
	private refetchSSRPreview = async (identifier: FileIdentifier) => {
		try {
			...
			const wrappedError = ensureMediaCardError('remote-preview-fetch-ssr', e as Error, true);
	...
	private resolvePreview = async (identifier: FileIdentifier, fileState: FileState) => {
		try {
	...
	private getPerformanceAttributes = () => {
		...
		const durationSinceCommenced =
			timeElapsedTillCommenced && timeElapsedTillEvent - timeElapsedTillCommenced;
	...
	private onDisplayImage = () => {
		...
		let payloadPart: Pick<MediaViewedEventPayload, 'fileId' | 'isUserCollection'>;
		if (isFileIdentifier(identifier)) {
			payloadPart = {
	...
	renderMediaViewer = (): React.ReactPortal | undefined => {
		...
		const { mediaClient, identifier, mediaViewerItems, contextId, featureFlags } = this.props;
	...
	renderCard = (
		...
		const { status, progress, cardPreview, error, cardRef, isCardVisible } = this.state;
		const { metadata } = this;
			...
			ssr === 'server' && (
				<script
					{...generateScriptProps(
						...
						dataURI,
						this.requestedDimensions,
						this.ssrReliability.server?.status === 'fail' ? this.ssrReliability.server : undefined,
					)}
				></script>
			)
		);
	...
	render() {
		...
		return this.props.intl ? innerContent : <IntlProvider locale="en">{innerContent}</IntlProvider>;
	}
...
const CardWithKeyAndDateOverrideContext = (props: CardBaseProps) => {
	...
	const dateOverride = isFileIdentifier(identifier) ? dateOverrides?.[identifier.id] : undefined;
