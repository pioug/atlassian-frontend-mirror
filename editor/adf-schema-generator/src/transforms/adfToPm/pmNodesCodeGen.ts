...
export function pmNodesCodeGen(nodeResMap: Record<string, NodeSpecResMap>): string {
	return _codeBlock(
		adfToPmAutogeneratedMessage,
		`import { createPMNodeSpecFactory, type PMNodeSpecFactoryInstance } from '../../schema/createPMSpecFactory'`,
		...
		objectToSortedArray(nodeResMap)
			.map(([key, { nodeTypeDefinition, pmNodeSpec }]) => {
				...
				return _codeBlock(
					...
					_functionCallToVariable(
						functionName,
						`createPMNodeSpecFactory<${nodeName}>`,
						[stringifyWithUndefined(pmNodeSpec)],
						`PMNodeSpecFactoryInstance<${nodeName}>`,
					),
				);
			})
			.join('\n\n'),
