...
function processQuery(query: string): QueryPerformance {
	...
	const validQuery = query.replace(/\?/g, '"?"').replace(/cf\[[^\]]*]/g, 'cf[1]');
...
function writeCsv(queriesWithPerformance: QueryPerformance[]) {
	console.log(`Writing results in csv file: ${__dirname}/output/performance.csv`);
...
parseFile(`${__dirname}/input/valid-queries.csv`, { headers: true })
	.on('error', (error) => console.error(error))
	.on('data', (record: QueryRecord) => queriesWithPerformance.push(processQuery(record.jql)))
