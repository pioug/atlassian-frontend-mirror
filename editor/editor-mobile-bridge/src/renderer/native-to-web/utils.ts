...
import { buildAnnotationMarkDataAttributes, AnnotationTypes } from '@atlaskit/adf-schema';
...
export function isValidNodeTypeForScroll(nodeType: ScrollToContentNode): boolean {
	return Object.values(ScrollToContentNode).indexOf(nodeType) >= 0;
}

export function scrollToElement(nodeType: ScrollToContentNode, id: string, index = -1): void {
...
export function getElementScrollOffset(selector: string, index = -1): { x: number; y: number } {
	const element = !!~index
		? document.querySelectorAll(selector)[index]
		: document.querySelector(selector);
...
export function getQuerySelectorForNodeType(nodeType: ScrollToContentNode, id: string): string {
	switch (nodeType) {
