...
import { c, c11, cCursor, h, h11, hCursor } from '../../../__tests__/__helpers/doc-builder';
...
describe('toggleHeader', () => {
	...
	it('should keep first cell as header when the column header is enabled', () => {
		const input = doc(p('x'), table()(tr(h11, c11), tr(hCursor, c11), tr(h11, c11)))(defaultSchema);

		const result = doc(p('x'), table()(tr(h11, h11), tr(h11, c11), tr(h11, c11)))(defaultSchema);
	...
	describe('new behavior', () => {
		it('turns a header column into regular cells without override header row', () => {
			...
			const result = doc(table()(tr(hCursor, h11), tr(c11, c11)))(defaultSchema);
