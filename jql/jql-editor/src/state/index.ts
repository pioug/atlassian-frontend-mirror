 ...
 */
const isLineNumbersVisible = (editorState: EditorState) =>
...
export const actions = {
  ...
  onEditorViewFocus:
    ...
    ({ setState, getState }, { onFocus }) => {
      const { autocomplete, editorViewBlurTimeout, editorViewHasFocus } =
        getState();
      ...
      if (!editorViewHasFocus) {
        setState({
          autocomplete: {
            ...
            offsetParentRect:
              autocomplete.container?.offsetParent?.getBoundingClientRect(),
          },
  ...
  appendOptionsForObservable:
    ...
    ({ getState, setState }) => {
      ...
      return observable.pipe(
        tap(data => {
          const nextSuggestions: SelectableAutocompleteOptions = data.map(
            option => ({
              ...option,
              context,
              matchedText,
  ...
  callAutocompleteProviders:
    ...
    ({ getState, setState, dispatch }) => {
      const { onFields, onOperators, onValues, onFunctions } =
      ...
      if (
        ...
        if (rules.value) {
          const values$ = onValues(
            rules.value.matchedText,
            rules.value.context?.field,
          );
        ...
        const operators$ = sortOperators(
          onOperators(matchedText, context?.field),
        );
  ...
  onApplyEditorTransaction:
    ...
    ({ getState, setState, dispatch }, { onUpdate }) => {
      const { query, editorState, editorView, enableRichInlineNodes } =
        getState();

      const oldSelection = editorState.selection;

      const updatedQuery = getNodeText(
        transaction.doc,
        0,
      ...
      if (
        query !== updatedQuery &&
        !transaction.getMeta(JQLAutocompletePluginKey)
      ) {
      ...
      if (
        query !== updatedQuery ||
        !updatedEditorState.selection.eq(oldSelection)
      ) {
      ...
      if (query !== updatedQuery) {
        ...
        onUpdate &&
          onUpdate(updatedQuery, getJastFromState(updatedEditorState));
      }
    },
    ...
    ({ getState }) => {
      ...
      if (editorView) {
        try {
          ...
          const attributeDiff = Object.keys(attributes).reduce(
            (result, key) => {
  ...
  externalErrorMessageViewed:
    ...
    ({ getState }, { createAndFireAnalyticsEvent, externalMessages }) => {
      const { editorView, onDebugUnsafeMessage, jqlError, editorState } =
        getState();
      ...
      if (externalErrors.length > 0) {
        ...
        if (editorView && !hasClientError && attributes.syntaxErrorCount > 0) {
          const message =
            'External JQL syntax error is shown without a client error.';
          sendDebugMessage(
            message,
            editorView,
            editorState,
...
export const useEditorView = createHook<State, Actions, EditorView | undefined>(
  Store,
  {
...
export const useIsSearching = createHook<State, Actions, boolean | undefined>(
  Store,
  {
...
export const useAutocompleteProvider = createHook<
  State,
  Actions,
...
const getScopedId = (state: State, idSuffix: string): string =>
  `${state.idPrefix}_${idSuffix}`;
...
export const useEditorViewHasFocus = createHook<State, Actions, boolean>(
  Store,
  {
...
export const useLineNumbersVisible = createHook<State, Actions, boolean>(
  Store,
...
export const useAutocomplete = createHook<State, Actions, AutocompleteState>(
  Store,
 ...
 */
export const useJqlError = createHook<State, Actions, JQLParseError | null>(
  Store,
  {
 ...
 */
export const useEditorStateHasJqlError = createHook<State, Actions, boolean>(
  Store,
  {
...
export const useExternalMessages = createHook<
  State,
  Actions,
...
export const useAutocompleteOptions = createHook<
  State,
  Actions,
  SelectableAutocompleteOption[]
>(Store, {
...
const autocompleteIsLoadingSelector = (state: State): boolean =>
  state.autocomplete.loading;

export const useAutocompleteLoading = createHook<State, Actions, boolean>(
  Store,
  {
...
export const useAutocompleteIsOpen = createHook<State, Actions, boolean>(
  Store,
  {
...
export const useAutocompletePosition = createHook<
  State,
  Actions,
  ...
  selector: state => {
    ...
    const viewportTop =
      editorViewContainerRect !== undefined
        ? clamp(
            autocompleteTop,
            editorViewContainerRect.top,
            editorViewContainerRect.bottom,
          )
        : autocompleteTop;
...
export const useHydratedDeprecations = createHook<
  State,
  Actions,
...
export const useRichInlineNodesEnabled = createHook<State, Actions, boolean>(
  Store,
  {
...
export const EditorStateContainer = createContainer<State, Actions, Props>(
  Store,
  {
    onInit:
      () =>
            ...
            window.ResizeObserver !== undefined
              ? new window.ResizeObserver(() => {
                  setState({
                    ...
                    editorViewContainerRect:
                      getState().editorViewContainer?.getBoundingClientRect(),
                  });
                })
              : undefined,
          onDebugUnsafeMessage,
