...
export class JIRATransformer implements Transformer<string> {
	...
	private encodeText(node: PMNode) {
		if (node.text) {
			...
			const { code, em, link, typeAheadQuery, strike, strong, subsup, underline, textColor } =
				this.schema.marks;
			...
			node.content.forEach((childNode) => {
				if (
					...
					if (list instanceof HTMLElement && list.tagName === 'UL') {
						...
						[].forEach.call(list.querySelectorAll('ul'), (ul: HTMLUListElement) => {
	...
	private addDataToNode(domNode: HTMLElement, mediaNode: PMNode, defaultDisplayType = 'thumbnail') {
		const { id, type, collection, __fileName, __displayType, width, height } = mediaNode.attrs;
	...
	private encodeMedia(node: PMNode) {
		...
		if (
			...
			if (node.attrs.collection && this.mediaContextInfo && this.mediaContextInfo.uploadContext) {
				img.setAttribute(
					'src',
					this.buildURLWithContextInfo(node.attrs.id, this.mediaContextInfo.uploadContext),
				);
			} else if (this.mediaContextInfo && this.mediaContextInfo.viewContext) {
				img.setAttribute(
					'src',
					this.buildURLWithContextInfo(node.attrs.id, this.mediaContextInfo.viewContext),
				);
			}
