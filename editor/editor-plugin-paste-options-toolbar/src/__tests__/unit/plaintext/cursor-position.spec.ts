...
describe('format plain text: cursor positioning', () => {
  it('should paste text at given position', () => {
    const state = createEditorState(doc(p('{<>}Type something')));
    ...
    expect(tr).toEqualDocumentAndSelection(
      doc(p('Some pasted text.{<>}Type something')),
    );
  });
  ...
  it('should not do anything when start position is invalid', () => {
    ...
    expect(tr).toEqualDocumentAndSelection(doc(p('{<>}Type something')));
  });
  ...
  it('should replace rich text with input plain text', () => {
    const state = createEditorState(doc(p(code('Type something{<>}'))));
    ...
    const tr = formatPlainText(state.tr, pluginState);
    expect(tr).toEqualDocumentAndSelection(doc(p('Pasted plain text.{<>}')));
  });

  it('should replace existing text with the input single character', () => {
    const state = createEditorState(doc(p('Lorem ipsum dolor sit amet.{<>}')));
    ...
    const tr = formatPlainText(state.tr, pluginState);
    expect(tr).toEqualDocumentAndSelection(doc(p('I{<>}')));
  });
});
