	...
	protocolTypeCombos.forEach((protocolTypes) => {
		describe(`with protocols: ${protocolTypes}`, () => {
			...
			beforeEach(() => {
				...
				protocol = protocols.find((protocol) => protocol.getType() === 'some-protocol')!;
			...
			describe('#leave', () => {
				...
				it.skip('should debounce call to subscribe ', () => {
					client.join(['ari:cloud:platform::site/666', 'ari:cloud:platform::site/667']);
					client.leave(['ari:cloud:platform::site/666']).then(() => {
						expect(protocol.subscribe).toHaveBeenCalledTimes(1);

						const lastCall = fetchMock.lastCall(`${baseUrl}/subscribe`) as any as [
							Request,
							undefined,
						];
						expect(lastCall).toBeDefined();
			...
			describe('#on', () => {
				...
				it('should subscribe to base events after joining channels', (done) => {
					client.join(['ari:cloud:platform::site/666', 'ari:cloud:platform::site/667']).then(() => {
				...
				it('should unsubscribe from base events after leaving all channels', (done) => {
					client
						.join(['ari:cloud:platform::site/666', 'ari:cloud:platform::site/667'])
						.then(() =>
							client.leave(['ari:cloud:platform::site/666', 'ari:cloud:platform::site/667']),
						)
						.then(() => {
							expect(protocol.on).toHaveBeenCalledTimes(4);
