...
test.describe('event handlers', () => {
	test.describe('with a single paragraph', () => {
		...
		test('should swap renderer to editor when clicking on paragraph', async ({ renderer }) => {
			const paragraph = renderer.page.locator('p');
			await paragraph.click();
			await expect(renderer.page.locator(editorPlaceholderSelector)).toBeVisible();
		});
	});
	...
	test.describe('with a multiple paragraphs', () => {
		...
		test('should not swap renderer to editor on selection', async ({ renderer }) => {
			...
			await renderer.page.waitForFunction(() => window.getSelection()?.toString() === 'second\n\n');
			await expect(renderer.page.locator(editorPlaceholderSelector)).toBeHidden();

			// Remove selection by clicking. Clicking after selection should not swap renderer to editor
			await paragraph4.click();

			await renderer.waitForRendererStable();
			await renderer.page.waitForFunction(() => window.getSelection()?.toString() === '');
			await expect(renderer.page.locator(editorPlaceholderSelector)).toBeHidden();
		});
	});
});
