...
import { getProviders, mediaProvider, quickInsertProvider } from '../../examples/5-full-page';
...
> {
	...
	private cardProviderPromise = Promise.resolve(new ConfluenceCardProvider('stg'));
				...
				<KitchenSinkEditor
					...
					annotationProviders={{
						inlineComment: {
							...
							supportedBlockNodes:
								this.props.featureFlags && this.props.featureFlags['comments-on-media']
									? ['media']
									: [],
						},
					}}
	...
	private validateDocument = () => {
		...
		this.validating = this.requestIdle(() => {
			...
			const errorCb: ErrorCallback = (entity, error, options) => {
				const wrappedEntity = validationErrorHandler(entity, error, options, marks, validate);
...
const findErrorsRecursively = (
	...
	if (entityMarks) {
		entityMarks.forEach((mark) => {
			if (mark.type === unsupportedMark.name || mark.type === unsupportedNodeAttribute.name) {
				errorCallback(getAttr(mark, 'error'), getAttr(mark, 'entity'));
			}
		});
	}
	if (entityType === unsupportedInline.name || entityType === unsupportedBlock.name) {
