...
describe('Navigation', () => {
	...
	it('should call onChange callback when left arrow is clicked', () => {
		...
		const el = mount(<Navigation onChange={onChange} items={items} selectedItem={identifier2} />);
	...
	it('should call onChange callback when right arrow is clicked', () => {
		const onChange = jest.fn();
		const el = mount(<Navigation onChange={onChange} items={items} selectedItem={identifier} />);
	...
	describe('Shortcuts', () => {
		it.skip('should call onChange callback when left ARROW key is pressed', () => {
			const onChange = jest.fn();
			mount(<Navigation onChange={onChange} items={items} selectedItem={identifier2} />);
		...
		it.skip('should call onChange callback when right ARROW key is pressed', () => {
			const onChange = jest.fn();
			mount(<Navigation onChange={onChange} items={items} selectedItem={identifier} />);
			const e = new KeyboardEventWithKeyCode('keydown', {
				bubbles: true,
				cancelable: true,
				keyCode: 39,
