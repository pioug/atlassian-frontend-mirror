...
describe('Pagination', () => {
	...
	describe('rendering with default props', () => {
		...
		it('should render aria-current on selected page', () => {
			...
			expect(screen.getByRole('button', { current: 'page' })).toHaveAccessibleName('page 1');
		});
	...
	describe('props', () => {
		describe('#components', () => {
			it('should render custom components', () => {
				function Page({ page, ...rest }: { page: number }, ref: React.Ref<HTMLDivElement>) {
					return (
				...
				function Navigation(props: { 'aria-label': string }, ref: React.Ref<HTMLDivElement>) {
					return (
						<div
							ref={ref}
							// eslint-disable-next-line @repo/internal/react/no-unsafe-spread-props
		...
		describe('#getPageLabel', () => {
			it('should render custom page labels', () => {
				...
				const getPageLabel = (page: { label: string }, index: number) => `${page.label}-${index}`;
				...
				const newGetPageLabel = (page: { label: string }, index: number) =>
					`${page.label}---${index}`;
				view.rerender(<Pagination {...props} pages={pages} getPageLabel={newGetPageLabel} />);
		...
		describe('#style', () => {
			it('should apply style to container', () => {
				...
				expect(screen.getByRole('navigation', { name: 'pagination' })).toHaveStyle(
				...
				expect(screen.getByRole('navigation', { name: 'pagination' })).not.toHaveStyle(
					'border: 1px solid red',
				);
				expect(screen.getByRole('navigation', { name: 'pagination' })).toHaveStyle('top: 50px');
			});
		});
		...
		describe('#renderEllipsis', () => {
			it('should re-render with new ellipsis', () => {
				...
				view.rerender(
					<Pagination {...props} renderEllipsis={jest.fn().mockReturnValue('\u2026Ell\u2026')} />,
				);
	...
	describe('interactions', () => {
		...
		it('should set aria-current on newly selected page', () => {
			...
			fireEvent.click(screen.getByText('5'));
			expect(screen.getByRole('button', { current: 'page' })).toHaveAccessibleName('page 5');
