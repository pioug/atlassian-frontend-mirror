 ...
 */
export function sendToBridge<K extends CombinedBridgeNames, EventName extends BridgeEventName<K>>(
	bridgeName: K,
	eventName: EventName,
	props = {},
) {
	if (window.webkit) {
		...
		if (bridge && eventName in bridge) {
			...
			try {
				...
				console.error(
					`Could not call ${bridgeName}.${String(eventName)}() with args: ${JSON.stringify(args)}`,
				);
			}
		}
	}
