...
const docAfterAddColumnAtThree = doc(
	table()(tr(tdColorA, tdColorB, tdColorC, tdEmpty), tr(tdColorA, tdColorB, tdColorC, tdEmpty)),
);

const docAfterAddColumnAtZero = doc(
	table()(tr(tdEmpty, tdColorA, tdColorB, tdColorC), tr(tdEmpty, tdColorA, tdColorB, tdColorC)),
);

const docAfterAddColumnAtOne = doc(
	table()(tr(tdColorA, tdEmpty, tdColorB, tdColorC), tr(tdColorA, tdEmpty, tdColorB, tdColorC)),
);

const docAfterAddColumnAtTwo = doc(
	table()(tr(tdColorA, tdColorB, tdEmpty, tdColorC), tr(tdColorA, tdColorB, tdEmpty, tdColorC)),
);

const docAfterRemoveColumnAtZero = doc(table()(tr(tdColorB, tdColorC), tr(tdColorB, tdColorC)));
const docAfterRemoveColumnAtOne = doc(table()(tr(tdColorA, tdColorC), tr(tdColorA, tdColorC)));
const docAfterRemoveColumnAtTwo = doc(table()(tr(tdColorA, tdColorB), tr(tdColorA, tdColorB)));
...
describe('AddColumnStep', () => {
  ...
  `, () => {
		...
		const threeByOne = doc(p(''), '{table}', table()(tr(tdColorA), tr(tdColorA), tr(tdColorA)));
		...
		it('should delete the table when I tried to remove the last column', function () {
			editorState = editorState.apply(removeColumnAtFactory('table', 0)(editorState, refs));

			expect(editorState.doc.toJSON()).toEqual(doc(p(''))(defaultSchema).toJSON());
		});
		...
		it('should add the table after been removed it for a remove column', () => {
			...
			expect(editorState.doc.toJSON()).toEqual(threeByOne(defaultSchema).toJSON());
		});

		it('should remove the table after undoing the last column ', function () {
			const editorState = testHistory(threeByOne, addColumnAtFactory('table', 0), [
		...
		it('should drop add column step when I remove the column', function () {
			const firstTransaction = removeColumnAtFactory('table', 0)(editorState, refs);
			let addColumnStep: Step | null | undefined = addColumnAtFactory('table', 0)(editorState, refs)
				.steps[0];
		...
		it('should remove the column instead of table when I add a column just before', function () {
			const firstTransaction = addColumnAtFactory('table', 0)(editorState, refs);
			let addColumnStep: Step | null | undefined = removeColumnAtFactory('table', 0)(
				editorState,
				refs,
			).steps[0];
		...
		it('should parse a json that we just generate', function () {
			...
			const removeTableStepFromJson = Step.fromJSON(editorState.schema, removeTableStep.toJSON())!;

			// Should keep the new created single column and remove the old one
			expect(editorState.apply(editorState.tr.step(removeTableStep)).doc).toEqual(
		...
		it('should add the removing table when I received a JSON', () => {
			...
			const removeLastRowInverted = Step.fromJSON(defaultSchema, removeLastRowInvertedJson);
			editorState = editorState.apply(editorState.tr.step(removeLastRowInverted));
		...
		it('should remove the table when I received a JSON', () => {
			...
			const removeLastRowInverted = Step.fromJSON(defaultSchema, removeLastRowJson);
			editorState = editorState.apply(editorState.tr.step(removeLastRowInverted));
  ...
  `, () => {
		...
		describe('invert', () => {
			...
			`('should be able to $description', ({ setup }) => {
				...
				editorState = applyAndInvertTransaction(originalDoc)(transaction, editorState);
		...
		describe('history plugin', () => {
			it('should be able to undo the right column', function () {
				const editorState = testHistory(twoBythreeTable, addColumnAtFactory('table', 2), [
					addColumnAtFactory('table', 1),
					addColumnAtFactory('table', 1),
				]);
		...
		describe('fromJson()', () => {
			...
			`('should add a column at $column of the table', ({ jsonStep, expectedDoc }) => {
			...
			`('should remove a column at $column of the table', ({ jsonStep, expectedDoc }) => {
				const addColumnStep = Step.fromJSON(defaultSchema, jsonStep);
				editorState = editorState.apply(editorState.tr.step(addColumnStep));
		...
		describe('serialize/deserialize', () => {
			...
			`('should serialize/deserialize a new column at $column', ({ column }) => {
				...
				const addColumnStep = AddColumnStep.create(editorState.doc, refs['table'], column, false);
			...
			`('should serialize/deserialize remove a column at $column', ({ column }) => {
				const { editorState, refs } = setup(twoBythreeTable);
				const addColumnStep = AddColumnStep.create(editorState.doc, refs['table'], column, true);
