...
describe('JIRA wiki markup - Links', () => {
	const testCases: Array<[string, string]> = [
		['should convert simple links to link marks', 'This is a [https://www.atlassian.com] link'],
		[
		...
		['should handle anchors with text and title', '[link-text|#anchor-text|link title]'],
		['should convert links with port to link marks', 'Link with port [http://localhost:8080] here'],
		[
		...
		['should convert IRC address text', 'An IRC address: irc://atlassian.com/confluence'],
		['should convert mailto address text', 'An IRC address: mailto:example@email.com'],
		['should ignore invalid protocal', 'An invalid URL: invalid://nowhere.com/hello'],
		[
		...
		['[CS-542] should correctly style superscript links', `[^link title^|http://example.com]`],
		['[CS-542] should correctly style superscript links', `[~link title~|http://example.com]`],
		['[CS-542] should not parse nested links', '[test[test|http://example.com]'],
		[
		...
		['[ADFS-80] should render color in links', 'asdasd[{color:red}test{color}|test]asdasdasd'],
		[
