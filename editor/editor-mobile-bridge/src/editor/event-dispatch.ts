...
type GetPayload<T extends EventTypes> = T extends EventTypes.SET_DOCUMENT_REFLOW_DETECTOR_STATUS
	? boolean
...
export class BridgeEventEmitter {
	...
	on<T extends EventTypes>(event: T, listener: (payload: GetPayload<T>) => void): EventEmitter {
		return this.emitter.on(event, listener);
	}

	off<T extends EventTypes>(event: T, listener: (payload: GetPayload<T>) => void): EventEmitter {
