...
const isOption = (option: Parameters[string]): option is Option => {
	return option && typeof option === 'object' && 'label' in option && 'value' in option;
};

const isOptions = (options: Parameters[string]): options is Option[] => {
...
function extract(value: Parameters[string], field: FieldDefinition, options?: ExtractOptions) {
	if (isOptions(value)) {
	...
	else if (options?.useDefaultValue && value === undefined && 'defaultValue' in field) {
...
export const findDuplicateFields = (fields: FieldDefinition[]): FieldDefinition | undefined =>
	findDuplicateFieldsInternal(flattenFields(fields));

const findDuplicateFieldsInternal = (fields: FieldDefinition[]): FieldDefinition | undefined => {
...
export const serialize = async (
	...
	const hasDuplicateFields =
		parentType === 'fieldset' && !!flattenedFields.find((field) => field.allowDuplicates);
...
const serializeFieldset = async (
	...
	const fieldParams = extract(data[field.name], field, { useDefaultValue: true }) || {};
...
function injectDefaultValues(
	...
	const copy: ParametersWithDuplicateFields = [...convertToParametersArray(data)];

	for (const field of fields) {
		const { name } = field;
		const fieldIndex = copy.findIndex((item: Parameters) => Object.entries(item)[0][0] === name);
...
const flattenFields = (fields: FieldDefinition[]): FieldDefinition[] => {
	const flattenAccumulator = (accumulator: FieldDefinition[], field: FieldDefinition) => {
		if (isTabGroup(field)) {
			if (field.hasGroupedValues) {
