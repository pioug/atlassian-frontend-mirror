...
import ButtonRovo from '../../examples/constellation/new-button/button/button-rovo';
...
import ButtonDiscovery from '../../examples/constellation/new-button/button/button-discovery';
...
Use a primary button to call attention to a [form](/components/form) submission or to highlight the
...
Review
[usage details for alignment examples](/components/button/button-legacy/usage#alignment-and-positioning).
...
wherever possible, avoid using `isDisabled`. Instead, use [validation](/components/button/usage) or
...
forms. Follow the [alignment guidance](/components/button/usage#alignment-and-positioning) for more
options.
...
<Example
	Component={ButtonWithIconBefore}
	packageName="@atlaskit/button"
	featureFlagDefaults={{ 'platform-dst-motion-uplift-button': true }}
/>
...
<Example
	Component={ButtonWithIconBeforeSize}
	packageName="@atlaskit/button"
	featureFlagDefaults={{ 'platform-dst-motion-uplift-button': true }}
/>
...
For more information see [truncation](/foundations/content/language-and-grammar#truncation).
...
<SectionMessage title="CustomThemeButton is deprecated" appearance="warning">
	<p>
		The{' '}
		<a href="/components/button/button-legacy/examples#custom-theme-button">CustomThemeButton</a> is
		deprecated. Use <a href="/components/primitives/pressable">Pressable</a> if you require a custom
