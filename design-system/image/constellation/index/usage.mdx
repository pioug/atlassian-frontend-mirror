...
The image component can be used interchangeably with the native `img` element. The key difference at
this stage is the image component's awareness of themes, like dark mode.
...
[typography](/foundations/typography/) provides display headings that are accessible and optimised
to work on every screen size.
...
- Avoid applying css directly to `image`. This isn't supported, and will be deprecated in favor of
  ...
  instead, but
  [avoid unsafe style overrides](/components/eslint-plugin-design-system/no-unsafe-style-overrides/usage).
