...
import { TableHeader, withSortableColumn } from '../../../../react/nodes/tableCell';
...
describe('Renderer - React/Nodes/TableHeader', () => {
	...
	describe('withCellProps', () => {
		...
		it('should render the colwidths', () => {
			...
			const intlTableHeader = mountWithIntl(<TableHeader colwidth={colwidth} />);
	...
	describe('withSortableColumn', () => {
		let WithSortableColumn: React.ComponentType<
			React.PropsWithChildren<Omit<CellWithSortingProps, 'intl'>>
		>;
		const TestComp = ({ children }: CellWithSortingProps) => <th>{children}</th>;
		...
		describe('when allowColumnSorting is the default value', () => {
			it('should not add sortable class name', () => {
				...
				expect(
					wrapper.find(TestComp).hasClass(RendererCssClassName.SORTABLE_COLUMN_WRAPPER),
				).toBeFalsy();
			});
		});
		...
		describe('when allowColumnSorting is false', () => {
			it('should not add sortable class name', () => {
				const wrapper = mountWithIntl(<WithSortableColumn allowColumnSorting={false} />);
		...
		describe('when allowColumnSorting is true', () => {
			it('should add sortable class name', () => {
				const wrapper = mountWithIntl(<WithSortableColumn allowColumnSorting />);
				expect(
					wrapper.find(TestComp).hasClass(RendererCssClassName.SORTABLE_COLUMN_WRAPPER),
				).toBeTruthy();
			});
			...
			describe('when onSorting function exist', () => {
				...
				it('should not call onSorting when clicking the wrapper', () => {
					wrapper = mountWrapper();
					wrapper.find(`.${RendererCssClassName.SORTABLE_COLUMN_WRAPPER}`).simulate('click');
				...
				it.each(keys)(
					...
					(key) => {
						wrapper = mountWrapper();
						wrapper.find(`[id="${MOCK_SORTING_ICON_ID}"]`).simulate('keydown', { key });
						expect(onSorting).toHaveBeenCalled();
					},
				);
