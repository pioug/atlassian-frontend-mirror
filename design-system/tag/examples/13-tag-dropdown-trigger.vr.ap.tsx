...
const SelectedToggleExample = () => {
	...
	return (
		<Stack space="space.100">
			...
			<Text as="p" size="small" color="color.text.subtlest">
				Toggle <code>isSelected</code> to preview the selected-state background transition.
			</Text>
			<Inline space="space.100">
				<Button onClick={() => setIsSelected((current) => !current)}>
					{isSelected ? 'Hide selected' : 'Show selected'}
				</Button>
			</Inline>
			<Inline space="space.100">
				<TagDropdownTrigger
