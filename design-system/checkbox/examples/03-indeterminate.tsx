...
const getInitialCheckedItems = (): Checkboxes => {
  ...
  childrenCheckboxes.forEach(
    (child) => (initialChildCheckboxes[child.id] = false),
  );
...
const IndeterminateCheckbox = () => {
  ...
  const getCheckedChildrenCount = () =>
    getAllChildren().filter(isChildChecked).length;

  const isParentChecked = () => getCheckedChildrenCount() > 0;
  const isChildChecked = (childCheckboxId: string) =>
    childCheckboxes[childCheckboxId];

  const isIndeterminate = () => {
    const checkedChildrenCount = getCheckedChildrenCount();
    const notAllChildrenAreChecked =
      checkedChildrenCount < getAllChildren().length;
