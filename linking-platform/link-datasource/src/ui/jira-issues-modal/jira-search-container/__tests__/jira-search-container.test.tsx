...
const setupBasicFilter = ({
	...
	const triggerButton = queryByTestId(`jlol-basic-filter-${filterType}-trigger`);
	...
	describe('BasicFilterContainer: should show basic filter container based on FF value', () => {
		ffTest(
			...
			() => {
				...
				expect(queryByTestId('jlol-basic-filter-container')).toBeInTheDocument();

				expect(queryByTestId('jlol-basic-filter-project-trigger')).toBeInTheDocument();
				expect(queryByTestId('jlol-basic-filter-status-trigger')).toBeInTheDocument();
				expect(queryByTestId('jlol-basic-filter-assignee-trigger')).toBeInTheDocument();
				expect(queryByTestId('jlol-basic-filter-type-trigger')).toBeInTheDocument();
			},
			() => commonBasicFilterFeatureFlagFalsyTest(),
		);
	});
	...
	describe('BasicFilterContainer: should disable basic mode on search when the query is complex based on FF', () => {
		ffTest(
			...
			() => {
				...
				expect(getByTestId('mode-toggle-basic').querySelector('input')).not.toBeDisabled();
			},
		);
	});
	...
	describe('BasicFilterContainer: should render options with those selected ordered first on menu reopen', () => {
		ffTest(
			...
			async () => {
				...
				const selectMenu2 = await findByTestId('jlol-basic-filter-status-popup-select--menu');
	...
	describe('BasicFilterContainer: should apply selection for each filter correctly', () => {
		ffTest(
			...
			async () => {
				...
				// Close menu
				const statusTriggerButton = await findByTestId(`jlol-basic-filter-status-trigger`);
				...
				const projectTriggerButton = await findByTestId(`jlol-basic-filter-project-trigger`);
	...
	describe('BasicFilterContainer: should reset filter labels when the cloudId changes', () => {
		ffTest(
			...
			async () => {
				...
				const { findByTestId, queryByTestId, mockOnSearch, rerender } = renderResult;
				...
				expect(queryByTestId('jlol-basic-filter-container')).toHaveTextContent('ProjectTypeStatus');
			},
			() => commonBasicFilterFeatureFlagFalsyTest(),
		);
	});
	...
	describe('BasicFilterContainer: should reset search term when the cloudId changes', () => {
		...
		ffTest(
			...
			async () => {
				...
				const { container, mockOnSearch, rerender, queryByTestId } = renderResult;
				...
				// Open menu
				const newInstaceOfTriggerButton = queryByTestId(`jlol-basic-filter-status-trigger`);
		...
		ffTest(
			...
			async () => {
				...
				availableBasicFilterTypes.forEach((filter) => {
					expect(
						queryByTestId(`jlol-basic-filter-${filter}-trigger--loading-button`),
					).not.toBeInTheDocument();
					expect(queryByTestId(`jlol-basic-filter-${filter}-trigger--button`)).toBeInTheDocument();
				});
			},
			() => commonBasicFilterFeatureFlagFalsyTest(),
		);
	...
	describe('BasicFilterContainer: should pre-populate basic mode search text hydrate returns input text', () => {
		ffTest(
			...
			async () => {
				...
				expect(queryByTestId('jira-datasource-modal--basic-search-input')).toHaveValue('hello');
			},
			() => commonBasicFilterFeatureFlagFalsyTest(),
		);
	});
