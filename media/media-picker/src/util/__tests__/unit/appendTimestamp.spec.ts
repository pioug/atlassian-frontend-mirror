...
describe('appendTimestamp', () => {
	...
	it('should work with normal file name', () => {
		expect(appendTimestamp('image.png', defaultDate)).toBe('image-20181212-153045.png');
	});

	it('should work with file names with dot', () => {
		expect(appendTimestamp('image.xyz.png', defaultDate)).toBe('image.xyz-20181212-153045.png');
	});
	...
	it('should work when file has no extension', () => {
		expect(appendTimestamp('my-image', defaultDate)).toEqual('my-image-20181212-153045');
	});

	it('should work when there is a dot in the begining', () => {
		expect(appendTimestamp('.my-image.png', defaultDate)).toEqual('.my-image-20181212-153045.png');
	});
});
