...
export class RuleContextVisitor implements JQLParserVisitor<JQLRuleContext> {
  ...
  visitJqlComparisonOperator = (
    ctx: JqlComparisonOperatorContext,
  ): JQLRuleContext => {
    return this.visitOperator(ctx);
  };
  ...
  visitJqlChangedOperator = (
    ctx: JqlChangedOperatorContext,
  ): JQLRuleContext => {
    return this.visitOperator(ctx);
  };
  ...
  private isReplacePosAtCtx = (ctx: ParserRuleContext): boolean => {
    ...
    return (
      (this.replacePositionStart >= start &&
        this.replacePositionStart <= stop) ||
  ...
  private getCtxToVisit = (
    contexts: MaybeParserRuleContext[],
  ): MaybeParserRuleContext => {
  ...
  private getCtxFromErrorNodes = (
    errorNodes: ErrorNode[] | undefined,
  ): JQLRuleContext => {
    if (errorNodes !== undefined) {
      const textBeforeReplacePosition = errorNodes
        .filter(
          errorNode => errorNode.payload.startIndex < this.replacePositionStart,
        )
