...
export class ViewportDebugger {
	...
	private navigationCanvas: HTMLCanvasElement = document.createElement('canvas');
	...
	renderNavigation() {
		...
		if (ctx) {
			...
			ctx.fillRect(innerBounds.left, innerBounds.top, innerBounds.width, innerBounds.height);
			// black outline
			ctx.strokeStyle = '#000';
			ctx.strokeRect(0, 0, outerBounds.width, outerBounds.height);
			// item bounds
			ctx.fillStyle = '#00ff002b';
			ctx.fillRect(itemBounds.left, itemBounds.top, itemBounds.width, itemBounds.height);
			// item border
			ctx.strokeStyle = 'green';
			ctx.strokeRect(itemBounds.left, itemBounds.top, itemBounds.width, itemBounds.height);
			// cross lines
			ctx.beginPath();
			...
			ctx.closePath();
			ctx.strokeStyle = '#fff';
			ctx.strokeRect(innerBounds.left, innerBounds.top, innerBounds.width, innerBounds.height);
		}
	}
