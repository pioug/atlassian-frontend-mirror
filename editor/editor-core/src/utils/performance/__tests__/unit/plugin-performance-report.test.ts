...
describe('PluginPerformanceReport.prototype.withEntryList', () => {
	...
	it('defaults step duration to 0 if missing', () => {
		...
		const report = PluginPerformanceReport.fromEntry(entry).withEntryList(
			MockPerformanceObserverEntryList.fromNames([EVENT_NAME_STATE_APPLY, EVENT_NAME_ON_CHANGE]),
		);
	...
	it('updates the slowPlugins field', () => {
		...
		const fastEntryList = MockPerformanceObserverEntryList.fromSeries(fastSeries);
		const slowEntryList = MockPerformanceObserverEntryList.fromSeries(slowSeries);
