...
export const getNestedDepth = (
	...
	const parent = conversation.comments.find((comment) => comment.commentId === parentId);
...
const updateComment = (comments: Comment[] | undefined, newComment: Comment) => {
...
const removeComment = (comments: Comment[] | undefined, commentToRemove: Comment) => {
	return (comments || []).filter((comment) => {
		return (
			(commentToRemove.localId && comment.localId !== commentToRemove.localId) ||
			comment.commentId !== commentToRemove.commentId
		);
	});
};

const updateConversation = (conversations: Conversation[], newConversation: Conversation) => {
...
const updateCommentInConversation = (conversations: Conversation[], newComment: Comment) => {
	return conversations.map((conversation) => {
...
const addOrUpdateCommentInConversation = (conversations: Conversation[], newComment: Comment) => {
	return conversations.map((conversation) => {
		if (conversation.conversationId === newComment.conversationId) {
...
const getCommentFromConversation = (
	...
	const [comment = null] = conversations.reduce<Array<Comment>>((acc, conversation) => {
		if (conversation.conversationId !== conversationId || !conversation.comments) {
			return acc;
		}
