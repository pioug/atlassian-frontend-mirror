...
const rule = createLintRule({
  ...
  create(context) {
    return {
      ...
      'CallExpression[callee.name="keyframes"] ObjectExpression Property': (
        node: Rule.Node,
      ) => StyleProperty.lint(node, { context }),
      'CallExpression[callee.name="cssMap"] ObjectExpression Property': (
        node: Rule.Node,
      ) => StyleProperty.lint(node, { context }),
      'CallExpression[callee.object.name=styled] ObjectExpression Property': (
        node: Rule.Node,
      ) => StyleProperty.lint(node, { context }),
      'CallExpression[callee.object.name=styled2] ObjectExpression Property': (
        node: Rule.Node,
      ) => StyleProperty.lint(node, { context }),
    };
  },
