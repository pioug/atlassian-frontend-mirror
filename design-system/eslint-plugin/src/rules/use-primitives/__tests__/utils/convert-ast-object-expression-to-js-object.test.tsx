...
describe('convertASTObjectExpressionToJSObject', () => {
  it('gets single property', () => {
    ...
    const result = convertASTObjectExpressionToJSObject(
  ...
  it('gets multiple properties', () => {
    const root = j(
      `const paddingStyles = css({ display: 'flex', padding: '8px' });`,
    );
  ...
  it('marks as unsupported properties where value is not a string', () => {
    const root = j(
      `const paddingStyles = css({ display: 'flex', padding: 8 });`,
    );
  ...
  it('gets tokenised property without fallback', () => {
    const root = j(
      `const paddingStyles = css({ padding: '8px', margin: token('space.200') });`,
    );
  ...
  it('records unsupported if styles are created with a spread', () => {
    const root = j(
      `const paddingStyles = css({ display: 'flex', ...moreStyles });`,
    );
  ...
  it('marks as unsupported properties where value is a random function call', () => {
    const root = j(
      `const paddingStyles = css({ display: 'flex', padding: blah() });`,
    );
