...
export const getClosestColorIndex = (
  ...
  themeRamp.forEach((value: CSSColor, index: number) => {
    ...
    if (
      closestColorDistance === null ||
      distance < (closestColorDistance as number)
    ) {
...
export const generateColors = (brandColor: CSSColor): CSSColor[] => {
  ...
  const isLowLuminance =
    relativeLuminanceW3C(baseRgb[0], baseRgb[1], baseRgb[2]) < 0.4;
  // Choose right palette
  const themeRatios = isLowLuminance
    ? lowLuminanceContrastRatios
    : highLuminanceContrastRatios;
...
export const generateTokenMap = (
  ...
  if (inputContrast < 4.5) {
    ...
    if (
      ...
      customThemeTokenMapDark['color.background.brand.bold'] =
        closestColorIndex;
      customThemeTokenMapDark['color.background.brand.bold.hovered'] =
        closestColorIndex - 1;
      customThemeTokenMapDark['color.background.brand.bold.pressed'] =
        closestColorIndex - 2;
    }
  }
