...
let mockGetMediaClient: jest.SpyInstance<(typeof MediaClientReactModule)['getMediaClient']> =
...
describe('SiteEmojiResource', () => {
	...
	describe('#generateTokenisedMediaURLS', () => {
		...
		it('tokenises mediaPath of representation and altRepresentatio with media jwt and client id', async () => {
			...
			if (isMediaRepresentation(representation) && isMediaRepresentation(altRepresentation)) {
		...
		it('returns original representation and does not call getToken if representation is not media', async () => {
			const siteEmojiResource = new TestSiteEmojiResource(tokenManagerStub);
			const { representation } = await siteEmojiResource.generateTokenisedMediaURLS(
	...
	describe('#deleteEmoji', () => {
		...
		it('Deleting a custom emoji fails for non-CUSTOM category', () => {
			...
			return siteEmojiResource.deleteEmoji(emoji).catch((status) => expect(status).toEqual(false));
		});
