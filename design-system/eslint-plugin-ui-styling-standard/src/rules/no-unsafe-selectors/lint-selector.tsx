 ...
 */
function checkNoAmbiguousPseudos({
  context,
  sourceNode,
  selector,
  config,
...
function checkNoRestrictedPseudos({
  context,
  sourceNode,
  selector,
}: CheckArgs) {
  selector.walkPseudos((pseudo) => {
     ...
     */
    if (
      legacyPseudoElements.has(pseudo.value) &&
      allowedPseudos.has(`:${pseudo.value}`)
    ) {
      return;
    }
