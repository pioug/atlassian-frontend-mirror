...
export function assertSelectionPayload(input: unknown): ValidationResult<Error, SelectionPayload> {
	...
	if (!objectShape.rect.hasOwnProperty('top') || !objectShape.rect.hasOwnProperty('left')) {
	...
	switch (softSelection.type) {
		...
		case 'text':
		case 'cell': {
			if (typeof softSelection.anchor !== 'number' || typeof softSelection.head !== 'number') {
