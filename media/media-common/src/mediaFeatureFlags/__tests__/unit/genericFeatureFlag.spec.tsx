...
describe('getGenericFeatureFlag', () => {
  describe('shoud return default if no value passed', () => {
    ...
    for (key in defaultFlags) {
      it(key, () => {
        expect(getGenericFeatureFlag(key, defaultFlags)).toEqual(
          defaultFlags[key],
        );
        expect(getGenericFeatureFlag(key, defaultFlags, {})).toEqual(
          defaultFlags[key],
  ...
  it('should return consumer value if passed through', () => {
    expect(
      getGenericFeatureFlag('my-first-flag', defaultFlags, consumerFlags),
    ).toEqual(false);
    expect(
      getGenericFeatureFlag('my-second-flag', defaultFlags, consumerFlags),
    ).toEqual(true);
    expect(
      getGenericFeatureFlag('my-third-flag', defaultFlags, consumerFlags),
    ).toEqual(1);
    expect(
      getGenericFeatureFlag('my-fourth-flag', defaultFlags, consumerFlags),
    ).toEqual('bye-mate');
  });
  ...
  it('should use localStorage override if available even if flags passed', () => {
    ...
    expect(getGenericFeatureFlag('my-second-flag', defaultFlags, {})).toBe(
      true,
    );
    ...
    expect(getGenericFeatureFlag('my-fourth-flag', defaultFlags, {})).toBe(
      'cya',
    );
