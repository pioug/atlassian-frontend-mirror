	...
	iconJSXWithSpacing.forEach((path) => {
		...
		if (hasSpreadAttributes(attrs)) {
			addEslintDisableComment(
				j,
				path,
				'Manually migrate spacing prop to Box primitive (spread props detected)',
			);
			return;
		}
		...
		if (isSpacingDynamic(attrs)) {
			addEslintDisableComment(
				j,
				path,
				'Manually migrate spacing prop to Box primitive (dynamic spacing value detected)',
			);
			return;
		}
		...
		if (!paddingToken) {
			addEslintDisableComment(
				j,
				path,
				`Manually migrate spacing="${spacingValue}" prop to Box primitive (unknown spacing value)`,
			);
			return;
		}
