...
export default class IosBridge implements NativeBridge {
	...
	showMentions(query: string) {
		if (this.window.webkit && this.window.webkit.messageHandlers.mentionBridge) {
	...
	updateTextFormat(markStates: string) {
		if (this.window.webkit && this.window.webkit.messageHandlers.textFormatBridge) {
	...
	submitPromise(name: string, uuid: string, args: string) {
		if (this.window.webkit && this.window.webkit.messageHandlers.promiseBridge) {
	...
	updateBlockState(currentBlockType: string) {
		if (this.window.webkit && this.window.webkit.messageHandlers.blockFormatBridge) {
	...
	stateChanged(canUndo: boolean, canRedo: boolean) {
		if (this.window.webkit && this.window.webkit.messageHandlers.undoRedoBridge) {
	...
	trackEvent(event: string) {
		if (this.window.webkit && this.window.webkit.messageHandlers.analyticsBridge) {
	...
	editorDestroyed(): void {
		if (this.window.webkit && this.window.webkit.messageHandlers.lifecycleBridge) {
	...
	editorError(error: string, errorInfo?: string): void {
		const editorError: EditorLifecycleAnalyticsEvents = {
			...
			attributes: {
				isBridgeSetup: !!(this.window.webkit && this.window.webkit.messageHandlers.lifecycleBridge),
				errorMessage: error,
			},
		};
	...
	onContentRendered(
		...
		if (this.window.webkit && this.window.webkit.messageHandlers.contentBridge) {
	...
	typeAheadItemSelected(quickInsertItem: Serialized<QuickInsertItem>): void {
		if (this.window.webkit && this.window.webkit.messageHandlers.typeAheadBridge) {
