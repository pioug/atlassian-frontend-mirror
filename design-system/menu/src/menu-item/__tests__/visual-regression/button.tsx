...
import { focus, hover, mouseDown, verifyElementIn } from '../../../__tests__/_helper';
...
const buttonLinkAtScaleBeforeAfter = "[data-testid='item-button-at-scale-before-after']";
...
describe('<ButtonItem />', () => {
	...
	it('should match disabled item in focused state', async () => {
		await verifyElementMatchProductionImage(buttonLinkDisabled, focus(buttonLinkDisabled));
	});
	...
	it('should match item with lots of text - wrap title and description', async () => {
		await verifyElementMatchProductionImage(buttonLinkAtScaleMultipleLineTitleAndDescription);
	});
