...
import { IS_SIDEBAR_COLLAPSING, PAGE_LAYOUT_LS_KEY } from '../../common/constants';
...
export const ResizeControlledConsumer = () => {
	...
	return (
		<>
			<button data-testid="collapse" onClick={collapseLeftSidebar} type="button" />
			<button data-testid="expand" onClick={expandLeftSidebar} type="button" />
			<p data-testid="isLeftSidebarCollapsed">{String(isLeftSidebarCollapsed)}</p>
			<p data-testid="leftSidebar">{String(leftSidebarState.leftSidebarWidth)}</p>
			<p data-testid="lastLeftSidebarWidth">{String(leftSidebarState.lastLeftSidebarWidth)}</p>
...
describe('SidebarResizeController', () => {
	...
	it('should not go into inconsistent state when expanded and collapsed quickly', () => {
		...
		expect(document.documentElement.getAttribute(IS_SIDEBAR_COLLAPSING)).toEqual(null);
	...
	it('should call `onLeftSidebarExpand` when expanding', () => {
		...
		const { getByTestId } = render(
			<PageLayout testId="grid" onLeftSidebarCollapse={collapseFn} onLeftSidebarExpand={expandFn}>
				<Content>
	...
	it('should add the correct data attributes while expanding and collapsing', () => {
		...
		expect(document.documentElement.getAttribute(IS_SIDEBAR_COLLAPSING)).toEqual('true');
	...
	it('should reset flyout when expandLeftSidebar is called', () => {
		...
		expect(getByTestId('component').firstElementChild!.nextSibling).toHaveStyleDeclaration(
			'width',
