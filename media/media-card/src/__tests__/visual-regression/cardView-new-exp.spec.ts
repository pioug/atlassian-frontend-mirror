...
function getURL(params: string): string {
	return getExampleUrl('media', 'media-card', 'Test-VR-card-view', global.__BASEURL__) + params;
}
...
describe('Media Card New Experience', () => {
	describe('has data URI', () => {
		describe('and metadata', () => {
			it.each(['uploading', 'processing', 'complete', 'failed-processing', 'error'])(
				'with filestate %s',
				async (status) => {
		...
		describe('and without metadata', () => {
			it.each(['uploading', 'processing', 'complete', 'failed-processing', 'error'])(
				'with filestate %s',
				async (status) => {
					const url = getURL(`&dataUri=true&disableMetadata=true&status=${status}`);
					const { image } = await setup(url);
					expect(image).toMatchProdImageSnapshot();
				},
			);
			...
			it.each(['uploading', 'processing', 'complete', 'failed-processing', 'error'])(
				'and is selected with filestate %s',
				async (status) => {
					const url = getURL(`&dataUri=true&disableMetadata=true&status=${status}&selected=true`);
	...
	describe('has broken data URI', () => {
		...
		describe('and without metadata', () => {
			it.each(['uploading', 'processing', 'complete', 'failed-processing', 'error'])(
				'with filestate %s',
				async (status) => {
					const url = getURL(`&brokenDataUri=true&disableMetadata=true&status=${status}`);
					const { image } = await setup(url);
					expect(image).toMatchProdImageSnapshot();
				},
			);
	...
	describe('has a transparent image preview', () => {
		...
		describe('and without metadata', () => {
			it.each(['uploading', 'processing', 'complete', 'failed-processing', 'error'])(
				'with filestate %s',
				async (status) => {
					const url = getURL(`&isTransparent=true&disableMetadata=true&status=${status}`);
					const { image } = await setup(url);
					expect(image).toMatchProdImageSnapshot();
				},
			);
		...
		describe('and disableOverlay', () => {
			it.each(['uploading', 'processing', 'complete', 'failed-processing', 'error'])(
				'with filestate %s',
				async (status) => {
					const url = getURL(`&disableOverlay=true&isTransparent=true&status=${status}`);
					const { image } = await setup(url);
					expect(image).toMatchProdImageSnapshot();
				},
			);
	...
	describe('does not have the data URI', () => {
		...
		describe('and has no metadata', () => {
			...
			it.each(['uploading', 'processing', 'complete', 'failed-processing', 'error'])(
				'with disableOverlay and filestate %s',
				async (status) => {
					const url = getURL(`&disableOverlay=true&disableMetadata=true&status=${status}`);
			...
			it.each(['uploading', 'processing', 'complete', 'failed-processing', 'error'])(
				'and is selected with filestate %s',
				async (status) => {
					const url = getURL(`&disableMetadata=true&status=${status}&selected=true`);
	...
	describe('is rate limited', () => {
		describe('has metadata', () => {
			...
			it('renders error state when disableOverlay', async () => {
				const url = getURL(`&isRateLimited=true&disableOverlay=true&status=error`);
				const { image } = await setup(url);
				expect(image).toMatchProdImageSnapshot();
			});
		});
	...
	describe('is polling error', () => {
		...
		it('has metadata and dataUri', async () => {
			const url = getURL(`&isPollingMaxAttemptsExceeded=true&status=error&dataUri=true`);
			const { image } = await setup(url);
			expect(image).toMatchProdImageSnapshot();
		});
	});
	...
	describe('is upload error', () => {
		...
		it('has metadata with disableOverlay', async () => {
			const url = getURL(`&isUploadError=true&status=error&disableOverlay=true`);
			const { image } = await setup(url);
			expect(image).toMatchProdImageSnapshot();
		});

		it('no metadata', async () => {
			const url = getURL(`&isUploadError=true&status=error&disableMetadata=true`);
			const { image } = await setup(url);
			expect(image).toMatchProdImageSnapshot();
		});
