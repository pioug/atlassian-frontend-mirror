...
export const renameScrollBehaviorToShouldScrollInViewport = (
	...
	const convertToBooleanProp = (attribute: ASTPath<JSXAttribute>, value: StringLiteral) => {
		...
		getJSXAttributesByName(j, element, fromProp).forEach((attribute: ASTPath<JSXAttribute>) => {
			...
			if (value.type === 'StringLiteral') {
				...
				if (expContainer.expression.type === 'StringLiteral') {
					convertToBooleanProp(attribute, value.expression as StringLiteral);
					...
					j(attribute).replaceWith(
						j.jsxAttribute(
							...
							j.jsxExpressionContainer(
								j.binaryExpression('===', expContainer.expression, j.stringLiteral('outside')),
							),
						),
					);
				}
