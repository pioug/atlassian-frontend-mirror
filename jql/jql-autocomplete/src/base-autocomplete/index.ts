...
export class BaseAutocomplete<RuleContext extends {} = {}> {
  ...
  private getMatchedText(
    ...
    const startTokenIndex = this.isCaretAtDelimiterOrHiddenToken(
      maybeCaretToken,
    )
  ...
  private getCandidateTokens(
    ...
    const matchedText = this.getMatchedText(
      maybeCaretToken,
      caretSelectionRange,
    );
  ...
  private getCaretToken(caretStartPosition: number): Token | void {
    ...
    for (const token of tokens) {
      if (
        (caretStartPosition > token.startIndex &&
          caretStartPosition <= token.stopIndex + 1) ||
  ...
  protected isCaretAtDelimiterOrHiddenToken(caretToken: Token) {
    return (
      this.isCaretAtDelimiterToken(caretToken) ||
      caretToken.channel === Token.HIDDEN_CHANNEL
    );
  }
  ...
  private getReplacePosition(
    ...
    const stopPosition =
      maybeCaretToken === undefined ||
      this.isCaretAtDelimiterOrHiddenToken(maybeCaretToken)
        ? stop
  ...
  getSuggestionsForTokenIndex(tokenIndex: number): Suggestions<RuleContext> {
    ...
    const caretSelectionRange: Position = [
      caretToken.stopIndex + 1,
      caretToken.stopIndex + 1,
    ];
