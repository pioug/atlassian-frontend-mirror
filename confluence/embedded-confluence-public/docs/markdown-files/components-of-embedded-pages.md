...
| `themeMode`                       | string                                                                                                                    | **(Optional)** an optional prop that can be provided to the Embedded components to ensure that they apply the same theme/color preference as their parent product. Currently, Atlassian supports "light", "dark" and "auto" (browser will use OS setting to determine which is shown). For the latest list of supported themes please visit the design systems documentation [here](https://atlassian.design/components/tokens/code#setglobalthemethemestate) and find the `colorMode` values. If no `themeMode` is provided, the Embedded component will display with it's original styles. |
