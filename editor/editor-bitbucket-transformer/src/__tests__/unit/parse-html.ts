...
describe('BitbucketTransformer: parser', () => {
	describe('block elements', () => {
		...
		it('should remove all zero-with-non-joiners', () => {
			expect(parse('<p>foo</p><p>&zwnj;</p><p>&zwnj;</p><p>bar</p>')).toEqualDocument(
				doc(p('foo'), p(''), p(''), p('bar')),
			);
		});
	...
	describe('images', () => {
      ...
      `);
		...
		describe('with parse option shouldParseImageResizingAttributes true', () => {
			const additionalOptions = { shouldParseImageResizingAttributes: true };

			it('should support images with resizing attributes', () => {
				const parsed = parse(
					'<p><img alt="Alt text" data-width="50.5" data-layout="align-start" src="http://path/to/image.jpg"></p>',
					additionalOptions,
				);

				expect(parsed).toEqualDocument(
					doc(
						mediaSingle({ width: 50.5, layout: 'align-start' })(
							media({
								url: 'http://path/to/image.jpg',
								type: 'external',
								alt: 'Alt text',
			...
			it('should parse images inside links generated by bitbucket with resizing attributes', () => {
				const parsed = parse(
					'<p><a href="https://example.com"><img data-width="90.66666" data-layout="align-end" data-width-type="pixel" alt="Alt text" src="http://path/to/image.jpg"></a></p>',
					additionalOptions,
				);
			...
			it('should support images in lists with resizing attributes', () => {
				const parsed = parse(
					`
        <ul>
          <li>
            Hello
            <img data-width="90.66666" data-layout="wrap-left" data-width-type="null" src="http://path/to/image.jpg" alt="Alt text" />
            World
          </li>
        </ul>
      `,
			...
			it('shoud split paragraphs with images with resizing attributes', () => {
				const parsed = parse(
					`
        <p>Hello <img data-width="50.5" data-layout="center" src="http://path/to/image.jpg"> World</p>
      `,
					additionalOptions,
				);
			...
			it('should split blockquotes with images with resizing attributes', () => {
				const parsed = parse(
					`
        <blockquote>
          <p>Hello</p>
          <p><img data-width="50.5" data-layout="center" src="http://path/to/image.jpg"></p>
          <p>World</p>
        </blockquote>
      `,
					additionalOptions,
      ...
      `);
			...
			// added below tests to check BE compatibility with shouldParseImageResizingAttributes option
			it('should support images', () => {
				const parsed = parse(
      ...
      `);
			...
			it('should not add resizing attributes if value is undefined', () => {
				const parsed = parse(
					'<p><img alt="Alt text" data-width="undefined" data-layout="undefined" src="http://path/to/image.jpg"></p>',
					additionalOptions,
				);
	...
	describe('inline elements', () => {
		it('should support emphasis', () => {
			...
			expect(textWithMarks(parse('<p><em>text</em></p>'), 'text', [em])).toBe(true);
		});

		it('should support strong', () => {
			const strong = schema.marks.strong.create();
			expect(textWithMarks(parse('<p><strong>text</strong></p>'), 'text', [strong])).toBe(true);
		});

		it('should support strikethrough', () => {
			const strike = schema.marks.strike.create();
			expect(textWithMarks(parse('<p><strike>text</strike></p>'), 'text', [strike])).toBe(true);
		});

		it('should support code', () => {
			const code = schema.marks.code.create();
			expect(
				textWithMarks(parse('<p><span style="font-family: monospace;">text</span></p>'), 'text', [
					code,
				]),
			).toBe(true);
		});
		...
		it('should support both strong and em', () => {
			...
			const strong = schema.marks.strong.create();
			expect(
				textWithMarks(parse('<p><strong><em>text</em></strong></p>'), 'text', [em, strong]),
			).toBe(true);
		});
	});
	...
	describe('code', () => {
		it('inline should be parsed', () => {
			expect(parse('foo <span style="font-family: monospace;">bar </span>baz')).toEqualDocument(
	...
	describe('links', () => {
		...
		it('created automatically by bitbucket should be removed', () => {
			expect(
				parse('<p>' + 'foo ' + '<a href="http://www.atlassian.com">#1234</a>' + ' baz' + '</p>'),
