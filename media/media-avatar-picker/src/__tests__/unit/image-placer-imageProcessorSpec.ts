...
describe('Image Placer Image Processing', () => {
	...
	describe('initialiseImagePreview()', () => {
		...
		it('should return null if image fails to load', async () => {
			mockLoadImageError();
			const result = await initialiseImagePreview(mockFileInfo, new Rectangle(0, 0), 1);
			expect(result).toBeNull();
		});

		it('should scale down image to largest size required by max zoom', async () => {
			const result = await initialiseImagePreview(mockFileInfo, new Rectangle(10, 10), 2);
		...
		test.each([1, 2, 3, 4])('should not flip orientation %i', async (orientation: number) => {
		...
		test.each([5, 6, 7, 8])('should flip orientation %i', async (orientation: number) => {
			mockLoadImage(1, 2, orientation);
			const { width, height } = (await initialiseImagePreview(
				mockFileInfo,
				new Rectangle(10, 10),
	...
	describe('renderImageAtCurrentView()', () => {
		...
		it('should apply circular clip if option passed', () => {
			renderImageAtCurrentView(mocks.mockImage, mockViewInfo, true, true, 'some-bg-color');
			expect(mocks.clip).toHaveBeenCalled();
		});

		it('should draw image correctly when constraints used', () => {
			renderImageAtCurrentView(mocks.mockImage, mockViewInfo, true, false, 'some-bg-color');
			expect(mocks.drawImage).toHaveBeenCalledWith(
				mocks.mockImage,
				sourceBounds.left,
				sourceBounds.top,
		...
		it('should draw image correctly when constraints not used', () => {
			renderImageAtCurrentView(mocks.mockImage, mockViewInfo, false, false, 'some-bg-color');
			const { left, top, width, height } = imageBounds.relativeTo(visibleBounds);
			expect(mocks.drawImage).toHaveBeenCalledWith(
				mocks.mockImage,
				0,
				0,
