...
function getDefaultSpinnerSpecifier(j: core.JSCodeshift, source: string): Nullable<string> {
...
function hasImportDeclaration(j: core.JSCodeshift, source: string, importPath: string): boolean {
	return (
		j(source)
			.find(j.ImportDeclaration)
	...
	return j(source)
		...
		.forEach((element) => {
			...
			getJSXAttributesByName({ j, element, attributeName: 'delay' })
				.filter((attribute) => {
					const toRemove = j(attribute)
						...
						.find(j.Literal)
						.filter(
							(literal) => typeof literal.value.value === 'number' && literal.value.value <= 150,
						);
			...
			getJSXAttributesByName({
				...
				j(attribute).find(j.JSXIdentifier).replaceWith(j.jsxIdentifier('appearance'));

				// For usages where the `invertColor` had no value (ie a true value):
				// we need to set it to 'invert'`
				j(attribute)
					.filter((attr) => attr.node.value == null)
					.replaceWith(j.jsxAttribute(j.jsxIdentifier('appearance'), j.stringLiteral('invert')));
