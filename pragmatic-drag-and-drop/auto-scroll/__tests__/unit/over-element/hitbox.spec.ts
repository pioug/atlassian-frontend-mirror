...
const smallGroup: Group = (() => {
  ...
  const scenarios: Scenario[] = [
    {
      ...
      hitbox: getRect({
        ...
        bottom:
          rect.top +
          rect.height *
            defaultConfig.startHitboxAtPercentageRemainingOfElement['top'],
        right: rect.right,
      }),
    },
    {
      ...
      hitbox: getRect({
        top:
          rect.bottom -
          rect.height *
            defaultConfig.startHitboxAtPercentageRemainingOfElement['bottom'],
        left: rect.left,
        bottom: rect.bottom,
        right: rect.right,
      }),
  ...
  scenarios.forEach(scenario => {
    ...
    const size =
      ...
      defaultConfig.startHitboxAtPercentageRemainingOfElement[
        mainAxis[scenario.side]
      ];
    invariant(
...
[smallGroup, largeGroup].forEach(
  ({ label, parentScrollContainer, child, scenarios }) => {
    describe(`Group: ${label}`, () => {
      ...
      scenarios.forEach(scenario => {
        ...
        const scrollProperty =
          scenario.axis === 'vertical' ? 'scrollTop' : 'scrollLeft';
          ...
          getInsidePoints(scenario.hitbox).forEach(point => {
            test(`point: [${point.label}] {x: ${point.x}, y: ${point.y}}`, () => {
              ...
              const initialScrollValue =
                child.getBoundingClientRect()[mainAxis.size] / 2;
              ...
              expect(parentScrollContainer[scrollProperty]).toBe(
                initialScrollValue,
              );
