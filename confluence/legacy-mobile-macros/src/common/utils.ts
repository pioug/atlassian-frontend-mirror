			...
			({ src, href, ...attributes }) =>
				new Promise((resolve, reject) => {
					if (
						...
						const tagElement = window.document.createElement(attributes[TAG_TYPE_KEY]);
						...
						if (tagElement instanceof HTMLLinkElement) {
							if (href) {
								tagElement.href = href.startsWith('//') ? `https:${href}` : href;
							}
						}
...
function parseTags(tagsString: string): { [key: string]: string }[] {
	...
	for (const [, tagType, attributes, body] of tagsString.matchAll(SCRIPT_REGEX)) {
...
const makeMacroViewedAnalyticsParams = (extensionKey: String, renderingStrategy: String) => {
	return {
		eventType: 'track',
		action: 'viewed',
		actionSubject: 'macro',
...
export const useMacroViewedAnalyticsEvent = () => {
	...
	return useCallbackOne(
		(extensionKey: string, renderingStrategy: string) => {
			const analyticsParams = makeMacroViewedAnalyticsParams(extensionKey, renderingStrategy);
