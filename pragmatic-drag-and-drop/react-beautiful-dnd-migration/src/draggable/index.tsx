...
export function Draggable({
  ...
  const { shouldRenderCloneWhileDragging, isDropDisabled } =
    useDroppableContext();
        ...
        listener(event) {
          if (event.key === ' ') {
            ...
            if (
              ...
              isEventInInteractiveElement(element, event)
            ) {
              return;
            }
    ...
    return combine(
      ...
      monitorForElements({
        canMonitor({ source }) {
          ...
          return (
            source.data.contextId === data.contextId &&
            source.data.draggableId === data.draggableId
          );
        },
        onDrag({ location }) {
          dispatch({
