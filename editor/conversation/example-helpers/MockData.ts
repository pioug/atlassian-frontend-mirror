...
function generateComment(
	...
	return {
		...
		conversationId,
		parentId,
		createdBy: MOCK_USERS[Math.floor(Math.random() * 10) % (MOCK_USERS.length - 1)],
		createdAt: Date.now(),
		document: {
			adf: {
				version: 1,
						...
						content: [
							{
								...
								text:
									MESSAGES[
										...
										Math.floor(Math.random() * MESSAGES.length) & MESSAGES.length
									] + extraText,
							},
						],
					},
...
export const generateMockConversation = (): Conversation => {
	...
	const reply = generateComment(conversationId, comments[0].commentId, ' (reply)');
	const replyOfReply = generateComment(conversationId, reply.commentId, ' (reply of reply)');
