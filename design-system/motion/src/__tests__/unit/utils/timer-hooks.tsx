...
describe('timer hooks', () => {
	...
	describe('useRequestAnimationFrame()', () => {
		...
		it('should not callback when the effect is cleared', () => {
			const callback = jest.fn();
			const { rerender } = render(<Component cleanup="next-effect" callback={callback} />);
		...
		it('should not callback when the effect is unmounted', () => {
			const callback = jest.fn();
			const { unmount } = render(<Component cleanup="unmount" callback={callback} />);
		...
		it('should have a stable reference', () => {
			const callback = jest.fn();
			const markup = () => <Component receiveHookResult={callback} cleanup="unmount" />;
