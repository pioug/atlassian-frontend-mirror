...
describe('getToolbarItems', () => {
	...
	const panelPreset = new Preset<LightEditorPlugin>().add(decorationsPlugin).add([
		panelPlugin,
		{
	...
	describe('when locale is en', () => {
		...
		it('should return default message when locale is en', () => {
			...
			const removeEmojiButton: AnyFloatingToolbarButton | undefined = toolbarItems.find(
				(item) => item.type === 'button' && item.id === 'editor.panel.removeEmoji',
	...
	describe('custom panel toolbar items', () => {
		...
		it(`should call changePanelType when clicked on emoji picker
          with changed emoji`, () => {
			...
			(emojiPickerConfig as AnyFloatingToolbarEmojiPicker)!.onChange(emojiId)(editorView.state);
		...
		it.each(standardPanels)(
          ...
          with previous icon for %p panel`,
			(value) => {
				const { editorView } = editor(doc(panel({ panelType: value })(p('text'))));
	...
	describe('analytics for custom panels', () => {
		...
		beforeEach(() => {
			({ editorView, editorAPI } = createEditor({
				...
				preset: panelPreset.add([featureFlagsPlugin, {}]).add([analyticsPlugin, {}]),
				providerFactory,
			}));
		...
		it('Should trigger analytics when background color is changed', () => {
			...
			expect(editorAPI?.analytics?.actions.attachAnalyticsEvent).toHaveBeenCalledWith(
				payload,
				undefined,
			);
		});
		...
		it('Should not fire analytics when the same background color is selected', () => {
			...
			expect(editorAPI?.analytics?.actions?.attachAnalyticsEvent).toHaveBeenCalledTimes(0);
			(colorPickerConfig as AnyFloatingToolbarColorPicker)!.onChange({
				label: 'Mintie',
				value: '#ABF5D1',
				border: DEFAULT_BORDER_COLOR,
			})(editorView.state, editorView.dispatch);
			expect(editorAPI?.analytics?.actions?.attachAnalyticsEvent).toHaveBeenCalledTimes(2);
			(colorPickerConfig as AnyFloatingToolbarColorPicker)!.onChange({
				label: 'Mintie',
				value: '#ABF5D1',
				border: DEFAULT_BORDER_COLOR,
			})(editorView.state, editorView.dispatch);
			expect(editorAPI?.analytics?.actions.attachAnalyticsEvent).toHaveBeenCalledTimes(3);
			expect(editorAPI?.analytics?.actions?.attachAnalyticsEvent).not.toHaveBeenNthCalledWith(3, {
