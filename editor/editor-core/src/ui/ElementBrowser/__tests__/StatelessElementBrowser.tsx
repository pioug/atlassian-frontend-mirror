...
jest.mock('../hooks/useContainerWidth', () => {
  const originalHook = jest.requireActual('../hooks/useContainerWidth').default;
  return (...args: any) => ({
    ...originalHook(...args),
...
describe('KeyboardNavigation for item listSize 10', () => {
  ...
  const testKeyCombination = (
    ...
    breakpointWidth: number = DEVICE_BREAKPOINT_NUMBERS.medium,
  ) => {
    ...
    if (keys.find(k => k === 'enter')) {
      it(`calls back onEnterKeyPress with selected item at index: ${expectedState.selectedItemIndex}`, () => {
        expect(testProps.onEnterKeyPress).toHaveBeenCalledWith(
  ...
  describe('Up/Down arrows with columns', () => {
    ...
    describe('Columns = 2 (ContainerWidth: 450px)', () => {
      arrowKeysWithcolumnSuite(2, 450);
      enterKeySuite(2, 450);
    });
    describe('Columns = 3 (ContainerWidth: 600px)', () => {
      arrowKeysWithcolumnSuite(3, 600);
      enterKeySuite(3, 600);
    });
