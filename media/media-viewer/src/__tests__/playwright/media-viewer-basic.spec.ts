...
test.describe('MediaViewer', () => {
  ...
  test('media-viewer-basic.ts: Should close on Close click', async ({
    page,
  }) => {
    const viewer = new MediaViewerPageObject(page);
    await viewer.init('vr-mocked-viewer');
  ...
  // eslint-disable-next-line playwright/expect-expect
  test('media-viewer-basic.ts: Should close on Escape press', async ({
    page,
  }) => {
    const viewer = new MediaViewerPageObject(page);
    await viewer.init('vr-mocked-viewer');
    ...
    expect(
      await page.webdriverCompatUtils.isDetached('media-viewer-popup'),
    ).toBe(true);
  });

  test('media-viewer-basic.ts: Should open and close sidebar', async ({
    page,
  }) => {
    const viewer = new MediaViewerPageObject(page);
    await viewer.init('vr-mocked-viewer');
