...
export const findChanged = (tr: Transaction | ReadonlyTransaction, state: EditorState) => {
	...
	for (let i = 0; i < tr.steps.length; i++) {
		...
		stepMap.forEach((oldStart, oldEnd, newStart, newEnd) => {
			...
			// Removed
			removedInRange.push(
				...findInNodeRange(before, oldStart, oldEnd, (node) => !!getNodeSubject(node)),
			);
			// Inserted
			insertedInRange.push(
				...findInNodeRange(after, newStart, newEnd, (node) => !!getNodeSubject(node)),
			);
...
const isTransactionQueuedForUpgrade = (tr: Transaction | ReadonlyTransaction) => {
...
const getResolvePositions = (tr: Transaction | ReadonlyTransaction, state: EditorState) => {
...
) => previousSubject === EVENT_SUBJECT.DATASOURCE && currentSubject === EVENT_SUBJECT.LINK;

const isDatasourceUpgrade = (
	previousSubject: EVENT_SUBJECT | null,
	currentSubject: EVENT_SUBJECT | null,
) => previousSubject === EVENT_SUBJECT.LINK && currentSubject === EVENT_SUBJECT.DATASOURCE;
...
export function eventsFromTransaction(
	...
	try {
		 ...
		 */
		const { action, inputMethod, sourceEvent } = getLinkMetadataFromTransaction(tr);
		...
		const pushEvents = (entities: Entity[], event: EVENT.CREATED | EVENT.DELETED) => {
