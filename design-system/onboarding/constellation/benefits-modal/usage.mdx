...
A benefits modal focuses a person's attention on a large or impactful update (for information on how
to size first impressions, see
[user journey sizes](/patterns/first-impressions/user-journey-sizes/)).

The modal can be followed by a [spotlight tour](/components/onboarding/usage) to highlight changes
...
- For more information about how to use buttons, see the
  [button usage guidelines](/components/button/usage).
