...
- When using a dropdown menu inside a modal dialog, use the `shouldRenderToParent` prop. This prevents loss of focus and ensures proper voicing by screen reading programs to render content in the nearest DOM node to the trigger element instead of `React.Portal`.
