...
interface EditorBrigdeModelInterface {
	...
	output: (props: { bridgeName: string; eventName: string }) => Promise<unknown>;
...
class EditorBrigdeModel implements EditorBrigdeModelInterface {
	...
	async output({ bridgeName, eventName }: { bridgeName: string; eventName: string }) {
	...
	async paste() {
		fixTest({
			...
			reason: 'Clipboard memory is shared on OS level so parallel tests can affect each other.',
		});
...
export const mobileBridgeEditorClipboardTestCase = mobileBridgeEditorTestCase.extend({
	bridge: async ({ page, params }, use) => {
		await page.visitExample('editor', 'editor-mobile-bridge', 'editor-with-copy-paste', params);
