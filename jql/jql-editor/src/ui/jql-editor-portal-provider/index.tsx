...
export const JQLEditorPortalRenderer = ({
  ...
  const actionsContext = useMemo<PortalActions>(() => {
    return {
      ...
      onDestroyPortal: (key: string) =>
        dispatch({ type: 'destroyPortal', payload: key }),
  ...
  return (
    <PortalActionsContext.Provider value={actionsContext}>
      ...
      {Object.entries(state.components).map(([key, value]) => {
        const container =
          value.container instanceof Element
            ? value.container
            : state.containers[value.container];
        return container !== undefined
          ? createPortal(value.portalComponent, container, key)
          : null;
      })}
