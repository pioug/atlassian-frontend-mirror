...
const IconExplorerCell: FC<IconExplorerCellProps> = ({
  ...
  if (oldName && typeof oldName === 'string') {
    ...
    metadata['Legacy Icon imports'] = oldName.reduce((acc, name) => {
      const legacyIconImport =
        Object.entries(legacyIconMetadata).find(
          ([_, value]) => value.componentName === name,
        )?.[1].package || '';
  ...
  const splitIconName =
    packageName.split('/').at(-1)?.split('-').join(' ') || '';
  const splitIconNameCapitalized =
    splitIconName.charAt(0).toUpperCase() + splitIconName?.slice(1);
