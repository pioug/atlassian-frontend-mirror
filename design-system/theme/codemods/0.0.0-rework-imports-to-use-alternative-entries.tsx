...
const constantsPredicate = (specifier: any) =>
	!specifier || !specifier.imported || constants.indexOf(specifier.imported.name) > -1;

const typesPredicate = (specifier: any) =>
	!specifier || !specifier.imported || typesImports.indexOf(specifier.imported.name) > -1;

function getConstantsImport(j: any, path: any) {
	const constantsSpecifierspath = path.value.specifiers.filter(constantsPredicate);

	if (constantsSpecifierspath.length === 0) {
		return null;
	}

	return j.importDeclaration(constantsSpecifierspath, j.literal(`${akTheme}/constants`));
}
const indexPredicate = (specifier: any) =>
	!specifier || !specifier.imported || themeIndexImports.indexOf(specifier.imported.name) > -1;
...
function getIndexImport(j: any, path: any) {
	...
	return j.importDeclaration(mainIndexSpecifierspath, j.literal(`${akTheme}/components`));
}
...
function getTypesImport(j: any, path: any) {
	...
	return j.importDeclaration(typesSpecifiersPath, j.literal(`${akTheme}/types`));
}
...
function getOtherImports(j: any, path: any, fileSource: any) {
	return path.value.specifiers
		.filter(
			(specifier: any) =>
				!indexPredicate(specifier) && !constantsPredicate(specifier) && !typesPredicate(specifier),
		)
		.map((specifier: any) => {
