...
jest.mock('@atlaskit/editor-common/quick-insert', () => ({
	...
	memoProcessQuickInsertItems: jest.fn().mockImplementation(() => mockSomeDefaultItems),
}));
...
describe('Quick-insert plugin state listener', () => {
	...
	it('should call set the quick-insert suggestions on the bridge when intl changes', async () => {
		...
		// Initial render
		const intlMock = {
			formatMessage: (messageDescriptor: any) => messageDescriptor?.defaultMessage,
		...
		const { rerender } = renderHook<
			...
			void
		>(({ intl }) => useQuickInsertListener(quickInsertSharedStateMock, bridge, intl), {
			initialProps: {
				intl: intlMock,
			},
		});
		...
		(memoProcessQuickInsertItems as jest.Mock).mockImplementation(() => mockSomeQuickInsertItems);
