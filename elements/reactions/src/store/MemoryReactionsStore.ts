...
export class MemoryReactionsStore implements Store {
	 ...
	 */
	private triggerOnChange = batch(() => this.callbacks.forEach((callback) => callback(this.state)));

	private setReactions = (containerAri: string, ari: string, reactions: ReactionsState) => {
		this.setState({
			reactions: {
	...
	private handleDetailedReactionResponse = (detailedReaction: ReactionSummary) => {
	...
	private setFlash(containerAri: string, ari: string, emojiId: string, flash: boolean) {
		this.setState({
			flash: {
				...this.state.flash,
				[`${containerAri}|${ari}`]: {
	...
	private flash = (reaction: ReactionSummary): void => {
		...
		window.setTimeout(
			() => this.setFlash(reaction.containerAri, reaction.ari, reaction.emojiId, false),
			700,
		);
	};
	...
	private withReaction(
		...
		return (containerAri: string, ari: string, emojiId: string) => {
			this.withReadyReaction(
				...
				const reaction: ReactionSummary = reactionsState.reactions.find(byEmojiId(emojiId)) || {
					containerAri,
					ari,
					emojiId,
					count: 0,
