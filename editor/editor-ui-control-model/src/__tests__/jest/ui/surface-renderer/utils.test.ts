...
import { createSurfaceContext } from '../../../../createSurfaceContext';
import type { ContextToken, RegisterComponent } from '../../../../types';
import type { ChildrenMap } from '../../../../ui/surface-renderer/types';
import {
	buildChildrenMap,
	findSurface,
	willComponentRender,
...
describe('buildChildrenMap', () => {
	it('should preserve key-only identity when both experiments are disabled', () => {
		resetAllExperiments();
		mockExpDisabled('platform_editor_slash_command');
		...
		expect(map.get('root')).toEqual(components);
		expect(map.has('toolbar:root')).toBe(false);
	});

	it('should use full parent identity when the slash command experiment is enabled', () => {
		resetAllExperiments();
		...
		expect(map.get('toolbar:root')).toEqual(components);
		expect(map.has('root')).toBe(false);
	});

	it('should group children by full parent identity', () => {
		const components: RegisterComponent[] = [
		...
		expect(map.get('toolbar:root')).toHaveLength(2);
		expect(map.get('toolbar:root')![0].key).toBe('section-1');
		expect(map.get('toolbar:root')![1].key).toBe('section-2');
	});
	...
	it('should sort children by rank within each parent', () => {
		...
		const map = buildChildrenMap(components);
		const children = map.get('toolbar:root')!;
	...
	it('should handle components with multiple parents', () => {
		...
		expect(map.get('group:group-a')).toHaveLength(1);
		expect(map.get('group:group-a')![0].key).toBe('shared');
		expect(map.get('group:group-b')).toHaveLength(1);
		expect(map.get('group:group-b')![0].key).toBe('shared');
	});
	...
	it('should not include root components (no parents) in the map', () => {
		...
		expect(map.has('toolbar:root')).toBe(true);
...
describe('willComponentRender', () => {
	...
	it('should return true for a container with at least one visible child', () => {
		...
		const childrenMap: ChildrenMap = new Map([['menu-section:section', [visibleItem, hiddenItem]]]);
	...
	it('should return false for a container where all children are hidden', () => {
		...
		const childrenMap: ChildrenMap = new Map([['menu-section:section', [hidden1, hidden2]]]);
	...
	it('should recurse through nested containers', () => {
		...
		const childrenMap: ChildrenMap = new Map([
			['menu-section:top', [nestedMenu]],
			['nested-menu:nested', [innerSection]],
			['menu-section:inner', [visibleItem]],
		]);
	...
	it('should return false when deeply nested leaf is hidden', () => {
		...
		const childrenMap: ChildrenMap = new Map([
			['menu-section:top', [nestedMenu]],
			['nested-menu:nested', [innerSection]],
			['menu-section:inner', [hiddenItem]],
		]);

		expect(willComponentRender(topSection, childrenMap)).toBe(false);
	});

	it('passes immutable surface context through recursive visibility checks', () => {
		const token = Symbol('visible') as ContextToken<boolean>;
		const surfaceContext = createSurfaceContext(token, true);
		const childIsHidden = jest.fn(
			({ surfaceContext: receivedContext } = {}) => receivedContext?.get(token) !== true,
		);
		const section: RegisterComponent = {
			key: 'section',
			type: 'section',
		...
		const item: RegisterComponent = {
			...
			parents: [{ key: 'section', type: 'section', rank: 1 }],
		};
		const childrenMap = buildChildrenMap([section, item]);

		expect(willComponentRender(section, childrenMap, surfaceContext)).toBe(true);
		expect(childIsHidden).toHaveBeenCalledWith({ surfaceContext });
		expect(surfaceContext.get(token)).toBe(true);
	});

	it('supports legacy zero-argument visibility callbacks', () => {
		const isHidden = jest.fn(() => false);
		const component: RegisterComponent = {
			key: 'item',
			type: 'button',
			isHidden,
			parents: [{ key: 'group', type: 'group', rank: 1 }],
		};

		expect(willComponentRender(component, new Map())).toBe(true);
	...
	it('should return false when container itself has isHidden true even if children are visible', () => {
		...
		const childrenMap: ChildrenMap = new Map([['menu-section:section', [visibleItem]]]);
...
describe('SurfaceContext', () => {
	it('returns undefined for any token other than its immutable value token', () => {
		const storedToken = Symbol('stored') as ContextToken<string>;
		const missingToken = Symbol('missing') as ContextToken<string>;
		const context = createSurfaceContext(storedToken, 'stored value');

		expect(context.get(storedToken)).toBe('stored value');
		expect(context.get(missingToken)).toBeUndefined();
	});
});
