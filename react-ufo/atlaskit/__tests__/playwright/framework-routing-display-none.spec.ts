 ...
 *
 * When the feature flag `platform_ufo_vc_ignore_display_none_mutations` is enabled,
...
test.describe('ReactUFO: Framework routing display:none mutations', () => {
	...
	test.describe('when platform_ufo_vc_ignore_display_none_mutations feature flag is enabled', () => {
		test.use({
			featureFlags: ['platform_ufo_vc_ignore_display_none_mutations'],
		});
	...
	test.describe('when platform_ufo_vc_ignore_display_none_mutations feature flag is disabled', () => {
		// No feature flags - default behavior

		for (const viewport of viewports.slice(0, 1)) {
			// Only test one viewport for the disabled case
			test.describe(`when view port is ${viewport.width}x${viewport.height}`, () => {
				test.use({
					viewport,
				});

				test('should still produce a valid UFO payload', async ({
					waitForReactUFOPayload,
				}) => {
					const reactUFOPayload = await waitForReactUFOPayload();
					expect(reactUFOPayload).toBeDefined();
