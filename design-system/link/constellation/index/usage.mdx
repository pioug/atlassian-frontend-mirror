...
Link has an underline by default but it can be disabled in some circumstances. Underlines help differentiate links from regular text and addresses a WCAG-A recommendation. Using color alone isn't accessible, especially if links are surrounded by other text.

Only remove underlines if the context surrounding the link makes it clear that it is interactive, such as in navigation.
...
- For navigational items, use [Atlassian navigation buttons](/components/atlassian-navigation/examples#button) and [menu link items](/components/menu/link-item/examples).
