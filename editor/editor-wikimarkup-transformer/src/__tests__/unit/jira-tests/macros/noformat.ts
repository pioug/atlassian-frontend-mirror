import { code, code_block, doc, em, p, strong } from '@atlaskit/editor-test-helpers/doc-builder';
...
describe.skip('WikiMarkup Transformer', () => {
	describe('noformat with marks', () => {
		...
		checkParseEncodeRoundTrips(
			...
			WIKI_NOTATION,
			doc(
				p(code(' some *preformatted* texts here. so this is not a template @variable@ anymore! ')),
			),
		);
	});
...
this is not a _emphesis_ text as well as a +inserted+ one{noformat} and here is the END of preformmatted block`;
		...
		checkParseEncodeRoundTrips(
			...
			WIKI_NOTATION,
			doc(
				p('this is a ', strong('bold'), ' text outside of ', em('preformatted'), ' block '),
				code_block({})(` some *preformatted* multi line texts
here:
...
c:\\\\work\\\\_tools\\\\flash7\\\\classes`),
	...
	describe('noformat filepath inline', () => {
		...
		checkParseEncodeRoundTrips(
			...
			defaultSchema,
			WIKI_NOTATION,
			doc(p('blah ', code(`c:\\\\work\\\\_common\\\\code\\\\flash *cat*`), ' blah blah')),
		);
	});
});
