...
describe('withMediaClient', () => {
	const mockFg = jest.mocked(fg);

	afterEach(() => {
		jest.clearAllMocks();
	});

	describe.each([true, false])('when media-perf-uplift-mutation-fix is %s', (featureGateValue) => {
		beforeEach(() => {
			mockFg.mockImplementation((flagName: string) => {
				if (flagName === 'media-perf-uplift-mutation-fix') {
					return featureGateValue;
				}
				return false;
			});
		});
