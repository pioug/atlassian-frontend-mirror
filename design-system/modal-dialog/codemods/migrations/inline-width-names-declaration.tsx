...
import { SHARED_VARIABLES_ENDPOINT, WIDTH_NAMES_TYPE_NAME } from '../internal/constants';

export const inlineWidthNamesDeclaration = (j: core.JSCodeshift, source: Collection<Node>) => {
	...
	source.find(j.TSInterfaceDeclaration).forEach((declaration) => {
		replaceTypeAnnotationForWidthNames(j, namedSpecifier, declaration.value.body.body);
	});
	...
	typeAnnotations
		...
		.forEach((property: TSPropertySignature) => {
			property.typeAnnotation = j.tsTypeAnnotation(
				j.tsUnionType(widthNames.map((name) => j.tsLiteralType(j.stringLiteral(name)))),
			);
		});
};
