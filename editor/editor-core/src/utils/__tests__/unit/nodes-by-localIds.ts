...
// eslint-disable-next-line import/no-extraneous-dependencies -- Removed import for fixing circular dependencies
import { doc, fragmentMark, p, table, td, tr } from '@atlaskit/editor-test-helpers/doc-builder';
...
describe('findNodePosByFragmentLocalIds', () => {
	...
	it('should return NodePos when localId is found', () => {
		const initDoc = doc(
			fragmentMark({ localId: 'fragment-local-id-1' })(
				table({ localId: 'table-id-1' })(tr(td({})(p()), td({})(p()), td({})(p()))),
			),
			fragmentMark({ localId: 'fragment-local-id-2' })(
				table({ localId: 'table-id-2' })(tr(td({})(p()), td({})(p()), td({})(p()))),
			),
			fragmentMark({ localId: 'fragment-local-id-3' })(
				table({ localId: 'table-id-3' })(tr(td({})(p()), td({})(p()), td({})(p()))),
			),
		);
		const { state } = createEditor(initDoc);
		const result = findNodePosByFragmentLocalIds(state, ['fragment-local-id-2']);
