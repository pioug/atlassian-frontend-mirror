...
describe('node utils', () => {
	describe('isNodeSelected', () => {
		it('should return null if not selected', () => {
			...
			expect(isNodeSelectedOrInRange(range.anchor, range.head, node.pos, node.size)).toBeNull();
		});
		it('should return selectedInRange if node is inside range', () => {
			const range = { anchor: 6, head: 10 };
			const node = { pos: 7, size: 2 };
			expect(isNodeSelectedOrInRange(range.anchor, range.head, node.pos, node.size)).toBe(
				SelectedState.selectedInRange,
			);
		});
