...
export class PluginPerformanceReport {
	...
	private isChild = (child: PerformanceEntry) =>
		...
		child.startTime + child.duration <= this.entry.startTime + this.entry.duration;

	private getEntryByName(entryList: PerformanceObserverEntryList, name: string) {
	...
	private hasOutlierMethods(methods: PluginMethodReport, outliers: OutlierReport): boolean {
		return (
	...
	public withPlugins(pluginNames: string[]): this {
		...
		this.plugins = pluginNames.reduce<PluginsReport>((acc, name) => {
			...
			const pluginViewUpdated = this.entryList
				? this.getEntryByName(this.entryList, `🦉${name}::view::update`) || emptyEntry
				: emptyEntry;

			const pluginOnEditorViewStateUpdated = this.entryList
				? this.getEntryByName(this.entryList, `🦉${name}::onEditorViewStateUpdated`) || emptyEntry
				: emptyEntry;
