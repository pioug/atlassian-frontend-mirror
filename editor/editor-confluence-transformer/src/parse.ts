			...
			case 'P':
				...
				if (textNodes.length) {
					return schema.nodes.paragraph.createChecked(
						{},
						ensureInline(schema, Fragment.fromArray(textNodes), convertedNodesReverted),
					);
				}
...
function convertTable(schema: Schema, node: HTMLTableElement) {
	...
	const tableBaseWidth = calcPixelsFromCSSValue(node.style.width || '100%', RELATIVE_TABLE_WIDTH);
	...
	for (let i = 0, rowsCount = rows.length; i < rowsCount; i++) {
		...
		for (let j = 0, colsCount = cols.length; j < colsCount; j++) {
			...
			if (background) {
				...
				background = tableBackgroundColorNames.get(background.toLowerCase()) || background;
			}
			...
			const attrs = {
				colspan,
				colwidth:
					colwidth && colwidth.length && colwidth.every((width) => width > 0) ? colwidth : null,
				background,
	...
	return table.createChecked(
		{
			...
			__autoSize: columnSizes.length === 0 || columnSizes.every((width) => width === 0),
		},
