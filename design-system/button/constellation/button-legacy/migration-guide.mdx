...
New button components with improved APIs are here. If you're using the old `@atlaskit/button`, you
can start migrating some default and loading button usages to the [new button](/components/button),
[icon button](/components/button/icon-button), and [link button](/components/button/link-button)
today.

## Use the codemod for automated migration assistance

Run the following command in your terminal, replacing the path with a path to the file you'd like to
migrate.
...
| `overlay` prop                                                  | The `overlay` prop has been deprecated in new buttons. It only existed in legacy buttons to support loading spinners, which can be achieved in the new button using the `isLoading` prop.  |
