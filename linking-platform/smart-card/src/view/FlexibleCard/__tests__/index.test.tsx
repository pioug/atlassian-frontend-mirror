...
describe('FlexibleCard', () => {
	...
	describe('placeholderData prop', () => {
		...
		ffTest.on('platform_initial_data_for_smart_cards', 'with feature flag enabled', () => {
		...
		ffTest.off('platform_initial_data_for_smart_cards', 'with feature flag disabled', () => {
			it('should not use placeholder data', () => {
				render(
					<FlexibleCard
						cardState={pendingCardState}
						url={url}
				...
				// Should render the pending state, not resolved state with placeholder data
				expect(screen.queryByTestId('smart-block-title-resolved-view')).not.toBeInTheDocument();
			});
		});
	});
