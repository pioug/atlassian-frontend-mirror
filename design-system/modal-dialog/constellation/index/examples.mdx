...
Modal dialog sets the [current surface CSS variable](/components/tokens/code#current-surface-color) to the Modal surface color.

Using the `utility.elevation.surface.current` design token allows elements to be styled with a color based on the current surface color.
