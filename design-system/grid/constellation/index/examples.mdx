...
We enable a shorthand here, using `span={6}` is the same as `span={{ xs: 6 }}`.  The shorthand will override the default full-width functionality across all breakpoints.
...
In the case that you do not want a GridItem rendered at all, use `span="none"` or `span={{ xs: 'none', md: 6 }}` (etc).
...
We enable a shorthand here, using `start={1}` is the same as `start={{ xs: 1 }}`.  The shorthand will override the default `'auto'` functionality across all breakpoints.
