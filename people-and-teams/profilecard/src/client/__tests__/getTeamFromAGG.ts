...
describe('getTeamFromAGG', () => {
	...
	describe('addHeaders', () => {
		it('should add headers for experiment', () => {
			...
			expect(result.get('X-ExperimentalApi')).toEqual('teams-beta, team-members-beta');
		});
	...
	describe('getTeamFromAGG', () => {
		...
		it('should return error when response is not ok', async () => {
			...
			await expect(getTeamFromAGG(serviceUrl, teamId)).rejects.toBeInstanceOf(HttpError);
		});
		...
		it('should make the correct query', async () => {
			...
			expect(gqlQuery).toHaveBeenCalledWith(
				...
				expect.objectContaining({
					query: expect.stringContaining('team: teamV2(id: $teamId, siteId: $siteId)'),
					variables: {
						siteId: 'site-id',
