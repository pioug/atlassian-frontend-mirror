...
const expandNode: ADFEntity = {
	type: 'expand',
	attrs: { title: 'expand-1' },
	content: [{ type: 'paragraph', content: [{ type: 'text', text: 'inside expand' }] }],
};
...
// Expand-in-panel fixtures (mirrors the table-in-panel fixtures above).
const panelC1WithExpand: ADFEntity = {
	type: 'panel_c1',
	attrs: { panelType: 'info' },
	content: [
		{ type: 'paragraph', content: [{ type: 'text', text: 'before' }] },
...
const docWithPanelC1WithExpand: ADFEntity = {
	type: 'doc',
	content: [panelC1WithExpand],
};

// A plain panel containing an expand wrapped in an LCM extension (the downgraded shape).
const docWithLcmWrappedExpandInPanel: ADFEntity = {
	type: 'doc',
	content: [
		{
			type: 'panel',
			...
			content: [
				{ type: 'paragraph', content: [{ type: 'text', text: 'before' }] },
				{
					...
					attrs: {
						...
						layout: 'default',
						parameters: {
							nestedContent: { version: 1, type: 'doc', content: [expandNode] },
							adf: JSON.stringify({ version: 1, type: 'doc', content: [expandNode] }),
							[PANEL_C1_FALLBACK_MARKER]: true,
						},
					},
				},
...
// A plain panel containing an expand wrapped in an unsupportedBlock (the safety-net shape).
const docWithUnsupportedBlockWrappedExpandInPanel: ADFEntity = {
	type: 'doc',
	content: [
		{
			type: 'panel',
			...
			content: [
				{ type: 'paragraph', content: [{ type: 'text', text: 'before' }] },
				{ type: 'unsupportedBlock', attrs: { originalValue: expandNode } },
				{ type: 'paragraph', content: [{ type: 'text', text: 'after' }] },
			],
		},
	],
...
const createSchemaWithNodes = (
	...
	if (includeExpand) {
		nodes.push('expand', 'nestedExpand');
	}
	if (includeExtension) {
...
describe('panelC1FallbackTransformV2', () => {
	describe('when no escape-hatch node type is available', () => {
		it('should return unchanged ADF with isTransformed false', () => {
			...
			const result = panelC1FallbackTransformV2(schema, docWithPanelC1WithTable);
		...
		it('should still downgrade a tableless panel_c1 to a plain panel', () => {
			...
			const result = panelC1FallbackTransformV2(schema, docWithPanelC1WithoutTable);
	...
	describe('when panel_c1 is supported by the schema', () => {
		...
		it('should restore an LCM-wrapped table back into the panel (round-trip)', () => {
			...
			const result = panelC1FallbackTransformV2(schema, docWithLcmWrappedTableInPanel);
		...
		it('should restore an unsupportedBlock-wrapped table back into the panel (round-trip)', () => {
			...
			const result = panelC1FallbackTransformV2(schema, docWithUnsupportedBlockWrappedTableInPanel);
		...
		it('should NOT restore an LCM-wrapped table whose parent is not a panel', () => {
			...
			const result = panelC1FallbackTransformV2(schema, docWithLcmAtRoot);
		...
		it('should NOT restore an unsupportedBlock-wrapped table whose parent is not a panel', () => {
			...
			const result = panelC1FallbackTransformV2(schema, docWithUnsupportedBlockAtRoot);
	...
	describe('idempotency and untouched documents', () => {
		it('should leave documents without panel_c1 unchanged', () => {
			...
			const result = panelC1FallbackTransformV2(schema, paragraphOnlyDoc);
		...
		it('should leave non-LCM extensions unchanged when panel_c1 is supported', () => {
			...
			const result = panelC1FallbackTransformV2(schema, docWithUnrelatedExtension);
		...
		it('should leave unrelated unsupportedBlock nodes unchanged when panel_c1 is supported', () => {
			...
			const result = panelC1FallbackTransformV2(schema, docWithNonTableUnsupportedBlock);
		...
		it('LCM downgrade output should round-trip back to the original when support returns', () => {
			...
			const off = panelC1FallbackTransformV2(offSchema, docWithPanelC1WithTable);
			...
			const back = panelC1FallbackTransformV2(onSchema, off.transformedAdf as ADFEntity);
...
describe('panelC1FallbackTransformV2 - expand in panel', () => {
	describe('when panel_c1 is unsupported by the schema', () => {
		it('should wrap the expand in an LCM extension when downgrading', () => {
			const schema = createSchemaWithNodes({
				includeExpand: true,
				includeExtension: true,
			});

			const result = panelC1FallbackTransformV2(schema, docWithPanelC1WithExpand);

			expect(result.isTransformed).toBe(true);
			const panel = (result.transformedAdf as ADFEntity).content?.[0];
			expect(panel?.type).toBe('panel');
			expect(isLcmExtension(panel?.content?.[1])).toBe(true);
			expect(getLcmWrappedNode(panel?.content?.[1])).toEqual(expandNode);
			expect(() => Node.fromJSON(schema, result.transformedAdf as ADFEntity)).not.toThrow();
		});
		...
		it('should fall back to unsupportedBlock when extension is unavailable', () => {
			...
			expect(panel?.type).toBe('panel');
			expect(panel?.content?.[1]?.type).toBe('unsupportedBlock');
			expect(panel?.content?.[1]?.attrs?.originalValue).toEqual(expandNode);
			expect(() => Node.fromJSON(schema, result.transformedAdf as ADFEntity)).not.toThrow();
		});
	});

	describe('when panel_c1 is supported by the schema', () => {
		it('should leave a panel_c1 containing an expand untouched', () => {
			const schema = createSchemaWithNodes({
				includePanelC1: true,
				includeExpand: true,
				includeExtension: true,
		...
		it('should restore an LCM-wrapped expand back into the panel (round-trip)', () => {
			const schema = createSchemaWithNodes({
				includePanelC1: true,
				includeExpand: true,
				includeExtension: true,
			...
			const result = panelC1FallbackTransformV2(schema, docWithLcmWrappedExpandInPanel);

			expect(result.isTransformed).toBe(true);
			const panel = (result.transformedAdf as ADFEntity).content?.[0];
			expect(panel?.content?.[1]?.type).toBe('expand');
			expect(panel?.content?.[1]).toEqual(expandNode);
		});

		it('should restore an unsupportedBlock-wrapped expand back into the panel (round-trip)', () => {
			const schema = createSchemaWithNodes({
				includePanelC1: true,
				includeExpand: true,
				includeUnsupportedBlock: true,
			...
			const result = panelC1FallbackTransformV2(
				schema,
				docWithUnsupportedBlockWrappedExpandInPanel,
			);
		...
		it('LCM downgrade output should round-trip back to the original when support returns', () => {
			const offSchema = createSchemaWithNodes({ includeExpand: true, includeExtension: true });
			const onSchema = createSchemaWithNodes({
				includePanelC1: true,
				includeExpand: true,
				includeExtension: true,
			...
			const off = panelC1FallbackTransformV2(offSchema, docWithPanelC1WithExpand);
			expect(off.isTransformed).toBe(true);
