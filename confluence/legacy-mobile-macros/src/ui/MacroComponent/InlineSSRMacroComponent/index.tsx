...
import { loadResourceTags, useMacroViewedAnalyticsEvent } from '../../../common/utils';
...
export const InlineSSRMacroComponent: ((props: InlineSSRMacroComponentProps) => JSX.Element) & {
	...
	useEffect(() => {
		createPromise('customLegacyMacro', legacyMacroArgs)
			...
			.then((response: any) => {
				...
				if (response.html) {
					loadResourceTags(
						[response.resources?.css, response.resources?.data, response.resources?.js],
						false,
	...
	}, [extensionKey, outputDeviceType, shouldFireAnalytics, fireMacroViewedAnalyticsEvent]);

	if (macroQueryResponse) {
		if (macroQueryResponse instanceof CustomLegacyMacroError) {
			return <SSRMacroWrapper data-testid={ssrMacroTestId}>{renderFallback()}</SSRMacroWrapper>;
		} else {
			...
			html = html.replace(/(src|href)="\/([^\/])/gi, `$1=\"${adjustedBaseUrl}$2`);
			return (
	...
	if (!InlineSSRMacroComponent.tocObserver) {
		...
		if (element) {
			...
			const observer = new MutationObserver((mutations) => {
				...
				if (shouldRefresh) {
					document.querySelectorAll('.client-side-toc-macro').forEach((node) => {
