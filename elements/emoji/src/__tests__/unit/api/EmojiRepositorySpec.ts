...
describe('EmojiRepository', () => {
	...
	describe('#search', () => {
		...
		it('all should not include non-searchable emoji', () => {
			...
			expect(emojis.filter((emoji) => emoji.shortName === ':police_officer:').length).toEqual(0);
		});
		...
		it('returns frequently used before others except for an exact shortname match', (done) => {
			...
			if (!greenHeart || !heart) {
				...
				window.setTimeout(() => {
					...
					expect(exactMatchResult.indexOf(heart) < exactMatchResult.indexOf(greenHeart)).toEqual(
						true,
					);
					done();
				});
	...
	describe('#addUnknownEmoji', () => {
		...
		it('add custom category when the first custom emoji is added', () => {
			...
			expect(repository.getDynamicCategoryList()).not.toContain([customCategory]);
		});
		...
		it('add non-custom emoji', () => {
			...
			expect(containsEmojiId(searchEmojis, toEmojiId(standardTest))).toEqual(true);

			expect(repository.findById(standardTest.id as string)).toEqual(standardTest);
			expect(repository.findByShortName(standardTest.shortName)).toEqual(standardTest);
		});
	});
	...
	describe('#findAllMatchingShortName', () => {
		it('returns a list of emoji with exact match in shortName', () => {
			...
			expect(repository.findAllMatchingShortName(':test:')).toEqual([siteTest, atlassianTest]);
		});
	...
	describe('#delete', () => {
		...
		it('should not be able to search by shortname for an emoji that has been deleted', () => {
			...
			expect(repository.search(':smiley').emojis.length).toEqual(numSmileys - 1);
		});
		...
		it('should not be able to find by shortname for an emoji that has been deleted', () => {
			const repository = new EmojiRepository(copyEmojis);
			repository.delete(smileyEmoji);
			expect(repository.findByShortName(smileyEmoji.shortName)).toEqual(undefined);
		});
		...
		it('should not be able to find by category an emoji that has been deleted', () => {
			...
			peopleEmojis.forEach((emoji) => expect(emoji.shortName).not.toEqual(smileyEmoji.shortName));
		});
	});
	...
	describe('getFrequentlyUsed', () => {
		it('should return frequently used with the correct skin tone', (done) => {
			...
			window.setTimeout(() => {
				...
				expect(emoji[0].shortName).toEqual(`${thumbsupEmoji.shortName}:skin-tone-5:`);
				done();
			});
		});
