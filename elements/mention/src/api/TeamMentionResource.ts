...
export default class TeamMentionResource extends MentionResource implements MentionProvider {
	 ...
	 * container.
	 */
	private async remoteInitialStateTeamAndUsers(contextIdentifier?: MentionContextIdentifier) {
		const emptyQuery = '';
	...
	private async handleBothRequests(
		...
		const notifyWhenOneRequestDone = (results: MentionsResult, hasTeamResults: boolean) => {
	...
	private convertTeamResultToMentionResult(result: Team[], query: string): MentionsResult {
