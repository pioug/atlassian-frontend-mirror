...
function buildToken(j: core.JSCodeshift, tokenId: string = '', node: any) {
	const callExpr = tokenId
		? j.callExpression(j.identifier('token'), [j.stringLiteral(tokenId), node].filter(Boolean))
		: node;
...
function replaceIdentifiers(j: core.JSCodeshift, source: Collection<any>, specifier: any) {
	...
	source
		...
		.filter((identifier) => !isDecendantOfType(j, identifier, j.ImportDeclaration))
		...
		.forEach((identifier) => {
			...
			const isDynamicReplacement =
				...
				(isDecendantOfCallExpression && identifier.parent.value.arguments.length > 0);
			...
			if (!isDecendantOfToken && replacement.tokenId) {
				(isDecendantOfCallExpression ? identifier.parent : identifier).replace(
					buildToken(j, replacement.tokenId, j.identifier(replacement.staticReplacement)),
				);
...
function replaceMemberExpressions(j: core.JSCodeshift, source: Collection<any>, specifier: any) {
	source
		...
		.forEach((identifier) => {
			...
			const isDynamicReplacement =
				!isDecendantOfCallExpression ||
				(isDecendantOfCallExpression && identifier.parent.parent.value.arguments.length > 0);
			...
			if (!isDecendantOfToken && replacement.tokenId) {
				(isDecendantOfCallExpression ? identifier.parent.parent : identifier).replace(
					buildToken(
						j,
						replacement.tokenId,
						j.memberExpression(
