...
import type { CallExpression, ImportDeclaration, Literal, Property } from 'estree';
...
const EASING_TOKENS: Array<{ value: string; token: string; params: number[] }> =
	EASING_TOKEN_NAMES.map((name) => {
		...
		if (!params) {
			throw new Error(
				`use-tokens-motion: could not parse cubic-bezier for token ${name}: ${rawValue}`,
			);
		}
...
const useTokensMotion: Rule.RuleModule = createLintRule({
	meta: {
		...
		docs: {
			description:
				'Enforces usage of motion design tokens rather than hard-coded duration and easing values.',
			recommended: false,
			severity: 'warn',
		},
		messages: {
