...
import LinkButton from '../../examples/constellation/new-button/link-button/link-button';
...
import LinkButtonLink from '../../examples/constellation/new-button/link-button/link-button-link';
import LinkButtonSubtleLink from '../../examples/constellation/new-button/link-button/link-button-subtle-link';
...
Link button consumes the router link component configured in the [app provider](/components/app-provider/examples#router-links) when possible. This means router links are configured once across an application (replacing [the component prop](/components/button/button-legacy/examples#routing)).
...
<SectionMessage title="Updates to link handling" appearance="information">
  <p>
    Link appearances now only support semantically correct links using link
    button. If you have a legacy button that's used as a link, consider
    replacing it with a link button or using another approach.
  </p>
</SectionMessage>

Use a link appearance for buttons that look and behave like inline text links.

<Example Component={LinkButtonLink} packageName="@atlaskit/button" />

### Subtle link

Similar to a subtle button, but looks like a link. Use subtle link buttons with caution. Because there isn't any color or other visual affordance built in, this style relies on the context around it to show that it can be interacted with.
...
<Example Component={LinkButtonSubtleLink} packageName="@atlaskit/button" />
