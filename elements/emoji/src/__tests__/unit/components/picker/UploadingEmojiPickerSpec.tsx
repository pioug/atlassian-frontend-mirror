...
describe('<UploadingEmojiPicker />', () => {
	...
	describe('delete', () => {
		...
		it('does not remove emoji from list on failure', async () => {
			...
			// Filter out the delete button since the class for visibility:hidden is present for the parent
			// but the style is not being applied
			const emoji = within(helperTestingLibrary.getVirtualList())
				.getAllByRole('button')
				.filter((emoji) => emoji.getAttribute('data-testid') !== 'emoji-delete-button');
			expect(emoji.length).toEqual(3);
			spy.mockRestore();
		});
