...
import type { Cell } from './utils/cells-at-column';
...
import type { SideEffects } from './utils/side-effects/types';
...
function createColumnInfo(cellsInfo: CellStep[]): ColumnInfo {
	return new Map<number, CellStep>(cellsInfo.map((cellInfo) => [cellInfo.from, cellInfo]));
}
...
export class AddColumnStep<S extends Schema = any> extends Step {
	...
	constructor(tablePos: number, addColumnStepInfo: AddColumnStepInfo, isDelete = false) {
		super();
	...
	map(mapping: Mappable): Step | null {
		...
		return new AddColumnStep(tablePosResult.pos, { cells: cellSteps, sideEffects }, this.isDelete);
	}
	...
	invert(originalDoc: ProseMirrorNode): Step {
		...
		const newCellSteps: CellStep[] = Array.from(this.columnInfo.values(), (oldCellStep) => {
		...
		return new AddColumnStep(this.tablePos, { cells: newCellSteps, sideEffects }, !this.isDelete);
	}
	...
	private applyCellSteps(tr: Transform, tableRect: TableRect, column: number, isDelete: boolean) {
		...
		while (!next.done) {
			...
			const newCellStep: CellStep = createCellStep(cell, column, isDelete, previousCellStep);
