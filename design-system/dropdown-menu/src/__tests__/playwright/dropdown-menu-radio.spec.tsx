...
test('Verify that radio in dropdown menu can only have one selection in a group', async ({
  ...
  await expect(
    page.locator(`#other-cities button[aria-checked]`).nth(0),
  ).toBeChecked();
  await expect(
    page.locator(`#other-cities button[aria-checked]`).nth(1),
  ).not.toBeChecked();
  await expect(
    page.locator(`#other-cities button[aria-checked]`).nth(2),
  ).not.toBeChecked();
