...
export default function asExperiment(
	...
	return class ExperimentSwitch extends Component<Record<string, any>, State> {
		...
		onReceiveContext = (experiments: Experiments, options?: ExperimentEnrollmentOptions) => {
			...
			if (!(experimentKey in experiments)) {
				throw new Error(`Experiment Key ${experimentKey} does not exist in configuration`);
			}
			...
			if (!enrollmentDetails) {
				throw new Error(`Experiment ${experimentKey} has missing enrollment details`);
			}

			const { cohort, isEligible, ineligibilityReasons } = enrollmentDetails;

			if (!(cohort in experimentComponentMap)) {
				throw new Error(`Cohort ${cohort} does not exist for experiment ${experimentKey}`);
			}

			const View = isEligible ? experimentComponentMap[cohort] : experimentComponentMap.fallback;
		...
		render() {
			return (
				<ExperimentConsumer>
					{({ experiments, options }) => this.onReceiveContext(experiments, options)}
				</ExperimentConsumer>
			);
		}
	};
